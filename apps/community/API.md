...
- **Check.** The server then checks the export in the background, without storing anything yet. Everything in it must be something this host's own members could have written and read: each message, channel, person, and file must read back through the member API, message text must fit `COMMUNITY_TEXT_BYTES`, each file `COMMUNITY_ATTACHMENT_BYTES`, and nothing may be dated after the upload. A channel name longer than 80 characters or a description longer than 1,000 is shortened with an ellipsis, and the report counts these as `shortened`. It also checks only `manifest.json` (first) and `attachments/<id>` entries, nothing encrypted, no duplicate or unsafe names, every file at the size and SHA-256 its manifest records, every message, reply, mention, and file pointing inside the export, exactly one active owner who made the export, and files that fit the community's storage limit. A passing import waits at `validated` with a report of counts and sizes (never names or text). Create it with `autoCommit: true` to skip the wait.
- **Commit.** `POST /api/v1/host/imports/:id/commit` with `{}` starts the restore (`409 STATE_CONFLICT` unless `validated`). An import left at `validated` for seven days is cancelled. The restore stores every file, then writes every channel, message, file, and audit event in one transaction, with new IDs, so nothing is visible until it is `ready`. Each channel's messages keep their order but are numbered again from 1. Past members and agents come back as former members with no account; email addresses are not kept, and restored audit events are marked as imported. Because a former member has no account on this host, they cannot erase their own past messages here; the community's owner can remove them instead. A version 1 export does not say who was erased on the old host, so an erased person arrives as an ordinary former member, with their messages already replaced. For the same reason a removed message arrives as an ordinary message showing its removal sentence; it is not marked removed. A restore that stops part-way picks up where it stopped.
