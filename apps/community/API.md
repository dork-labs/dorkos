...
| Imports             | `POST /api/v1/host/imports`; `GET /api/v1/host/imports/:id`; `POST /api/v1/host/imports/:id/cancel`; `PUT /api/v1/imports/:id/archive`                                                                                                     | `communities:import`, `communities:read` to read          |
...
- **Start.** `POST /api/v1/host/imports` with `idempotencyKey`, `name`, and optionally `description`, `admissionPolicy`, `limits`, and `autoCommit`. The answer (`201`, `Cache-Control: no-store`) carries the import and an `uploadToken`, shown this once. Repeating the same request answers `200` with `replayed: true` and `uploadToken: null`; the same key with different inputs is `409 IDEMPOTENCY_CONFLICT`. Give the new community a short name afterwards through the host's own routes.
- **Upload.** `PUT /api/v1/imports/:id/archive` with the export as the body, its size in `Content-Length` (at most 1 GiB; larger is `413 IMPORT_TOO_LARGE`), and its SHA-256 as lowercase hex in `X-Archive-SHA256`. Authenticate with `Authorization: Bearer <uploadToken>`, so the host can hand the upload to whoever holds the file, or with host authority that has `communities:import`. A body that does not match its size or digest, or is not a zip file, is `400 IMPORT_ARCHIVE_INVALID` and keeps nothing; neither that nor a dropped connection uses up the token, which works until `uploadExpiresAt` (24 hours). After that it answers `401`. Success uses up the token and moves the import to `validating`. Sending the same file again is `200`; a different one is `409 IDEMPOTENCY_CONFLICT`.
- **Read.** `GET /api/v1/host/imports/:id` returns the state, a report of counts and sizes once the export has been checked, and a `failureCode` when it failed. It never returns the token, a name, or any text.
- **Cancel.** `POST /api/v1/host/imports/:id/cancel` with `{}` works in any state before `ready`. The unclaimed community and every file of the import are then removed in the background, and the import answers `communityId: null`. An upload window that closes first ends the import the same way, as `cancelled`.

Until an import is `ready`, its community refuses an owner claim (`409 STATE_CONFLICT`) and cannot be abandoned with `DELETE /api/v1/host/communities/:id`; cancel the import instead. The uploaded export shows in usage as `importStagingBytes` and never counts against a storage limit.
