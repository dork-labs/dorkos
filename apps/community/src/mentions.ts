...
const QUOTE = /^ {0,3}>/;
const INLINE_CODE = /`[^`]*`/g;
const ADDRESS = /@([A-Za-z0-9][A-Za-z0-9_.-]*)/g;
...
function maskedText(text: string): string {
...
export function resolveCommunityMentions(
  ...
  for (const match of maskedText(text).matchAll(ADDRESS)) {
    const raw = match[1].toLowerCase();
    const found = byHandle.get(raw) ?? byHandle.get(raw.replace(/[.\-_]+$/, ''));
