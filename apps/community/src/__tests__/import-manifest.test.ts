...
describe('checkManifest', () => {
  ...
  // Purpose: a file larger than an attachment may be is a size refusal, not a damaged file.
  it('refuses an oversized file as too large', () => {
    expect(failureOf((m) => void (m.attachments[0].byteSize = 25 * 1024 * 1024 + 1))).toBe(
      'IMPORT_TOO_LARGE'
    );
  });
});
