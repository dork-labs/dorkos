    ...
    try {
      await transaction(pool, async (client) => {
        ...
        const referenced = await client.query(
          `SELECT 1 FROM attachments WHERE blob_key=$1 UNION ALL SELECT 1 FROM export_archives WHERE blob_key=$1
           UNION ALL SELECT 1 FROM export_segments WHERE blob_key=$1
           UNION ALL SELECT 1 FROM communities WHERE icon_blob_key=$1 LIMIT 1`,
          [candidate.blob_key]
        );
