...
test('add → needs sign-in → Sign in → approve → connected with N tools', async ({
  ...
  // `exact` because the success panel's own copy starts with the same two words.
  await expect(mcp.mcpSection.getByText('Signed in', { exact: true })).toBeVisible();
  ...
  await expect(mcp.mcpSection.getByText('Connected')).toBeVisible({ timeout: 15_000 });
