...
- **`getByText('Saved')` matches "Not saved yet".** A plain string is a _case-insensitive substring_ match, so a status line that cycles through several sentences will satisfy the wait with the wrong one — instantly, and without failing. Everything after it then races the thing it was meant to wait for, and reads as a broken feature (`rooms/room-files.spec.ts`, where it read as "the save never landed" against a save that had). Use `{ exact: true }` for any state word that is a substring of a neighbouring state, or assert on a role/testid instead.
