 ...
 * `'none'` means the column is left NULL and the runtime decides.
 */
const TABLE: ReadonlyArray<readonly [TurnOrigin, OriginPermissionSeed]> = [
  // A person is holding the stream open, so a stop they set is one they can answer.
  [{ kind: 'interactive' }, 'configured-stop'],
  // Nobody is watching a room turn, which is the reason it follows the
  // operator's level rather than the reason it may not (DOR-1917).
  [{ kind: 'room', roomId: 'room-1', externalAuthor: false }, 'configured-stop'],
  ...
  [{ kind: 'room', roomId: 'room-1', externalAuthor: true }, 'none'],
  ...
  [{ kind: 'schedule', taskId: 'task-1', runId: 'run-1' }, 'none'],
  ...
  [{ kind: 'agent-dm' }, 'none'],
  [{ kind: 'connector-event', agentId: 'agent-1' }, 'none'],
