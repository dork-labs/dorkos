 ...
 *
 * One package's exact command strings, in one project. The stored record binds
 * all three: `harness.approvedHooks` holds `<packageName>@<digest>`, where the
 * digest covers the project path and the commands as they would be written. So an
 * update that changes what a package runs asks again instead of riding an old
 * yes, and consent given in one project does not carry into another.
 *
...
export interface HookProjectionRequest {
  ...
  packageName: string;
  /** The exact commands that would land, `${CLAUDE_PLUGIN_ROOT}` already resolved. */
  commands: string[];
}
 ...
 * packages they have allowed. The digest behind it covers the project and the
 * exact commands, so the entry stops matching the moment either changes.
 *
 * @param request - The package, project, and commands to identify.
...
export function hookApprovalEntry(request: HookProjectionRequest): string {
  const digest = createHash('sha256')
    .update(JSON.stringify([request.projectPath, request.commands]), 'utf8')
    .digest('hex');
 ...
 * these commands into exactly this project.
 *
...
/**
 * Record a person's yes, so the same package projecting the same commands into
 * the same project is never asked about again.
 *
 ...
 *
 * Leads with what will run, because that is the whole decision — a package name
 * tells a person nothing about what it is about to execute. Every value is
 * quoted, escaped and capped, so a command carrying its own quotes and
 * connectives cannot forge the rest of the sentence.
 *
 * @param request - The package, project, and commands being asked about.
...
export function summariseHookProjection(request: HookProjectionRequest): string {
  ...
  const commands = request.commands.map(renderCommandForCard).join(', ');
  const count = request.commands.length;
  return (
    `Let the installed package ${name} run ${count} command${count === 1 ? '' : 's'} ` +
    `automatically in ${where}: ${commands}`
  );
}
 ...
 * @param request - The package, project, and commands to ask about.
