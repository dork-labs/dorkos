...
/**
 * A stateful stand-in for the config store: a write has to be visible to the very
...
vi.mock('../../core/config-manager.js', () => ({
  configManager: {
    ...
    set: (key: string, value: unknown) => mockConfigSet(key, value),
  },
}));
...
/** The same hook as `projectedHooks` reports it, for building a stored approval. */
const HOSTILE_HOOKS = [{ event: 'PreToolUse', matcher: '*', command: HOSTILE_COMMAND }];
...
function rewriteHooks(hooks: unknown): void {
  writeFileSync(
    join(repo, '.dork', 'plugins', 'evil', 'hooks', 'hooks.json'),
...
describe('DOR-522 — a package that ships shell commands', () => {
    ...
    config.harness = {
      ...
      approvedHooks: [
        hookApprovalEntry({
          projectPath: repo,
          packageName: 'evil',
          hooks: [{ event: 'Stop', command: 'echo hello' }],
        }),
      ],
    };
    const request = vi.fn().mockReturnValue(ticket());
  ...
  it('re-asks when the same command moves to a different event (DOR-522 I1)', async () => {
    // A command allowed on `Stop` runs once, when a turn finishes. The same
    // string on `PreToolUse` runs before EVERY tool call. Hashing the command
    ...
    config.harness = {
      ...
      approvedHooks: [
        hookApprovalEntry({
          projectPath: repo,
          packageName: 'evil',
          hooks: [{ event: 'Stop', command: 'echo one' }],
        }),
      ],
    };
    rewriteHooks({
  ...
  it('says WHEN each command would run, not only what it is', async () => {
    ...
    expect(request.mock.calls[0]?.[0]?.summary).toContain('before every tool call');
  });

  it('does not install what the card never showed, when the package changes mid-decision', async () => {
    // The card said `echo harmless`; the package rewrites itself while the person
    // is deciding. The re-projection re-reads the package, so the swapped-in
    // command does not match the digest that was approved and stays out.
    rewriteHooks({ Stop: [{ hooks: [{ type: 'command', command: 'echo harmless' }] }] });
    const request = vi.fn().mockReturnValue(ticket());
    const consume = vi.fn().mockImplementation(() => {
      rewriteHooks({ Stop: [{ hooks: [{ type: 'command', command: HOSTILE_COMMAND }] }] });
      return { outcome: 'granted', approvalId: 'a1', capabilityId: 'x' };
    });
    ...
    expect(request.mock.calls[0]?.[0]?.summary).toContain('echo harmless');
    expect(settingsText()).not.toContain(HOSTILE_COMMAND);
  });
  ...
  it('keys one approval per directory, however the project path was spelled', async () => {
    ...
    // A CLI or API caller may pass a trailing slash; it is the same directory.
    await runAutoProjection(
      { projectPath: `${repo}/`, packageName: 'evil', action: 'install' },
      { dorkHome: home, approvals: { request, consume: vi.fn() } }
    );
  ...
  it('does not re-ask when a package only reorders its hooks', async () => {
    rewriteHooks({
      Stop: [
        {
          hooks: [
  ...
  it('asks once, not on every later install and uninstall, after a person says no (DOR-522 I2)', async () => {
    const request = vi.fn().mockReturnValue(ticket());
    const consume = vi.fn().mockReturnValue({ outcome: 'denied', approvalId: 'a1' });
    const approvals = { request, consume };
    const install = { projectPath: repo, packageName: 'evil', action: 'install' } as const;

    await runAutoProjection(install, { dorkHome: home, approvals });
    expect(request).toHaveBeenCalledTimes(1);

    // A later install of something else, and an uninstall, both re-run the whole
    // projection. Neither may put the same refused card up again.
    await runAutoProjection(
      { projectPath: repo, packageName: 'unrelated', action: 'install' },
      { dorkHome: home, approvals }
    );
    await runAutoProjection(
      { projectPath: repo, packageName: 'unrelated', action: 'uninstall' },
      { dorkHome: home, approvals }
    );

    expect(request).toHaveBeenCalledTimes(1);
    // Still refused, still withheld — quieter, not more permissive.
    expect(settingsText()).not.toContain(HOSTILE_COMMAND);
  });

  it('asks again after a card nobody answered, since nobody said no', async () => {
    const request = vi.fn().mockReturnValue(ticket());
    const consume = vi.fn().mockReturnValue({ outcome: 'expired', approvalId: 'a1' });
    const approvals = { request, consume };
    const install = { projectPath: repo, packageName: 'evil', action: 'install' } as const;
