...
- open_file: { sourcePath: string } — open a file from the session's working directory in the workbench. DorkOS picks the right viewer (code editor, image, PDF, 3D model, audio player, video player, CSV table, or rich markdown) from the file's type and opens it as a new document, so the user can read or edit it in place. Use this instead of pasting a file's contents into the chat when you want the user to look at or edit a real file.
