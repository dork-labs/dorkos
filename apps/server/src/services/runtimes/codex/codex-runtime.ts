 ...
 * messaging, storage, and capabilities land with the remaining P2 tasks
 * (permission mapping, thread map, event mapper, full facade). Registration
 * in the composition root is gated on `runtimes.codex.enabled` and happens
 * when the facade is complete.
 *
...
/**
 * Throw for CodexRuntime methods the remaining P2 tasks implement
 * (permission mapping, thread map, event mapper, full facade).
 */
function notImplemented(method: string): never {
  throw new Error(`CodexRuntime.${method} is not implemented yet (additional-agent-runtimes P2)`);
}

/**
 * Codex runtime implementing the universal AgentRuntime interface.
 *
 * One DorkOS session maps to one Codex thread (ADR-0307). Only the
 * dependency check is live today; every other method throws until the
 * thread map, event mapper, and facade tasks fill them in.
 */
export class CodexRuntime implements AgentRuntime {
  ...
  getCapabilities(): RuntimeCapabilities {
    // Finalized by the permission/sandbox/approval verification task (2.2).
