...
async function* captureClaudeCodeMedia(
  ...
  while (state.pendingMedia.length > 0) {
    ...
    try {
      ...
      const stored =
        existing ??
        (await store.put(sessionId, attachmentId, image.mediaType, decodeBase64(image.base64)));
      yield {
      ...
      logger.warn('[ClaudeCode] could not store an image a tool returned', { err, sessionId });
...
export async function* mapSdkMessageWithMedia(
  ...
  for await (const event of mapSdkMessage(message, session, sessionId, toolState, wasStopped)) {
    yield event;
    yield* captureClaudeCodeMedia(store, sessionId, toolState);
  }
  yield* captureClaudeCodeMedia(store, sessionId, toolState);
}
