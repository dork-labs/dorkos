...
## 9. The SDK types EVERY request body as optional; the server does not (DOR-1668)

Found by a live paid conformance run, not by a mocked test. `client.session.summarize({ path })` —
no body — compiled, passed the mocked suite, and failed every real `/compact` with:

```json
{ "name": "BadRequest", "data": { "message": "Expected object, got undefined", "kind": "Payload" } }
```

**The cause is generator-wide, not specific to compaction.** `@opencode-ai/sdk` is generated by
hey-api, which emits `body?:` for every endpoint that has a payload at all, whether or not the server
requires one. So the compiler cannot tell a genuinely-optional body from a required one, and a mocked
client — which accepts whatever it is handed — cannot either. Only a live sidecar answers.

The server's own rule is legible in the binary's route table: a route that tolerates an absent body
declares `payload: [NoContent, Schema]`, and one that requires a body declares `payload: Schema` on its
own. `kind: "Payload"` in the error is the Effect HttpApi's payload-decode failure.

Audited against `opencode-ai@1.18.15` (route table + `Struct` payload schemas read out of the shipped
binary), for every call this adapter makes:

| DorkOS call                                                                                                                        | server payload        | body DorkOS sends       | exposed?                 |
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ----------------------- | ------------------------ |
| `session.summarize`                                                                                                                | `Struct` (required)   | `{providerID, modelID}` | **was the bug** — fixed  |
| `session.promptAsync`                                                                                                              | `Struct` (required)   | `{parts, model?}`       | no — `parts` always sent |
| `session.update`                                                                                                                   | `Struct` (required)   | `{title}`               | no                       |
| `session.create`                                                                                                                   | `[NoContent, Struct]` | `{}` or `{title}`       | no                       |
| `session.fork`                                                                                                                     | `[NoContent, Struct]` | `{}` or `{messageID}`   | no                       |
| `mcp.add`                                                                                                                          | `Struct` (required)   | `{name, config}`        | no                       |
| `postSessionIdPermissionsPermissionId`                                                                                             | `Struct` (required)   | `{response}`            | no                       |
| `session.get` / `list` / `messages` / `todo` / `abort`, `mcp.status` / `disconnect`, `config.get`, `provider.list`, `global.event` | no payload declared   | —                       | no                       |

Fix live-verified 2026-09-01 against `opencode-ai@1.18.15` on a real `opencode serve`, driving a turn
and then a compaction on a local Ollama model (`ollama/qwen2.5-coder:7b`, pinned through
`OPENCODE_CONFIG`) — passing with the body, and reproducing the exact `BadRequest` above with the body
removed.

**The table's axis is payloads. There is a second axis: `query.directory`.** Caught in review of the fix
above. Every non-session-scoped route resolves per directory and falls back to
`searchParams.directory → x-opencode-directory → the SIDECAR's own process.cwd()` (§1), and
`server-manager.ts` spawns the sidecar with no `cwd` of its own — so it inherits the DorkOS server
process's. An unscoped `config.get()` therefore answers for whatever project DorkOS happens to be
running in, not the caller's, which is silent and wrong rather than loud and wrong. Session-scoped
routes (`/session/{id}/…`) are exempt: a separate middleware resolves them from the session's stored
directory. Demonstrated live on 1.18.15 — the same sidecar answered `ollama/gemma4:latest` with
`directory=<projB>` and `ollama/qwen2.5-coder:7b` with no directory at all.

**Two calls sit on this axis, and BOTH now pass it.** `config.get` (compaction rung 3, and
`mcp-status.ts`) and **`provider.list`** — `GET /provider` reads `enabled_providers` /
`disabled_providers` off the same `Config` service `GET /config` does. `provider.list` feeds
`getSupportedModels()`, so an unscoped read builds the model picker from the wrong project: a provider
declared in this project's `opencode.json` (a local vLLM or other OpenAI-compatible endpoint) never
appears, and one disabled here is still offered.

**Passing a cwd to `getClient(cwd)` does NOT scope the request.** That argument is accepted and ignored
(`server-manager.ts` — one shared sidecar for every directory, routed per request), and the client is
built with only `baseUrl` + `Authorization`, so it sends no `x-opencode-directory` either. The query
parameter is the only channel. A call site that reads as directory-aware because it passed a cwd to
`getClient` is the trap here — `getSupportedModels` looked scoped and was not.

So `summarize` was the only exposed call, and the rest are safe by construction rather than by luck —
every DorkOS call to a required-payload route already sends a complete body. The standing rule for new
calls: **a `body?:` in the SDK types proves nothing.** Check the route, and cover it in the live arm of
`__tests__/conformance.test.ts` (`DORKOS_OPENCODE_LIVE=1`), because a mocked test never can.
