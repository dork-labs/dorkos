 ...
 * picture in a visible turn, a missing turn. Images now map, so such a message
 * survives with its picture; one whose image genuinely cannot be re-materialized
 * still returns `null`, because there is then honestly nothing to show.
 *
    ...
    return async (file, identity) => {
      ...
      try {
        ...
        if (existing) {
          ...
          void store.touch(sessionId, attachmentId, storableImageExtension(file.mime) ?? '');
