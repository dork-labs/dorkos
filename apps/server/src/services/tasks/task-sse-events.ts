 ...
 *
 * Deliberately fired from the task ROUTES rather than the store. The routes are
 * the only writer that can raise a task's permission mode: `task-write-policy`
 * classifies that field operator-only, and both content paths into the same row
 * (a Shape manifest, a SKILL.md on disk) are clamped by
 * `schedule-permission-clamp`, so no watcher or reconciler write can introduce
 * the posture this signal exists to announce. Broadcasting from the store would
 * add a burst of events during reconciliation for no reader.
 *
