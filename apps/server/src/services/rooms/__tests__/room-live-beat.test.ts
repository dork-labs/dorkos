 ...
 * place — including the ones a type cannot reach: the server's presence
 * republisher, which keeps its constant module-private, and the browser, which
 * cannot import server code at all.
 *
...
describe('the room’s one ephemeral beat', () => {
  ...
  it('leaves no bare copy of the number behind', () => {
    const files = [
      '../room-trigger.ts',
    ...
    for (const file of files) {
      expect(source(file), file).not.toMatch(/=\s*10_000;/);
    }
  });
});
