...
export class RoomTurnControl {
   ...
   * NARROW — it takes no viewer and refuses nobody by name, so it cannot become
   * a second way to make an agent speak. Every bound a normal trigger has still
   * applies, and the turn says whatever it decides to say by calling the posting
   * tool, exactly as any room turn does; see
   * {@link RoomTriggerDispatcher.askAside}.
   *
   * **Never throws.** A missing room, an archived one, a deleted entry and a
   * failed turn all resolve with nothing said, because the greeting this belongs
   * to must not fail the read-state write that revealed it.
   *
