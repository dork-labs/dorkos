...
/**
 * One roster member as mention resolution sees it.
 *
 * `names` is a list rather than a single handle for exactly one reason, and it
 * is not a display-name fallback: a bridged room threads the bot's platform
 * username in beside the agent's own handle, because Telegram addresses a bot as
 * `@botusername` and a person typing that in a Telegram group means the agent
 * (chats-as-channels §5.4). Tried in order, so the agent's real handle stays
 * preferred and the extra name can never displace a handle another member holds.
 */
 ...
 * at all. That used to be a real cost — `agents.name` is `z.string().min(1)`,
 * and 7 of the 40 agents registered on the install this was written against have
 * a space in theirs — and `authors.handle` removed it: an author is addressed by
 * a handle the grammar guarantees is typeable, never by a name somebody wrote
 * into a manifest.
 *
 ...
 * class is gone at the source rather than guarded against here: the handle
 * grammar requires a handle to start AND end alphanumeric, so nobody can be
 * called `ana.` (`@dorkos/shared/handle`).
 *
 ...
 * wins.
 *
 * **Private, and that is the change.** It used to be exported and read by a
 * second half of the mention contract — `advertisedHandle`, which decided what a
 * picker could offer over the same ownership map. Both are gone with the
 * display-name addressing path (spec `handles` S1): a handle is unique by index,
 * so the author who has it is the author it reaches, and there is no ownership
 * to compute. What is left is a lookup table for ONE pass over ONE message,
 * built here and used nowhere else.
 *
 * First-claimant-wins still matters, for the one case that still supplies more
 * than one name per member: a bridged room's platform username, appended after
 * the agent's own handle so it can never displace one.
 *
 * @param roster - The room's members and the names each answers to, in the
 *   order that decides ties.
 */
function claimNames(roster: readonly MentionCandidate[]): Map<string, string> {
