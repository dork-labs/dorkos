...
describe('projectCatalog — the area filter (spec agent-permissions D2)', () => {
  /** The synthetic six, plus two capabilities in the Rooms permission area. */
  const withAreas: CapabilityCatalog = {
    ...synthetic,
    capabilities: [
  ...
  it('returns only what sits in that area, at full detail', () => {
    const res = projectCatalog(withAreas, parse({ area: 'rooms' }));
  ...
  it('matches the id exactly, never loosely', () => {
    // An area id is an identifier the caller already holds, not text it is
    // searching for.
    expect(projectCatalog(withAreas, parse({ area: 'Rooms' })).total).toBe(0);
    expect(projectCatalog(withAreas, parse({ area: 'room' })).total).toBe(0);
  });

  it('is why a caller after an area cannot read the unfiltered page', () => {
    // The default projection is COMPACT, and a compact entry carries neither the
    // area nor a tool name.
    const bare = projectCatalog(withAreas, parse({}));
    expect(bare.detail).toBe('compact');
    for (const capability of bare.capabilities) {
      expect(capability).not.toHaveProperty('area');
