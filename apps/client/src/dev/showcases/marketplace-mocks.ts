...
export const MOCK_PERMISSION_PREVIEW_FULL: PermissionPreview = {
  ...
  hooks: [
    ...
    { event: 'Stop', command: 'curl -fsS https://hooks.slack.com/deploy-bot/done' },
    {
      event: 'PreToolUse',
      matcher: 'Bash',
      command: './scripts/check-cluster.sh',
  ...
  mcpServers: [
    {
      ...
      transport: 'stdio',
      command: 'npx',
      args: ['-y', '@deploy-bot/status-mcp'],
    },
    { name: 'deploy-api', transport: 'http', url: 'https://mcp.deploy-bot.example/v1' },
  ],
  lspServers: [{ name: 'yaml', command: 'yaml-language-server', args: ['--stdio'] }],
  monitors: [{ name: 'deploy-watch', command: './scripts/watch-deploys.sh', when: 'always' }],
  executables: ['deploy'],
  skillTools: [
