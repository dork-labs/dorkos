 ...
 * (e.g., `claude-code`) that must never surface in connection pickers.
 */
 ...
 * that presents adapters as "connections" to the user. Runtime-bridge
 * adapters (the `claude-code` adapter is the canonical example) are never
 * a "connection" to add or configure like Telegram or Slack — the Settings
 * Connections tab surfaces the claude-code instance separately, under its own
