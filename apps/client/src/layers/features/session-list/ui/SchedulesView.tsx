...
function RecentRunItem({ run, scheduleName }: RecentRunItemProps) {
  ...
  const displayStatus: PulseRun['status'] =
...
export function SchedulesView({ toolStatus, agentId }: SchedulesViewProps) {
  ...
  const { data: allRuns = [] } = useRuns(undefined, enabled);
  ...
  const { runningRuns, recentRuns } = useMemo(() => {
    const running: PulseRun[] = [];
    const terminal: PulseRun[] = [];
  ...
  const openPulse = () => (agentId ? openPulseForAgent(agentId) : setPulseOpen(true));
