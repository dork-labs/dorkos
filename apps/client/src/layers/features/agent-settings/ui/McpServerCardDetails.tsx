  ...
  return (
    <div className="mt-1.5">
      {shown.map((tool) => (
        ...
        >
          <code className="text-foreground shrink-0 font-mono text-2xs">{tool.name}</code>
          {tool.description && (
            <span className="text-muted-foreground truncate text-2xs">{tool.description}</span>
          )}
        </div>
      ))}
      {hidden > 0 && (
    ...
    <div className="border-border/60 mt-2 space-y-1 border-t pt-2">
      ...
      {rawName !== displayName && (
        <DetailRow label="Raw id">
          <code className="font-mono text-2xs">{rawName}</code>
        </DetailRow>
      )}
      ...
      {error && (
        <DetailRow label="Error">
          <code className="font-mono text-2xs break-all">{error}</code>
        </DetailRow>
      )}
    </div>
  );
