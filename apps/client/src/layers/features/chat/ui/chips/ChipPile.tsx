  ...
  return (
    ...
    >
      ...
      >
        {stack.map((chip, index) => (
          <span
            ...
            aria-hidden="true"
            className={cn(
              'bg-card border-border text-3xs flex size-5 items-center justify-center rounded-full border leading-none',
              STACK_LAYER[index]
            )}
          >
            {VERB_ICON[chip.verb]}
      ...
      <span
        ...
        className="bg-primary text-primary-foreground text-3xs rounded-full px-1.5 tabular-nums"
      >
        {chips.length}
      </span>
    </button>
