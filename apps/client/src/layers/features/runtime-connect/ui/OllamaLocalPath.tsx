 ...
 *
 * This file owns the RUNNING half only; the not-installed half (explainer,
 * one-click or manual install, download link) is the sibling
 * {@link OllamaAbsentState}, split out to keep both under the repo's 500-line
 * ceiling (DOR-424).
 *
...
import { OllamaAbsentState } from './OllamaAbsentState';
...
export function OllamaLocalPath({ active, onConnected, onConnectDirectly }: OllamaLocalPathProps) {
  ...
  if (!running) {
    return <OllamaAbsentState installMethod={detection.data?.installMethod} />;
  }
...
function TierBadge({ tier }: { tier: ModelTier }) {
  return (
    <Badge size="xs" variant="outline" className="font-normal">
      {TIER_LABELS[tier]}
    </Badge>
  );
}
