...
describe('Switch', () => {
  ...
  it.each([
    ...
    ['default', 'w-9', 'w-4', 'data-[state=checked]:translate-x-4'],
    ['md', 'w-11', 'w-5', 'data-[state=checked]:translate-x-5'],
    ['lg', 'w-14', 'w-6', 'data-[state=checked]:translate-x-7'],
  ] as const)('sizes the track and its thumb together at %s', (size, track, thumb, travel) => {
    render(<Switch size={size} aria-label={size} />);
    const root = screen.getByRole('switch');
    expect(root).toHaveClass(track);
    expect(thumbOf(root)).toHaveClass(thumb, travel);
  });

  // Responsive is mobile-first and must REPLACE the default's fixed height, not
  // sit beside it — tailwind-merge is what makes the second axis safe.
  ...
  it('an explicit size opts out of the ladder', () => {
  ...
  it('exports its variants for callers that need the recipe', () => {
    expect(switchVariants({ size: 'lg', responsive: false }).root()).toContain('w-14');
  });
});
