...
import { PANEL } from '../film/film-tokens';
 ...
 * second thing to get wrong. `source` is `tutorials-modal`, so signups from
 * here can be told apart from the footer's: the API validates that field
 * against a closed enum, and the value is declared in both
 * `db/newsletter-schema.ts` and the subscribe route's schema.
 *
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <div className="mt-5">
          <NewsletterSignupForm source="tutorials-modal" variant="compact" />
        </div>
      </motion.div>
    </div>,
    document.body
