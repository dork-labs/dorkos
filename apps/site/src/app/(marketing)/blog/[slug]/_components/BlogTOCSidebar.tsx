...
export function BlogTOCSidebar({ toc }: { toc: TOCItemType[] }) {
  ...
  return (
    <aside className="sticky top-24 hidden h-fit w-56 shrink-0 xl:block">
      <p className="text-charcoal mb-3 text-sm font-semibold">On this page</p>
      <nav>
        <ul className="border-warm-gray-light/30 space-y-1.5 border-l">
          {toc.map((item) => {
            const id = item.url.slice(1);
            const isActive = activeId === id;
            return (
              <li key={item.url} style={{ paddingLeft: `${(item.depth - 2) * 12 + 12}px` }}>
                <a
                  href={item.url}
