...
function cronRequest(authorization?: string): Request {
  ...
  return new Request('https://dorkos.ai/api/cron/instance-expiry', { method: 'GET', headers });
}
...
describe('GET /api/cron/instance-expiry', () => {
  ...
  // The reason the two routes are separate: event-retention work no longer
  // shares this invocation, so it cannot fail this one.
  it('does not touch the database — event retention is the other route', async () => {
