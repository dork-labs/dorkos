...
describe('usePresentationMode', () => {
  ...
  it('returns true when ?present=true', () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new URLSearchParams('present=true') as ReturnType<typeof useSearchParams>
    );
    const { result } = renderHook(() => usePresentationMode());
  ...
  it('returns false when ?present=false', () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new URLSearchParams('present=false') as ReturnType<typeof useSearchParams>
    );
    const { result } = renderHook(() => usePresentationMode());
    expect(result.current).toBe(false);
  });
  ...
  it('returns false when ?present has an unexpected value', () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new URLSearchParams('present=1') as ReturnType<typeof useSearchParams>
    );
    const { result } = renderHook(() => usePresentationMode());
    expect(result.current).toBe(false);
  });
