/**
 * Anonymous product telemetry tables for the apps/site Neon Postgres database.
 *
 * This file is the **privacy contract** for install telemetry and heartbeats.
 * Adding a column here is the single chokepoint for what we are allowed to
 * collect — the ESLint config and `__tests__/schema.test.ts` enforce that no
 * field which could identify a user (IP address, user agent, hostname,
 * username, working directory, etc.) ever lands here.
 *
 * Both tables are hard-isolated from the account tables: no foreign keys, join
 * columns, or shared identifiers cross the account ↔ telemetry boundary (see
 * `auth-schema.ts`). That isolation is what lets them sit in the **public**
 * half of the split — see `public-schema.ts`.
 *
 * @module db/telemetry-schema
 */
import {
  bigserial,
  boolean,
