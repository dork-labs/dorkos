...
/**
 * Per-URL cache of in-flight/completed markdown fetches, so repeat clicks on
 * the same page's Copy-Markdown button don't re-fetch.
 */
const markdownCache = new Map<string, Promise<string>>();

/**
 * "Copy Markdown" button for a docs page.
 *
 * Fetches the page's raw markdown from `markdownUrl` (the `/docs/<slug>.mdx`
 * route) and copies it to the clipboard.
 *
 * Local, project-owned adaptation of fumadocs-ui's `MarkdownCopyButton`.
 * Vendored by hand (not scaffolded via `npx @fumadocs/cli add
 * ai/page-actions`, which requires network access this environment doesn't
 * have) because the installed fumadocs-ui 16.7.16 does not expose
 * `layouts/shared/page-actions` as a public subpath — only its sibling
 * primitives (`components/ui/button`, `components/ui/popover`,
 * `utils/use-copy-button`) are. See
 * https://fumadocs.dev/docs/integrations/llms#page-actions for the upstream
 * pattern this mirrors.
 *
 * @param markdownUrl - URL to fetch the raw Markdown content of the page.
 */
 ...
 * scaffolded).
 *
 * @param markdownUrl - URL to the raw Markdown content of the page.
 * @param githubUrl - Source file URL on GitHub.
 */
export function ViewOptions({
  ...
  const items = useMemo<ViewOptionsItem[]>(() => {
    ...
    const candidates: (ViewOptionsItem | undefined)[] = [
      githubUrl
        ? {
    ...
    return candidates.filter((item): item is ViewOptionsItem => Boolean(item));
...
/** GitHub mark, used by the "Open in GitHub" deep-link. */
function GitHubIcon() {
  return (
...
/** Anthropic mark, used by the "Open in Claude" deep-link. */
function ClaudeIcon() {
  return (
...
/** OpenAI mark, used by the "Open in ChatGPT" deep-link. */
...
/** Scira AI mark, used by the "Open in Scira AI" deep-link. */
function SciraIcon() {
  return (
    <svg
      width="910"
...
/** Cursor mark, used by the "Open in Cursor" deep-link. */
function CursorIcon() {
  return (
    <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <title>Cursor</title>
