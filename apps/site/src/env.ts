...
const webEnvSchema = z.object({
  ...
  // delivery's `Linear-Signature` header — an HMAC-SHA256 hex digest of the raw
  // body, keyed by this secret — before trusting anything in the payload.
  // Provisioned once by ops alongside the webhook subscription itself (not
  // generated by this code). Optional by design: when unset, the webhook route
  // has nothing to verify against and refuses every delivery (401) rather than
  // trusting an unverifiable payload — fail closed, not open.
