...
ci/config.yaml        repo specifics          reports/YYYY-Www.md  weekly deterministic report
...
- **Subcommands:** `census`, `ledger-check`, `collect`, `verdicts`, `ratchet-assert`, `report`, `status`, `local-export`.
...
- `ci-steward census` generates the rest from the real YAML (triggers, "reports on" events, required, timeout, retries, shards, invoked scripts) and fails when:
  ...
  - a required job contains `continue-on-error` or a step-level `if:` that is not on an allowlist with an expiry date.
...
- `verified`, `failed` and `inconclusive` exist only in `verdicts/<id>.json` on the data branch. The validity check rejects them on `main`. `/ci:status` joins the two.
- **Coverage check** (a `typecheck.yml` step, PR only; validity runs on PR and merge group): a PR touching any gate source, any script a gate invokes (from the generated atlas), `turbo.json`, `lefthook.yml`, `.claude/settings.json` or any `ci/` hand file must add or edit a ledger entry. On failure it prints the exact `/ci:record` command. It runs with `continue-on-error` for its first 7 days; that allowlist entry's expiry date **is** the flip to blocking. No separate flip exists.
...
A daily scheduled workflow (`ci-steward.yml`, 05:00 UTC, plus `workflow_dispatch`) runs `collect`, then `verdicts`, then `report` (Mondays), and pushes to `ci-steward-data` with `GITHUB_TOKEN`. That push triggers no workflows, which is the intent.
...
- **Verdicts:** for each ledger entry whose after-window has closed:
  - compute the hypothesis metric over a 7-day before-window and an `after_days` after-window, both anchored on the merge time;
  ...
  The SLO movement is reported beside every verdict. A gate that got faster while its SLO got worse is visible. Fixtures: #1391 (held), #1135 (held partly), #1246 (failed without a confounder, `inconclusive` with a planted same-week confounder), the only past changes with a quantified hypothesis and a measured outcome.
...
- **Weekly report** (`reports/YYYY-Www.md`, deterministic, no LLM): the headline is the SLO trend, not the verdict count. It also shows the constraint, verdicts issued, ratchets, real catches per gate, the tracked metrics and health.
...
- **The daily local export:** `local-export` runs on the operator's machine as the scheduled skill `ci-local-export`, through the DorkOS task scheduler.
...
- the `merge-tail.yml` tick, every 10 min in Actions.
...
| Question              | Answer                                                                                                                                       |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Cadence               | Daily deterministic collection to the data branch, weekly report. No weekly PR.                                                              |
| Location              | `ci/` on `main` for intent, `ci-steward-data` for observations. The guide is `contributing/ci.md`.                                           |
| New required context? | No. Census and ledger checks are steps in `typecheck.yml`. Coverage is advisory for week one via an expiring allowlist.                      |
...
- **Round 3 (Vesper): SIGNED**, with 19 non-blocking follow-ups. The five marked "fix before landing" are applied above:
  ...
  - F: the coverage flip is the allowlist expiry.
...
- **Phase 1:**
  - E: queue shards upload `vitest-shard-report-N` artifacts, and the fan-in downloads them.
  - H: the PR advisory reports the required-path minutes a PR adds (gate budget).
  - I: `floor-release` ledger field.
  - O: both data-branch writers fetch, rebase and retry.
  - P: the time-wrap is POSIX shell, not node.
- **Phase 2:**
