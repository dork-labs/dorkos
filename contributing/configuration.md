...
| `__tests__/migration-append-only.ts` + `merged-migration-hashes.ts` (DOR-1222) | a hash per key of its whole reachable closure — table slice plus every helper it calls — against a pinned value | any change to a merged body or to a helper it reaches, from the moment it merges, with no tag needed    | module-level constants a helper spreads, and anything outside the file |
