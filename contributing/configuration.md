...
7. **State what the default does for the person who never touches it** in `CONFIG_DEFAULT_VERDICTS`' three lists (`apps/server/src/services/core/safe-defaults/default-verdicts.ts`): `no-risk`, `safe`, or `permissive`. A third drift guard, in `safe-defaults/__tests__/default-verdicts.test.ts`, fails until every leaf has a verdict. `no-risk` means the default cannot send data off the machine, grant an agent a capability, or relax a bound — most fields. `safe` means the default _is_ the protective option (a gate that starts closed, a list that starts empty). `permissive` means it is not, which is allowed but must be argued: the entry carries a written reason and the guard rejects an empty one. See ADR 260727-181825.
