...
Both come out once the migration has shipped. `'a config written before DOR-579 is not condemned'` in `config-manager.test.ts` boots a real `ConfigManager` over a real prior-shape file and fails if either is removed early — the mock-store tests cannot see this failure, and neither can `UserConfigSchema.parse`, because Zod strips unknown keys where Ajv rejects them.
