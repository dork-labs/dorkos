...
- **`preserveWidenedLeaves`**, called from the write path, carries the stored value back onto any write that did NOT name that leaf. Without it the loss just arrives quietly: this build read `theme` as `system`, so the section it saves says `system`.
