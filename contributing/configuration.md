...
7. **Document the field** in the Settings Reference table above. The `check-docs-changed.sh` hook will remind you at session-stop if you forget, but doing it inline is cleaner.
8. **Mirror the doc to `docs/getting-started/configuration.mdx`** if the field is user-visible.
9. **Add a test** in `apps/server/src/services/core/__tests__/config-manager.test.ts` that exercises the migration against a realistic stale-config blob.
10. **Wire a CLI flag** in `packages/cli/src/cli.ts` if the field needs one, following the precedence rule (CLI flag > env var > config > default).
...
1. **`tolerateLegacySidebarEncoding`** (`config-manager.ts`) widens the JSON Schema handed to conf so Ajv accepts the old encoding instead of condemning the file. It lives at that seam, not in the Zod schema, because Ajv is the only validator on the config read path and `z.toJSONSchema` emits a pipe's *output* schema — so a Zod `.preprocess` would neither run nor widen what Ajv sees. Keeping it here also leaves the exported types strict and the operator disclosure walker untouched.
