...
| `0.63.0` | `backfillPromoDismissals`         | Writes `ui.promos: { dismissedIds: [] }` onto an existing `ui` block when the section is absent (spec `sidebar-simplification` D4). A no-op ANCHOR, unlike every other body in this table: conf builds Ajv with `useDefaults`, so the section is written in during validation whether or not this runs — measured, not assumed. It earns its line by making the seed reviewable here rather than implicit in a schema default. Seeds EMPTY, and skips an existing `ui.promos`, so it can never bring back a card somebody dismissed.                       |
