...
| ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `__tests__/migration-safety.ts` (DOR-339, DOR-988, DOR-1135)                   | a shipped key's table slice byte for byte, **plus** the normalized source of every declaration in the file it reaches, against the newest release tag | a key authored at or below a released version; any edit — code or comment — to a slice that has shipped; any edit to the BEHAVIOR of a helper or constant a shipped key reaches | anything between merge and tag; prose inside a helper (the slice is byte-compared, the helpers are not); anything one import hop out of `config-manager.ts` — `0.57.0` calls `toSidebarItemRef` in `packages/shared/src/config-schema.ts`, and editing that function is invisible here                                                              |
| `__tests__/migration-append-only.ts` + `merged-migration-hashes.ts` (DOR-1222) | a hash per key of its whole reachable closure — table slice plus every function and constant in the file it reaches — against a pinned value          | any change to a merged body, to a helper it calls, or to a constant it reads, from the moment it merges, with no tag needed                                                     | anything outside `config-manager.ts`, and it is only ONE import hop away — narrowing the `ONBOARDING_STEPS` constant changes what `0.55.0`'s scrub deletes, and editing the `toSidebarItemRef` FUNCTION that shipped key `0.57.0` calls rewrites what its rename produces. Both live in `packages/shared/src/config-schema.ts`; neither moves a pin |
