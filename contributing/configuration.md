...
| `ui.composer.richText`                           | boolean                                                                  | `true`               | Whether the message box shows formatting as you type — bold, headings and lists take shape while you write (DOR-948). Ships **on** (owner decision, 2026-08-12), and only the chat composer reads it; rooms and onboarding stay plain. The Settings → Preferences switch is the way back to the plain box                                                                                                                                                                                                                                                                                             |
| `ui.communityNavigation`                         | object                                                                   | version 1, no owners | Owner-qualified manual Community order and up to 100 remembered room/thread/scroll destinations per owner. It stores opaque IDs only; message drafts and staged files never enter shared config                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `relay.maxAgentTurnsPerAgentPerHour`             | number \| null                                                           | `1000`               | The most turns any ONE agent — or one scheduled task — may be sent through the adapter dispatch in an hour, counted there because every surface crosses it, and without reading who is calling (ADR 260824-120429). `null` = no limit; `0` starts no turns at all, scheduled runs included. `operator-only` to write                                                                                                                                                                                                                                                                                  |
| `relay.maxAgentTurnsTotalPerHour`                | number \| null                                                           | `5000`               | The most turns the adapter dispatch may start on this install in an hour, across every agent AND every scheduled task — the ceiling on what agents messaging each other, and the scheduler, can cost. `null` = no limit; `0` starts no turns at all, so it stops the schedule too. `operator-only` to write                                                                                                                                                                                                                                                                                           |
...
| `memory.provider`                                | string                                                                   | `'builtin'`          | Which backend holds what your agents remember. `'builtin'` is one small `MEMORY.md` beside each agent, on this machine. An id nothing registered falls back to `'builtin'` with one warning, and a registered backend that throws is benched for the rest of the run with `'builtin'` taking over — memory never takes down a turn. Either case is more than a log line: `GET /api/system/memory` reports the configured and active ids and why they differ, and a standing app banner says so in plain words. Read once at boot; a change takes effect on the next restart. `operator-only` to write |
...
| `rooms.responseGate`                             | `'off' \| 'routing'`                                                     | `'routing'`          | Whether an overhearing agent may skip a message that named somebody else, instead of running a whole turn to decide it has nothing to add                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `rooms.maxPostsPerTurn`                          | integer (1--10)                                                          | `3`                  | How many messages one agent may post into a room inside a single turn. The fourth is refused with `TOO_MANY_POSTS_THIS_TURN`                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `rooms.maxCanvasOpsPerTurn`                      | integer (1--10)                                                          | `3`                  | How many times one agent may change a room's shared canvas inside a single turn — opening a document, updating one, closing one. The fourth is refused with `TOO_MANY_CANVAS_OPS_THIS_TURN`                                                                                                                                                                                                                                                                                                                                                                                                           |
| `rooms.maxConcurrentTurnsPerAgent`               | integer (1--8)                                                           | `3`                  | How many conversations one agent may work in at the same time. A message that finds the agent at the limit is held, and starts when one of its turns ends. Never more than one turn per room. `1` is the behaviour before this setting existed                                                                                                                                                                                                                                                                                                                                                        |
| `rooms.repo.enabled`                             | boolean                                                                  | `true`               | Whether a room may have files of its own. Off makes every room behave like a room without files; nothing on disk is deleted, so turning it back on returns everything                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `rooms.repo.worktreeReapDays`                    | integer (1--365)                                                         | `14`                 | How many days an agent's working copy may sit untouched before DorkOS tidies it away. Only an empty one is ever removed — unsaved edits and unmerged work are left alone and surfaced instead                                                                                                                                                                                                                                                                                                                                                                                                         |
| `rooms.repo.maxFileBytes`                        | integer                                                                  | `5242880` (5 MB)     | The largest single file an agent may merge into a room. A merge carrying something bigger is refused whole and says which file it was                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `rooms.repo.maxRepoBytes`                        | integer                                                                  | `524288000` (500 MB) | How large one room's files may grow in total                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `rooms.repo.maxRoomMdBytes`                      | integer                                                                  | `24576` (24 KB)      | How much of a room's `ROOM.md` reaches its agents. Past this size the agents are told the file is too long rather than being sent part of it                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `rooms.repo.mergeQueueWaitMs`                    | integer (1000--600000)                                                   | `30000`              | How long a merge waits its turn before giving up. Merges into a room run one at a time, so two agents finishing together queue rather than collide                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `uploads.maxFileSize`                            | integer                                                                  | `10485760` (10 MB)   | Maximum file size in bytes per uploaded file. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field))                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `uploads.maxFiles`                               | integer (1--50)                                                          | `10`                 | Maximum number of files per upload request. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field))                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `uploads.allowedTypes`                           | string[]                                                                 | `["*/*"]`            | Allowed MIME types (e.g., `["image/*", "text/plain"]`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `agentContext.relayTools`                        | boolean                                                                  | `true`               | Whether the Relay messaging tools are documented to sessions in their context (`tool-filter.ts`); it does not unregister them, and access is still the tier gate's call. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field))                                                                                                                                                                                                                                                                                                                                             |
| `agentContext.meshTools`                         | boolean                                                                  | `true`               | Whether the Mesh discovery tools are documented to sessions in their context. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field))                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `agentContext.adapterTools`                      | boolean                                                                  | `true`               | Whether the chat-adapter tools are documented to sessions in their context. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field))                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `agentContext.tasksTools`                        | boolean                                                                  | `true`               | Whether the Tasks scheduler tools are documented to sessions in their context. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field))                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `extensions.approvedSources` | `Record<string, { path, plugin? }>` | `{}`    | Which copy each approval in `approvedToRun` is for: the extension directory, plus the carrying marketplace plugin when it came inside one (DOR-2383). An approved id with no entry counts as not approved. Seeded empty by the `'0.86.0'` migration; the first discovery binds each older id-only approval to the direct install at `{dorkHome}/extensions/<id>`, and never to a plugin copy. Removed with the approval. `operator-only`, withheld from agent reads (absolute paths) |
...
| Key                        | Type           | Default | Description                                                                                                                                                                                                                                      |
| -------------------------- | -------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `harness.autoSync`         | boolean        | `true`  | Run Harness Sync projection unprompted — on a marketplace install/uninstall, an agent registered against a directory, a skill change, the sweep, or a turn end. `operator-only` to write ([wipe floor](#step-by-step-adding-a-new-config-field)) |
| `harness.autoAdopt`        | boolean        | `false` | Let DorkOS move a plainly-portable skill out of one agent tool's folder into `.agents/skills` on its own, and only inside the agent folders and room folders DorkOS owns. Off everywhere by default (operator-only)                              |
| `harness.approvedHooks`    | string[]       | `[]`    | Hook projections a person has allowed, as `<packageName>@<digest>` (operator-only)                                                                                                                                                               |
| `harness.refusedHooks`     | string[]       | `[]`    | Hook projections a person has turned down, same form and same key (operator-only)                                                                                                                                                                |
| `harness.global.harnesses` | HarnessId[]    | `[]`    | The agent tools DorkOS shares your globally installed packages with. Empty IS off (operator-only)                                                                                                                                                |
| `harness.global.askedAt`   | string \| null | `null`  | When the one-time sharing question was answered, ISO-8601. `null` means never asked (operator-only)                                                                                                                                              |

When `harness.autoSync` is `true` (the default), DorkOS re-projects `.agents/` and installed plugins to every harness a project enables without being asked. It gates all five unprompted triggers — a marketplace install/uninstall, an agent registered against a directory, the `.agents/skills` watcher, its comparison sweep, and the turn-end re-projection ([Harness Sync §10](harness-sync.md#10-triggers--what-starts-a-projection)). Set it to `false` to manage projection manually via `dorkos harness sync`.

`harness.autoAdopt` is off everywhere by default, and it is READ in exactly two places — the boot backfill's per-workspace loop (`services/harness/project-agent-workspace.ts`, per workspace that passed `isAgentHome`) and `RoomWorktreeManager`'s seed-and-project pairing. Both have already established that DorkOS owns the directory they are standing in, so a `true` in somebody's own repository does nothing at all: the three unprompted project triggers never ask, and `dorkos harness sync` prints one line saying so. Turning it on lets DorkOS move a skill from `.claude/skills/<name>` (or any other tool's own folder) into `.agents/skills/<name>`, where every agent tool reads it, and leave the link Claude Code reads it through. The guard is an **allowlist**, not a denylist: only a skill whose frontmatter holds nothing outside the six agentskills.io base fields and whose body carries no `${CLAUDE_…}` token is moved, so a field a vendor adds tomorrow fails closed. Everything else is reported with the one sentence that says why and left where it is. It is `operator-only` for a stronger reason than `harness.autoSync`: turning this on makes DorkOS move a person's own files, unattended. Seeded `false` for existing configs by the `'0.78.0'` migration (DOR-1853), and `dorkos config set harness.autoAdopt true` is the only surface — there is deliberately no Settings toggle.

`harness.approvedHooks` and `harness.refusedHooks` are the record of who may install hooks: shell commands a harness runs unattended, projected into `.claude/settings.local.json` and the generated `.codex/hooks.json` and friends. A package's hooks do not project until a person answers the approval card naming those exact commands, or runs `dorkos harness sync --fix --allow-hooks <package>`, which writes the same entry (DOR-522, DOR-1849). The entry's digest covers the resolved project path and every hook (command, event and matcher), so an update that changes what a package runs, or when it runs it, asks again — which is also how a refusal expires. Recording either side removes the matching entry from the other, so an entry is never in both. Both are `operator-only`, like `extensions.approvedToRun`: an agent cannot write itself an approval, nor delete the record of one being refused. `services/harness/hook-consent.ts` owns both lists and states the guarantee and its limits; `dorkos harness hooks --list` / `--revoke <package>` are the surface. Backfilled empty for existing configs by the `'0.57.0'` and `'0.75.0'` migrations, so nothing already installed is pre-approved and no upgrade turns anything down on somebody's behalf.

`harness.global` is the answer global scope has instead of a manifest. A project's enabled agent tools come from `.agents/harness.manifest.json`; a package installed for all your projects has no repository, so its two decisions live here — the same store, the same reader and the same migration chain as the hook decisions above, which are the same kind of decision. `harnesses` lists the agent tools DorkOS may share those packages with, by linking each package's skills into `~/.agents/skills` (read by Codex, OpenCode, Cursor, Gemini CLI and Copilot) and `<claudeRoot>/skills` (read by Claude Code). There is no separate on/off flag: an empty list IS off, so the two can never disagree. `askedAt` is what makes a decline durable — `null` means the question has never been put, and a timestamp with an empty list means somebody said no and is not asked again. `dorkos harness global --enable <tool>` / `--disable <tool>` / `--list` is the surface, and `--disable` sweeps the links that tool's directory no longer needs before it forgets the tool. Seeded empty and unasked for existing configs by the `'0.77.0'` migration, so no upgrade puts a link in anybody's home directory on their behalf (DOR-1924).
...
A dangling reference (env var unset, file/keychain entry missing) resolves to a typed failure. The runtime credential helper logs the failure reason and injects no key, leaving supported environment or delegated-login authentication available. Claude reads `providers.anthropic` (injected as `ANTHROPIC_API_KEY`); OpenCode reads `providers[<runtimes.opencode.provider>]` (injected as the provider's key). Codex uses its own login/auth files and the projected `CODEX_API_KEY` / `OPENAI_API_KEY` variables. Every Codex client receives a complete `CodexOptions.env`; `runtimes.codex.credentialRef` remains dormant and is not resolved by either the launch or delegated-login path.

Runtime launches and their helper processes use `services/runtimes/shared/runtime-environment-config.ts`, with the pure policy in `runtime-environment.ts`. The owner-only `runtimes.environment.inherit` lists add exact names; reserved server names cannot be added. Always choose the matching runtime/purpose and pass the complete result: omitting `env` or spreading the parent environment afterward restores the exposure this policy prevents. See the [runtime environment settings](../docs/getting-started/configuration.mdx#runtime-environment-settings) for upgrade and restart behavior.
...
The `profile` section is what the user has told DorkOS about themselves (spec `user-profile-onboarding`). It is projected into every session's agent context as a `<user_profile>` block and feeds the role → recommendations mapping in `@dorkos/shared/profile-recommendations`. Every field is local-only; never included in any telemetry payload (pinned by sentinel-exclusion tests on the heartbeat and usage-event catalogs). All five leaves are `expose`, and four of them are `agent-writable` on purpose — agents knowing and updating the profile is the feature.

`profile.displayNameSource` is the exception, and the only `operator-only` leaf here (DOR-1022). It is not a preference but the RECORD of who wrote `profile.displayName`, which since DOR-979 is also the name the roster and the account menu show. No caller supplies it: the three doors that write the name stamp it as a side effect — `PATCH /api/profile` records `operator` (the only door that refuses agents, so the only one that can name a person), `config_patch` records the calling agent, and `PATCH /api/config` and `dorkos config set` record `null`, because with login off neither can tell a person from a process running as them. An agent able to write this leaf through its own capability surface could stamp its own suggestion `operator` and erase the "Suggested by …" note it had raised, which is why `config_patch` refuses it outright.

The `operator-only` verdict carries the usual qualifier, and this leaf is no exception to it: it closes the patch path in every posture, but with Require login OFF a local program that omits its agent header can still write any `operator-only` setting through `PATCH /api/config` — the residual DOR-505 could not close, documented in that route and reproduced against this leaf. Require login closes it. So the accurate claim is "an agent using the surface it is given cannot forge this", not "this cannot be forged". See `apps/server/src/services/identity/display-name-provenance.ts`.
...
| `profile.displayNameSource`         | object \| null             | `null`  | Who wrote the stored `displayName`: `{ kind: 'operator' }`, `{ kind: 'agent', agentName }`, or `null` for "no record". Machine-managed; the only `operator-only` leaf of the profile (DOR-1022)                                                  |
...
| `profile.identityPromptDismissedAt` | string \| null             | `null`  | ISO timestamp when the one-time "what should we call you?" question was closed, by saving or skipping it (DOR-677). Machine-managed. Asked only while the operator has no name or no handle. Local-only; never included in any telemetry payload |
...
| ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `__tests__/migration-safety.ts` (DOR-339, DOR-988, DOR-1135)                             | a shipped key's table slice byte for byte, **plus** the normalized source of every declaration in `config-manager.ts` it reaches, against the newest release tag                                          | a key authored at or below a released version; any edit — code or comment — to a slice that has shipped; any edit to the BEHAVIOR of a helper or constant a shipped key reaches                                                     | anything between merge and tag; prose inside a helper (the slice is byte-compared, the helpers are not); anything one import hop out of `config-manager.ts` — deliberately, because a tag has no escape hatch and a schema symbol sometimes has to change. The pins below cover that hop instead                                       |
| `__tests__/migration-append-only.ts` + `merged-migration-hashes.ts` (DOR-1222, DOR-1732) | a hash per key of its whole reachable closure — table slice plus every function and constant it reaches in `config-manager.ts` **and** in `packages/shared/src/config-schema.ts` — against a pinned value | any change to a merged body, to a helper it calls, or to a constant it reads, from the moment it merges, with no tag needed — including a helper one import hop out, such as the `toSidebarItemRef` that shipped key `0.57.0` calls | anything `config-manager.ts` imports from a third file; and anything inside `UserConfigSchema`, excluded by name so ordinary schema growth does not move the five pins seeding from `USER_CONFIG_DEFAULTS` — so **changing an existing default** those keys seed (`memory.provider`, say) is invisible here, not merely adding a field |
...
The pins live in `apps/server/src/services/core/__tests__/merged-migration-hashes.ts`, one line per key. **Add a key's pin in the same pull request that adds the key** — the guard fails until you do, because a pin added later leaves a window where the body could be rewritten with nothing to show for it. Changing an existing pin is the only escape hatch, and it is a single changed line in a file that exists for nothing else, so it cannot pass review unseen: bump one only with a justification in the pull request that names the population which could have run the old body and says why it is empty. On this repository it never has been — the one bulk repin on record (DOR-1732, ten keys at once) changed no body at all; it widened what the pins cover, so each of the ten hashes strictly more source than before.

**The import boundary is crossed by ten of the keys, and the pins now follow them across it** (DOR-1732). `0.55.0` reads `ONBOARDING_STEPS`, `0.57.0` calls `toSidebarItemRef`, `0.59.0` reads `ComposerPrefsSchema`, `0.64.0` reads `NotificationPrefsSchema`, `NOTIFICATION_PREFS_DEFAULTS` and `EscalationDelaySchema`, `0.65.0` calls `claudeAccountId`, and `0.69.0` through `0.73.0` seed from `USER_CONFIG_DEFAULTS` — ten keys. Until this landed, none of that was watched: editing `toSidebarItemRef` to lowercase every stored path — which changes what every upgrading install's sidebar members become — left all 382 guard tests green. It now moves the `0.57.0` pin, while rewording that function's docblock still moves nothing.

**What still is not watched, and it is one thing.** `UserConfigSchema` is excluded from the walk by name. It is the whole schema, so following it would move five shipped pins for every ordinary field addition, and a pin bumped in a batch is a pin nobody reads. The line that derives `USER_CONFIG_DEFAULTS` from it IS pinned, so swapping the derivation for a literal is seen — but **changing a default that a `0.69.0`-and-later key seeds wholesale is not.** If you change one, check those five keys yourself.

**Where a schema symbol changes and no new key can answer it.** The tag guard freezes what a shipped key reaches inside `config-manager.ts` absolutely, and rightly: those helpers exist for migrations and nothing else. A `config-schema.ts` symbol is shared with the running app and does occasionally have to move: four such changes have landed after the key reaching them merged — `ONBOARDING_STEPS` widened twice (v0.57.0, v0.64.0), the `ComposerPrefsSchema.richText` default flipped (v0.59.0), and the ReDoS fix to `claudeAccountId` at v0.73.0, two releases after `0.65.0` shipped. Freezing those against a tag would have left no legal move at all, so the import hop is covered by the pins, where the answer is a visible repin with the affected population written down.
...
The hash normalizes comments and formatting away, so correcting a stale sentence inside a merged body does not trip it — the byte-identity guard still will, if the key has shipped, which is why the docblock above `CONFIG_MIGRATIONS` says to correct that kind of thing above the key rather than inside it. That is about the TABLE SLICE only: both guards normalize the helpers a key reaches, so a helper's docblock can be corrected in place.

**Both guards now follow a key into the helpers it calls, and it took two goes to get there.** The tag-based guard used to compare the table slice and nothing else, which is a comparison of the CALL rather than of what runs — and most of this table is a composite, with several keys a bare function reference. Seeding a tamper into `backfillAutonomyAcknowledgement`, which the shipped `'0.57.0'` key calls, passed; the identical tamper written inline in the table went red (DOR-1135, found while reviewing DOR-1121). Both rules share one reader now, `__tests__/migration-closure.ts`. They still answer to different authorities and that is the point: a pin is a value in this repository and repinning it is the escape hatch, while a release tag cannot be edited at all, so a key that has SHIPPED has no escape hatch.
...
`0.70.0` is the newest key: `seedRoomRepoDefaults` reserves `rooms.repo` — whether
a room may have files of its own, and the bounds on them (spec `project-rooms`
§3.12, DOR-1591). Read it as the OPPOSITE of the anchor above it. `rooms.repo` is
a **nested leaf** inside a section every stored config already carries, so conf's
shallow defaults pre-write never reaches it and this body is the only thing that
writes it to disk — suppress the body and the upgrade-boot test goes red while
`store.get('rooms').repo` still answers from the copy Ajv filled and discarded.
It is safety-neutral: the block ships enabled, but a room only gets files when a
person gives them to it, and the migration writes no room's binding. Disjoint
from `0.66.0`, which is the only other key that touches `rooms.*` and rewrites
sibling numbers rather than this member.
...
A room's own files: a git repo under the DorkOS data directory, a standing working copy per agent, and the merges that bring that work back (spec `project-rooms`).

`enabled` is the whole feature in one switch. Off, every room behaves exactly like a room without files — no new repo can be created, and a room that already has one stops offering it. Nothing on disk is deleted, so turning it back on returns everything. It is `reach`-stake operator-only: the surface it opens is a writable checkout on this machine that agents run tools in, so an agent that could switch it back on would be re-opening a door a person shut.
...
All four of those survive a config wipe (`PROTECTIVE_CARRYOVERS`): an off switch and three tightened ceilings are choices a person made, and corrupt-recovery landing back on the shipped defaults would reverse every one of them silently. `enabled` is also the one entry this change added to `PERMISSIVE_DEFAULTS`, which raised that list's ceiling from fifteen to sixteen — the raise is argued in the guard itself rather than done quietly.

`mergeQueueWaitMs` is the one leaf here an agent may set. Merges into a room are serialized, so two agents finishing together queue rather than collide; waiting longer buys no file, no byte and no turn, which puts it on the same side of the line as `rooms.replyWaitMinutes`.

```bash
dorkos config set rooms.repo.enabled false
...
How many conversations one agent may work in at the same time (DOR-2104). Offered in Settings → Rooms → **Conversations at once**.

An agent's rooms and chats all run in its own folder (only a repo-enabled room gets a worktree of its own), so two turns at once are two writers in one tree — the contention ADR `260726-170125` (DOR-500) measured, where six agents on one tree clobbered each other and halving the writers roughly doubled what survived. `claimBusyWith` in `apps/server/src/services/rooms/room-claims.ts` therefore counts the live claims on an agent's path and holds a new turn when the count is **at or above** this number. A hold is never a refusal (ADR `260818-234541`): the message waits, the room shows "will pick this up when it finishes in …", and the turn starts when one of the agent's other turns ends.
...
The wire DTO (`GET /api/config` → `rooms.maxConcurrentTurnsPerAgent`) and the held indicator's `heldBehind.severalInTheWay` flag are the two places it reaches a client. The latter exists so the lane stops promising the one room it names when several of the agent's turns are in the way.

```bash
dorkos config set rooms.maxConcurrentTurnsPerAgent 1
```

### rooms.responseGate

Whether listening is free (spec `engaged-response-gate`).

`engaged` is what every channel membership starts on, so an agent you talked to a minute ago is weighing every message that lands after it. Until DOR-1203 that meant a full model turn per overheard message, ended by the agent producing no text — correct, and the exact thing `meta/agent-etiquette.md` E7 says must not cost anything: _"if an agent is charged for listening, restraint becomes something the product punishes."_ DOR-1434 made it worse by raising the reply ceilings roughly tenfold, since those ceilings were the accidental brake.

`routing` skips three shapes, and reads no message text to do it — the inputs are mentions, author kinds, and the `answersEntryId` the room stamps on every agent post:

- the message named **another** agent in the room and not this one (`@nova ship the release`);
- an agent was **answering somebody else's** question;
- an agent was **acknowledging something this agent said**, without asking it anything.

All three are `meta/agent-etiquette.md` E2 — do not answer a question addressed to someone else — and the codebase already made exactly this call for the `#team` fallback seat.

A skipped message is **ambient, not lost**: the read cursor does not move, so the agent sees it as background on whatever turn it does run. Nothing is charged, no session is bound, and no working indicator appears. A message that **names** the agent, and anything in a direct message, is never skipped — being asked creates an obligation, and the gate is deliberately out of reach of it.

Every skip is written to the refusal log as `not_addressed_to_me` with `visibility: 'chosen'` and the rule that reached it, so `jq 'select(.reason=="not_addressed_to_me") | .rule'` tunes this without guessing. `chosen` logs at `info`, unlike the other three visibilities, because it is the expected outcome rather than an incident.

`off` restores the previous behaviour exactly. Operator-only, for the reason every other number here is: an agent that could switch its own gate off would be voting itself a turn per message.

```bash
dorkos config set rooms.responseGate off
```
...
**It is no longer an experiment.** It graduated in spec `full-power-defaults` (D1): the registry entry was deleted in the same change that flipped the default, per the doctrine [above](#experimental-fields), and with it went the only switch the setting had. **Its control moved rather than vanishing:** the Control Center carries it as the **Warm agents** switch (#1209, `ControlCenterSwitches.tsx`), so the one failure mode of this doctrine — a graduation that removes the UI — was avoided by a hair rather than by design. There was a window between the two PRs when `PATCH /api/config`, `dorkos config set` or the file were the only way in; that window is closed. The `0.67.0` migration moves a stored `false` — which the `0.59.0` backfill wrote into every upgraded config — up to `true`, and cannot tell that seed from a person who tried it and turned it off. That caveat is recorded in the migration comment and named in the changelog.
...
- **`expose` to read, and `operator-only` to write since DOR-1497** — the write half changed, the reasoning behind the old `agent-writable` verdict did not. It is still true that this says how work runs rather than whether anybody is asked: the same executable is spawned under the same trust stop, the boundary check runs per dispatch either way, and no credential is reachable through it. What decides the verdict is the [wipe floor](#step-by-step-adding-a-new-config-field) instead. The cost is memory — up to about a gigabyte per warm agent — so the leaf has a `PROTECTIVE_CARRYOVERS` rule, and a leaf a config wipe refuses to reverse cannot be one an agent reverses on purpose. What made the gap bite is that nothing tells a person when the setting goes back on — an agent could hand itself the gigabyte back and they would find out from their machine, not the app. That the Control Center's **Warm agents** switch exists is what makes the verdict cheap: refusing the agent takes nothing away from the person, who reaches for the switch, not for a config door.
...
- **`resolveSessionDefaults` answers the permission tier only when it is handed the runtime's declared modes**, and one place decides whether to hand them over: `seedForNewRow`, from the required `TurnOrigin` its caller passed to `persistSessionRuntime` (DOR-2105). Withholding the descriptors is how an origin that seeds no power says so, so the failure direction is "a preference does not apply" rather than "a session starts looser than intended".
- **Which origins follow the stop is one exhaustive switch**, `permissionSeedForOrigin` in `services/session/origin/turn-origin.ts`. It answers three policies, not two, because _when_ a stop may be seeded differs by origin (ADR `260919-010733`):
  ...
  The two `configured-stop*` policies differ on exactly one row: an UNBOUND row an earlier settings change created (the pre-launch picker above). That row is a person's own, from their own sitting, so their binding write claims and seeds it. For a room the same row is evidence the conversation already exists, and ADR `260908-170643` promises such a conversation is untouched, so a room seeds only a row it mints. The registry honours the difference by handing the seed to the INSERT branch always and to the UPDATE branch (the claim) only for `configured-stop`.

  The switch ends in a `never` check, so adding a turn-starting surface fails the build until somebody decides what power it starts at, and `services/session/origin/__tests__/turn-origin-call-sites.test.ts` fails until the new call site is written into the census with the origin it meant.

- **A surface that needs the stop for the TURN it is about to start asks for it BY NAME**, through `resolveUnattendedPermissionMode` (`resolve-session-defaults.ts`), which maps it onto a runtime's own mode id with the same `resolveTrustStops` the dial uses. Two callers do: a scheduled run (`tasks/scheduled-run-power.ts`, which falls back to `acceptEdits` when no stop is set) and a room turn's first launch (`rooms/room-turn-runner.ts`, which falls back to nothing and lets the runtime decide). Neither seeds the row; that is the origin's job.
- **Relay bindings and agent-to-agent DMs are the exception, deliberately.** They carry a grant a person set on the binding itself, and an absent grant there is not consent (DOR-604), so the global stop must not overwrite it. The bypass clamp on file-sourced schedules is untouched by any of this: content power and operator power stay separate ladders (ADR `260822-235802`, amended by `260908-170643` to add rooms).
...
Room turns take one detour. A room session's `session_metadata` row is written AFTER its turn is known to have started, so a runtime that throws leaves no orphan row — which is too late to seed the turn that is starting. `room-turn-runner.ts` therefore resolves the defaults itself for a session with no row yet and passes them to `triggerTurn`; the registry writes the same values onto the row it then creates, so the second turn inherits the ordinary way. The trust stop rides both halves of that (DOR-1917): the per-turn seed, so the first reply already runs at the operator's level, and the `{ kind: 'room', externalAuthor }` turn origin handed to `persistSessionRuntime`, so the row carries it too (DOR-2105 moved that half out of the runner and into the one mapping; the runner now states a fact and the registry decides). The row seed fills a column still holding NULL, and for a room only on a row the binding write MINTS — a row that already exists is a conversation somebody configured, and it is left alone. One case is not covered by "never overwrites a mode already there": a stored mode the runtime being bound does not DECLARE is replaced by the seed, because the session could not run in it (`claimedPermissionMode`). That leaves one window, documented at the seam: a settings change landing between the row check and the turn is outranked for that single turn, and self-heals from the next one.
...
**Success response (200)** — abbreviated, but every presence flag below really does ship on every answer, and so does `providersConfigured`:
...
```json
{
  ...
  "config": {
    "version": 1,
    "server": { "port": 8080, "cwd": null, "boundary": null, "open": true },
    "tunnel": {
      "enabled": false,
      "domain": null,
    ...
    "ui": { "theme": "dark", "...": "the rest of the exposed ui leaves" },
    "logging": { "level": "info", "maxLogSizeKb": 500, "maxLogFiles": 14 },
    "relay": { "enabled": true, "dataDir": null, "...": "the two turn ceilings" },
    "scheduler": {
      "enabled": true,
      "maxConcurrentRuns": 4,
      "retentionCount": 100
...
**What is NOT in there**, so nobody concludes from the abbreviation that these merely got trimmed:

| Stored                                       | In the answer                                                                  |
| -------------------------------------------- | ------------------------------------------------------------------------------ |
| `tunnel.authtoken`, `tunnel.auth`            | gone; `tunnel.authtokenConfigured` / `authConfigured`                          |
| `mcp.apiKey`                                 | gone; `mcp.apiKeyConfigured`                                                   |
| `cloud.instanceToken`                        | gone; `cloud.instanceTokenConfigured`                                          |
...
Two ways a setting lands on that list. Most are a judgement, field by field: changing it removes or widens a control, or widens what agents do unbidden on your bill. The rest land there without a judgement at all — **every leaf with a `PROTECTIVE_CARRYOVERS` rule is `operator-only`**, because a config wipe already refuses to reverse it and a wipe is the accident (DOR-1497). Both halves of that are pinned by drift guards in `config-write-policy.test.ts`, and the write side refuses **both directions**: the guard reads paths, never values, so an agent may not tighten one of these on your behalf either. That has always been true of the seventeen carried leaves that were already `operator-only`; the change made it true of the other nine.
...
Since DOR-1507 the list above is **pinned by a test**: `configManager.set(` is a protected effect in `services/core/capabilities/__tests__/gate-bypass-scan.test.ts`, allowlisted module by module with the gate each one stands behind. A new purpose-built writer turns that red until its author writes down which door it is and what refuses an agent there. Two neighbours are pinned with EMPTY allowlists for the opposite reason: `configManager.setDot(` takes a caller-supplied path — a general-purpose door with none of what a door owes — and `configManager.reset(` puts a section back to defaults. No server module calls either, and the day one does it has to say why.
