...
**The import boundary is crossed by ten of the keys, and the pins now follow them across it** (DOR-1732). `0.55.0` reads `ONBOARDING_STEPS`, `0.59.0` reads `ComposerPrefsSchema`, `0.57.0` calls `toSidebarItemRef`, `0.65.0` calls `claudeAccountId`, and `0.69.0` through `0.73.0` seed from `USER_CONFIG_DEFAULTS`. Until this landed, none of that was watched: editing `toSidebarItemRef` to lowercase every stored path — which changes what every upgrading install's sidebar members become — left all 382 guard tests green. It now moves the `0.57.0` pin, while rewording that function's docblock still moves nothing.
...
**Where a schema symbol changes and no new key can answer it.** The tag guard freezes what a shipped key reaches inside `config-manager.ts` absolutely, and rightly: those helpers exist for migrations and nothing else. A `config-schema.ts` symbol is shared with the running app and does occasionally have to move — the ReDoS fix to `claudeAccountId` at v0.73.0 landed while `0.65.0` had been shipped for eight releases. Freezing that against a tag would have left no legal move at all, so the import hop is covered by the pins, where the answer is a visible repin with the affected population written down.
