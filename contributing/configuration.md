...
| `ui.sidebar`                            | object                                                                   | see below          | Sidebar organization (DOR-329, DOR-339, DOR-525, DOR-579): pinned items, user-defined groups, per-section sort/collapse/display-filter state, muted items/groups, and the Channels / Direct-messages collapse flags                                                    |
...
| `0.57.0` | `backfillSidebarRoomSections`     | Writes `ui.sidebar.channelsCollapsed: false` and `ui.sidebar.dmsCollapsed: false` onto an existing `ui.sidebar` (rooms sidebar, DOR-525). Both seed expanded, so an upgrade shows the two new sections instead of hiding them behind collapsed headers.                                                                                                                                                                        |
...
`GET /api/config` reports this block **resolved**, as `claudeCode: { resolvedAccount, inherited, accounts[] }`, for the same reason it resolves `claudeCliPath` and `agents.defaultDirectory`: the cockpit cannot see the server process's `CLAUDE_CONFIG_DIR`, so it would otherwise show an empty field where the effective default belongs. Each account also carries `exists`, the same structural check, so the UI can say honestly which registered directories DorkOS can no longer find.
