...
`profile.displayNameSource` is the exception, and the only `operator-only` leaf here (DOR-1022). It is not a preference but the RECORD of who wrote `profile.displayName`, which since DOR-979 is also the name the roster and the account menu show. Nothing writes it through a patch: the three doors that write the name stamp it as a side effect — `PATCH /api/profile` records `operator` (the only door that refuses agents, so the only one that can name a person), `config_patch` records the calling agent, and `PATCH /api/config` and `dorkos config set` record `null`, because with login off neither can tell a person from a process running as them. An agent able to write this leaf directly could stamp its own suggestion `operator` and erase the "Suggested by …" note it had raised. See `apps/server/src/services/identity/display-name-provenance.ts`.
