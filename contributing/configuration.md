...
**It is no longer an experiment.** It graduated in spec `full-power-defaults` (D1): the registry entry was deleted in the same change that flipped the default, per the doctrine [above](#experimental-fields), and with it went the only switch the setting had. **The Control Center's "Warm agents" switch is now its control** (`apps/client/src/layers/widgets/control-center/`, DOR-1431): a graduation that removes the UI is the one failure mode of this doctrine, and the gap was closed rather than left open — `GET /api/config` now exposes the field on the `claudeCode` block so the switch can read real state, and `PATCH /api/config` or `dorkos config set` still work too. The `0.67.0` migration moves a stored `false` — which the `0.59.0` backfill wrote into every upgraded config — up to `true`, and cannot tell that seed from a person who tried it and turned it off. That caveat is recorded in the migration comment and named in the changelog.
