...
**What it does not cover.** The floor rests on every writer going through `ConfigManager`. Editing `~/.dork/config.json` in a text editor — including via `dorkos config edit`, which hands you the raw file — narrows the posture without stamping anything, so an off-then-on round trip done that way can still wake permissions inside one server lifetime. A restart re-establishes the invariant only if the settings are still narrowed when it happens.
