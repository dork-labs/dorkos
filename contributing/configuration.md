...
The one subscriber today is `applyAndWatchConfiguredDefaultRuntime` (`services/core/runtime-registry.ts`), which re-applies `runtimes.default`. The execution defaults (`runtimes.*.defaultModel` / `defaultEffort`) need nothing: they are read when a session is created, so a change governs the next session on its own.
