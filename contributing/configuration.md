...
| `ui.composer.richText`                  | boolean                                                                  | `false`            | Whether the message box shows formatting as you type — bold, headings and lists take shape while you write (DOR-948). Ships off; on at ship time for the chat composer only. Making it default-`true` is gated on the graduation criteria in `specs/composer-rich-text/02-specification.md`                                                         |
...
| `runtimes.claudeCode.activeAccount`     | string \| null                 | `null`        | Absolute Claude config directory new sessions run and bill on; `null` = inherit `$CLAUDE_CONFIG_DIR`, else `~/.claude`   |
| `runtimes.claudeCode.accounts`          | array of `{ path, label }`     | `[]`          | Claude accounts the operator registered, for showing which one a session belongs to (`label` may be `null`)              |
...
| `runtimes.claudeCode.persistentSession` | boolean                        | `false`       | Keep a claude-code session's agent process running between messages instead of starting one per message                  |
...
**Never edit a shipped migration.** Once a migration has run on real users, its body is frozen — editing it would leave users in divergent states (those who ran the old body vs. those who ran the new one).

**An UNSHIPPED key is different, and the difference is the whole point of the rule.** A key strictly above the newest `v*` tag has run for nobody — no release carries it, and a dev tree resolves `SERVER_VERSION` to `0.0.0` and runs none of them — so there are no divergent states to create, and its body may be amended in place. DOR-1121 is the worked example: it reversed the `welcomeBack.offersEnabled` seed inside the still-unreleased `'0.59.0'` rather than appending a key. The licence comes from the TAG, and you check it yourself (`git tag -l 'v*' | sort -V | tail -1`); do not lean on the guard for it. The guard reads only the `CONFIG_MIGRATIONS` table slice, so it sees a body written inline in the table and is blind to any helper function the table merely CALLS — the `backfillWelcomeBackDefaults` DOR-1121 actually edited sits outside its reach, and a tampered `backfillAutonomyAcknowledgement` passes it today.

Reach for a follow-up key only when the first one really has shipped, and check first that the follow-up can tell a value the migration seeded from one a person chose. Where it cannot, appending is the more destructive option, not the safer one: it overwrites the choice.

To fix a migration that HAS shipped:

1. Leave the broken migration in place.
2. Append a **new** migration at the next version that reverses the damage and applies the correct change.

All three rules are enforced, not just documented — within the reach stated above. `apps/server/src/services/core/__tests__/migration-safety.ts` compares every migration key's source text against the newest release tag; `config-manager.test.ts` runs that comparison over the real repository on every CI run, and fails loudly (rather than skipping) in a checkout that cannot see tags. Its source text is the `CONFIG_MIGRATIONS` slice, so it catches a key at the wrong version and an edit to a body written inline in the table, and does not catch an edit to a helper the table calls.
...
4. **Append a migration** to the `migrations` block, under a NEW key strictly greater than the newest `v*` tag (see [Append-only rule](#append-only-rule) — never extend a key that has shipped). Guard every `store.set/delete` with `store.has()` so the migration is idempotent.
...
| `0.59.0` | `backfillComposerPrefs`           | Writes `ui.composer: { richText: false }` onto an existing `ui` block when the section is absent (composer rich text, DOR-948). Seeds off — an upgrade must never change what someone's message box does under them. Skips an existing `ui.composer`, so it never flips a preference back.                                                                                                                                                                                                                                                                 |
...
`0.59.0` is a composite as well, and unlike `0.57.0` it is still OPEN: it carries `backfillWelcomeBackDefaults` and `backfillClaudeCodePersistentSession` (seeds `runtimes.claudeCode.persistentSession: false` on an existing `runtimes.claudeCode` block — one process per message, i.e. today's behavior) alongside `migrateSidebarSectionPrefs`, and the sidebar body sits there deliberately rather than under a key of its own, because the schema that removes the eight keys ships in that same release — a `0.60.0` key would leave everyone upgrading into 0.59.0 with a config the new schema no longer describes. **That reasoning expires the moment `v0.59.0` is tagged.** If a release cuts 0.59.0 before this lands, the body has to move to `0.60.0` or it ships dead.

Being open is also what let DOR-1121 flip `backfillWelcomeBackDefaults` to seed `offersEnabled: true`, a few hours after DOR-1046 wrote it seeding `false`, by amending the body rather than appending a key: nobody had run it, so nobody carries the old seed — and the fill-if-absent rule leaves an explicit `false` on disk exactly where it is.
...
So a rename needs two things the additive backfills never did, and both are back-compat for exactly one release:

1. **`tolerateLegacySidebarEncoding`** (`config-manager.ts`) widens the JSON Schema handed to conf so Ajv accepts the old encoding — both the pre-DOR-579 bare paths and the eight keys the redesign retired — instead of condemning the file. It lives at that seam, not in the Zod schema, because Ajv is the only validator on the config read path and `z.toJSONSchema` emits a pipe's _output_ schema — so a Zod `.preprocess` would neither run nor widen what Ajv sees. Keeping it here also leaves the exported types strict and the operator disclosure walker untouched.
2. **`normalizeSidebarPrefs`** (`@dorkos/shared/config-schema`) converts at read time, so an install that never runs the migration still behaves correctly rather than quietly showing empty groups.

A third piece, `ConfigManager.canonicalUi`, makes the read-time conversion the class's own boundary rather than each consumer's problem — `applyConfigPatch` re-validates a merged config with Zod, which is strict, so an un-normalized read there refuses every config write and silently strips a legacy `agentPaths`.

All three come out once the migration has shipped. `'a config written before DOR-579 is not condemned'` in `config-manager.test.ts` boots a real `ConfigManager` over a real prior-shape file and fails if either is removed early — the mock-store tests cannot see this failure, and neither can `UserConfigSchema.parse`, because Zod strips unknown keys where Ajv rejects them.
...
dorkos config set runtimes.claudeCode.activeAccount ~/.claude2
```
...
- **`activeAccount` sits IN FRONT of the env var, not behind it.** `resolveActiveClaudeRoot()` (`services/runtimes/claude-code/claude-config-dir.ts`) resolves `activeAccount ?? $CLAUDE_CONFIG_DIR ?? ~/.claude`, so a configured account is deterministic instead of depending on the launching shell. At the `null` default the chain is byte-for-byte the SDK's own, so nothing changes for anyone who does not set it. It is stored as a **path, not an index into `accounts`**, so removing an account can never silently repoint the selection at a different client.
...
Both leaves are `operator-only`. A Claude config directory carries its own sign-in, so moving the active account moves the operator's spend onto a different subscription — for the operator this was written for, a different paying client. Under login-on that means the account switcher writes through `PATCH /api/config` with a session cookie and must handle a 403 rather than assume success.

`GET /api/config` reports this block **resolved**, as `claudeCode: { resolvedAccount, inherited, accounts[] }`, for the same reason it resolves `claudeCliPath` and `agents.defaultDirectory`: the cockpit cannot see the server process's `CLAUDE_CONFIG_DIR`, so it would otherwise show an empty field where the effective default belongs. Each account also carries **`isAccountRoot`** — the same structural check, deliberately not named `exists`, because a directory that is really there but holds no `projects/` reports `false` — so the UI can say honestly which registered directories DorkOS can no longer find.
...
The persistent-session opt-in (spec `persistent-session-runtime`, P3). Off — how it ships — every message launches its own SDK `query()`, resumes the transcript, answers and exits, which is what claude-code has always done. On, the session's pump holds that process open between messages, so the next message reaches an agent that is already running.
...
- **It never guesses ON.** An unreadable config (the singleton before `initConfigManager()`, a config written before the field existed) answers `false`. Reading a launch decision must not be able to fail a launch, and an absent value is not consent.
...
**Its row in `docs/getting-started/configuration.mdx` is deliberately missing, and lands with the wiring.** Every other field in that table does something the moment a person writes it; this one does nothing at all until the pump is on the dispatch path, so a user-facing row today would describe behavior that is not there — the demo-claim gate, applied to a settings table. Add the row in the same change that turns the pump on (`supportsPersistentSession: true`), not before.
