...
| `welcomeBack.offersEnabled`             | boolean                                                                  | `false`            | Whether a greeting may also carry a next-step offer. Off by default: an offer is the one part of a return that spends a model turn, because asking the agent is the only honest way to learn whether it has one                                                                                                                                     |
...
All three rules are enforced, not just documented. `apps/server/src/services/core/__tests__/migration-safety.ts` compares every migration key's source text against the newest release tag; `config-manager.test.ts` runs that comparison over the real repository on every CI run, and fails loudly (rather than skipping) in a checkout that cannot see tags.
...
`offersEnabled` is the one leaf here that costs money, which is why it is its own switch and why it ships **off**. The status lines above are derived from session state and spend nothing; an OFFER — "want me to open the PR?" — cannot be known about without asking the agent, and asking runs it for a turn. Turning this on says yes to that spend, and the bound on it is the cap you already set: the only agents ever asked are the ones `maxPosts` let speak, each asked at most once per return. Every way an offer can fail (a busy agent, an exhausted turn budget, a failed turn, an agent with nothing to say) is silence — the status lines still post, and the room writes no notice about the offer that did not happen. An offer that simply takes a long time is late rather than lost: it is waited out to `rooms.lateReplyCeilingMinutes` and posted when it lands, like every other slow turn in a room.
...
dorkos config set welcomeBack.offersEnabled true
```
