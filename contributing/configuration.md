...
8. **Decide whether it must survive a config wipe.** If the default is permissive _and_ a person could move it to a protective value, add a rule to `PROTECTIVE_CARRYOVERS` in `safe-defaults/protected-state.ts`. `~/.dork/config.json` is replaced wholesale by corrupt-recovery and by `reset()`, and without a rule the wipe silently reverses their choice — which is exactly how an explicit telemetry opt-out came back as the opt-out-tier defaults (DOR-584). A field whose default is already protective needs nothing: the wipe lands on it for free.
