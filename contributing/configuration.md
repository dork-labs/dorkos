...
- **Which origins follow the stop is one exhaustive switch**, `permissionSeedForOrigin` in `services/session/origin/turn-origin.ts`. A person watching (`interactive`, from `POST /api/sessions/:id/messages`) and a room turn somebody on this machine started (`room` with `externalAuthor: false`) follow it. A scheduled run, a relay binding, an agent-to-agent DM, a connector event and the test harness seed nothing. The switch ends in a `never` check, so adding a turn-starting surface fails the build until somebody decides what power it starts at, and `services/session/origin/__tests__/turn-origin-call-sites.test.ts` fails until the new call site is written into the census with the origin it meant.
