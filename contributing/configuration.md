...
- **`resolveSessionDefaults` answers the permission tier for attended sessions only.** It answers one only when the caller hands it the runtime's declared modes, and only the attended paths do — `persistSessionRuntime(..., { interactive: true })` from `POST /api/sessions/:id/messages`, and every settings write (`saveSessionSettings`). Withholding the descriptors is how that is enforced, so forgetting to pass them makes a preference not apply — the safe direction.
- **A surface nobody is watching asks for the stop BY NAME**, through `resolveUnattendedPermissionMode` (`resolve-session-defaults.ts`), which maps it onto a runtime's own mode id with the same `resolveTrustStops` the dial uses. Two callers do: a scheduled run (`tasks/scheduled-run-power.ts`, which falls back to `acceptEdits` when no stop is set) and a room turn (`rooms/room-turn-runner.ts`, which falls back to nothing and lets the runtime decide). Naming it at the call site is the point — an escalation should be visible in a diff, not hidden in an argument.
