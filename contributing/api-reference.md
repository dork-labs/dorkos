...
Everything but the ceiling is **refused, not stripped**: a partial write that silently dropped the field would let a caller report the change as done. Naming a refused field at all — `true`, `false`, `null`, or an empty object above it — refuses the whole patch, and the check runs before the schema parse and before the manifest is read, so the answer never depends on the rest of the body being well-formed or on which agent lives at the path.
