...
- **It runs against `main` or it fails.** `workflow_dispatch` is ref-free, so `--ref <a PR's branch>` would post a required context's check run on that pull request's head. `scripts/assert-canary-ref.sh` runs first in every canary job and **fails** such a run — never skips it, because a skipped job satisfies a required context.
