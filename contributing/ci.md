...
| 4   | a gate's `failure_rate` over 7 days is at least `failure_spike_ratio` (1.5) times the previous 7 days, with `failure_spike_min_n` (20) completed runs in each          | amber                                              |
| 5   | a gate's `duration_p90` grew by `duration_growth` (25%) week over week, or job minutes per merged PR grew by `minutes_growth` (20%)                                    | amber                                              |
| 6   | one job caused `repeat_ejection_min` (3) or more failed-checks ejections in `repeat_ejection_days` (7)                                                                 | amber                                              |
| 7   | any red episode on `main`, with its duration                                                                                                                           | red                                                |
