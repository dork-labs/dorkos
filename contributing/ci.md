...
- **Run one now:** `gh workflow run test.yml --ref main` (or the Actions tab). That is the same leg plan §4.9's targeted canary will dispatch on the first failed-checks ejection.
- **What four crons a day actually buy.** GitHub throttles this repo's cron hard: measured over 200+ runs of merge-tail's `*/10` schedule, about 7 runs a day, a median 2.7 h late, p90 5 h, max 12 h. So no schedule here can promise a run inside a named window. What the four slots buy is a break on `main` found in hours rather than days, and the 00:00–02:00 UTC window hit perhaps one day in three — near-certainly within a week. Throttling only ever delays a run, so the 18:3x slots are also late-night slots.
- **`credential-free-build` and `copy-spec-drift` are not in it.** Both read a base SHA to diff against, which a scheduled run does not carry. Neither do the PR-diff checks (`fragment-present`, `no-fragment-under-skip-label`, `version-outranks-base`, `db-check`): they measure a diff, and `main` has none.
- **The list is a registry, not a convention.** `ci/config.yaml`'s `canary.workflows` is what the collector reads a run against, and the census fails when a workflow named there has no `schedule` or `workflow_dispatch` trigger. Otherwise either half could be removed without the other noticing, and a canary that stopped running reads exactly like a healthy `main`.
- **What it does to the data.** Canary runs carry the same gate ids as the PR and queue legs, so every reader that aggregates a gate across events skips them for a gate that also runs on the merge path (`onMergePath`); a gate that runs on nothing but a schedule keeps its own samples, so `headroom` still watches merge-tail, the evals, CodeQL and the collector. `main-green` counts `push` runs only and is untouched. The canary's own results are `snapshot.canary`, and `tracked.time-to-detect` reads the gap between one workflow's consecutive results.
