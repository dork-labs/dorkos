...
**The page never scrolls sideways**, at any width. Prose sits in a 960px column; the SLO table and the trigger list break out to `min(1440px, 100vw - 2rem)`; a table too wide to fit scrolls inside its own panel, never on the page. Under 720px the table becomes cards, CSS only. A date list in any cell is compressed to a range, `day(s)` is never printed, and a sentence still over 120 characters loses its trailing clause, never its numbers. Measured in Chromium at 1440x900, 1024x768 and 390x844: `documentElement.scrollWidth - window.innerWidth` is at most 0 at all three.
