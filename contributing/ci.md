...
PUSH     lefthook pre-push: prettier check on changed files, then turbo test --affected on a ~2-minute budget (TURBO_SCM_BASE pinned to origin/main); over budget PASSES with a note
...
**The local gate is a smell test. The queue is the guarantee.** Say it that way round, because the two are not the same kind of thing and treating them as one is what made the local gate unusable (DOR-2160).

What the local hooks promise: that the cheap, certain mistakes are caught in seconds — a formatting miss that would red the required `lint` job twenty minutes later, a type error in the package you just edited — and that whatever else fits in the budget gets run. What they do **not** promise, and cannot: that your change is good. They test your working tree, and your working tree is not what merges. The queue builds your branch on top of `main` plus everything ahead of it in the queue and runs the full suites against **that**, which is the only tree the merge is about.

So the pre-push test command is bounded at about **two minutes of wall clock, and exceeding the budget is a PASS with a loud note**, not a failure. The reasoning is in `lefthook.yml` beside the setting, and it is short: on this machine the hook's p90 was 26.6 minutes with runs at 43, so failing on the budget would red green trees several times a day, and the thing an agent does about that is `--no-verify` — which also skips the two-second formatting check, the part most worth having. Nothing unreviewed reaches `main` as a result: the queue runs the same suites in full and refuses the merge if they fail. What is lost is the local warning, not the protection.

It is not allowed to be silent about it. The watchdog records a `budget_exceeded` note in the local timings, `ci-local-export` carries it to the data branch, and the daily report says how many hook runs were cut short and how many the operating system killed. If that share climbs, the budget is wrong.

**Heavy local commands hold one of a small number of machine-wide slots** (`scripts/heavy-run-lock.sh`, `local.heavy_run_slots` in `ci/config.yaml`, default 3). Several agents each running a suite is what pinned this box: each `turbo test --concurrency=1` still lets vitest keep a worker per core, so four at once is four machines' work on one. The lock lives in `$(git rev-parse --git-common-dir)/ci-steward/heavy-locks`, which every worktree of the clone shares. It never blocks indefinitely: a slot whose owner is gone is reclaimed, waiting is bounded by `local.heavy_lock_wait_seconds`, and a wait that runs out runs the command anyway with a note rather than hanging or skipping. Fast checks are not capped.
...
A third event type, `O`, is a **note**: an outcome the exit status cannot carry. `budget_exceeded` is a gate its own wall-clock bound cut short and passed anyway, `lock_timeout` a heavy command that gave up waiting for a slot and ran uncapped, `lock_wait` one that waited and got a slot. That is how `tracked.gate-cut-short` can tell a gate that checked and was happy from a gate that stopped checking — the condition on the pre-push gate being allowed to pass on a timeout at all.
