...
A server account is not a membership. Host operators use the `/host/*` routes to create communities, reissue or revoke owner claims, and change hosting lifecycle, and they receive metadata only, never channel content. Host administration shows each new claim as a link to the browser route `/claim#claim=<secret>`. That page's inline bootstrap reads the fragment into page memory and strips it from the URL before any module loads, exchanges it through `POST /api/v1/owner-claims/preflight` for an HTTP-only cookie, and redeems it with `POST /api/v1/owner-claims/claim` after sign-in or sign-up. Only the non-secret community ID is kept in `sessionStorage`, so a reload or OAuth callback can resume. Content authority comes from the live member row inside the resolved community. Each community carries a lifecycle (`pending_owner`, `active`, `archived`, `suspended`, `deletion_pending`) that is checked before member traffic runs.
...
Cloud-hosted communities are a planned consumer of this general host API. The public image retains independent operation; Cloud manages hosting rather than becoming a member or accessing the host database. Machine keys and host-set limits have landed. Import, read-only holds, optional OIDC, and the local app's hosted start/move flow remain separate implementation work at `ea6e06765`. Since that snapshot, `c8fc697ad` (#2037) landed the start/move flow in the local app and its `/api/cloud/communities/*` relay; it stays unannounced until the hosted service serves those routes, and its launch copy is held in `specs/community-host-operator-api/launch-copy/`. The [system map](system-architecture.md#cloud-hosted-communities-planned-management-path) and [public host specification](../specs/community-host-operator-api/02-specification.md) separate shipped primitives from that intended connection.
