...
| **Drawer entrance** | `ProfileDrawer`               | 300ms rather than the Sheet primitive's 500                                                                    |
...
- **Travel is won structurally, not with `layoutId` — and `popLayout` needs a forwarded ref.** A card animates to a new position only while it stays the _same component instance_, so both roster arrangements are one flat list of grid children (a cluster header is a `col-span-full` row, not a `<section>` wrapping a nested grid). `layoutId` looks like the shortcut and is a trap: it hands the element to motion's shared-layout system, which animates it only on a handoff between trees — so surviving siblings stop animating entirely — and an exiting `layoutId` element inside `AnimatePresence` waits for a handoff that never comes and is **never unmounted**, leaving invisible absolutely-positioned ghosts in the DOM. Separately, `AnimatePresence mode="popLayout"` writes `position: absolute` onto an exiting node through a **ref**, so any component between it and the DOM must pass `ref` along; without that the mode silently does nothing and every survivor waits out the fade before closing the gap. All three were invisible to jsdom, to typecheck and to lint, and all three were found by measuring element positions frame by frame in a real browser.
- **A reveal holds its width at rest — and does not exist where there is no hover.** Animating `width`/`max-width` reflows, and a suffix that appeared by taking space would shove the thing you are pointing at, so the owner echo is in the DOM from first paint at `opacity-0` and only opacity moves. The cost is stated rather than hidden: a long handle truncates slightly sooner. That cost is only worth paying where the preview can be seen, so the echo is gated on `[@media(hover:hover)]` — the query asks about the **pointer**, not the viewport, so a phone reserves nothing while a narrow desktop window keeps the echo it can use.
- **Retime a shared primitive at the call site, not in the primitive.** The 300ms lives on `ProfileDrawer`'s `ResponsiveSheetContent` className, which `cn`'s tailwind-merge lets outrank the primitive's own duration. Changing `sheet.tsx` would have re-timed Settings' panels too — a decision about every sheet in the app, which this was not.
