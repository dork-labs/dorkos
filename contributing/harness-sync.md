...
- **Namespacing is mandatory, and `__` alone does not mean managed.** Installed skills are always `<pkg>__<name>` so they can never overwrite an authored skill. The predicate for "this is an engine-managed projection" is the PAIR: the name carries `__` **and** the entry is a symlink, because a projection is the only thing the engine writes and it only ever writes one as a link. Both halves are required everywhere it is asked — `scanSkillDirs` (`scan/scanner.ts`), the orphan sweep (`sweepInstalledOrphans`), and the `.gitignore` patterns — so a real directory a person named `my__helper` is authored content: scanned, projected, and never swept (DOR-1844).
