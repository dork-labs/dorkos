...
- **Namespacing is mandatory, and `__` alone does not mean managed.** Installed skills are always `<pkg>__<name>` so they can never overwrite an authored skill. The predicate for "this is an engine-managed projection" is the PAIR: the name carries `__` **and** the entry is a symlink, because a projection is the only thing the engine writes into these dirs and it only ever writes one as a link. `scanSkillDirs` (`scan/scanner.ts`) and the orphan sweep (`sweepInstalledOrphans`) both ask for both halves, so a real directory a person named `my__helper` is authored content and is scanned and projected like any other skill (DOR-1844). Two things still read only the NAME, and both cost an authored `__` skill something. The `EPHEMERAL_GITIGNORE_PATTERNS` globs (`.agents/skills/*__*`, `.claude/skills/*__*`) cannot ask git what a symlink is, so such a skill's SOURCE is gitignored and never reaches a teammate's clone. And `sweepInstalledOrphans` builds its keep-set from `provenance: 'installed'` actions only, so with `sweepOrphans: true` — what auto-projection uses after every install and uninstall — it deletes the authored `.claude/skills/my__helper` link the same `applyPlan` just created (reproduced 2026-09-07; the source directory is untouched and Codex, which reads `.agents/skills` natively, is unaffected). Fixing the second one means keeping every `symlink` action's target rather than only the installed ones. Until then: **`__` is a bad name for an authored skill**, and the engine no longer pretends the skill does not exist.
