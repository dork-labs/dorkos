...
| `deliverIntoTurn({ mode: 'stage' })`  | appends to the process; the next scripted answer repeats the words                                                                            | refused — only reachable by a caller that ignored `canStageSession` |
