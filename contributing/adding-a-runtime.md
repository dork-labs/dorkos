...
### Disposition honesty (C1, C2, C3, C6, C7)

The three disposition flags above are not free to declare — the suite holds every runtime to them, because **a capability that is not conformance-tested is one a runtime can lie about**, and a lie here sends a person's steer somewhere it did not go. Five cases enforce it, and they follow the same "measure, don't guess" stance as presence: fabrication fails, honest absence passes or skips by name.
...
- **C7 — per-session steerability.** Only for a runtime that implements `canSteerSession`; the rest take a named skip. It answers a boolean for a session it has never heard of as readily as for a live one, and it never reports a steerable session on a runtime that declares `supportsSteer: false`. No driver; the whole point is that it is answerable at rest, because the composer asks it before the turn it would steer is open.
