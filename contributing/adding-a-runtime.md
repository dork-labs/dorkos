...
- **A dead sign-in is DorkOS's sentence, never the backend's.** Route every failure you turn into an `error` event through `describeRuntimeError({ runtimeType, message, code })` (`packages/shared/src/runtime-error-classification.ts`). It passes ordinary failures through verbatim, and when it recognises an expired, revoked or invalid credential it swaps in one DorkOS sentence naming YOUR runtime, tags it `category: 'auth_error'` (what earns the client's "Fix sign-in" affordance), and moves the backend's words to `details`. Do it on **every** channel a failure can arrive on — codex has three (an error item, `turn.failed`, a thrown stream) and they must agree. Before DOR-1656 a single Claude Code expiry reached people in two different voices depending on which SDK channel caught it, one of them a raw CLI literal. Wire `RuntimeConformanceOpts.authFailure` with a scripted credential failure plus the exact vendor text you scripted it with, and the suite holds you to all three halves.
