...
**You must decide, in writing, whether you can say when the person last wrote.** `Session.lastUserMessageAt` is half the sidebar's Today order key (`lastInteractionAt = max(lastUserMessageAt, userLastOpenedAt)`, spec `sidebar-now-today-library` BC-16), and it exists because `updatedAt` moves whenever the _agent_ writes — ordering on that makes rows jump under a cursor. There are two honest answers and the suite makes you pick one:

- **You can say.** Wire `lastUserMessageAtSession`: a probe returning a Session for a conversation a person wrote to **and an agent worked on afterwards**. The suite asserts the reported instant is strictly earlier than that session's `updatedAt` — the only check that tells a real derivation apart from `updatedAt` under a second name, which is also why a probe fixture that ends on the person's turn is rejected.
- **You cannot.** Declare `lastUserMessageAtOmittedReason` with a sentence saying why (whitespace declares nothing). The suite then asserts the field is **absent** on a session that has just taken a user message — not null, not an empty string, not `updatedAt` quietly filling the gap. Implementing the field later means _deleting_ that reason.

Claude-code takes the first arm (it rides the transcript tail read the session list already performs); codex, opencode and test-mode take the second. Derive it inside a pass the listing path already makes — this endpoint is read on every cockpit boot, and a whole-conversation read per row to answer it is the wrong trade. The rules live in `chooseLastUserMessageAtArm` / `evaluateLastUserMessageAtPresence` / `evaluateLastUserMessageAtOmission` (`packages/test-utils/src/runtime-conformance.ts`), and `packages/test-utils/src/__tests__/runtime-conformance-last-user-message.test.ts` proves each one rejects the wrong answer.
