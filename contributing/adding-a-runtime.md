...
- **Interactive flows** (`approveTool`, `submitAnswers`, `submitElicitation`, `stopTask`, `interruptQuery`): return `false` for interactions your backend cannot support — except the two STOP verbs, which answer an `InterruptReceipt` and say `not-running` / `no-open-turn` when there is nothing to stop (see the next bullet), and declare that honestly in capabilities (Codex sets `supportsToolApproval: false` because `codex exec` closes stdin after the prompt; OpenCode sets it `true` and routes `approveTool` to `POST /session/{id}/permissions/{permissionID}`). `approveTool`'s `options` carries `alwaysAllow` on an approval and `denyReason` on a refusal; deliver the reason to your backend if it has a channel for one, and — this is the part that matters — only tell the projector `reasonGiven: true` when you actually did. The transcript receipt's "agent was told why" is that flag and nothing else, so a runtime that drops the reason must stay silent rather than claim it (OpenCode's respond endpoint takes `once`/`reject` and does exactly that).
- **Stop is bounded (C11, DOR-1299).** If `interruptQuery` (or `stopTask`) awaits a backend ack over any wire — an SDK control call, an HTTP request to a sidecar — that ack can go unanswered forever on a wedged backend, and an unbounded `await` there means Stop does nothing until the backend ends on its own. Race the ack against a wall-clock bound instead (claude-code's `sessions/bounded-control.ts`, `STOP_ACK_TIMEOUT_MS`; opencode's `INTERRUPT_ACK_TIMEOUT_MS` in `runtime-constants.ts`) and answer honestly on expiry with the receipt that names what happened — `acked` only for a stop the backend confirmed, `closed` when you escalated to a session-scoped kill, `unconfirmed` when nothing answered and you have no escalation, `failed` when the call itself blew up (claude-code closes the CLI subprocess, so its expiry is `closed` / `ack-timeout`; opencode manages exactly ONE `opencode serve` child for the whole server, shared by every session across every project, so it has no session-scoped escalation and its expiry is `unconfirmed` / `ack-timeout`). A backend with nothing to await — a synchronous local abort, like codex's `controller.abort()` — is bounded by construction and owes the suite nothing extra. Conformance case C11 proves it: wire `RuntimeConformanceOpts.hangingInterrupt` to stage a genuinely open turn whose interrupt call your test double never acks, and the case races your real `interruptQuery` against a generous ceiling; omit the driver and C11 still runs, asserting the at-rest call resolves without spending real wall-clock time (the shape a truly synchronous interrupt has). The driver returns the EXACT receipt your adapter must settle to on expiry, so a runtime that silently swapped `closed` for `unconfirmed` goes red.
- **Stop answers a receipt, never a boolean** (spec `runtime-interrupt-receipts`). `interruptQuery` and `stopTask` return `InterruptReceipt` — one of `acked` (your backend confirmed the stop), `closed` (DorkOS ended the process or turn after the graceful path failed — a SUCCESS, not an error), `not-running` (there was nothing to stop), `unconfirmed` (the request went out and you cannot say whether it landed; the turn may still be running), or `failed` (the call could not be delivered) — plus the `reason` your mapping owes and your own `type` in `runtime`. **Never throw for an ordinary refusal**: `failed` / `delivery-failed` is the receipt for that, the same rule ADR `260816-143752` set for `deliverIntoTurn`. Never report a turn as ended that you did not observe end — the client says "stopped" only about `acked` and `closed`, and "stop requested" about the rest, so a hopeful `acked` becomes a sentence telling a person their agent stopped while it goes on spending their money. Conformance I1, I2 and I5 gate all of it.
...
- **Storage queries** (`listSessions`, `getSession`, `getMessageHistory`, `getSessionSnapshot`, `subscribeSession`, `subscribeSessionList`, `getSessionTasks`, `getSessionETag`, `getLastMessageIds`, `getSessionCwd`, `getSessionAccount`, `readFromOffset`): serve from your backend's native storage or the EventLog (see the split below). `getSession` returns `Session | null`, never throws, and must stamp `runtime: this.type`. `getSessionCwd` is optional (DOR-1322): a synchronous, best-effort answer to "what directory is this session id bound to right now", using only a LIVE binding your adapter already holds — no directory lookup, no filesystem scan. Omit it and callers fall back to an explicit `projectDir`; implement it and C10 holds you to the same shape `getLastMessageIds` already assumes internally (claude-code's is `this.sessionStore.findSession(sessionId)?.cwd`): it must answer `undefined` for an id you have never heard of, and **must never throw, for any id** — a route that calls it is on a graceful-degradation path and has nowhere to send an exception.

  `getSessionAccount` is its sibling and optional on the same terms (DOR-1682): synchronous, never-throwing, and answering _which of your backend's credentials_ a turn on this session runs on. **Implement it only if your runtime genuinely has more than one sign-in.** Codex and opencode each have exactly one home directory, so they omit it, and the sign-in watch (`services/observability/runtime-signin-watch.ts`) reads that absence as "do not distinguish" and behaves exactly as it did before accounts existed — omission is the right answer, never a gap. Claude-code implements it because a credential belongs to an account root (`CLAUDE_CONFIG_DIR`) and DorkOS supports several; without it, a clean turn on a healthy account resolved a dead one's standing condition, which cancelled the escalation ladder every time and meant the phone leg never ran.

  Two rules if you do implement it. The string is **opaque** — consumers compare it with `===` and never parse it, so you may answer with a path, an email, a workspace id, anything you can tell your own credentials apart by. And it is **canonical for the runtime**: every session on one account answers the same string, whichever route put it there. That second rule is the one that bites, because the watch's two edges are different sessions — one turn discovers the dead credential, a LATER turn proves it works again — so an identity spelled two ways reads as two accounts and the notice stands forever. Claude-code's answer is `path.resolve`d for exactly this reason: the launch ladder's rungs hand back config-spelled roots verbatim, and nothing in the schema makes them agree. Read the account **late** — at the edge that needs it, never at turn setup — because a brand-new session's account is decided by its launch, and the launch has not happened until the first event is pulled.
...
- **A dead credential must reach the user as `category: 'auth_error'`, in human copy, with the backend's words under `details` — on every channel it can arrive on.** That is the invariant; `describeRuntimeError({ runtimeType, message, code })` (`packages/shared/src/runtime-error-classification.ts`) is the usual way to satisfy it, and claude-code's result mapper shows the carve-out: it needs its own `mapErrorCategory` for the non-auth subtypes (`max_turns`, `budget_exceeded`), so it calls `detectAuthError` + `describeAuthError` directly instead. Either way the copy comes from that module, never a string you type — before DOR-1656 one Claude Code expiry reached people in two different voices depending on which SDK channel caught it, one of them a raw CLI literal. Three things this invariant needs from you:
  - **Every channel, not the obvious one.** Codex has three (an error item, `turn.failed`, a thrown stream). The item is the one live traffic uses, and `turn.failed` dedupes against it, so fixing only the terminal channel changes nothing a user sees.
  - **A category HIDES the message.** `ErrorMessageBlock` renders the category's fixed copy in place of `message`, falling back to `message` only when no category is set. So never stamp a category on a channel whose message is the only account of the failure — codex's diagnostic error item stays category-less unless the failure really is a credential one.
  - **A diagnostic channel needs a narrower test.** One that also carries failures about a TOOL rather than the session (again the codex item) must pass `unambiguousOnly` to `detectAuthError`, or a tool's own "Failed to authenticate with server github" gets answered with sign-in advice about your runtime.
  - **Prove it.** Wire `RuntimeConformanceOpts.authFailure` with a scripted credential failure plus the exact vendor text you scripted it with — script it on the channel a real failure uses, not the tidiest one — and the suite holds you to category, copy and details together.
...
**`mediaOutput` is required, and `'none'` is a real answer.** Say what happens to an image your model or your tools produce: `'attachments'` when you store it through a `SessionAttachmentStore` and emit an `image_attachment` event referencing the stored bytes, `'none'` when you do not carry it. Declare it from what your adapter DOES, not from what its backend could do, and resolve it per INSTANCE when it depends on wiring: all three production adapters answer `'attachments'` only when the composition root actually handed them a `SessionAttachmentStore`, and `'none'` when it did not, because the field is a promise and a runtime with nowhere to put a picture must not make it (DOR-1664). Codex is the honest example of a real ceiling: `@openai/codex-sdk`'s `ThreadItem` union carries no image OUTPUT item at all, so it can only ever surface images an MCP tool returned, and its declaration describes that rather than pretending to more. Declaring `'attachments'` obliges you to wire the `mediaTurn` opt: a turn that really produces an image, drained through your real adapter, so the suite can check the picture is announced by URL and never inlined as bytes. **Script that turn so it publishes the same image TWICE.** The gate's other half is "one picture, one announcement", and it only bites when the driver actually republishes — a fixture that emits each image once passes it for a reason unrelated to the property, which is how both claude-code and codex first shipped an inert half-gate (DOR-1664). Whatever re-delivery your backend really has (OpenCode's cumulative `message.part.updated`, a replayed `tool_result`, a repeated terminal item), script it. That last rule is the load-bearing one — the session stream is replayed whole on every reconnect, and its replay buffers hold a bounded window (capped in both events and bytes), so an inlined image is re-sent forever and spends on itself the window the turn's own words needed (ADR 260901-135657). Never put base64 in a part or an event; put it behind the store and carry the URL the store answered.
...
### Disposition honesty, cwd resolution and account identity (C1, C2, C3, C6, C7, C9, C10, C12)

The three disposition flags above are not free to declare — the suite holds every runtime to them, because **a capability that is not conformance-tested is one a runtime can lie about**, and a lie here sends a person's steer somewhere it did not go. Six cases enforce it, and they follow the same "measure, don't guess" stance as presence: fabrication fails, honest absence passes or skips by name. C10 and C12 are unrelated to steer/stage but live in the same catalogue for the same reason: each is an optional per-session method held to the same "answer or skip by name, never fabricate" bar.
...
- **C12 — account identity (DOR-1682).** Only for a runtime that implements `getSessionAccount`; the rest take a named skip, and for a runtime with ONE set of credentials that skip is the correct outcome, not a gap. Three claims. It ANSWERS `undefined` for an id it has never heard of rather than throwing or guessing — the caller is the sign-in watch, wrapped around somebody else's turn, and a guess files a dead credential under the wrong key. It NAMES the account once a turn has actually run, because a constant `undefined` is indistinguishable from not implementing the method and the absence is already a supported answer. And two DIFFERENT sessions on one account answer the SAME string, which is the claim the watch depends on: its failing edge and its resolving edge are never the same session. That last one fails any identity derived per session; an identity that varies with how a session was STARTED needs a fixture that can produce two spellings, which only your own suite can build (claude-code's sits beside its `runtimeConformance` call and drives the launch ladder's two spellings of one root). Numbered C12 because C11 was already taken by the bounded-interrupt case.
