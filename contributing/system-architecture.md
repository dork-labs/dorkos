...
**Code snapshot:** `f4d1e36db8787aa5e52c2ea35b3d051129f4de07`, with the
architecture-doc foundation from `a203bad52ea3fde6b0b55726ccfbe5294763e654`,
reviewed 2026-09-16. “Implemented” below means present in this public source snapshot, not a claim that a hosted deployment or every platform has passed end-user verification. Cloud is described only through public app code and `packages/cloud-api`.
...
| Community port and actual wiring       | [CommunityAdapter](../packages/shared/src/community-adapter.ts), [remote Community implementation](../apps/server/src/services/communities/remote), [qualified Community routes](../apps/server/src/routes/remote-communities.ts), [remote room stream](../apps/server/src/services/communities/remote/remote-community-stream.ts) |
...
| `apps/community`                | Independent Hono service with its own browser, PostgreSQL, sign-in, channels, files, exports, and pairing endpoints. Approved local installations participate through qualified local-server routes and a remote-room SSE stream. |
...
| ------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Transport`         | How the React client reaches app operations         | `HttpTransport` uses HTTP, WebSockets, and Community SSE; `DirectTransport` calls an in-process subset.                                                                                                                                                                           |
