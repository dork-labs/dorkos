...
The two `destructive` tools, `tasks_delete` and `mesh_unregister`, appeared in those lists but were never actually exposed: they are gated in the handler by `gateHandRegisteredMcpTools` (DOR-468), a layer `allowedTools` cannot reach. The real exposure was the 29 to 34 `act` and `observe` tools. That split is the argument for where enforcement lives: consequence is gated by tier in `services/core/mcp-tool-gate.ts`, below every caller, and never by a list of names a config toggle can rewrite.
