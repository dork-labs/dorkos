...
- `persistSessionRuntime(sessionId, type, origin, agentPath?)` — records the binding when the session starts, **first-write-wins** (a session's runtime never changes after that). A row created earlier by a settings change carries no runtime, and this call claims it; a row that already names one is left alone. `origin` is a required, exhaustive `TurnOrigin` union (`services/session/origin/turn-origin.ts`, DOR-2105) naming what started the session — `interactive`, `room`, `schedule`, `relay-binding`, `agent-dm`, `connector-event`, `test-harness`. It is the only input to how much power the new row is seeded with, through the single `permissionSeedForOrigin` mapping, which answers `configured-stop` (seed on insert and on a claim — `interactive`), `configured-stop-on-insert` (seed only a row this write mints — a `room` nobody bridged) or `none` (everything else). A new turn-starting surface cannot compile until that switch handles it; the policies and the reason they differ are ADR `260919-010733`
...
`CanvasService` (`apps/server/src/services/canvas/`) owns the `canvas_documents` table and everything that is about the TABLE: dedupe by source key, the twelve-document LRU, the edit lock, `rev` ordering, and publishing. It is **its own service domain** because it serves two owners — a room's shared table under a `room:<id>` scope, and one person's session canvas under `session:<id>` — and a canvas that serves both is a rooms concern in neither direction. Leaving it under `services/rooms/` would have `services/session/` importing from `services/rooms/` to answer a question with nothing to do with rooms.

`RoomCanvasService` (`apps/server/src/services/rooms/canvas/`) is the room FLAVOUR over it: membership, the archived-room refusal, the six-verb allow-list, the per-turn ceiling and the ledger. It still hangs off the room service (`getRoomService().canvas`) because the one line a turn writes about it is a room entry, and that class owns the single write path into a room's log.

A session's canvas is the same table under a `session:` scope (spec `canvas-agent-seat`), and the differences are decisions rather than omissions: no per-turn ceiling (an audience of one, who asked for the turn), and no ledger (a session's history is its transcript, which already records every `control_ui` call). A session's changes travel as a `canvas` SESSION event — seq'd, ingested through the projector every runtime feeds, and replayed from the ring, so unlike a room's frame it needs no whole-set resync. `HTTP /api/sessions/:id/canvas` is the person's door onto it and refuses any agent `PEOPLE_ONLY`; `control_ui` and `ui.read_canvas_document` are the agent's, and neither takes a session id it could point somewhere else.
...
**A change wakes nobody** (ADR `260911-200302`). Three channels and no fourth: a `canvas` section in every turn's room context, one coalesced entry per turn composed from the service's own per-turn ledger, and an ordinary `@mention` when an agent wants eyes. The `canvas` stream frame is modelled on `reaction` rather than on `entry` — whole-document state, no `seq`, re-sent in full on a resume — so the room stream keeps exactly one cursor. `apply` appends every operation it applies to a per-`turnId` ledger whichever caller called it, and `finishTurn(turnId)` — called once from the collector's `finally`, so a turn the deadline killed still reports what it put up — composes that one line, posts it and clears the ledger. Composing from what the tap happened to observe instead would make "has a row" and "is named in the entry" two different sets.

**Reading it is a room capability, not a claude-code tool.** `read_canvas` is registered in `services/rooms/room-capabilities.ts` as `rooms.read_canvas` on both MCP servers, so claude-code, codex and opencode reach it identically, membership-gated by `callerAuthor` like every other room verb. A document whose tree the READER could not already read comes back as metadata plus one sentence, checked at read time against the row's stored `resolvedCwd` — which is what makes §8.1's property ("a canvas document never lets a member read a tree it could not already read") hold for a member who joined after the document was opened.

**On the client, the room's table is stream-hydrated state, never a query cache and never `localStorage`.** `app-store-room-canvas.ts` holds it keyed by room, filled from `snapshot.canvas` on a cold connect and kept current by the `'canvas'` branch in `use-room-stream.ts`; a resync replaces the whole set rather than merging, which is how a close missed while disconnected self-corrects. The Canvas and Browser right-panel contributions (`app/init-extensions.ts`) gain `routeShowsRoom` and render `RoomCanvasBody` there instead of the session slice; `canvasViewForContent` (`@dorkos/shared/canvas-view`) is the single definition of which view a document belongs to, derived from the viewer dispatch so the tab split cannot drift from it. Every write from the client is a request to `routes/room-canvas.ts`, never a local mutation — which is what keeps what one viewer sees identical to what everyone else sees.
