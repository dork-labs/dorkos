...
`CommunityAdapter` (`packages/shared/src/community-adapter.ts`) is the fourth swappable seam beside `AgentRuntime`, `Transport` and `ConnectorProvider`. It lets one local server read and write rooms in more than one place — this machine's SQLite rooms, a foreign relay, a hosted community — without the client, the router or the session spine learning that more than one place exists. **The client's `Transport` is unchanged**: the seam is entirely server-side, so keys never touch the browser and there is one render path and one streaming model.
...
**Actual wiring:** `GET /api/rooms` lists local rooms through caller-aware `RoomService` operations, not through the single-identity local port. It queries registered remote communities but reports their unavailable rooms through warnings; the local UI cannot yet resolve those remote addresses. The Buzz implementation is not exported or registered at startup. The independent `apps/community` service now has browser chat, invitations, files, exports, and pairing endpoints, while its connection to the local app is still in development. See [Community development](community-server.md).
