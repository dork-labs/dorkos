...
- **Re-key on rebind.** Nothing is stranded under a retired id, because the adapter moves the row the moment it binds a new canonical one (`SessionStore.rebindSdkSession` → `rekeySessionSettings`, DOR-493). Claude-code is the only runtime that aliases at all; the others return `undefined` from `getInternalSessionId` and never re-key. Without the move, a turn sent under the canonical id after eviction or a restart hydrates a miss and the runtime ENFORCES its default mode instead of the operator's.
