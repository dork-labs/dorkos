...
| -------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/server`  | `apps/server/src/env.ts`  | DORKOS_PORT, DORKOS_HOST, NODE_ENV, DORKOS_DEFAULT_CWD, DORKOS_BOUNDARY, DORKOS_LOG_LEVEL, DORK_HOME, DORKOS_VERSION_OVERRIDE, CLIENT_DIST_PATH, MCP_API_KEY, DORKOS_TASKS_ENABLED, DORKOS_RELAY_ENABLED, DORKOS_A2A_ENABLED, DORKOS_CLOUD_URL, DORKOS_ALLOW_INSECURE_BIND, DORKOS_TRUSTED_HOSTS, DORKOS_MANAGED_BY, DORKOS_SKIP_INSTANCE_LOCK, TUNNEL_ENABLED, TUNNEL_PORT, TUNNEL_AUTH, TUNNEL_DOMAIN, NGROK_AUTHTOKEN, DORKOS_CORS_ORIGIN¹ |
| `apps/client`  | `apps/client/src/env.ts`  | MODE, DEV (Vite built-ins)                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `apps/site`    | `apps/site/src/env.ts`    | NODE_ENV, NEXT_PUBLIC_POSTHOG_KEY, NEXT_PUBLIC_POSTHOG_HOST, BETTER_AUTH_SECRET, BETTER_AUTH_URL, GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, RESEND_API_KEY, RESEND_FROM                                                                                                                                                                                                                                 |
...
| DORKOS_ALLOW_INSECURE_BIND | server | boolean                  | `false`                          | Permit a non-loopback bind without a login, and switch OFF both request-level locality checks: the `/api` host guard (`middleware/host-guard.ts`) and the local-only gate on the runtime connect/provision routes (`routes/runtimes.ts`). Set only by container images that own their own network boundary (both `Dockerfile` targets set it). |
