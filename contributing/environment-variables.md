...
| DORKOS_ALLOW_INSECURE_BIND | server | boolean                  | `false`                          | Permit a non-loopback bind without a login, and switch OFF the `/api` host guard. Set only by container images that own their own network boundary (both `Dockerfile` targets set it).                                                          |
