...
2. **Writes the record** (`lib/installed-files.ts`) into the staged tree, so it activates with the package: every shipped regular file and its SHA-256. The record never lists owned paths (`node_modules`, and `package-lock.json` when the npm step ran), the installer's own files, reserved paths, or an agent package's four identity files (`agent.json`, `SOUL.md`, `NOPE.md`, `MEMORY.md`), which DorkOS writes and which are the agent's.
