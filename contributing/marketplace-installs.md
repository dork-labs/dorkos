...
- **Prepare** (DOR-2320): `POST /packages/:name/prepare { projectPath?, installRoot? }` runs `rebuildRecordStrict` on one installation and answers `{ outcome, message }`. `installRoot` narrows `locateInstallRoot` and can never widen it. The route is deliberately not tier-gated: it writes only a record that must match the live files byte for byte, so it cannot change what runs or claim a person's file. It is also exposed as `dorkos marketplace prepare <name>`, as the Prepare button on an older install's row, and as `prepareMarketplacePackage` on the Transport.
...
| POST   | `/packages/:name/prepare`   | `{ projectPath?, installRoot? }`                             | `{ outcome, message }`: records an older install's files from its exact commit, or says why not (§5.3)           |
