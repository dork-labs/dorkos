...
3. **Rebuilds a missing record** for an install made before records existed (`lib/legacy-record.ts`): the sidecar's `commitSha` is fetched exactly (`fetchAtCommit`, by the install-time name so the `trees/` cache is reused) and trusted unless more than 10% and at least 3 of the present recorded files differ; otherwise, and when nothing can be fetched, a file is the package's only if an obtainable tree has the same bytes at that path.
...
- **`rebuildRecordStrict(root)`** (`strict-record.ts`) gives an install made before records existed its record, and it never guesses:
  ...
  - It has no byte-matching fallback, unlike `rebuildInstalledFiles`, which the update and uninstall paths still use.
...
- **Check files** (DOR-2320): `POST /packages/:name/check-files { projectPath?, installRoot? }` runs `rebuildRecordStrict` on one installation and answers `{ outcome, message }`. `installRoot` narrows `locateInstallRoot` and can never widen it. The route is deliberately not tier-gated: it writes only a record that must match the live files byte for byte, so it cannot change what runs or claim a person's file. It is also exposed as `dorkos marketplace check-files <name>`, as the Check files button on an older install's row, and as `checkPackageFiles` on the Transport. The last answer that recorded nothing is kept in memory (`check-results.ts`) and reported on the install's `integrity.check`, with `source: 'fetchable' | 'local'`, so the app shows the reason instead of a button that cannot help.
...
| POST   | `/packages/:name/check-files` | `{ projectPath?, installRoot? }`                             | `{ outcome, message }`: records an older install's files from its exact commit, or says why not (§5.3)                   |
