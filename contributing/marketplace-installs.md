...
- **Files an update kept unproven** (DOR-2322, `unproven.ts`). Verify names them in `integrity.unproven` (never in `added`), with whether Check files can sort them. `sortUnprovenFiles`, run only by Check files (`rebuildRecordStrict(root, deps, { sortUnproven: true })`, never by the sweep), fetches the earlier version the record names.
  - Under the lock, it removes each kept file that is a regular file with exactly that version's bytes at its old path, and that the current version does not ship. Those are leftovers an online update would have removed.
...
- **Check files** (DOR-2320): `POST /packages/:name/check-files { projectPath?, installRoot? }` runs `rebuildRecordStrict` on one installation and answers `{ outcome, message }`. `installRoot` narrows `locateInstallRoot` and can never widen it. The route is deliberately not tier-gated. It writes only a record that must match the live files byte for byte. The only files it removes are byte-identical to a version the package itself shipped and that the current version no longer ships. So it can stop a stale leftover from running, but it can never start anything or take a person's file. It is also exposed as `dorkos marketplace check-files <name>`, as the Check files button on an older install's row, and as `checkPackageFiles` on the Transport. The last answer that recorded nothing is kept in memory (`check-results.ts`) and reported on the install's `integrity.check`, with `source: 'fetchable' | 'local'`, so the app shows the reason instead of a button that cannot help.
