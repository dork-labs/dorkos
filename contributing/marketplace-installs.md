...
1. `updatesNotAsShown(updates, targets)`: any reinstall whose `latestVersion` or `disclosed` differs from its target (compared with `sameDisclosedEffects`, the approval hash's canonicalization) refuses the whole batch with 409 `{ code: 'disclosure_changed', changed }`. Nothing runs.
...
**Global activation consent (`global-plugin-consent.ts`, DOR-2306).** A global plugin, skill-pack or adapter is handed to the Claude Agent SDK whole, and the SDK starts its hooks, MCP and language servers, monitors and `bin/` commands in every session. `refreshActivatedPlugins` therefore activates only `listConsentedPluginNames`: `partitionGlobalPlugins` reads each installed package with `readRunnableDeclarations` (the preview's own reader), takes `activationEffectsOf` (the disclosure minus scheduled jobs, which the SDK never loads and which have their own `pending_approval` gate), and activates it only when it runs nothing on its own or a stored `<dirName>@global-<digest>` entry in `harness.approvedHooks` matches exactly. Checked refused → approved → unasked; unreadable declarations or an unreadable config file withhold. A yes is recorded only where a person approved exactly that set, before the refresh fires: a trusted HTTP install that sent `approvedDisclosure` (the installer held the install to it), a trusted HTTP update, a granted HTTP or MCP card (`GlobalConsentRecorder`, injected into both surfaces; never for `preApproved`). Everything else is withheld, and `ask-withheld-global-plugins.ts` raises one card per withheld package (capability `marketplace.activate_global_plugin`) at boot and after every global package change; grant records and refreshes, deny records a refusal, expiry records nothing. A list too long for one card, or unreadable declarations, is never shown and stays withheld. `dorkos harness hooks --list` names these entries, and `--revoke <name>` forgets them.
