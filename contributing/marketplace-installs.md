...
1. **Settle an interrupted earlier install.** `settleInterruptedInstall(target)` rolls back whatever a crashed transaction left beside the target (see §5.0). It refuses — changing nothing — when a rollback fails or another running DorkOS may be mid-install on the target.
...
Recovery runs in three places, always under the target's lock: at the start of every transaction and every uninstall (for that target), at server startup for every global install root before the app serves anything, and once Mesh has reconciled for every registered agent's project (`backup-janitor.ts`, `recoverInterruptedInstalls(scopeRoots, logger)`).

**Another process's live install is never touched.** The lock is per process, and two servers with different data directories can share a project. Each record's name carries its writer (`lib/record-owner.ts`: pid, start time read by `ps`, and a hostname hash), and recovery acts on a record only when its writer is this process, is provably not running, or the record is older than `IN_FLIGHT_FLOOR_MS` (10 minutes — far longer than any record lives, since the slow `npm install` runs in `stage`, before the record exists). Unreadable start times, Windows, and another machine (a synced project folder) all fall back to that floor. A target with such a record is left entirely alone by the sweep, and a new install of it refuses to start.

**Adding a kind of sibling** (DOR-2245's in-place uninstall is the next): add a row to `INSTALL_RECORD_POLICIES` in `install-recovery.ts` (marker, suffix, and `roll-back` with its `undo`, or `discard`), and add its marker to `MARKETPLACE_INSTALL_SIBLING_MARKERS` in `@dorkos/shared/marketplace-schemas`. Every reader of an install root — the installed scanner, conflict detector, update walk, Mesh discovery and Harness Sync's plugin scan — skips siblings through `isInstallSiblingName`, and the sweep, the ownership check and the per-target recovery pick the new row up unchanged.
