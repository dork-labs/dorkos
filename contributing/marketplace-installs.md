...
**Global activation consent (`global-plugin-consent.ts`, DOR-2306).** A global plugin, skill-pack or adapter is handed to the Claude Agent SDK whole, and the SDK starts its hooks, MCP and language servers, monitors and `bin/` commands in every session. `refreshActivatedPlugins` activates only `listConsentedPluginNames`, and the runtime re-checks that list at the start of every turn (`setConsentedPluginNames`). `partitionGlobalPlugins` reads each installed package with `readRunnableDeclarations` (the preview's own reader) and hashes its tree with `installedContentHash` (`lib/content-hash.ts`: every file's path, execute bit and bytes, every in-tree link's target, DorkOS's runtime state skipped, cached behind an lstat fingerprint of size, mtime, ctime and inode so an unchanged tree costs one `lstat` walk). It activates a package only when it runs nothing on its own, or a stored `<dirName>@global-<digest>` entry in `harness.approvedHooks` matches its name, its declarations (minus scheduled jobs, which have their own gate) and its content hash. Checked refused → approved → unasked; unreadable declarations, an out-of-tree link, or an unreadable config file withhold.

A package keeps at most ONE approval: recording one replaces the rest, and every global install, update and uninstall forgets them first (`GlobalConsentRecorder.settle` / `removed`), so a downgrade to old approved bytes is held back. A yes is recorded only AFTER the install or update landed and only when the landed copy declares what the person was shown and its shipped files (`shippedContentHash`: runtime state, `node_modules`, the lockfile, `.npmrc` and links left out, the same on the staged preview and the install) hash the same as the `contentHash` they were shown. The places a person is shown it: the app's install (`approvedDisclosure` + `approvedContentHash`), a trusted HTTP update (targets carry `contentHash`), a granted HTTP or MCP card (bound to the same hash), and `dorkos marketplace held-back --allow`. Never for `preApproved`.

An AGENT's HTTP install of a global package of an activated type that runs anything, or that replaces an existing global package, is previewed and goes through the same `ConfirmationProvider` card `marketplace_install` raises (202 `requires_confirmation`, retried with `confirmationToken`), bound to the disclosure and staged content hash; the card's detail names who asked, the version and the source. Everything else that is held back gets its own card from `ask-withheld-global-plugins.ts` at boot and after every global change: one open card per package name, at most one per `CARD_COOLDOWN_MS` per name, bound to the content hash and re-checked before a grant is recorded. `GET /api/marketplace/held-back` lists held-back packages with a plain `note`; `POST /held-back/:name/review` raises the card again (the Installed row's Review button); `POST /held-back/:name/decision` records the person's allow or refuse bound to the hash they were shown (operator only). `GET /installed` marks held-back rows with `heldBack`. The residuals, stated: a file rewritten in the middle of a turn runs in that turn; a hook's script that reads files outside its package is not bound; with login off, a caller that omits its agent header counts as the person (DOR-505).
