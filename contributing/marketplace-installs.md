...
6. **DorkOS removes or replaces only the files it can prove the install put there, unchanged (DOR-2245).** Every install records its files and their hashes in `.dork/installed-files.json`. Everything else in an install root belongs to the person or their agent, survives update, reinstall and a plain uninstall, and is removed only by `--purge`. A person's file is never moved out of its own directory. See §5.2.
7. **Nothing a package controls is read or unpacked without a limit (DOR-2319, DOR-2321).** Package text files are read through `@dorkos/shared/bounded-read` (1 MB each, catalogs 5 MB), never through a symbolic link inside a package. A whole package is held to `PACKAGE_SIZE_LIMITS` in `@dorkos/marketplace/package-size` (250 MB, 20,000 files and folders, 50 MB per file): the validator checks it first, and `stagePackageContents` checks it again before copying. A git download is held to `CLONE_SIZE_LIMITS` (1 GB, 100,000 files and folders): its `.git` and the disk's free space are watched while every fetch and the checkout run, and git's whole process tree (its process group on POSIX, `taskkill /T` on Windows) is stopped past the byte limit; `git ls-tree -r -t` is streamed before the checkout and stopped past the entry limit; every file is sized before the checkout, counted once per place it appears (`-l` when the files are already downloaded; on the blobless path the files are fetched in batches under the watch and sized with `git cat-file --batch-check`, with `GIT_NO_LAZY_FETCH=1`); and the checked-out tree is measured before anything reads it. A failed fetch always leaves its destination empty. A local source is copied with its `node_modules`, so the size error says how much of the tree is there.
...
**In place, and only the package's files (DOR-2245).** Uninstall no longer moves the install root anywhere. It moves only the files the installed-files record proves are the package's (listed, reached through real directories, unchanged bytes), plus installer-owned paths (`node_modules`, the npm lockfile, `install-metadata.json`), into a same-filesystem sibling `<root>.dorkos-uninstall-<createdAt>-<owner>-<uuid>` (`lib/uninstall-journal.ts`). Each move is journaled before it happens, and the package's manifest files move last. The side effects run from inputs read off the live root first, so an extension whose files the person edited is still disabled. Anything written into the sibling in the meantime is moved back. Then a `committed` phase is written, the sibling is deleted, and the record is pruned to what the person kept (`uninstalledAt` set). A failure before the commit puts every move back, manifest files first. A root that holds only kept files has no package manifest, so it counts as not installed anywhere (`hasPackageIdentity`), and an untouched package leaves no folder at all. `purge: true` removes the whole root once the uninstall commits. **An agent package** leaves the team as the last side effect: `agent.json` is parked as `.dork/uninstalled-agent.json`, then Mesh unregisters it (the full cascade: rooms, paused schedules, task roots, sign-ins, tokens, community enrollments, connection access). A reinstall of the same package restores the parked identity, but nothing the cascade removed. An update's uninstall half passes `replacing: true`, which skips this and keeps an active Shape active. Before that half runs, an update stages the new version and runs every refusal that depends only on its content (`assertInstallable` in `marketplace-installer.ts`: schedules croner rejects, a `SKILL.md` the parser rejects) plus the disclosure check, so a version that can never install leaves the old one installed. A failure the content cannot predict (npm, the disk, an extension compiled against its npm dependencies) still leaves the package uninstalled, with the person's files and the pruned record in place for a retry. A **linked install** (a symlink to a working copy) is removed by removing the link.
...
5. **Only through `UpdateFlow.applyPlan(plan, approved)`**, and only the installations `approved` names, each held to the disclosure it maps to (DOR-2306: there is no unbound apply, and `approved` is required), reinstall each `update-available` installation through the injected `InstallerLike.update()`, **in the scope the installation was found in**: its project for a project or agent install, none for a global one ([ADR 260923-163034](../decisions/260923-163034-updates-are-per-installation-in-their-own-scope.md)). It also passes the exact install root the check resolved (`InstallRequest.installRoot`), which `update()` and the uninstall flow use to narrow their by-name probe, so a plugin and an agent sharing a name never have the wrong one replaced. It never uses the scope the request named, because the installer uninstalls by probing project roots then global ones, and reinstalls into whatever `projectPath` it is given. Before DOR-2194 an apply passed the request's `projectPath`, so updating a global package with `--project` deleted the global install and reinstalled it into the project. The installer uninstalls as the first half of a replace and then reinstalls, keeping every file the person or their agents added or changed (§5.2), and runs the full install pipeline, validation included. It installs with `force: true`, so conflicts are detected but do not block the reinstall. An apply never runs on `unknown` or `current`.

**Two doors.**

- `UpdateFlow.run({ name, installation })` is the per-package route's (`POST /packages/:name/update`). **Advisory only** (DOR-2306): the route's body schema is strict, so a retired `apply: true` is a 400. The route scans the global scope plus the project's merged view and resolves the installation with `pickInstallation` (the project's own first). `run` returns `{ checks }`. A name missing from its scope is one `unknown` check ("not installed in this scope"). The route answers 404 for a name installed in no scope at all, since it can see every scope and the flow cannot.
- `UpdateFlow.planInstallations({ installations, disclose? })` then `UpdateFlow.applyPlan(plan, approved)` is the all-packages door's (`GET` / `POST /updates`, and `marketplace_update`). The caller scans once and hands the records in. That is why `marketplace_list_installed { checkUpdates }` (DOR-2195) can list and check from one walk. A plan's checks are `InstallationUpdateCheck[]`: one entry per installation, in scan order, each with the installation's identity in the installed list's own field names (`installPath`, `type`, `scope`, `agentPath`, `agentId`, `agentName`). `installPath` is the join key, the same one the Installed view keys rows on. With `disclose`, each `update-available` check also carries `disclosed`, what its new version runs. After an apply, an entry carries `applied` (the `InstallResult`) or `applyError` (why it failed). `selectInstallations(records, { names, installPaths })` narrows a scan. Any unmatched name or path throws `PackageNotInstalledForUpdateError`, whose 404 body carries `packageNames` and `installPaths`, before anything runs. A symlinked install (a working copy linked into place; the record's `linked`) is checked as `unknown` with "linked install — update its source instead" and never reinstalled.

The door's orchestration lives in `flows/update-installed.ts` so the HTTP route and the MCP tools share it: `scanUpdateView`, `callerSpelling`, `checkInstalledUpdates(deps, projectPath, selector?)` (always plans with `disclose`), `reinstallInputsFor` (the distinct `marketplace.install` tier-gate inputs, one per update name and scope), `updatesNotAsShown`, and `applyApprovedUpdates(deps, request, gate)`, where only the permission step (an `ApprovalGate`) is surface-specific. `GET /installed?projectPath` scans at the same canonical path, so its rows and these checks join on `installPath` even through a symlinked project.
...
**The HTTP apply (`POST /updates`, DOR-2306).** The body is `{ apply: true, projectPath?, targets: [{ installPath, latestVersion, disclosed }], confirmationToken? }`, strict: each target is an installation exactly as a check reported it and a person was shown it, `disclosed` sent back untouched. First the tier gate: when `marketplace.install`'s tier could ask a person (above `act`) and the caller is not trusted, the batch is refused with a 403 `batch_update_needs_approval` before any gate call; otherwise every distinct reinstall is authorized as `marketplace.install` (`reinstallInputsFor`, the caller's own spelling for the requested project) before any network work, stopping at the first refusal. Then `applyApprovedUpdates` plans with `disclose` and the route's gate:

1. `updatesNotAsShown(updates, targets)`: any reinstall whose `latestVersion`, `contentHash` or `disclosed` differs from its target (compared with `sameDisclosedEffects`, the approval hash's canonicalization) refuses the whole batch with 409 `{ code: 'disclosure_changed', changed }`. Nothing runs.
2. A trusted caller (`trustedCaller`: the person in the app or their own terminal) proceeds.
3. Anyone else (an agent) gets the card `marketplace_update` raises, through the same `ConfirmationProvider`: 202 `{ status: 'requires_confirmation', confirmationToken, updates, message }`, a retry with `confirmationToken` in the body, 403 `{ status: 'declined', reason }`. An agent can read any disclosure the app can, so echoing one proves nothing about a person having looked.

After the gate, `applyPlan` holds each reinstall to its disclosure and the installer's stage-once update refuses a `DisclosureChangedError` before uninstalling: one check between the check and the apply, one between the apply and the install. The route then fires `onPluginsChanged` once per `applied`, with that installation's project (none for a global one). The app sends `targets` from the checks `ConfirmUpdatesDialog` rendered; `dorkos update --apply` prints the same disclosures, asks (or `--yes`), and sends what it printed.

**Global activation consent (`global-plugin-consent.ts`, DOR-2306).** A global plugin, skill-pack or adapter is handed to the Claude Agent SDK whole, and the SDK starts its hooks, MCP and language servers, monitors and `bin/` commands in every session. `refreshActivatedPlugins` (at boot and after every marketplace change) activates only `listConsentedPluginNames`. `partitionGlobalPlugins` reads each installed package's declarations with `readRunnableDeclarations` (the preview's own reader) and the content hash its install RECORDED (`InstallMetadata.contentHash`); it hashes nothing. It activates a package only when it runs nothing on its own, or a stored `<dirName>@global-<digest>` entry in `harness.approvedHooks` matches its name, its declarations (minus scheduled jobs, which have their own gate) and its binding: the recorded hash, or `linked:<realpath>` for a linked (developer) install, whose card and row say it runs whatever is in that folder. Checked refused → approved → unasked. Withheld too: `unrecorded` (installed before hashes were recorded; its Review card shows it as it is now and a decision writes that hash into its metadata), `unreadable` (declarations that cannot be read, a declaration pointing into a runtime-state path, or any error reading that one package), and `unreadable-config`. If the partition fails as a whole the runtime loads no global plugin.

**The threat boundary.** An approval binds what arrives through the install and update channel. A local process editing an installed package's files is out of scope: it can already write `~/.claude/settings.json` hooks directly, so the install folder is never re-hashed.

**The install event.** `MarketplaceInstaller.installStaged` hashes the STAGED package exactly as it came through the channel, before the flow copies it and npm writes into the copy, and records that `packageContentHash` in the install metadata (`lib/content-hash.ts`: every regular file's path, execute bit and bytes, a shipped `node_modules` and lockfile included because npm obeys them; only the root `.npmrc` and links, which staging strips, and DorkOS's runtime state are left out). The preview, the update check and the approval card hash the same staged tree the same way; the landed folder is never re-hashed. `stageAndValidate` refuses (`ShipsRuntimeStateError`, 400) a package that ships any runtime-state path (`RUNTIME_STATE_PATHS`: `.dork/data`, `.dork/secrets.json`, the install records): refused rather than stripped, so what installs is what the author published, and a shipped install record can never stand in for the installer's.

A package keeps at most ONE approval: recording one replaces the rest, and every global install, update and uninstall forgets them first (`GlobalConsentRecorder.settle` / `removed`), so a downgrade to old approved bytes is held back. A yes is recorded only AFTER the install or update landed and only when the hash its install recorded is the `contentHash` the person was shown. The places a person is shown it: the app's install (`approvedDisclosure` + `approvedContentHash`, from the preview's or detail's `contentHash`), a trusted HTTP update (targets carry `contentHash`), a granted HTTP or MCP card (bound to the same hash), and `dorkos marketplace held-back --allow`. Never for `preApproved`.

**Warm processes.** A plugin reload can add a plugin but never unload one, so `compareLaunchFingerprints` relaunches a warm process whose wanted plugin set drops one it loaded, before its next turn, and the fan-out reload after a shrinking refresh is never held for prompt-cache cost.

An AGENT's HTTP install of a global package of an activated type that runs anything, or that replaces an existing global package, is previewed and goes through the same `ConfirmationProvider` card `marketplace_install` raises (202 `requires_confirmation`, retried with `confirmationToken`), bound to the disclosure and staged content hash; the card's detail names who asked, the version and the source. Everything else that is held back gets its own card from `ask-withheld-global-plugins.ts` at boot and after every global change: one open card per package name, at most one per `CARD_COOLDOWN_MS` per name, bound to the package's binding and declarations and re-checked before a grant is recorded. `GET /api/marketplace/held-back` lists held-back packages with a plain `note` and `bindsTo`; `POST /held-back/:name/review` raises the card again (the Installed row's Review button); `POST /held-back/:name/decision` records the person's allow or refuse bound to the `effects` and `bindsTo` they were shown, behind the same bar as deciding a card (`trustedCaller`, so under sign-in only a session cookie; an API key is pointed at the Review card). `GET /installed` marks held-back rows with `heldBack`. The residuals, stated: a local edit to an installed package is not caught (the boundary above); a hook's script that reads files outside its package is not bound; with login off, a caller that omits its agent header counts as the person (DOR-505).

**The MCP tool (`marketplace_update`, DOR-2195).** Advisory by default: `checkInstalledUpdates` with the same `names` / `installPaths` selection, answered as `{ status: 'checked', checks }`, with no confirmation and no reinstall. With `apply: true` it goes through `applyApprovedUpdates`, not the route's door, because it asks a person and a person must see what they approve. It checks first (`UpdateFlow.planInstallations({ disclose: true })`), which also stages every `update-available` installation's new version through `installer.preview` and records `disclosedEffectsOf` it on the check (`disclosed`: hooks, scheduled jobs, MCP and language servers, monitors, `bin/` commands). A new version that cannot be read, or that declares anything the preview could not read (`unreadableHooks`, `unreadableDeclarations`), becomes `unknown` with the paths in its note and can never be approved from the card. The confirmation provider then asks once, as operation `update` (capability `marketplace.update`, tier `act`), about exactly the stale installations: the card's `detail` lists each one's name, type, place, old and new version, and everything its new version runs, written out whole with hidden and direction-changing characters shown (`revealHiddenCharacters`). A plugin's own programs are said to start "in every session" only for a global installation; a project's copy says they are not started, because project installs are projected as files and those programs are not among them. A list longer than `APPROVAL_DETAIL_MAX_LENGTH` is refused with a plain message, never cut. The approval binds the set of `{ installPath, latestVersion, disclosed }` (sorted), so it cannot stretch over another installation of the same name, a newer version, or a version that runs something else; the retry recomputes everything and re-asks with a reason if it moved. After a yes, `UpdateFlow.applyPlan(plan, approved)` reinstalls only the approved installations, each with `InstallRequest.approvedDisclosure`. `MarketplaceInstaller.update()` resolves and stages the new version once, BEFORE it uninstalls anything, checks the approved disclosure against that stage, and then installs exactly that stage, so a commit that lands after the yes is refused for that installation (`applyError`) and the old version stays in place. When nothing could be applied the tool answers `nothing-to-update`. A `preApproved` caller (a spent tier-gate approval, or a trusted caller) is not asked again. `marketplace_list_installed { checkUpdates: true }` adds an `update` summary (`status`, `latestVersion`, `hasUpdate`, `note`) to each entry from the same one scan; without `checkUpdates` it never calls the flow, so it makes no network call.

**What the preview reads (`lib/package-hooks.ts`, `lib/package-programs.ts`).** Everything a Claude Code plugin runs on its own, from its default files and from `.claude-plugin/plugin.json` (a path, an inline value, or a list of either): hooks (`hooks/hooks.json`, `hooks`), MCP servers (`.mcp.json`, `mcpServers`), language servers (`.lsp.json`, `lspServers`), monitors (`monitors/monitors.json`, `experimental.monitors`), and the files in `bin/`. A hook of a type other than `command` is reported unreadable rather than dropped. Skills and commands (`skills/**/SKILL.md`, a root `SKILL.md`, plugin.json `skills`, and `commands/**/*.md` or what plugin.json `commands` replaces it with) are read for their frontmatter `hooks`, which Claude Code registers while the skill is in use, each tagged with its `source`, and for `allowed-tools` (`skillTools`), because the model picks a skill by its description (`lib/package-skills.ts`). Their TEXT is read too, and so is that of agents and output styles (`agents/`, `output-styles/`, and the same folders under `.claude/`), for the shell commands written into it (`skillCommands`, DOR-2327, `@dorkos/skills/shell-commands`, ADR 260925-000832). Claude Code runs them while it renders a skill or command, before the model sees it, plugin skills included. Its own patterns, read from the CLI the Agent SDK bundles, are a block `/```!\s*\n?([\s\S]*?)\n?```/g` anywhere in the text and an inline ``/(?<=^|\s)!`([^`]+)`/gm`` after it blanks ordinary code spans. OpenCode runs the inline form anywhere in a command template (``/!`([^`]+)`/g``), and Harness Sync copies plugin commands into `.opencode/commands/` wrappers. The reader reports the union of both patterns, a Markdown reading of fences (tildes, longer fences, an info string starting with `!`, fences in a blockquote or list, an unclosed block), and every double-backtick span after a `!`, on text whose `\r\n` and `\r` are normalised first. An extra line on the card is cheap; a command that runs unseen is the bug. Both programs fill `$ARGUMENTS`, `$N` (and in Claude Code `$ARGUMENTS[N]` and a named `$name` from frontmatter `arguments`) BEFORE running the command, so a command that names one is marked `usesArguments` and every surface says it uses the text typed after the command. Codex runs no such syntax, and a scheduled task's body is sent as a plain prompt, which renders nothing. Each command is bound in `disclosedEffectsOf` (sorted by file in code-point order, document order within one), so a version that adds or edits one needs approval again. An older client's disclosure without the field reads as having none, and the stored global-approval digest omits the field while it is empty, so v0.83 approvals keep matching. Frontmatter is read with `parseFrontmatter` from `@dorkos/skills/frontmatter`, the only frontmatter reader in the repo: it accepts YAML or JSON `key: value` fields only, so `---js` is refused and the file reported unreadable (DOR-2308), as is a header that is too large or expands too large through YAML aliases (DOR-2311). Every declaration is read through `readDeclarationJson` (`lib/package-declarations.ts`): a path out of the package, a symbolic link, or a file whose real location is outside the package's real root is reported unreadable and never opened, so the preview cannot leak a file such as `~/.claude.json` to the agent or client that asked. Agents are not disclosed: Claude Code ignores `hooks`, `mcpServers` and `permissionMode` in a plugin agent. Workflows run only when invoked by name. An output style marked `force-for-plugin` changes every session's instructions; it is not a program and is a known leftover.

**An agent package's working directory (DOR-2314, ADR 260924-212826).** An agent package installs into the folder its sessions run in, so what a harness loads from a working directory is either shown or refused.
...
- **Creating a marketplace agent in the app (DOR-2325):** `CreateAgentDialog` sends `package` to `POST /api/agents/create`, which runs the installer (`approvedDisclosure` + `approvedContentHash`, person only), so the agent is validated and created from the staged copy the preview showed. A raw template (`template`, or `dorkos agent create --template`) is not a package: it is cloned to a staging folder and read by `inspectTemplate` (`services/core/agent-templates/template-gate.ts`) with the same `findAgentWorkspaceConfig` rule, then shown to a person (409 `template_needs_review`) or carded for an agent (202 `requires_confirmation`) before it lands.
...
The net guarantee, **crashes included**: either the package is fully installed and visible, or (for a fresh install) it never existed, or (for a reinstall) the previous installation is intact.
...
**Staging and uninstall siblings (DOR-2245).** Two more rows share the table and the owner stamp. A `.dorkos-stage-` directory (installs that keep the person's files stage beside their target) is deleted; it only ever holds copies. A `.dorkos-uninstall-` sibling is settled by its journal: not committed, every move goes back (manifest files first, a parked `agent.json` restored); committed, the uninstall is finished; no readable journal, it is kept and reported, never deleted. Before a `.committed` backup is discarded, the live install is checked against its own installed-files record (`isInstallWhole`); one missing a recorded file keeps the leftover rather than lose the only whole copy.

**What recovery does not cover.** Rolling back restores the target directory only: a Mesh registration, the agent-created hook or an enabled extension that `activate` produced is not undone, on the crash path exactly as on the activate-failure path.

**Adding a kind of sibling**: add a row to `INSTALL_RECORD_POLICIES` in `install-recovery.ts` — its marker, whether its stamp carries an owner, its suffix, and its phase: `finished` (deleted) or `unfinished` with a `recover` that settles it and reports `rolled-back`, `rolled-forward` or `kept` — and add its marker to `MARKETPLACE_INSTALL_SIBLING_MARKERS` in `@dorkos/shared/marketplace-schemas`. Every reader of an install root or a skills root skips siblings through `isInstallSiblingName` — including watchers, whose events never pass through a scanner (the task file watcher arms a schedule straight from an event); `__tests__/install-sibling-readers.test.ts` fails when a new reader (a `readdir`, `chokidar.watch` or `fs.watch` over such a root) does not, or when anything spells a marker of its own. The sweep, the ownership check (`lib/record-owner.ts` is exported for siblings outside this grammar) and the per-target recovery pick the new row up unchanged.
...
### 5.2 Whose files these are (DOR-2245)

An install that passes `ownership` to `runTransaction` (all five flows do, through `lib/flow-ownership.ts`; the Shape fork does not):

1. **Stages beside the target** (`<target>.dorkos-stage-…`, same filesystem), not in `os.tmpdir()`, so activation is a true rename.
2. **Writes the record** (`lib/installed-files.ts`) into the staged tree, so it activates with the package. Anything the installer itself writes into the package's own files happens first, through `ownership.prepareStaged`: a `skillRef` schedule is injected into the staged `SKILL.md` there (DOR-2318), so the record holds that file as installed and an untouched update or an uninstall treats it as the package's. A legacy rebuild injects the old version's `skillRef` schedules into its fetched tree for the same reason. The record lists every shipped regular file and its SHA-256. The record never lists owned paths (`node_modules`, and `package-lock.json` when the npm step ran), the installer's own files, reserved paths, or an agent package's four identity files (`agent.json`, `SOUL.md`, `NOPE.md`, `MEMORY.md`), which DorkOS writes and which are the agent's.
3. **Rebuilds a missing record** for an install made before records existed (`lib/legacy-record.ts`). The sidecar's `commitSha` is fetched exactly (`fetchAtCommit`, by the install-time name so the `trees/` cache is reused).
   - The strict rule is tried first (`strictDifferences`, shared with Check files, §5.3).
   - On a miss the record is still trusted unless more than 10% and at least 3 of the present recorded files differ.
   - Otherwise, and when nothing can be fetched, a file is the package's only if an obtainable tree has the same bytes at that path. Every other live file is listed as `unproven` (why, the commit to compare with later, and each path) and kept as the person's (DOR-2322).
   - An update then gives each unproven file a `kept-unproven` notice, with one warning naming them, and records them on the new install with where each now sits.
   - An uninstall keeps them and returns them as `unproven`.
4. **Clones the person's files** from the live target into the staged tree (`lib/carry-over.ts`), applying the rule table in spec §4 (`planCarryOver`). An edited shipped file is replaced and saved as `<file>.dork-old` unless the package lists it in `userEditable`, in which case the person's copy stays and a changed default is saved as `<file>.dork-new`. Saved names never overwrite. Collisions (file vs folder, case-only) are checked on disk. Special files are skipped and reported, symlinks recreated verbatim, timestamps kept. The live target is only read. A linked install is never walked.
5. **Late-write pass**, after activation and before the commit: anything written into the old install since step 4 (by an agent working during its own update, say) is brought over when its clone is untouched, or saved as `.dork-old`; deletions are honoured. A process whose working directory is the old install keeps writing there afterwards, and those writes go with the backup (documented residual).

Each flow copies the notices (`InstallResult.fileNotices`) onto its result, with one plain sentence each on `warnings`. Every flow creates the package's `.dork/data`, which `${CLAUDE_PLUGIN_DATA}` resolves to (`contributing/harness-sync.md`). A package may never ship a reserved path (`.dork/data/**`, `.dork/secrets.json`, the installer's files, `*.dork-old`, `*.dork-new`): `validatePackage` refuses it (`RESERVED_PATH_SHIPPED`) and `stagePackageContents` strips it.
...
- **`verifyInstall(root)`** (`verify-install.ts`) reads the live folder against the record. A recorded file that differs is `changed`, one that is gone or no longer a regular file reached through real directories is `missing`, and an unrecorded file at or under an `EFFECT_BEARING_PATHS` entry, or a location the install's plugin.json declares, is `added`, because Harness Sync projects it. A `userEditable` file the person edited, or replaced with a folder or a link, is `customized`, which is not a modification. With no record, the status is `unknown` (`no-record`, `unreadable-record`, or `linked`). It is read-only and takes no lock. Hashes go through `cachedHashFile` (`file-hash-cache.ts`): the record's own `hashFile`, memoized while a file's `lstat` size, mtime, ctime and inode are unchanged. It adds no second hash. `withIntegrity(installations)` verifies a list four at a time.
- **Surfaces:** `GET /installed?verify=true` and `/installed/:name?verify=true`, the `marketplace_list_installed { verify }` MCP flag, and `dorkos marketplace installed --verify` (a FILES column) add `integrity` per installation. `dorkos doctor --deep` names, without paths, the changed packages and the older installs. The Installed view shows a row note from one verified request, never one per row: a disclosure counting edited, added and removed files, opening to their paths, and, only when an update is on offer, what it does to each (replaces edited files and keeps the person's copies, keeps added ones, puts back removed ones). The update-all confirm says the same per installation. Only the note's icon is coloured.
- **`rebuildRecordStrict(root)`** (`strict-record.ts`) gives an install made before records existed its record, and it never guesses:
  - It holds `withInstallTargetLock`, and re-checks inside the lock that there is still no record file.
  - It fetches the sidecar's exact commit through `stageInstalledCommit`, the same helper the tolerant rebuild uses, so both see the tree that install saw, `skillRef` schedules included.
  - It writes the record only when every recorded file (bar `userEditable` ones) is a regular file with the same bytes live.
  - Offline, with no exact commit (a local-path install), or on any mismatch, it writes nothing.
  - It has no byte-matching fallback, unlike `rebuildInstalledFiles`, which the update and uninstall paths use after trying this rule first.
  - A record that fallback only guessed (`inferred`) counts as no record: verify reports it `unknown` with reason `inferred`, and the sweep and Check files rebuild it strictly (DOR-2322).
- **Files an update kept unproven** (DOR-2322, `unproven.ts`). Verify names them in `integrity.unproven` (never in `added`), with whether Check files can sort them. `sortUnprovenFiles`, run only by Check files (`rebuildRecordStrict(root, deps, { sortUnproven: true })`, never by the sweep), fetches the earlier version the record names.
  - Under the lock, it sets aside each kept file that is a regular file with exactly that version's bytes at its old path, and that the current version does not ship. Those are leftovers an online update would have replaced. Setting aside is a rename to the first free `<path>.dork-old[.n]` (a reserved name nothing loads) with the execute bits cleared, never a delete, so an edit that lands between the comparison and the move survives in the moved file. A leftover already under a set-aside name stays put.
  - It keeps every other file as the person's, then drops the list.
- **The list lasts** until Check files sorts it. Every later update carries it through `placeUnproven` (whatever the old record was), mapping each file to where it now sits and keeping the path it had in the version the list was made against. An entry leaves only when the new version ships that file byte for byte. The uninstall reads it from any record.
- **Kept files that run.** `runningUnproven` is the list intersected with `addedEffectFiles`: kept files where a package keeps what it runs. Verify returns them as `integrity.unproven.running`, the update warning names them, the row note takes the amber weight, and the CLI FILES column says `(1 still runs)`.
- **Global consent.** A person's approval binds the content hash taken of the package as it arrived, before the update carried anything, so kept files are never in it. `globalConsentRecorder.settle` records the live declarations. So while a kept file runs, it records nothing: such a file was never in what the person was shown. `partitionGlobalPlugins` puts the files on the withheld entry (`keptRunning`), and the held-back note names them and points at Check files.
  - Nothing is removed when the version cannot be fetched or the record changed meanwhile.
- **The background sweep** (`legacy-record-sweep.ts`) runs `rebuildRecordStrict` on every legacy install after boot, one at a time. It is chained after the project install-recovery sweep in `index.ts`, and it covers the global roots plus every registered project's `.dork/` roots. What it cannot rebuild is left untouched until the next boot.
- **Check files** (DOR-2320): `POST /packages/:name/check-files { projectPath?, installRoot? }` runs `rebuildRecordStrict` on one installation and answers `{ outcome, message }`. `installRoot` narrows `locateInstallRoot` and can never widen it. The route is deliberately not tier-gated. It writes only a record that must match the live files byte for byte. It deletes nothing: the only files it moves are byte-identical to a version the package itself shipped and that the current version no longer ships, and it moves them to a reserved `.dork-old` name. So it can stop a stale leftover from running, but it can never start anything or lose a person's file. It is also exposed as `dorkos marketplace check-files <name>`, as the Check files button on an older install's row, and as `checkPackageFiles` on the Transport. The last answer that recorded nothing is kept in memory (`check-results.ts`) and reported on the install's `integrity.check`, with `source: 'fetchable' | 'local'`, so the app shows the reason instead of a button that cannot help.
- **Exact means exact.** Besides every recorded file matching, the live folder must hold no unrecorded file where a package keeps what it runs (`addedEffectFiles`, which also covers the locations plugin.json declares). That rejects an extra skill and a case-only rename. `userEditable` entries are only trusted where `UserEditablePathSchema` accepts them (`validUserEditable`), whether read off an old manifest or a record. The fetch and staging run outside the install lock; only the re-check, the comparison and the write hold it. The sweep stops between installs on shutdown (an `AbortSignal`), and boot removes rebuild scratch folders older than an hour (`removeRecordTempLeftovers`).
...
`services/tasks/task-file-update.ts` was the last plugins-only holdout and was fixed in DOR-1789, but it asks the question differently because it decides whether DorkOS may **write** a schedule's file. Since DOR-2272, `packageOwnershipOf` answers it from the installed-files record:

- **Candidates.** The file's install root is found three ways, any of which may name it: under a `plugins/` or `shapes/` root, under an `agents/` root (the first segment below the root in both cases), and the owning agent's own directory from `meshCore.getProjectPath(agentId)`. The agent probe is the only way to reach a **project-scoped** agent package (its `<repo>/.dork/agents` root is not derivable from anything the route holds); the root walks are the only ways that survive **without mesh** (mesh down, an agent that left the registry, a file another agent's skills root links into).
- **With a record.** The file is the package's exactly when the record lists it (in `files` or under `ownedPaths`) and it does not match `userEditable`: that is "the next update puts the package's copy back" (DOR-2245 §4). Whether the bytes still match is deliberately ignored, unlike `isProvenPackageFile`: an edited shipped file is still replaced. A record left by an uninstall (`uninstalledAt`) claims nothing.
- **Without a record** (installed before DOR-2245 and not updated since) the DOR-1789 answer stands for that install: location alone for `plugins/` and `shapes/`, which hold nothing a person put there, and a `.dork/manifest.json` or `.dork/install-metadata.json` marker for an `agents/` install or the agent's own directory, since that root also holds every agent a person makes, DorkBot included.
- Any candidate claiming the file is enough, so a plugin installed inside an agent package's directory is still claimed by its own record.

Both task doors ask this one question. `update-task-file.ts` refuses to rewrite a package-owned file (its switch and timing still land on the row, FB-26 and DOR-2302), and `create-task.ts` asks it about the file it is about to write: under a recorded agent package a new name is the person's and is made in the agent's own `.agents/skills/`, where it survives every update, while a name the record lists, or any name under an install with no record, is refused with a sentence saying why and what to do. Discovery asks it too (`packageOwnershipInRoot`), so the sync and the doors never disagree about whose a file is, and keeps the answer on the task row (`package_owned`): the app shows it before an edit, and when a file stops being a package's, the sync keeps the person's switch from the row and writes it into the file (`carrySwitchIntoReleasedFile`). Deleting a package's schedule is still allowed. A separate preserved task root for person-made schedules (DOR-1791's T3) was not built: DOR-2245 already keeps every unrecorded file. If a later package version ships a schedule under a name the person already used, the update saves the person's copy as `SKILL.md.dork-old` and says so (DOR-2245 row 10).
...
**Every git call leads with the hardening (DOR-2326).** `git-tree.ts` and `execGitClone` put `-c safe.bareRepository=explicit -c core.fsmonitor= -c core.hooksPath=/dev/null` (`NUL` on Windows) ahead of every subcommand, from `internalGitArgs()` in `lib/git-safety.ts`, and `hardenedGitEnv()` carries the same settings in the environment. Argv is the copy that counts: git before 2.31 ignores the environment one. `core.fsmonitor` is empty rather than `false` because git before 2.36 reads it as a program's path and would run one named `false`. `safe.bareRepository` is ignored before 2.38, so on older git only the other two apply; the probe runs every step through a wrapper that adds all three, and 2.26.2, 2.30.0, 2.40.1 and 2.49.1 pass with them. Staging (`stage-package.ts`) drops every `.git`, folder or file, at any depth, and the content hash leaves them out; the validator refuses a root shaped like a repository (`GIT_REPOSITORY_SHAPED`).
...
```typescript
{
  ...
  yes?: boolean;
  projectPath?: string;
  approvedDisclosure?: DisclosedEffects; // install only: the preview's `disclosed`, sent back (DOR-2306)
  approvedContentHash?: string; // install only: the preview's `contentHash`, sent back
  confirmationToken?: string; // an agent's retry after a person approved the card
}
```
...
| `ConflictError`                                 | 409         |
| `DisclosureChangedError` (`disclosure_changed`) | 409         |
...
dorkos marketplace update --apply <name>                  # Print what it runs, ask, then update exactly that
dorkos marketplace update --apply [--yes]                 # Same for every available update (one check, one apply)
dorkos marketplace update --apply --approval <token>      # Retry an agent's update a person approved
...
`dorkos marketplace add <url>` derives a default name from the URL's last path segment (minus `.git`, shaped into a name the server accepts, so `.github` becomes `github`); pass `--name` as the explicit escape hatch. After saving, `POST /sources` fetches the new source's listing once through `fetchNewSourceListing` (`services/marketplace/source-listing.ts`), which calls the same `PackageFetcher.fetchMarketplaceJson` the refresh route calls: the same URL policy, the same `MARKETPLACE_JSON_TIMEOUT_MS` deadline, an in-place read for `file://`, the same plain-words failure reasons, and the same cache write. So the first install from a new source does not need a refresh first (DOR-2304). Two differences keep `fetched: true` honest, because listings are cached by source **name**: the add first forgets anything cached under that name (`MarketplaceCache.removeMarketplace`, which also clears caches that removals made before DOR-2304 left behind), and it fetches with `{ staleFallback: false }`, so an old copy never passes for a fresh one. `DELETE /sources/:name` forgets the listing too, and clears the update check's memos. A source added with `enabled: false` is not fetched, though the name's old listing is still forgotten first. A new source's name must match `^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$` (`MarketplaceSourceManager.add` throws `InvalidSourceNameError`, answered as a 400), because the name keys the cache directory that removal deletes; `x/..` named the cache root itself. Names already on disk are not re-checked, so an older config still loads; `assertContainedIn` refuses a path equal to its root, and `readMarketplace` treats such a name as a cache miss. **Last-fetch record (DOR-2324).** `PackageFetcher.fetchMarketplaceJson` is the one door every listing fetch goes through (add, refresh, the browse list, the update check, the MCP tools), so it writes how each attempt went to `.last-check.json` beside the cached listing (`MarketplaceCache.writeFetchStatus`: `{ startedAt, checkedAt, ok, reason?, packageCount? }`), best effort. The write is atomic and a read-modify-write under `withFileLock`: attempts finish out of order, so a record for an attempt that STARTED earlier than the one on disk is dropped, and a failure keeps the count of the copy still listed. A failure is recorded even when an old copy is then served. It lives in the cache, not in `marketplaces.json` or `config.json`: it is derived state, rewritten on every fetch, and dropped with the listing by `removeMarketplace` and by clearing the cache, so no config migration is involved. `describeLastFetch` pairs it with the copy's `.last-fetched` stamp (`readMarketplaceFetchedAt`, which does not parse the listing; the count comes from the record) for `GET /sources` and the `marketplace_list_marketplaces` MCP tool. A failure whose attempt started before the copy on disk was written reads as `fetched`. The states: `failed` (no copy), `stale` (the copy's `copyFetchedAt` and count), `fetched`, or `never`; a copy with no record (cached before DOR-2324) reads as `fetched` at the copy's date. The app's rows and `dorkos marketplace list` draw from it, so a failure survives a reload and every window agrees. Refresh (`refreshSourceListing`) is "check now": it fetches with `{ staleFallback: false }`, and when that fails with a copy cached it answers that copy with `stale: true`, the reason and the copy's `fetchedAt`; the CLI and the app say "couldn't reach it, still showing the last copy" and the CLI exits 1. The fetch never throws: a failure comes back as `listing: { fetched: false, reason }` with the source still saved. `reason` is written for a person: `fetchMarketplaceJson` turns a 404 into "there's no marketplace listing at that address" and unwraps undici's bare `fetch failed` through `cause.code` (`ENOTFOUND`, `ECONNREFUSED`, …), which refresh shares. The CLI prints the reason and the `dorkos marketplace refresh <name>` line that retries; the app's sources page shows it on the source's row, next to a Refresh button (`Transport.refreshMarketplaceSource`, the same refresh route). The route is the only way in: agents may not add sources (DOR-502), so there is no MCP tool for it. `dorkos marketplace refresh` without a name iterates every configured source via `Promise.allSettled`, so a single failing source never aborts the batch.
...
- **`${CLAUDE_PLUGIN_DATA}` for global plugins.** Project-scope plugins get the install's own `.dork/data` through projection. Global plugins are still handed to the Claude Agent SDK (DOR-174), which sets its own `~/.claude/plugins/data/<id>/`. Both survive a DorkOS update; DOR-174 must move that data when global scope switches to projection.
- **The project-then-global probe.** A root an uninstall left in a project (kept files, no manifest) is treated as if that project never installed the package, so a project-scoped command then reaches a global install of the same name, as it always has for a package installed only globally.
