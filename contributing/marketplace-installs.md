...
| POST   | `/updates`                  | `{ apply: true, names?, projectPath? }` | `{ checks: InstallationUpdateCheck[] }` — with `applied` / `applyError` |
