...
`services/tasks/task-file-update.ts` was the last plugins-only holdout and was fixed in DOR-1789, but it asks the question differently because it decides whether DorkOS may **write**. It walks only the roots `INSTALL_ROOT_HOLDS_PACKAGES_ONLY` marks — `plugins/` and `shapes/`, where location alone proves ownership — and answers for agent packages by probing the owning agent's own directory (`meshCore.getProjectPath(agentId)`) for a `.dork/manifest.json` or `.dork/install-metadata.json` marker. It deliberately does **not** search `agents/`: that root also holds every agent a person makes, and the agent directory it needs is not derivable from a root walk anyway — for a project-scoped agent package the scope root derived from the agent's path is `<agentDir>/.dork`, which never contains `<repo>/.dork/agents`. Both task doors use it: `update-task-file.ts` refuses to rewrite a package-owned file, and `create-task.ts` refuses to file a new schedule under a package-owned agent at all, since `agentSkillsRoot()` would put it inside the checkout.
