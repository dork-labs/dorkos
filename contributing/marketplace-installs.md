...
**The install event.** `MarketplaceInstaller.installStaged` records the landed package's `packageContentHash` (`lib/content-hash.ts`: every regular file's path, execute bit and bytes; DorkOS's runtime state, the npm step's root `node_modules`, the lockfile, `.npmrc`, `.git` and links left out, so a staged preview and its installed copy hash the same) in the install metadata. `stageAndValidate` refuses (`ShipsRuntimeStateError`, 400) a package that ships any runtime-state path (`RUNTIME_STATE_PATHS`: `.dork/data`, `.dork/secrets.json`, the install records): refused rather than stripped, so what installs is what the author published, and a shipped install record can never stand in for the installer's.
