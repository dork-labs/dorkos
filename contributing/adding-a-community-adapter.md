...
3. **Resume is gap-free or it throws** `StaleCommunityCursorError` **before it serves an event**. An in-process adapter normally validates at call time; an HTTP adapter may let its authoritative server validate the opaque cursor on the first pull. In either case it may not emit a snapshot or entry before refusing. Check the room first, and refuse an id you cannot stream with `CommunityRoomNotFoundError` before you look at the cursor. That order is contractual, not stylistic — see the trap below for what it leaks.
...
- **Serving before an authoritative refusal.** An HTTP adapter may validate an opaque cursor only on its first pull, but it must refuse before it emits a snapshot or entry. Do not turn a server `410` into a partial replay.
...
- **Emitting `room_removed` from a state rather than a transition.** "This identity cannot see room X" is true of a room it was just ejected from _and_ of every room on the machine it was never in — the same collapse `CommunityRoomNotFoundError` exists to keep. So an adapter that answers a membership change by asking "is it visible now?" broadcasts the id of somebody else's room the moment anybody leaves one, on a stream whose whole job is to be listened to. Keep the set of rooms you have already reported (the local adapter's `visible`, the Buzz adapter's `rooms` cache) and emit only on a **change** to it. The same rule governs `room_added` and `room_updated`: read visibility, never mere existence, or a listener learns the title and topic of every room on the machine.
