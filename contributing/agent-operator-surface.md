...
The registry carries **51** capabilities today: 7 operator, 9 marketplace, 7 connector, 11 MCP-server-management (`mcp.*`, spec `mcp-server-management`, sign-in verbs included), 15 rooms, `memory.write`, and `capabilities.list`. (Count them from the domain files, or from `dorkos capabilities` against a running instance, before repeating the number — this line was three domains out of date when DOR-1698 touched it.) Alongside them, roughly 40 tools per MCP server (the `UNREGISTERED_TOOL_FAMILIES` in `self-description/capabilities-domain.ts`: tasks, relay, mesh, binding, trace, extension, devtools, UI, plus the hand-registered `create_agent`) have no registry entry. They appear in an agent's tool list and are unreachable by `dorkos call`.
...
**Scope check before you describe this to a user.** DOR-468 widened tier coverage to the hand-registered tools, so the gate is now system-wide across both surfaces: all 47 hand-registered MCP tools carry a tier (`services/core/mcp-tool-tiers.ts`), and the server's typecheck fails on a tool that declares none. Eight actions are `destructive` today: `marketplace.uninstall`, `operator.update_agent_boundaries`, `operator.update_agent_execution`, `mcp.add`, `mcp.import`, and `mcp.update` on the registry side, plus `tasks_delete` and `mesh_unregister` among the hand-registered tools. Everything else is `act` or `observe`, which is the honest thing to say: the three groups exist and cover everything, but only one of the three actually stops a call. Do not describe `act` as gated. It runs, audited. (Count these from the source before you repeat the number — `grep "tier: 'destructive'"` across the `*-capabilities.ts` files under `services/`, plus `MCP_TOOL_TIERS`. This sentence has gone stale once already.)

That coverage is load-bearing, and DOR-519 is the proof. A separate bug handed a session an SDK `allowedTools` list that skipped the approval prompt for 31 to 35 tools at a time, `tasks_delete` and `mesh_unregister` among the names (the only two destructive actions a tool group can contain; `marketplace.uninstall`, `operator.update_agent_boundaries`, `operator.update_agent_execution` and every other registry-side one are in no group). Neither ran: `allowedTools` decides only whether the SDK asks before invoking a tool, and the tier gate sits below that, inside the handler. The layer that failed was not the layer that mattered. See ADR-0070 and ADR-260726-171347.
...
- **Only a person writes permissions.** `updateAgentManifest` refuses any patch naming `permissions` (`REFUSED_OFF_WIRE_AGENT_PATHS` in `agent-write-policy.ts`), `PATCH /api/mesh/agents/:id` refuses it with `USE_PERMISSIONS_API`, and the generic config writers refuse `permissions.*` the same way. The only way in is `/api/permissions/*` and `/api/agents/:id/permissions`, behind `resolveDecisionAuthority` plus the operator cookie under login, and every write records a `permission.changed` Activity event (category `permissions`, never pruned). "Only a person writes" is a claim about DorkOS's own surfaces, not about the file: an agent with file tools can edit its own `.dork/agent.json`, and login does not stop that. The file stays the source of truth (ADR-0043), so the edit is honoured, but `PermissionObserver` (`services/core/permissions/permission-observer.ts`) compares every read with the last value DorkOS wrote or saw, kept under dork-home by agent id rather than in the agent's directory (a speed bump, not a wall: an agent with a shell that edits both can hide a change), and records the difference once as `attribution: 'outside'` ("Changed outside DorkOS"); the agent page marks the row with `changedOutsideAt`.
...
- **`operator.update_agent_boundaries`** is the same write, split out so it can carry its own tier (DOR-1698). NOPE.md is fed to the runtime as `safetyBoundaries` every turn, and it used to ride `update_agent` at tier `act` — so an agent could rewrite the file that says what it must not do, with no card in front of anybody. A tier is per-capability, not per-field, so the fields moved rather than the tier: this one is `destructive` and a person approves the change, alongside `marketplace.uninstall`, `tasks_delete` and `mesh_unregister`.
- **`operator.update_agent_execution`** is the same split for an agent's runtime, model and effort (DOR-2328). A schedule that leaves those unset follows its agent, and its approval records the unset value, so an agent that could move its agent's defaults would move every such approved schedule with nobody asked. `update_agent` refuses the three with a pointer; this capability is `destructive`, and its card shows each change old → new through `describeApprovalChange`, which also binds the approval to the values shown. `PATCH /api/agents/current` and `PATCH /api/mesh/agents/:id` refuse the three for a caller that has not cleared the agent bar (`clearsTheAgentBar`, `lib/caller-authority.ts`); a person is not asked. A hand edit of `.dork/agent.json` is the person's domain and is not gated.
  ...
  **It covers TWO fields, and leaving either behind would have gated nothing.** `nopeContent` replaces the text; `conventions.nope: false` is the stronger one — `runtimes/shared/agent-context.ts` drops the whole `<agent_safety_boundaries>` block when it is off, so the file survives on disk saying what it always said while no turn is ever given it. A mute leaves no diff to notice. Both are refused on `update_agent`, both are accepted here.
  ...
  The cockpit's own boundaries editor is unaffected: it is a person, on `PATCH /api/agents/current`.
...
Tiers are the opposite case: they are **not** registry-only. `defineCapability` declares one for a capability and `MCP_TOOL_TIERS` declares one for each hand-registered tool, and both feed the same gate. A count of tiered things is therefore a count over two tables, not one. Getting that wrong produces the specific error of calling `tasks_delete` and `mesh_unregister` "the two destructive actions" when `marketplace.uninstall`, `operator.update_agent_boundaries` and `operator.update_agent_execution` are on the other table.
