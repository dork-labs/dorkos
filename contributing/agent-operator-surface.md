...
| JSX string props and inline captions  | Copy inside a component, often a one-line caption under a hero.                                                                                                                                                                                            | `ActivityFeedHero.tsx:401`                             |
| Skill-pack prose in template literals | `packages/operating-skills/src/skills/*.ts` reads as code, so nobody looks, but `seedOperatingSkills` writes it into every new agent's `.agents/skills/` and DorkBot's. A false claim here reaches every agent in the product before it reaches any human. | None found; audited clean, listed to keep it that way. |
...
**Scope check before you describe this to a user.** The gate is real but it is not system-wide: exactly **one** capability is `destructive` today (`marketplace.uninstall`), and the ~40 hand-registered tools per MCP server carry no tier at all, so `tasks_delete` deletes a schedule with no approval. Any doc that says DorkOS "sorts everything an agent can do into three groups" is false, and false in the direction that gets a user's data deleted while they believe otherwise: `docs/guides/action-approvals.mdx` was corrected for exactly that. Widening tier coverage to the remaining families is tracked as **DOR-468**; until it lands, describe the gate by naming what it covers.
...
- **Behaviorally, on a credentialed run**: the `governance-approval-gate` eval (`packages/evals/src/suite/governance.ts`) asks a real model to uninstall a seeded package with no approval in hand, then asserts the gate stopped it, an approval row landed in the sandbox database, and the package tree is byte-identical. It is `core`-tagged and `quarantined`, so it runs and reports but never gates until a credentialed run promotes it.
  ...
  You no longer need CI to run that case credentialed: `pnpm evals:local` boots the credentialed tier on your own machine against your own `claude` sign-in. **That is not the same as having the evidence to promote it.** This case drives a tool, and nothing in the harness answers the runtime's `approval_required` yet, so the turn parks until the 90-second timeout and the case ends as a runner `error` rather than a verdict. What works today is the credential path and the turn itself; what is missing is an answer to the approval. Until that lands, a local run tells you the harness reached a model, not that the gate held. See the known-gap section of [`packages/evals/README.md`](../packages/evals/README.md), which also covers credential precedence, the docker tier's key requirement, and how to read the GATING line.
...
Two more places where "generated" is narrower than it sounds: only **2** capabilities declare an `http` surface (`operator.activity_list` and `capabilities.list`), so 2 of the OpenAPI document's operations come from the registry against 82 hand-written `registerPath` sites. And a tier is a property of a _capability_, so the ~40 hand-registered tools per MCP server carry none. Neither is a bug; both are routinely overstated in prose. Check a claim against the registry before you write it.
