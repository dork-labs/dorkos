...
| `context-menu/`               | The right-click menu over the renderer: spelling, link and editing items. Electron shows none unless the embedder builds one (DOR-1297); a page that `preventDefault()`s its own `contextmenu` — every Radix row menu — keeps the click.                                                                                |
...
- **`DORKOS_PARENT_PID`** is set in **dev only**, and is `delete`d from the packaged child's env rather than merely omitted — a packaged app inherits whatever the launching shell exported, and an inherited stale pid would arm the child's orphan watchdog (`exitWhenOrphaned`, `src/orphan-watchdog.ts`) against a process that is already gone, so the child would exit 0 a poll later and the shell would report a crash it caused itself. The child cannot derive this itself: `tsx` runs the server as a _grandchild_, so its `process.ppid` is the tsx wrapper. A packaged build needs none of it, because Electron tears a UtilityProcess down with the app.
...
**Silencing the install-location prompt.** Set `DORKOS_DESKTOP_SUPPRESS_INSTALL_PROMPT=1` and the wrong-home guard (`main/install-location/`) stays quiet wherever the app is running from. Anything that launches a **packaged** build from outside `/Applications` without a human present needs this: the guard's offer is a modal dialog raised _before_ the server starts, so an unanswered one means the app never serves `/api/health` — a 120s timeout with no output, wearing the same face as the Gatekeeper hang in §6. `scripts/smoke-packaged.ts` sets it for exactly this reason (it launches from `release/`). Main-process only, same as the heartbeat switch. Deliberately an explicit variable rather than a build-path or CI heuristic: a guessable special case is one a real install can fall into, and going quiet for someone who genuinely cannot update is the one failure this guard must not have.
