...
**Say what the server said.** A server that refuses to boot has already written the actionable sentence to stderr — most often "another DorkOS server is already using this data directory", when someone runs `dorkos` in a terminal and then opens the app. `server-output.ts` keeps a bounded, redacted tail of that stream, and both the startup-failure error and the crash dialog carry it. Pass it through verbatim: never parse it, match on it, or reword it. The server owns that copy; the shell owns delivery.

**A retry that keeps failing stops being offered.** After `MAX_RESTART_FAILURES` restarts that never produce a running server, the crash dialog drops "Restart Server" and offers "Open Logs" plus "Quit". Some failures cannot be retried out of, and an unlimited retry button against a locked data directory is a button that can never succeed. The counter resets whenever a server does start.
