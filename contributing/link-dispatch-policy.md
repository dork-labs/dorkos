...
**Since DOR-924 that includes the anchors and the router calls, wherever the URL came from outside this codebase.** A bare `<a href>` is followed by the browser with none of our code running, and a `router.navigate` call takes whatever string it is handed — so neither asked this seam anything. Every one of those fed by a package catalog, a server response, an adapter manifest, an extension or an agent now goes through `ExternalLinkAnchor` or `internalRoutePath`; the settled per-site list is [below](#anchors-and-router-calls-dor-924). Anchors whose href this codebase wrote as a literal stay plain anchors, and that is the whole remaining exclusion.

**A refusal is not silent.** Whenever `classifyLink` blocks a link, `openLink`/`openExternalLink` show one toast naming the scheme ("DorkOS doesn't open irc: links") before returning `false`. The message lives in the seam, in a single `reportRefusal`, precisely so no surface grows its own copy of the allowlist to restate — one did, and went stale the first time the list moved. Where a refusal is knowable **before** the person acts, it is shown before rather than after: `LinkSafetyModal` asks `linkRefusalHere` when it opens and, for a refused link, explains itself and offers only "Copy link" instead of an "Open link" button whose one possible outcome is a refusal.
...
`classifyLink` sits behind `openLink` and `openExternalLink`, the two ways the app's own code opens a link programmatically. The last known bypass of those helpers, the touch chip strip's embedded-mode `window.open`, was closed by DOR-921 and is listed below as one of the surfaces this seam protects. Two things used to sit outside it — bare `<a href>` anchors and direct `router.navigate` calls — and since DOR-924 they are inside it wherever the URL was not written here: see [Anchors and router calls](#anchors-and-router-calls-dor-924). The seam's callers include untrusted or semi-trusted surfaces whose URLs the app did not author:
...
This is defense in depth, not redundancy: the client-side allowlist is what every surface (web app, phone, desktop) shares, and the desktop shell narrows it further for its own outbound traffic. A scheme has to clear both gates to leave a desktop window; on the web app, only this page's policy applies.
...
1. `linkRefusalHere` (`link-navigation.ts`) answers the **surface's** question, not just the policy's: `classifyLink` first, then the desktop mirror `isWebUrl`. `openInBrowser` refuses on it **before** dispatch, so the refusal is synchronous and `openExternalLink`'s return value — which `ElicitationPrompt` and the OpenRouter sign-in flow both gate on — is honest on desktop.
2. The `open-external` IPC handler resolves `true`/`false` rather than `void`, and the seam reports a `false`. This is **future-drift insurance, not a live second case**: everything reaching the bridge has been through `new URL`, which lowercases the scheme, so a `HTTP://` spelling arrives normalised and clears the shell's `startsWith('http://')` exactly as it clears `isWebUrl` — they cannot disagree today. They are still two predicates in two processes and two bundles, and the failure mode when one widens is silent: a link reported as opened that never opened. An older preload resolves `undefined`, which is treated as "no answer", not as a decline — a version-skewed host stays quiet rather than accusing itself.

Neither is a substitute for the other: the first keeps the return value honest, the second makes a future divergence loud on the day it happens.

**Both halves have to ask the same question, and for one round they did not.** `LinkSafetyModal` asked `classifyLink` — surface-blind — while dispatch also consulted `isWebUrl`, so a `mailto:` link in the desktop app still drew an "Open link" button and still declined when pressed. That is the promise-then-refuse shape this whole ticket removes, reappearing inside the fix. Anything deciding whether a link will open asks `linkRefusalHere`; `classifyLink` alone answers a narrower question than any UI wants.

Whose policy refused a link is part of the message, too. `describeRefusal` is the single source for the toast's and the modal's wording, because when they were separate strings the desktop branch reused the generic title and produced "DorkOS doesn't open https: links" — false, since the web app opens them fine.

## Anchors and router calls (DOR-924)

DOR-921 closed the last programmatic bypass of `openLink`/`openExternalLink`. It also pointed out that closing it did not cover two shapes no `openLink` call is involved in: a bare `<a href={someUrl}>`, which the browser follows with none of our code running, and a `router.navigate({ to: somePath })`, which takes whatever string it is handed. Several of both were fed by strings this codebase did not write. DOR-924 is the census and the closure.

**Two seams cover them, and which one applies is decided by where the link goes, not by who supplied it.**

- **Out of the app** — `ExternalLinkAnchor` (`apps/client/src/layers/shared/ui/external-link-anchor.tsx`). A real `<a href>` for as long as the link is one, so hovering shows the destination and "Copy Link Address" has something to copy; a modified click on an absolute `http(s)` URL is still left to the browser, exactly as `MarkdownLink` decides it. When `linkRefusalHere` says this surface would refuse the link, **no href is rendered at all** — an href a middle-click might carry somewhere is not something to leave in the DOM once we know we would refuse it — and the element stays operable as a `role="button"` so a press still gets the seam's one-sentence explanation. There is no confirmation modal: these URLs sit inside flows the person deliberately started, and pressing "Open sign-in" is already the confirmation. Its `onOpened` callback fires only when the link actually left, so a refusal cannot be recorded as "they opened the sign-in page".
- **Inside the app** — `internalRoutePath` (`link-navigation.ts`). Returns the router-relative path when the href names a route the cockpit serves, and `null` for everything else. Deliberately not `openLink`: `openLink` answers an unrecognised path by opening a browser tab, which is the wrong answer for a **supplied** path, where the honest outcome is not navigating at all.

**Schema validation was considered per site and deliberately not chosen.** Two reasons. First, `z.string().url()` is not a scheme gate — under the pinned Zod it accepts `javascript:`, `data:`, `vbscript:` and `blob:` alike, because all it does is `new URL(...)`; the `.url()` already on `authorizeUrl` and `docsUrl` buys nothing here and should not be read as if it did. Second, a parse failure takes down the **whole payload** — the connector flow, the adapter manifest, the runtime-doctor response — where a seam refusal takes down only the link and says why. A connector naming an OAuth deep link (`myapp://`) should get a link that explains itself, not a response that fails to load.

The settled outcome per site:

| Site                                                                        | Where the URL comes from | Outcome                                                            |
| --------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------------------ |
| `marketplace/ui/PackageDetailSheet.tsx` `homepage`                          | remote package catalog   | `ExternalLinkAnchor`                                               |
| `connections/ui/ConnectDialog.tsx` `authorizeUrl`                           | connector-flow response  | `ExternalLinkAnchor`                                               |
| `connections/ui/ManagementReviews.tsx` `authorizeUrl`                       | connector-flow response  | `ExternalLinkAnchor` (+ `onOpened`)                                |
| `entities/agent/ui/McpSigninBody.tsx` `authorizeUrl`                        | the remote MCP server    | `ExternalLinkAnchor` (+ `onOpened`)                                |
| `relay/ui/wizard/ConfigureStep.tsx` `manifest.actionButton.url`             | adapter manifest         | `ExternalLinkAnchor`                                               |
| `entities/runtime/ui/DependencyInstallHint.tsx` `infoUrl`                   | runtime-doctor payload   | `ExternalLinkAnchor`                                               |
| `activity-feed-page/ui/ActivityRow.tsx` `item.linkPath`                     | server activity row      | `internalRoutePath` — an unknown path leaves the row inert         |
| `extensions/model/extension-api-factory.ts` `navigate(path)`                | extension code           | `internalRoutePath` — an unknown path warns and does nothing       |
| `canvas/ui/CanvasVideoContent.tsx`, `CanvasAudioContent.tsx` `resolved.url` | agent `content.src`      | media-kind `data:` gate in `fetch-src.ts`, matching `media-src.ts` |

**Be honest about that last row: it is defense in depth, not a hole anyone could click.** The anchor it protects lives in `<video>`/`<audio>` **fallback content**, which a browser renders only when it cannot play the element at all — no browser DorkOS supports does that, so on every real surface the link was never drawn. Even where it was, browsers have blocked top-level navigation to `data:` since 2017, so the worst reachable outcome was a click that did nothing. The gate went in because the resolver's rule differed from `media-src.ts`'s for no stated reason and the difference happened to sit under an `href` — a divergence worth removing while it is still theoretical, not a fix for a live exploit. The same honesty applies to the whole of DOR-924: React neutralises a literal `javascript:` href by itself, and no working exploit was constructed for any row in the table. This is hardening.

Accepted as they are, with the reason:

- **`canvas/ui/CanvasPdfContent.tsx`** and the image canvas. `resolveCanvasMediaSrc` (`canvas/lib/media-src.ts`) already refuses every explicit scheme but `http(s)`, and admits a `data:` URI only when its prefix matches the media kind. Routing the anchor through the seam would refuse the `data:application/pdf` sources that gate exists to allow — a working link traded for no safety.
- **`relay/ui/wizard/ConfirmStep.tsx`** ``href={`https://t.me/${botUsername}`}``. The scheme and host are literals this repo wrote; the supplied part can only vary the path under `t.me`.
- **`chat/ui/message/MessageImage.tsx`, `chat/ui/message/FileAttachmentList.tsx`, `conversation/ui/message/MessageAttachments.tsx`.** These URLs are **minted** by the attachment store, not supplied to it — see `sessionImageShape` in `@dorkos/shared` — or built here from a literal `/api/...` prefix. An agent chooses the bytes, never the address.
- **The adapter-manifest markdown** (`SetupGuideSheet.tsx`, `ConfigureStep.tsx`'s `setupInstructions`, `ConfigFieldInput.tsx`). The `linkSafety`-off concern DOR-921 raised was resolved by DOR-1272: every Streamdown instance in the app is handed `components={{ a: MarkdownLink }}` unconditionally, so manifest markdown links confirm and dispatch through the seam like any other markdown link. Nothing is left to switch on.
- **First-party anchors** — the runtime-connect docs links, the dev playground's own `#anchor` hrefs, and every other `href` written here as a literal or a module constant. The allowlist protects them the same way if they ever route through it; they are not what it exists for.

When you add an anchor or a `navigate` call, the question is only "did this codebase write the URL?". If not, use one of the two seams above.
...
**Desktop Electron note (DOR-1297).** The carve-out above hands the right-click to the shell, and until DOR-1297 the shell dropped it: Electron shows no context menu unless the embedder builds one, and none was registered on any `webContents` (the only match in the tree was `apps/desktop/src/main/tray.ts`, the system tray icon — unrelated). `apps/desktop/src/main/context-menu/index.ts` now builds it, applied per window in `window-manager.ts` beside the two link guards, and it takes `isWebLink` from this same seam — so "Open Link in Browser" can never offer to leave for a scheme the guards would refuse. Chromium only raises that event when the page did not `preventDefault()` its `contextmenu`, which is exactly why the Radix row menus above still win where they claim the click.
