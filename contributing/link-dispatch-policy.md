...
The two gates compose in one direction: the sanitizer can only ever remove an anchor before this seam sees it, and the seam can only ever refuse an href the sanitizer already passed. Neither can widen the other, which is why `MarkdownLink` refusing a `file:` href is not dead code — `LinkifiedText` renders the same component over raw machine output, where no sanitizer ran.
