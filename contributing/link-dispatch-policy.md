...
`classifyLink` sits behind `openLink` and `openExternalLink`, the two ways the app's own code is supposed to open a link. One known bypass exists today: the touch chip strip's embedded-mode path (`apps/client/src/layers/features/chat/ui/chips/TouchChipStrip.tsx`) calls `window.open` directly on a url chip whose target comes from the agent's `WebFetch` input, with no scheme check (tracked as DOR-921). Everything else routes through this seam. Its callers include untrusted or semi-trusted surfaces whose URLs the app did not author:
...
**Markdown links in chat and in static `MarkdownContent` do not go through `classifyLink` at all.** Streamdown (`streamdown` npm package, pinned `2.5.0`) renders its own anchors and dispatches them itself, under Streamdown's own sanitizer, not this module's allowlist. Clicks are confirmed through `LinkSafetyModal` where the surface opts in via the `linkSafety` prop (chat does), and through Streamdown's built-in confirmation otherwise.
