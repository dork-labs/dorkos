...
Apply is the seam that writes things, so it is the seam with a permission gate (DOR-625). `POST /api/shapes/:name/apply` runs the tier gate before it resolves anything, as tier `destructive`. The reason is **creation, not deletion**: apply writes a `SKILL.md` into the person's own skills root for every schedule the manifest declares, records a write receipt naming what it wrote, rewrites `ui.shapes.active` in `~/.dork/config.json`, and turns extensions on and off. Each of those outlives the call. That is the same ground `gate-bypass-scan.test.ts` already protects `createTask(` on. Step 4b's deletion is provenance-gated and confined to this Shape's own schedules, which is the shape of thing `relay_inbox` is allowed to do at tier `act` — so it is what the tier costs, not why it is right.

The reason is **not** that one apply arms unattended execution with the prompts off, and this guide said so until DOR-1713. It cannot: `clampSchedulePermissionMode` downgrades a manifest's `bypassPermissions` to `acceptEdits` before the row is written and warns the operator (DOR-607); a schedule is created enabled only when the manifest set `startEnabled` **and** its agent already exists (`startEnabled` defaults to `false`); and the row lands through `upsertFromFile({ source: 'discovery' })`, which `resolveFileArmStatus` parks at `pending_approval` on first sighting, so nothing runs on a clock until a person approves it (DOR-1486).
...
The agent-facing route to apply is `control_ui { action: 'apply_layout' }`, which is gated one level earlier, in the interactive `canUseTool` gate — see `contributing/interactive-tools.md`. `fork` is deliberately ungated: it copies a manifest and writes nothing else.
