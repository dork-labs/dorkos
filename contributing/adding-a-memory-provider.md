...
Two consequences worth planning for. **The fallback swaps which memory an agent has**, not just which code serves it: `builtin` starts from its own scaffold, so an agent whose notes live in your backend reads an empty file and its next `write` reports `created: true`. To a person watching, that reads as amnesia rather than as an outage, which is why a bench is not left to the log alone: `GET /api/system/memory` reports `configuredId`, `activeId` and why they differ, a standing banner in the app says the same thing in plain words (gated on `configuredId !== activeId`, so an id that never registered at all shows up too, not only one the registry actually benched), and — when the fallback already has content by the time an agent's turn injects it — the agent itself is told the notes it is reading come from a different local store. And **a `write` that faults is re-attempted against `builtin`**, so a backend that faults AFTER committing has stored the note twice — once with you, once in the file. Fail before you commit, or make your writes idempotent.
