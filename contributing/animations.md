...
**Neither reset reaches opacity, colour, or `repeat: Infinity`** — `MotionConfig` doesn't suppress those, since they're written as inline styles from JS regardless of the setting. A `motion.*` component animating any of the three must call `useReducedMotion()` itself and branch off — put the branch in a pure function that also reports itself as a `data-` attribute (the `shouldAnimateRoster()` shape in `features/team-roster/lib/roster-layout.ts`), so the attribute can't drift from the behavior. `use-session-border-state.ts` and `LaneContent.tsx` gate their infinite colour/opacity loops this way.
