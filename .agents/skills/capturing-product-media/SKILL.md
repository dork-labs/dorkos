...
4. **Poster** extracted from the loop itself — frame 0 by default (invisible poster→video handoff), or the last clean pre-seam frame for `posterFrame: 'settled'` shots (loops that open on streaming text or a loading skeleton; the poster doubles as the still that release notes and docs embed). No separate dark screenshots are taken.
