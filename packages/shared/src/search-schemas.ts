 ...
 *
 * Named here so the HTTP route and shared search service use one refusal shape
 * for malformed queries (DOR-691).
 */
 ...
 *
 * **The whole decision, as data.** The route renders the shared service result
 * as a status plus a body, keeping validation and query behavior together.
 */
