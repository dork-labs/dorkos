...
export interface SerializedCapability {
   ...
   *
   * It rides the catalog so the one declaration on the capability has one answer
   * everywhere: the server gate resolves it, the permissions pages list each
   * area's actions from it, and the docs projection reports it from the same
   * place. A bare string rather than the area union: this package is the wire
