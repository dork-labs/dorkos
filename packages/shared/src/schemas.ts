...
export const StreamEventTypeSchema = z
  .enum([
    ...
    'mcp_signin_required',
    'mcp_signin_resolved',
    // An image the turn produced — a model that draws pictures, a tool that
    // hands back a screenshot. Carries a REFERENCE to stored bytes, never the
    // bytes themselves; {@link SessionImageEventSchema} says why that
    // distinction is load-bearing on a stream that gets replayed.
    'image_attachment',
  ])
...
/**
 * The fields that identify ONE stored image, shared by the wire event
 * ({@link SessionImageEventSchema}) and the rendered part
 * ({@link ImagePartSchema}) — the same one-shape-two-users arrangement
 * {@link MemoryEntrySchema} has.
 *
 * **There is no `data` field here, and there must never be one.** An image is
 * addressed by `url`, and the bytes stay behind that URL. Inlining them as
 * base64 would work on the first render and fail everywhere afterwards: a
 * session's event stream is replayed in full on every reconnect and every
 * `Last-Event-ID` catch-up, so a single 2 MB picture becomes ~2.7 MB of base64
 * re-sent on every refresh. The replay buffers hold a bounded WINDOW of a
 * session (`RING_BUFFER_MAX_EVENTS`/`RING_BUFFER_MAX_BYTES`,
 * `EVENT_LOG_MAX_EVENTS`/`EVENT_LOG_MAX_BYTES`), so inlined images do not grow
 * memory without limit — they do something subtler and worse, which is spend
 * the window. Two pictures can evict the words of the turn that produced them.
 * The reference is a few hundred bytes, and the browser fetches the picture
 * itself once and caches it.
 */
const sessionImageShape = {
  /**
   * Opaque id of the stored bytes. Never a path: the store that minted it
   * refuses any id that could be read as one.
   */
  attachmentId: z.string().min(1),
  /**
   * Where to fetch the bytes — **whatever the attachment store answered**,
   * stored verbatim and never rebuilt from the ids. The same reasoning as the
   * room attachment row's `url` column: today's local store answers a
   * server-relative `/api/sessions/…` that a reader could in principle
   * reconstruct, and a bucket-backed one would answer an absolute `https://…`
   * that nothing could. Recomputing it would work right up until the bytes
   * moved, which is the day the seam exists for.
   */
  url: z.string().min(1),
  /** IANA media type the bytes were stored as, from a fixed allowlist. */
  mediaType: z.string().min(1),
  /** Byte length as stored, so a renderer can budget before it fetches. */
  size: z.number().int().nonnegative(),
  /** What the picture is, for a screen reader and for when the bytes are gone. */
  alt: z.string().optional(),
} as const;

/**
 * An image the turn produced, announced on the session stream.
 *
 * Emitted by a runtime adapter AFTER it has stored the bytes, so by the time a
 * client sees this the image is already fetchable.
 */
export const SessionImageEventSchema = z.object(sessionImageShape).openapi('SessionImageEvent');

/** Inferred type for {@link SessionImageEventSchema}. */
...
/**
 * A picture in a message — generated by the model, or handed back by a tool.
 *
 * A reference, not a payload: see {@link sessionImageShape} for why the bytes
 * live behind `url` and may never travel in the part. Rendering one is a fetch
 * the browser caches; re-reading a transcript costs nothing extra.
 */
export const ImagePartSchema = z
  .object({ type: z.literal('image'), ...sessionImageShape })
  .openapi('ImagePart');

/** Inferred type for {@link ImagePartSchema}. */
