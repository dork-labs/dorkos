 ...
 * authors**, not a private UI detail. Model display names come from each runtime's
 * account-aware catalog and are bounded on the way out instead of rejected, which
 * is what this number is for. Not everything DorkOS writes fits it: shipped
 * permission-mode labels reach 18 characters (`'Bypass permissions'`), so this is
 * a price rather than a guarantee.
 *
 * 13 is the width of the longest asserted DorkOS-authored name (`Claude Code` in
 * the client's runtime descriptors). Deliberately counted in characters, not
 * pixels: the composer's `text-xs` is 1.25× larger below 768px and scales again
 * with `--user-font-scale`, so a pixel ceiling would be a lie at exactly the widths
 * that matter.
 */
