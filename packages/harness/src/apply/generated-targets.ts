...
export function findGeneratedOrphans(repoRoot: string, plan: ProjectionPlan): string[] {
  ...
  for (const rel of GENERATED_HOOK_TARGETS) {
    ...
    if (onDisk === undefined) {
      ...
      if (hasGeneratedSidecar(abs)) orphans.push(sidecarRel);
      continue;
    }
    if (!ownsGeneratedFile(abs, onDisk)) continue; // somebody else's file — never ours to delete
    orphans.push(rel, sidecarRel);
  }
  return orphans;
}

/**
 * Remove the generated hook files (and sidecars) {@link findGeneratedOrphans}
 * names.
 *
 * Every path it hands back is one of the two shapes this module owns — a hooks
 * file the sidecar proves the engine wrote, or a sidecar whose file has already
 * gone — so removing them by path is the whole of the deletion, and the two
 * always go together because the finder always names them together.
 *
 * @param repoRoot - absolute path to the repository root.
 * @param plan - the current projection plan (its generate targets are kept).
...
export function sweepGeneratedOrphans(repoRoot: string, plan: ProjectionPlan): string[] {
  const orphans = findGeneratedOrphans(repoRoot, plan);
