...
function Progress({ value = 0, className, ...props }: ProgressProps) {
  ...
  return (
    <div
      ...
      aria-valuemax={100}
      aria-valuenow={pct}
      className={cn('bg-dui-secondary relative h-2 w-full overflow-hidden rounded-full', className)}
      {...props}
    >
      <div
        data-slot="progress-indicator"
        className="bg-dui-primary h-full rounded-full transition-[width] motion-reduce:transition-none!"
        style={{ width: `${pct}%` }}
      />
    </div>
  );
