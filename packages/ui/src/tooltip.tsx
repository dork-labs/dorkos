  ...
  return (
    <TooltipPrimitive.Portal container={container}>
      <TooltipPrimitive.Content
        ...
        className={cn(
          'animate-in bg-dui-foreground text-dui-background fade-in-0 zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 z-50 w-fit origin-(--radix-tooltip-content-transform-origin) rounded-md px-3 py-1.5 text-xs text-balance motion-reduce:animate-none!',
          className
        )}
        {...props}
      >
        {children}
        <TooltipPrimitive.Arrow className="bg-dui-foreground fill-dui-foreground z-50 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px]" />
      </TooltipPrimitive.Content>
