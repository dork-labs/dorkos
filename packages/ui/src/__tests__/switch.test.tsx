...
describe('Switch', () => {
  // Uncontrolled state must update through Radix while preserving the callback.
  it('toggles uncontrolled state and reports the new value', () => {
    const changed = vi.fn();
    render(<Switch aria-label="Notifications" onCheckedChange={changed} />);
  ...
  // Controlled callers own state; a click reports intent without mutating props.
  it('leaves a controlled value unchanged until its owner updates it', () => {
    const changed = vi.fn();
    const { rerender } = render(
      <Switch aria-label="Managed" checked={false} onCheckedChange={changed} />
    );
    const control = screen.getByRole('switch');
    fireEvent.click(control);
    expect(changed).toHaveBeenCalledWith(true);
    expect(control).toHaveAttribute('data-state', 'unchecked');
    rerender(<Switch aria-label="Managed" checked onCheckedChange={changed} />);
    expect(control).toHaveAttribute('data-state', 'checked');
  });
