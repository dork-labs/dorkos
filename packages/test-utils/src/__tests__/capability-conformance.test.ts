 ...
 *
 * The permission-area check gets seeded drifts of its own: a capability with no
 * area, an area-less one with no note, an area capability with no card fields,
 * and one whose card would show an opaque id with no subject naming it.
 *
...
function observeCapability(): ConformanceCapability {
  return {
    ...
    tier: 'observe',
    area: null,
    areaNote: 'a conformance fixture',
    surfaces: {
...
describe('permission-area check (spec agent-permissions D2)', () => {
  /** A conformant capability in the Rooms area. */
  function roomsCapability(): ConformanceCapability {
    return {
      id: 'demo.manage',
      title: 'Manage demo',
      description: OK_DESCRIPTION,
      tier: 'act',
      area: 'rooms',
      approvalDisplayFields: ['roomId', 'name'],
      approvalSubject: { field: 'roomId', kind: 'room' },
      surfaces: { mcp: { toolName: 'demo_manage', servers: ['in-session', 'external'] } },
    };
  }

  /** The permission-area violations for a registry holding these capabilities. */
  function areaViolations(...extra: ConformanceCapability[]) {
    const registry = conformantRegistry([observeCapability(), actCapability(), ...extra]);
    return checkCapabilityConformance(registry, conformantFixtures(registry)).filter(
      (v) => v.check === 'permission-area'
    );
  }

  it('passes a well-declared area capability', () => {
    expect(areaViolations(roomsCapability())).toEqual([]);
  });

  it('seeded drift: a capability that declares no area at all', () => {
    const { area: _area, ...noArea } = roomsCapability();
    expect(
      areaViolations(noArea).some((v) => v.detail.includes('declares no permission area'))
    ).toBe(true);
  });

  it('seeded drift: area null with no note saying why', () => {
    const bare = { ...roomsCapability(), area: null, areaNote: '  ' };
    expect(areaViolations(bare).some((v) => v.detail.includes('no areaNote'))).toBe(true);
  });

  it('seeded drift: an area capability with no card fields', () => {
    const { approvalDisplayFields: _f, ...noFields } = roomsCapability();
    expect(
      areaViolations(noFields).some((v) => v.detail.includes('no approvalDisplayFields'))
    ).toBe(true);
  });

  it('seeded drift: a card that would show an opaque id with no subject naming it', () => {
    const { approvalSubject: _s, ...noSubject } = roomsCapability();
    expect(areaViolations(noSubject).some((v) => v.detail.includes('opaque id'))).toBe(true);
  });

  it('seeded drift: a secret-shaped card field on an area capability', () => {
    const leaky = { ...roomsCapability(), approvalDisplayFields: ['roomId', 'apiToken'] };
    expect(areaViolations(leaky).some((v) => v.detail.includes('credential material'))).toBe(true);
  });
});
