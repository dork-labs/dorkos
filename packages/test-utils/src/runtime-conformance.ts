    ...
    describe('media a turn produces (DOR-1663)', () => {
      ...
      if (mediaTurn) {
        it('announces an image as a REFERENCE, never as bytes on the stream', async () => {
          ...
          expect(
            images.length,
            'a runtime that declares it carries media must emit exactly one `image_attachment` ' +
              'for a turn that produced one picture — `toBeGreaterThan(0)` here would have passed ' +
              'the duplicate-announcement bug that a republished cumulative snapshot causes'
          ).toBe(1);
