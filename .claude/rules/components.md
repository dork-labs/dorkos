...
- **Shadcn primitives** (`layers/shared/ui/`): follow the existing files — `cva` variants, `data-slot="component-name"` attribute on the root element AND on every sub-part, `cn()` for class merging, export both the component and its `componentVariants`, plus its `*Props` type on the barrel — give it a named `*Props` interface/type rather than an inline `React.ComponentProps<...>` so a test can pin that it made it onto the barrel; that test only sees named types, so it can't yet catch a component whose props type is inline. `data-slot` is a styling and testing seam, not decoration: `field.tsx` selects on `[data-slot=field-label]`, the sidebar's browser tests address `[data-slot="sidebar-row-title"]`, and `index.css` hides a scrollbar by `data-slot` because a utility class could not beat an unlayered global. A component whose root IS another `shared/ui` primitive (e.g. `SettingRow` rendering a `Field`, `CopyButton` rendering a `Button`) inherits that primitive's `data-slot` rather than adding a second, conflicting one — the attribute holds one value, and the inner primitive's own selectors depend on that value staying what it is. New components with a root of their own (a bare `div`/`span`, or a new primitive) still need their own `data-slot`. This is not yet true of every file in the directory — DOR-1761 stamped it onto the components that were being touched anyway rather than sweeping the whole tree in one pass, so a missing `data-slot` on an untouched file is a known gap, not a signal that the pattern is optional.
