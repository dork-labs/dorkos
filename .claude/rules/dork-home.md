...
| `services/runtimes/codex/codex-home.ts`              | Mirrors the Codex CLI's own `$CODEX_HOME ?? ~/.codex` resolution 1:1, so the message-search index reads the rollout files that CLI actually wrote (DOR-683). Same shape as the row above, same import-spelling constraint.                                                                                                                                                                                                                                                                                                                                    |
| `services/runtimes/opencode/opencode-data-dir.ts`    | Mirrors the OpenCode CLI's own `$XDG_DATA_HOME \|\| ~/.local/share` + `opencode` resolution 1:1, so the message-search snapshot reads the store that CLI actually writes (DOR-688). Same shape as the two rows above, same import-spelling constraint.                                                                                                                                                                                                                                                                                                        |
| `services/harness/agents-user-home.ts`               | Mirrors the `$HOME/.agents/skills` resolution five vendor pages document as a user-scope skills read path (Codex, OpenCode, Cursor, Gemini CLI, Copilot), so Harness Sync at global scope can link a globally installed package's skills where they look (DOR-1924). The first carve-out exempt from BOTH halves, so it carries no import-spelling constraint: a declared carve-out whose job is to call `homedir` decides nothing by which spelling it uses, and the SDK confinements are handed back in its own block so nothing else is dropped with them. |
