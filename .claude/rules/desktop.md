...
- **A Finder-launched app inherits `PATH=/usr/bin:/bin:/usr/sbin:/sbin` and `cwd=/`.** Both are wrong and both have to be replaced before the server child is forked: `shell-path.ts` reads the user's login-shell PATH once (`$SHELL -ilc`, bounded, fallback-on-failure, skipped in dev and on Windows), and `server-cwd.ts` resolves `DORKOS_DEFAULT_CWD` plus the fork's `cwd` from `server.cwd`, clamped into the boundary and required to exist. Do not "simplify" either away: without the first, no tool outside `/usr/bin` is findable; without the second, the server's own fallback lands on `…/Contents/Resources`, outside the boundary, and every boot degrades the session list.
