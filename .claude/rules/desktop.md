...
- Native modules (`better-sqlite3`, `node-pty`) and the Claude Code binary must be `asarUnpack`ed — a native executable cannot run from inside `app.asar`. **Every packaged target needs its own per-platform SDK package**, in two places: an `asarUnpack` glob in `electron-builder.yml` _and_ an os/cpu-guarded `optionalDependencies` entry in `package.json`. Both `…-darwin-arm64` and `…-win32-x64` are wired today; miss either half for a new target and that target packages green and ships with no runnable Claude Code.
