...
2. For each category in standard order (Added, Changed, Deprecated, Removed, Fixed, Security), merge every bullet from every fragment under a single `### Category` heading — applying the Phase 4 curation verdicts: skip bullets marked **drop**, and apply each **fold** by amending the surviving feature entry. A fragment whose every bullet was dropped is still deleted in step 4 like the rest. **Never compile a fragment's `covers:` frontmatter** — it declares which commits the fragment covers for the PR check and is not changelog content. An unread, hook-seeded entry (`changelog/README.md#seeded-fragments`) should never reach this step at all — an HTML comment is not a bullet, so this compile step would not catch one even if it tried; the `--validate` gate (no `skip-changelog` bypass) is what keeps a seeded fragment off `main` in the first place.
