...
| Client                                     | `apps/client/src/layers/widgets/room-view/ui/` — `RoomHeader`, `RoomComposer`, `RoomMessage`, `RoomThreadPanel`, `RoomLiveLane`; the row itself, `NoticeRow` and the live lane are `apps/client/src/layers/features/conversation/` |
