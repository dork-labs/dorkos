...
- **The halt ordering is deliberate:** notice first, then drop parked batches, then interrupt live claims — dropping after releasing would let held messages run one macrotask past the halt. The per-agent stop repeats it at one key and touches no other, so the agents nobody stopped keep their claims and their gathered messages.
