...
- **Streaming is direct SSE** in the POST `/api/sessions/:id/messages` response body; a separate EventSource handles cross-client sync only (ADR-0117). Pending permission requests are **not** persisted and **not** replayed on reconnect — the crux of the switching bug.
