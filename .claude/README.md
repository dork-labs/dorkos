...
| ------------------ | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PreToolUse(Bash)` | file-guard, git-guard                                              | file-guard blocks bash access to secrets (`.env`, keys); file-tool access is denied natively via `permissions.deny` tool-scoped rules. git-guard refuses `git stash` (except `list`/`show`), `git checkout -- <path>` and the same discard spelled `git restore` — fixtures in `scripts/test-git-guard.sh` |
