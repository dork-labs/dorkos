...
| `TS7056: The inferred type of this node exceeds the maximum length…` in `apps/server/src/services/core/auth/index.ts`, or `BetterAuthError: Drizzle schema mismatch` in tests | Either a **duplicated peer instance** — two copies of `@better-auth/core` / `@better-auth/utils` / `jose` after a `better-auth` bump (DOR-1538 shape) — or a **real upstream change**: 1.7.3 dropped `account.issuer` and outgrew the inferred type, with one core in the tree (DOR-2036) | Count the cores first. Two: move the whole family together (ledger, `better-auth` family). One: read the release notes for a schema change, migrate, and keep `AuthOptions` in `auth/index.ts` in step. Holding is the last resort, never the fix.           |
