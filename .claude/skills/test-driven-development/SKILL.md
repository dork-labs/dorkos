...
Tests live in `__tests__/` directories alongside source code, not in a separate test tree:

```
apps/server/src/services/__tests__/transcript-reader.test.ts
apps/client/src/layers/features/session/__tests__/SessionPanel.test.tsx
```

### Client Component Tests

Component tests require the `@vitest-environment jsdom` directive and a mock `Transport` wrapper:
...
### Server Tests with FakeAgentRuntime

Use `FakeAgentRuntime` from `@dorkos/test-utils` instead of hand-rolling mock objects:
...
See `.claude/rules/testing.md` for complete DorkOS testing conventions.
