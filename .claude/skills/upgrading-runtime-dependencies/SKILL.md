...
| `@openai/codex-sdk`              | `runtimes/codex`       | Versions track the Codex CLI train; active `alpha` dist-tag; event-mapper `never` exhaustiveness checks are the intended tripwire for new `ThreadEvent`/`ThreadItem` members (ADR-0309). **`CODEX_MODELS` in `runtime-constants.ts` is a hand-written snapshot of the pin — its model list, `CODEX_CONTEXT_WINDOW`, `CODEX_EFFORT_LEVELS` and `CODEX_MODEL_CAPABILITIES` (tool use / vision / image output) are static claims nothing reds automatically. Re-verify them against the new CLI's model manifest on every bump.** |
