...
| `@openai/codex-sdk`              | `runtimes/codex`       | Versions track the Codex CLI train and must move in lockstep; active `alpha` dist-tag; event-mapper `never` exhaustiveness checks are the intended tripwire for new `ThreadEvent`/`ThreadItem` members (ADR-0309). The model picker comes from the resolved CLI's account-aware `app-server` catalog, so verify the `initialize` + paginated `model/list` protocol and its mapped capabilities after every bump. |
