...
- A shipped default meant to be edited goes in `userEditable` in `.dork/manifest.json` (exact paths or `dir/**`); then the person's copy stays and a changed default is saved as `<file>.dork-new`. Set `minDorkosVersion` when relying on it. Nothing that decides what the package runs can be `userEditable` (hooks, `.mcp.json`, `.lsp.json`, monitors, `bin/`, extensions, tasks, skills, commands, subagents (`agents/`), output styles, `package.json`, or a path plugin.json declares one at, or a harness config folder: `.claude/`, `.agents/`, `.codex/`, `.opencode/`, `.gemini/`, `opencode.json(c)`): the schema and `validatePackage` refuse it (`EFFECT_BEARING_PATHS`, `USER_EDITABLE_EFFECT_PATH`).
- An agent package's folder is its sessions' working directory, so it may not ship harness config for them: `.claude/settings*.json`, a root `.mcp.json`, `.codex/`, `opencode.json(c)`, `.opencode/`, `.agents/harness.manifest.json`, `.gemini/settings.json`, a nested `.claude/`, or a `.claude/agents/` subagent setting `hooks`/`mcpServers`/`permissionMode` (or with a non-YAML header, a repeated key, or too deep) (`AGENT_WORKSPACE_CONFIG_FORBIDDEN`, ADR 260924-212826). Its `.claude/skills`, `.claude/commands` and `.agents/skills` are read by the install preview. Ship hooks or servers in a plugin the agent `requires`.
