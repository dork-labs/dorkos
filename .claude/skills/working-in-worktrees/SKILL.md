...
- You are running the `/flow:execute` stage (the workspace-choice phase of the `implementing-specifications` skill) — the unified `/flow` execution gate.
...
The `/flow` intent stages — `/flow:ideate`, `/flow:specify`, `/flow:decompose` — write **only `specs/` markdown** (plus tracker breadcrumbs). They do not mutate code, so they run in `main` without a worktree. Isolation begins at **execution** — the `/flow:execute` stage, the workspace-choice phase of the `implementing-specifications` skill.
...
- Execution gate: the `/flow:execute` stage, the workspace-choice phase of the `implementing-specifications` skill
