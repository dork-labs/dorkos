---
title: Architecture
description: How DorkOS connects your browser to your coding agents using a hexagonal architecture
---
...
DorkOS sits between you and your coding agents, providing a browser-based command center. It works with Claude Code, Codex, and OpenCode through one shared runtime interface. Understanding how the pieces fit together helps you troubleshoot issues, choose the right deployment mode, and build integrations.
...
Transport (interface)
  |
  +---> HttpTransport ---> DorkOS Server (Express) ---> AgentRuntime ---> Claude Code, Codex, or OpenCode
  |
  +---> DirectTransport ---> AgentRuntime ---> Claude Code (Obsidian plugin, no server)
```

Two abstractions make this work. The **Transport interface** decouples the React client from its backend — the client calls `transport.postMessage()` and subscribes to session events without knowing whether they came from an SSE stream or an in-process generator. The **AgentRuntime interface** decouples the server routes from specific agent backend implementations — routes call `runtimeRegistry.getDefault()` and never reference `ClaudeCodeRuntime` directly.
...
- The client sends REST requests and receives all session events on durable, resumable SSE streams
...
1. **ChatPanel** captures your input and calls `transport.postMessage()`
2. The **Transport adapter** delivers the trigger to the backend (via HTTP or directly), which acknowledges immediately with the canonical session id
3. The server resolves the session's `AgentRuntime` via the `RuntimeRegistry`
4. The **AgentRuntime** calls the chosen runtime (Claude Code, Codex, or OpenCode) and the turn runs server-side
5. Events flow to every subscribed client over the session's durable event stream: text deltas, tool calls, approvals, and the final result
...
The `RuntimeRegistry` holds all registered runtimes keyed by type string. The default is `ClaudeCodeRuntime`, which integrates the Claude Agent SDK. DorkOS also ships `CodexRuntime` and `OpenCodeRuntime`, and more can be registered without touching route code.
