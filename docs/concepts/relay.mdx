...
<Callout type="info">
  Looking to actually connect Telegram or Slack to your agents? That is the **Messaging** half of
  the [Connections](/docs/connections) page, no code required. This page explains how Relay works
  underneath it.
</Callout>
...
Every message travels to a named address, like `relay.human.telegram.{chatId}` for one specific Telegram chat, or `relay.agent.{namespace}.{agentId}` for one specific agent. You rarely need to write these yourself. DorkOS and its adapters generate them for you, and each agent's own address is shown as its `relaySubject`; the pattern is here mostly so log lines make sense when you're troubleshooting.
...
- `relay.agent.{namespace}.{agentId}`: messages addressed to a specific agent (its `relaySubject`)
...
An expired message never runs. A message can run out of time after it is sent — while it waits for a busy agent, or sits behind another turn — and when that happens it is turned away and the sender is told, rather than answered later as if it had just arrived. Nothing gets a fresh clock because it arrived late.
...
- **Slack adapter**: bridges Slack workspaces into Relay using Socket Mode (Slack's WebSocket-based connection, which avoids exposing a public webhook URL). Supports streaming responses, threading, and a working indicator driven by the turn itself.
