...
| **enabledToolGroups** | Two different settings share this field. The four domain groups (tasks, relay, mesh, adapter) decide what the agent is **told about** — turning one off changes its instructions, not what it may reach. `roomsManage` is a **grant**, not a hint: DorkOS enforces it, and it stays off until you turn it on. All five are yours alone: an agent cannot set any of them for itself |
