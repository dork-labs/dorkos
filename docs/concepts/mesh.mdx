...
**Try it:** open the **Team** page and click **New Agent**, then choose **Import Project**. Mesh scans your folders and shows you what it found. From the Team page you can also switch to the **Topology** view to see the network map, or the **Access** view to manage messaging rules between groups.
...
<Callout type="info">
  DorkOS itself uses an admin view that sees past those walls, so it can draw the full map for you.
</Callout>
...
When an agent's state changes, the map updates on the spot. You also get a running count of how many agents are active, inactive, and stale.
...
| **behavior**          | Response mode: `always`, `direct-only`, `mention-only`, `engaged`, or `silent`. Only sets what a new direct message starts as. A channel always starts `engaged`, and a room someone already set up keeps what it has                                                                                                                    |
...
| **enabledToolGroups** | The four domain groups (tasks, relay, mesh, adapter) decide what the agent is **told about** — turning one off changes its instructions, not what it may reach. All four are yours alone: an agent cannot set any of them for itself                                                                                                     |
| **permissions**       | What this agent may do where it differs from everyone else (see [Permissions](/docs/guides/permissions)). DorkOS enforces it and only the permission pages write it; an edit made to the file directly takes effect and is recorded as changed outside DorkOS. An older file's `enabledToolGroups.roomsManage` is read as the Rooms area |
...
The **Let all my agents talk to each other** switch in Team → Access is one cross-namespace allow at priority 50, written with `*` on both sides. It outranks every namespace's catch-all deny, so while it is on, any agent can message any other agent on this machine. Turning it off removes that one rule and leaves the pairs you added untouched. `GET /api/mesh/topology` reports it as `openMesh`.
...
- **Lifecycle signals** broadcast health-status changes on the `mesh.agent.lifecycle.*` subject pattern, which DorkOS subscribes to for real-time map updates.
