...
Each registered agent also has a `.dork/agent.json` file in its project directory. This file is the portable representation of the manifest and the canonical source of truth (ADR-0043); the database is a derived cache. Because of that, the database is fully rebuildable from disk: if you delete `dork.db`, the reconciler walks the managed agents home directory (`~/.dork/agents/`) and the scan roots of surviving agents on its next pass, re-importing every `.dork/agent.json` it finds. Deeper project trees can always be recovered with an explicit discovery scan.
