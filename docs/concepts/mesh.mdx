...
<Callout type="info">
  The DorkOS cockpit uses an admin view that sees past those walls, so it can draw the full map for
  you.
</Callout>
...
When an agent's state changes, the cockpit updates the network map on the spot. You also get a running count of how many agents are active, inactive, and stale.
...
| **enabledToolGroups** | Which MCP tool groups this agent is told about (tasks, relay, mesh, adapter). Not a restriction: it changes the agent's instructions, not what it can call                                                            |
...
- **Lifecycle signals** broadcast health-status changes on the `mesh.agent.lifecycle.*` subject pattern, which the cockpit subscribes to for real-time map updates.
