...
| **permissions**       | What this agent may do where it differs from everyone else (see [Permissions](/docs/guides/permissions)). DorkOS enforces it, and only the permission pages write it. An older file's `enabledToolGroups.roomsManage` is read as the Rooms area |
