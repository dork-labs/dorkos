...
<TypeTable
  type={{
    ...
    'rooms.repo.worktreeReapDays': {
      description:
        "How many days an agent's copy of a room's files may sit untouched before DorkOS tidies it away. Only an empty copy is ever removed. Anything unsaved, or saved but not yet merged, is left alone however long it sits there, and the room shows you who is holding it.",
      type: 'integer (1--365)',
      default: '14',
    },
...
The three size limits are also written down for each room when its files are first set up. So a
room keeps the limits it was made under, and raising or lowering these later never makes an
existing room's files suddenly illegal.
