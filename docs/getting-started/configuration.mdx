...
<TypeTable
  type={{
    ...
    'rooms.maxAutomaticTurnsPerHour': {
      description:
        'The most automatic replies one room may run in an hour, counted no matter who the message looked like it came from. This is the ceiling on what a room can cost you. Set it to 0 to stop automatic replies entirely. Only you can change this.',
...
<Callout type="warn">
  ...
  can reset that limit. The hourly limit is the one that holds either way, because it never asks
  who is calling. Turn on **Require login** to close the gap.
</Callout>
