...
<TypeTable
  type={{
    ...
    'rooms.maxTurnsPerAgentPerCascade': {
      description:
        'How many of those replies any one agent may run in a single back-and-forth. The limit above counts the whole chain; this one counts each agent, so a pair trading answers stops after ten each. Only you can change this.',
      type: 'integer (1--100)',
      default: '10',
    },
