...
<TypeTable
  type={{
    'rooms.maxAgentDepth': {
      description:
        'How many replies in a row agents may send each other before the room stops them and says so. Your own messages always start the count over, so a room that has gone quiet is one message away from running again. Set it to 0 to turn automatic replies off. Only you can change this.',
      type: 'integer (0--10)',
      default: '3',
    },
  }}
