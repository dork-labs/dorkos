...
**Claude Code chats can keep their agent running between messages.** Normally, every message you send starts the agent up again, and it shuts down when the reply is finished. Turn on `runtimes.claudeCode.persistentSession` and the agent stays running in between, so your next message reaches an agent that is already awake instead of one that has to start from scratch.

It ships off. When you turn it on, it applies to the next message you send in each chat — nothing you have open changes underneath you. Turning it back off works the same way round, with one wrinkle worth knowing: a chat whose agent is already running keeps it until that agent goes away on its own, which happens after five quiet minutes, when the chat is closed, or when you restart DorkOS. An agent that has been sitting idle is shut down automatically, and you will not notice — your next message simply picks the conversation up where it left off. DorkOS keeps at most twelve of these running at once and gives back the one you have used least when it needs room.

This is new, and it is off by default because it is new. Everything else works the same either way.
...
<TypeTable
  type={{
    ...
    'runtimes.claudeCode.persistentSession': {
      description:
        'Keep a Claude Code chat’s agent running between messages instead of starting it again for each one. Applies to the next message in each chat; a chat whose agent is already running keeps it until that agent is shut down for being idle, the chat is closed, or DorkOS restarts.',
      type: 'boolean',
      default: 'false',
    },
...
You can also make Full autonomy where every new chat starts, in Settings under the defaults for new sessions, or right after you change a chat's setting — a quiet line offers it there for a few seconds. Either way DorkOS asks you to confirm what it means once, at the moment you choose it, and writes down that you did. From then on new chats start without asking, and Settings keeps a note saying so with a link to change it back. This covers the chats you open yourself; scheduled runs, integrations, and rooms have their own settings and their own, stricter rules.
