...
**If you have more than one Claude account**, the `runtimes.claudeCode` settings say which one your work runs on. A Claude account is a folder — usually `~/.claude`, and a second one might be `~/.claude2` — holding that account's chat history and its own sign-in. Point `activeAccount` at a folder and your next session runs and bills there. Leave it `null` and nothing changes: DorkOS uses whatever your shell already set, or `~/.claude`. List your accounts under `accounts` and give each one a name you'll recognise, like the client you bill it to. Nothing outside DorkOS changes either way — your shell, your aliases, and a plain `claude` all keep working exactly as before.
