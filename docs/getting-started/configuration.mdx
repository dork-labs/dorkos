...
<TypeTable
  type={{
    ...
    'notifications.escalation.phoneAfterMinutes': {
      description:
        'How long something may wait on you here before DorkOS tries to reach you another way. Nothing reads this yet. Set it to "never" to keep everything on this computer. Only you can change this.',
      type: '1 | 2 | 5 | 15 | "never"',
      default: '2',
    },
  }}
...
That last setting is saved, but nothing uses it yet. Reaching you somewhere else, like on your phone or in a chat app, is still being built. Until it is, nothing tries another way of reaching you, whatever this is set to.

On a phone, open DorkOS using your tunnel address (see [Tunnel Setup](/docs/self-hosting/tunnel-setup)), then choose "Add to Home Screen." It installs like an app: full screen, with its own icon, no browser bar. iPhone only shows notifications from an app installed this way, not from a browser tab. DorkOS does not send notifications to a phone yet, installed or not; that is still being built.
...
<TypeTable
  type={{
    ...
    'runtimes.claudeCode.activeAccount': {
      description:
        'The Claude account new sessions run and bill on, as the full path to its Claude folder (for example `/Users/you/.claude2`). When `null`, DorkOS uses whichever folder your shell already points at, or `~/.claude`.',
      type: 'string | null',
      default: 'null',
    },
    'runtimes.claudeCode.accounts': {
      description:
        'The Claude accounts you have told DorkOS about, each with a path and a name you choose (or `null` for no name). DorkOS never goes looking for accounts on its own.',
      type: 'Array<{ path: string; label: string | null }>',
      default: '[]',
    },
