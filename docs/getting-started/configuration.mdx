...
DorkOS supports ngrok tunnels for remote access from any device. See the [Tunnel Setup](/docs/self-hosting/tunnel-setup) guide for full details.
...
Linking this instance to a DorkOS account lets the account see and manage it. Run `dorkos cloud login` to link (it prints a code to approve in your browser, and works headless), `dorkos cloud status` to check the link, and `dorkos cloud logout` to unlink. Local login and the cloud link are independent — either can exist without the other.
...
Set `DORKOS_CLOUD_URL` to point at a non-production cloud (defaults to `https://dorkos.ai`). While linked, the instance heartbeats the cloud on startup and every 15 minutes; if the account revokes the instance, the next heartbeat clears the local token and the instance is unlinked.
