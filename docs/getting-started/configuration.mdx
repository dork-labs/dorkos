...
DorkOS is private by default. Every telemetry channel is off until you opt in, and the first-run banner shows you the exact payload before you decide. See the full [telemetry contract](/docs/self-hosting/telemetry) for what the weekly heartbeat sends and how to opt in or out.
...
<TypeTable
  type={{
    ...
    'telemetry.heartbeat': {
      description:
        'Send an anonymous weekly heartbeat to dorkos.ai (version, OS, configured runtimes, and rough counts). Never prompts, code, paths, or session content.',
      type: 'boolean',
      default: 'false',
    },
    'telemetry.install': {
      description: 'Send anonymous marketplace install events to dorkos.ai.',
      type: 'boolean',
      default: 'false',
    },
    'telemetry.errorReporting': {
      description:
        'Send scrubbed crash reports to your own Sentry or self-hosted GlitchTip project. A separate opt-in from the anonymous channels; also needs a SENTRY_DSN env var. Sends the error type and a cleaned stack trace, never the message, paths, or session data. See the telemetry page.',
      type: 'boolean',
      default: 'false',
    },
