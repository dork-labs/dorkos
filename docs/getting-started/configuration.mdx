...
Running `dorkos harness sync --fix` yourself follows the same answers. It sets up everything else the package brought, holds its hooks back, and prints each command it did not install. To install them, run `dorkos harness sync --fix --allow-hooks <package>`, which saves your answer in the same list. `dorkos harness hooks --list` shows every package you have decided about, and `dorkos harness hooks --revoke <package>` forgets one so DorkOS asks again.
