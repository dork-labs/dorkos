...
It reads your other tools' folders too, so a project that never used Claude Code is not handed an empty list. Skills you keep in `.opencode/skills/`, `.cursor/skills/`, `.gemini/skills/`, `.github/skills/` or `.codex/skills/` each get a line saying which tools read them where they are and which do not, and the Skills page offers to move one to `.agents/skills/`, where every tool can read it — nothing is moved for you. And if your MCP servers are declared in `opencode.json`, `.codex/config.toml` or `.cursor/mcp.json` rather than `.mcp.json`, you get one line saying how many are in there and that DorkOS only passes on the ones in `.mcp.json`. It counts them and reads nothing else, so nothing from those files ever gets printed or stored.
