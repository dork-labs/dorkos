...
On a phone, open DorkOS in your browser and choose "Add to Home Screen." It installs like an app: full screen, with its own icon, no browser bar. iPhone only shows notifications from an app installed this way, not from a browser tab. DorkOS does not send notifications to a phone yet, installed or not; that is still being built.
