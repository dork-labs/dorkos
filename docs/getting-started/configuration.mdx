...
A note can also end with one thing the agent wants you to decide — "want me to open the PR?". That part is off unless you turn it on, and the reason is cost: there is no way to know an agent has a next step without asking it, and asking runs it for a turn. When it is on, only the agents that already left you a note are asked, and each of them only once.
...
<TypeTable
  type={{
    ...
    'welcomeBack.offersEnabled': {
      description:
        "Whether a note may end with one next step for you to decide on. Off by default, because finding out costs a turn of the agent's time: only the agents that already left you a note are asked, and each only once. Only you can change this.",
      type: 'boolean',
      default: 'false',
    },
  }}
