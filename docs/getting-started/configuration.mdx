...
There is a switch for this in the app, at Settings → Preferences → **Welcome-back notes**, and it changes the same `welcomeBack.enabled` setting below. The other two are file-only for now. See [When you come back after being away](/docs/concepts/rooms#when-you-come-back-after-being-away) for what the notes look like.
