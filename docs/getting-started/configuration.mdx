...
<Callout type="info">
  DorkOS stores no runtime API keys. Codex authenticates via `codex login` (or `CODEX_API_KEY` in
  the server's environment); OpenCode provider credentials live in OpenCode's own `auth.json` via
  `opencode auth login`.
</Callout>
