...
2. Check the connection indicator in the status bar. It only appears when the live connection isn't healthy, so no indicator is good news. If it says **Reconnecting**, DorkOS is recovering the connection automatically and nothing is lost, just wait. If it says **Connection lost**, refresh the page; if that doesn't help, confirm the DorkOS server itself is still running (see the health check above). On a narrow window that same warning reads **Offline**.
