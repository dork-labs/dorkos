...
## "Restart DorkOS and Reset all data in the desktop app"

Both buttons in **Settings → Danger zone** work in the desktop app. The app runs the server itself, so it does the work for you:

- **Restart DorkOS** stops the server, starts it again, and points the window at the new one. If the restart fails, the app tells you and leaves a server running.
- **Reset all data** stops the server, deletes your data folder, then starts fresh.

Two things worth knowing. The app will not reset while another copy of DorkOS is using the same data folder, so quit the other one first. And if you open a desktop-run DorkOS in a normal browser tab or on your phone, both buttons refuse there and say why: only the app window can ask the app to restart itself.

You can always start over by hand instead. Quit DorkOS, delete your data folder (`~/.dork`), then open DorkOS again and it sets itself up from scratch.
...
## "The doctor says my git is too old to protect my agents"

Your agents use git in their folders. A folder can be set up to look like a git repository and carry a setting that makes git run a program the moment anyone looks inside. DorkOS tells git to ignore those settings every time it or one of your agents runs git. Git 2.38 and later honor all of it. Git 2.31 to 2.37 honor part of it, and older git none of it for your agents.

1. Check your version with `git --version`.
2. Update git to 2.38 or later. On a Mac, `brew install git`. On Linux, your package manager's git or the [git downloads page](https://git-scm.com/downloads). On Windows, the installer from the same page.
3. Restart DorkOS and run `dorkos doctor` again.

DorkOS also refuses to install a marketplace package shaped like a git repository, whatever your git version.
...
Inside DorkOS, open the help menu and pick **Send feedback**. Choose Bug or Idea, write what happened, and press Send. You can follow what you've sent under **Your reports** in the same menu. Prefer a public GitHub issue? Run `dorkos feedback` in your terminal, or use **Open one instead** at the bottom of the feedback form. Either one opens a prefilled issue with your version, operating system, and settings already filled in. You review and edit everything before you submit; DorkOS sends nothing on its own.
