...
2. Check the live stream indicator in the status bar. It only appears when the live stream isn't healthy, so no indicator is good news. If it says **Reconnecting**, DorkOS is recovering the stream automatically and nothing is lost, just wait. If it says **Live stream lost**, refresh the page; if that doesn't help, confirm the DorkOS server itself is still running (see the health check above). On a narrow window that same warning reads **Offline**.
...
4. Open the session in a fresh browser tab. If it opens fine there, the problem was the original tab or its live stream, not the session itself.
