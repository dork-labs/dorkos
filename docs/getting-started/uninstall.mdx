...
The Obsidian plugin is retired. Disable **DorkOS Copilot** in **Settings > Community plugins**. Then uninstall it, or remove only its copied plugin folder or symbolic link. Keep your vault, DorkOS data folder, and agent conversation history. The plugin could use data outside the vault. See the [retirement guide](/docs/guides/obsidian-plugin) for migration and source recovery.
...
| `~/.dork/dork.db`     | SQLite database (agents, schedules, relay state, conversations)            | DorkOS      |
| `~/.dork/backups/`    | Automatic spare copies of that database (see the warning below)            | DorkOS      |
...
<Callout type="warn">
  `~/.dork/backups/` holds the only spare copies of your channels, DMs, and threads. Deleting
  `~/.dork` deletes those too. If you might want that history later, move the newest file out of
  `backups/` first. Each one is a complete database on its own.
</Callout>
