---
title: Installation
description: Install DorkOS and open the one place where you run, schedule, and check on your coding agents
---
...
<Callout type="info">
  ...
  desktop app and Docker bring their own, so you don't have to install anything else.) If your
  agents use git, use git 2.38 or later: older git can be tricked into running a program when an
  agent looks inside a folder, and `dorkos doctor` tells you which you have.
</Callout>

## Try it without installing

Curious before you commit to anything? If you have Node.js 22.22.3 or newer, you can run DorkOS straight from npm with `npx`, no install step first:
...
This checks for Node.js 22.22.3 or later, installs DorkOS, and asks if you'd like to run the setup wizard. Say yes and it'll ask for your preferred port, a UI theme, and which folder you want DorkOS to work in by default. You can change any of these later.
...
DorkOS starts on port 4242 and opens your browser to the app.
...
<Callout type="warn">
  ...
  active development, so expect a rough edge here and there. If you hit trouble, the web app from
  the tabs above is the most battle-tested way to run DorkOS today.
</Callout>

A real Mac app: it's the same DorkOS, in its own window and dock icon, with nothing to install in a terminal. It bundles everything it needs, including Claude Code, so you don't need Node.js.
...
<Callout type="warn">
  ...
  on a real machine, so treat it as an early preview. If something does not work, the web app from
  the tabs above is the most reliable way to run DorkOS today.
</Callout>
...
The app runs its own DorkOS server, normally at `http://localhost:4242` just like the CLI. **Settings
→ Server** shows the exact address, copies it, and opens it in your browser, which is what you need
to bookmark DorkOS or point another tool at it.

See the [Desktop App guide](/docs/getting-started/desktop-app) for the full menu, deep link, and
auto-update details, plus what it doesn't do that the web app does.
...
Navigate to [http://localhost:4242](http://localhost:4242) in your browser. You'll land on DorkOS.
...
<Callout type="warn">
  ...
  a dependable daily driver today, use the web app above; come back here once it's further along.
</Callout>
...
<Callout type="info">
  ...
  machine, though it also means the plugin needs Node.js 22.22.3 or later installed on your system.
</Callout>
...
This starts the Express API server and Vite dev server together, with hot reload as you edit. DorkOS is available at [http://localhost:6241](http://localhost:6241).
...
Nothing to do: the desktop app updates itself. It downloads new versions in the background, then offers you a restart to install one. An update also installs itself the next time you quit and reopen the app. Closing the window doesn't quit the app, so take the restart when it's offered. (On Windows, updates ride the same unverified alpha mechanism.)
