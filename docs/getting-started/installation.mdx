...
<Callout type="info">
  ...
  [Anthropic](https://console.anthropic.com/) that lets DorkOS talk to Claude on your behalf). If you
  don't have one yet, create it there first. Everything except Docker also needs
  [Node.js](https://nodejs.org/) 18 or later, the JavaScript runtime DorkOS runs on.
</Callout>
...
<Callout type="warn">
  ...
  community plugin directory, and it's under active development, so expect rough edges. If you
  want a dependable daily driver today, use the web cockpit above; come back here once it's further
  along.
</Callout>
