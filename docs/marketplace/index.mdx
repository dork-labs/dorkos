...
A source address is a git repository DorkOS clones from, so it has to be one git understands: an
`https://`, `git://` or `ssh://` URL, or the short `git@host:owner/repo` form. A folder on this
machine works too, written as `file:///path/to/folder`. Anything else is turned down when you add
it, with a note saying so — git accepts some exotic addresses that run a command instead of
fetching a repository, and DorkOS does not pass those along.
