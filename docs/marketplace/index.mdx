...
Click a package to open the detail sheet. You'll see the README, the requested permissions, the external hosts the package needs to reach, and an install button. Review the permission preview and confirm — DorkOS handles the rest.
...
Marketplace is the in-product browse experience. It mirrors the public catalog but adds the install machinery — you can go from "interesting package" to "running on my machine" without leaving the app.
...
- **A specific agent**, shown by name — installed only for that agent's sessions. If the same package is also installed globally, the agent row is marked **Overrides global**: for that agent, the agent's copy takes precedence.
...
<Callout type="info">
  ...
  extensions everywhere. DorkOS warns you about this in the install dialog when it applies.
</Callout>
