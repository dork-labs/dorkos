...
Click a package to open the detail sheet. You'll see the README, the requested permissions, the external hosts the package needs to reach (for example, a Discord adapter reaching Discord's API), and an install button. Review the permission preview and confirm. DorkOS handles the rest, and if anything goes wrong partway through, it puts things back exactly as they were.
