...
Click the **Marketplace** entry in the cockpit's sidebar. In a browser you can also go straight to `/marketplace` on your DorkOS address, which is [http://localhost:4242/marketplace](http://localhost:4242/marketplace) on the default port. **Settings → Server** shows yours if it is different.
