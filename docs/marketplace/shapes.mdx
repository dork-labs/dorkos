...
That gives you a new Shape named `my-ops`, a copy of `linear-ops` that you own and can edit. The copy remembers where it came from, so the switcher shows a small "forked from linear-ops" note under its name. The original is left exactly as it was.

If you fork the Shape you have applied right now, add `--capture-current` to snapshot your live setup into the copy:

```bash
dorkos shape fork linear-ops --as my-ops --capture-current
```

This captures how your workspace is arranged at that moment, along with the extensions and schedules the Shape set up, so your version starts from what you actually have in front of you.
