...
<Callout type="info">
  A Shape changes your whole screen, so it belongs to you, everywhere. It is not tied to one agent
...
**It sets up the Shape's schedules.** These are the recurring tasks the setup runs for you. Most arrive turned off, and you switch them on when you are ready: a Shape only starts a timer on arrival if its author asked for that, and even then only once the agent it runs as exists. If a schedule is meant for an agent you have not created yet, DorkOS sets it up turned off and picks it back up the moment you create that agent, so you do not have to come back and re-apply the Shape. DorkOS remembers which Shape created each schedule, so it can clean them up later.

A Shape also cannot hand the timers it sets up for you a free pass. If one asks to run with every approval prompt turned off, DorkOS sets it up asking instead and leaves you a note saying so. You can still raise it yourself on the task, once you have read what it does.
...
Found a Shape that is close to what you want, but not quite? You can make a copy and change it, without touching the original. Developers call this forking.

Both paths make the same thing: a new Shape that is yours to edit. The copy remembers where it came from, so the switcher shows a small "forked from linear-ops@dorkos-community" note under its name. The original is left exactly as it was.

### From the Shape switcher

This is the way to keep the setup you are using right now. Open the Shape switcher and click **Make your own version** at the bottom, next to **Reset to defaults**. Name your copy and click **Create**.

Your copy records the arrangement you are in:

- the extensions you have turned on, and
- how your workspace is arranged — whether the sidebar is open, and which panels are open.

Anything the switcher cannot see stays exactly as the original Shape had it, so your copy never records a setting you did not choose. Panels close on their own when you reload the page, so if none are open when you make your copy, it keeps the original's panels rather than guessing you wanted them gone.

Your copy is saved and shows up in the list right away, but DorkOS does not switch you over to it: the original stays the applied Shape. When you want to work in your copy, click it in the list like any other Shape.

<Callout type="info">
  The button copies the Shape you have applied right now — that is the only one whose arrangement
  DorkOS can see. To copy any other Shape, use the command line.
</Callout>
...
That gives you a new Shape named `my-ops`, a copy of `linear-ops` that you own and can edit. It works for any installed Shape, applied or not.

If you are copying the Shape you have applied right now, add `--capture-current`:

```bash
dorkos shape fork linear-ops --as my-ops --capture-current
```

From the command line, `--capture-current` snapshots which of the Shape's extensions you have turned on right now, so your copy starts from that set instead of the original's. It cannot capture your layout: the command line has no way to see how your workspace is arranged, so your copy keeps the original's. Use the switcher when the layout is what you want to keep.
