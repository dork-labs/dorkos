...
Open **Connections**, find **Accounts**, and search the current service catalog. A supported service can use DorkOS-managed OAuth without a separate setup entry. An explicit custom setup takes priority when one exists. Services that need unsupported or custom setup stay visible and explain what is missing. When more than one route is ready, you can change the route before continuing.
