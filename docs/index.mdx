---
title: DorkOS Documentation
description: The operating system for autonomous AI agents — scheduling, communication, and coordination.
---

import { Cards, Card } from 'fumadocs-ui/components/card';

# DorkOS

The operating system for autonomous AI agents. DorkOS gives your coding agents — Claude Code, Codex, and OpenCode — a scheduling engine, a messaging layer, and a discoverable agent network, so they can work independently, reach you when they need to, and coordinate across projects.

Open source. Runs on your machine.

## Get Started

<Cards>
  <Card title="Installation" href="/docs/getting-started/installation">
    Install via npm, Obsidian plugin, or clone for self-hosting.
  </Card>
  <Card title="Quickstart" href="/docs/getting-started/quickstart">
    Launch DorkOS and open your first session in under two minutes.
  </Card>
  <Card title="Configuration" href="/docs/getting-started/configuration">
    Environment variables, config file, and CLI flags.
  </Card>
</Cards>

## Guides

Task-oriented walkthroughs for common workflows.

<Cards>
  <Card title="CLI Usage" href="/docs/guides/cli-usage">
    Flags, subcommands, and config management from the terminal.
  </Card>
  <Card title="Obsidian Plugin" href="/docs/guides/obsidian-plugin">
    Run DorkOS as an Obsidian sidebar panel — query agents without leaving your notes.
  </Card>
  <Card title="Runtimes" href="/docs/guides/runtimes">
    Run Claude Code, Codex, and OpenCode sessions side by side — including local models.
  </Card>
  <Card title="Tool Approval" href="/docs/guides/tool-approval">
    Configure tool approval flows and permission modes.
  </Card>
  <Card title="Slash Commands" href="/docs/guides/slash-commands">
    Discover and create custom slash commands from `.claude/commands/`.
  </Card>
  <Card title="Tunnel Setup" href="/docs/guides/tunnel-setup">
    Access DorkOS from any device via ngrok tunnel.
  </Card>
  <Card title="Keyboard Shortcuts" href="/docs/guides/keyboard-shortcuts">
    Navigate the interface with hotkeys.
  </Card>
  <Card title="Task Scheduler" href="/docs/guides/task-scheduler">
    Schedule agents on a cron — independently, not tied to your terminal.
  </Card>
  <Card title="Relay Messaging" href="/docs/guides/relay-messaging">
    Connect agents to Telegram, webhooks, and each other.
  </Card>
  <Card title="Relay Observability" href="/docs/guides/relay-observability">
    Track message delivery, inspect traces, and debug failures.
  </Card>
  <Card title="Agent Discovery" href="/docs/guides/agent-discovery">
    Register, discover, and monitor agents with Mesh.
  </Card>
  <Card title="Agent Coordination" href="/docs/guides/agent-coordination">
    Patterns for coordinating multiple agents with Relay, Mesh, and Tasks.
  </Card>
  <Card title="Agent Identity" href="/docs/guides/agents">
    Give your projects a name, color, and icon — think in agents, not file paths.
  </Card>
  <Card title="Persona Injection" href="/docs/guides/persona">
    Shape Claude's behavior per-project with custom system prompt context.
  </Card>
  <Card title="Building Relay Adapters" href="/docs/guides/building-relay-adapters">
    Create custom adapters to connect agents to any external channel.
  </Card>
</Cards>

## Concepts

How DorkOS works under the hood.

<Cards>
  <Card title="Architecture" href="/docs/concepts/architecture">
    How DorkOS connects to Claude Code via the Agent SDK.
  </Card>
  <Card title="Sessions" href="/docs/concepts/sessions">
    The session model, runtime-owned storage, and multi-client visibility.
  </Card>
  <Card title="Transport" href="/docs/concepts/transport">
    The Transport abstraction that decouples UI from backend.
  </Card>
  <Card title="Relay" href="/docs/concepts/relay">
    Subject-based messaging between agents and external channels.
  </Card>
  <Card title="Mesh" href="/docs/concepts/mesh">
    Agent discovery, registry, and network topology.
  </Card>
</Cards>

## Integrations

Build custom clients or connect DorkOS to your workflow.

<Cards>
  <Card title="Building Integrations" href="/docs/integrations/building-integrations">
    Use the Transport interface or REST API to build custom clients.
  </Card>
  <Card title="SSE Protocol" href="/docs/integrations/sse-protocol">
    Wire format, event types, and connection lifecycle.
  </Card>
  <Card title="API Reference" href="/docs/api">
    Interactive OpenAPI documentation for all endpoints.
  </Card>
</Cards>

## Self-Hosting

Run DorkOS on your own machine or server.

<Cards>
  <Card title="Deployment" href="/docs/self-hosting/deployment">
    Build, configure, and run DorkOS in production.
  </Card>
  <Card title="Reverse Proxy" href="/docs/self-hosting/reverse-proxy">
    nginx and Caddy configs with SSE support.
  </Card>
</Cards>

## Contributing

<Cards>
  <Card title="Development Setup" href="/docs/contributing/development-setup">
    Clone, install, and run the dev servers.
  </Card>
  <Card title="Architecture" href="/docs/contributing/architecture">
    Hexagonal architecture, FSD layers, and service overview.
  </Card>
  <Card title="Testing" href="/docs/contributing/testing">
    Vitest patterns, mock Transport, and test conventions.
  </Card>
</Cards>
