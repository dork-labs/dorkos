...
### `interaction_pending` goes only to whoever may answer it

Every other event on this stream is written to every connection. `interaction_pending` is not.
It carries what an agent is waiting for — the tool it wants to run, the command or path it
would run it against, and the session's working directory — so it is delivered only to a
