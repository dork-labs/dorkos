...
<TypeTable
  type={{
    ...
    queue_update: {
      ...
      description:
        "The session's message queue changed. Carries the whole queue every time, never a diff — a client that missed an update is corrected by the next one. `outcome` is present when the sender's own message caused this update.",
    },
  }}
/>
