...
### Five events go only to whoever they are about

Most events on this stream are written to every connection. Five are not:
`interaction_pending`, `approval_pending`, `notification`, `notification_read` and
`config_changed`. Each carries something about what your agents are doing, what is waiting on
