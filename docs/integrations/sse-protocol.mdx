...
Most events on this stream are written to every connection. Four are not:
`interaction_pending`, `approval_pending`, `notification` and `notification_read`. Each carries
something about what your agents are doing or what is waiting on you, so each goes only to a
connection that could act on it.
