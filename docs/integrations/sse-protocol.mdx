...
- **Refusals arrive as a close code, not an HTTP status.** A browser cannot see the status of a
  ...
  `event.code` in `onclose` and subtract 4000. This is how every refusal of these three streams
  arrives, for browser and non-browser clients alike: the handshake completes and the socket closes
  immediately, so do not wait for an HTTP status that will not come.
