...
| `agents_changed`       | `{ kind: 'registered' \| 'updated' \| 'removed', agentId: string, projectPath?: string, name?: string, displayName?: string, changedAt: string }` | An agent was registered, renamed or removed, by any path. Names and ids only — read the agent back for the rest                                    |
| `config_changed`       | `{ sections: string[], changedAt: string }`                                                                                                       | Settings were written. Section names only, never a value. Addressed — see below                                                                    |
...
for agents as well as people, and the payload is names and ids: whatever else the manifest says,
a reader has to go and ask for.
