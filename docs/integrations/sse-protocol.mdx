...
<TypeTable
  type={{
    ...
    system_status: {
      ...
      description:
        'Out-of-band status notice from the model or SDK — e.g., a refusal ("the model declined to respond"), a max-tokens cutoff, or a model-unavailable message. `message` is human-readable; `status` carries the raw SDK status value when present.',
    },
    done: {
