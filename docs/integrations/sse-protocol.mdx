...
The stream opens with a `connected` event, and then — before any live traffic — one
`session_status` event for every session the server currently sees as working, waiting on a
person, or stopped with an error. Sessions that are idle send nothing, because there is nothing
to say about them.
...
can tell you. Treat these exactly like any later `session_status` — same shape, same handler. The
next real transition replaces whatever the opening ones told you.

The server only remembers a session's live state while the session is recent (about 30 minutes of
quiet, or until the server restarts), so this is a picture of the recent fleet, not of all
history.
