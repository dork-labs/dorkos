...
| `dorkos://capabilities`  | List     | The live capability catalog: id, title, description, permission tier, input/output schema, and surfaces for everything you can invoke by id.         |

Session and skill resources are scoped to the server's default working directory; the external surface has no per-request `cwd` parameter to key on. Each list resource is the complete enumeration for its kind, so the item templates aren't re-listed separately. A client reads them once it already has an id or name.
