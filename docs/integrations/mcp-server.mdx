...
Clicking **Render app** remembers that choice for the server (stored per-server, so a trusted server never asks twice) and renders the app inline in the message. Leaving the card alone simply skips the app for now. A **fullscreen** control on a rendered app pops it out to the full canvas for apps that need more room. A **pop out** control floats the app in a small window that stays on top while you move around DorkOS, so you can keep a live app in view while you work elsewhere (on a wide enough screen). These are all backed by the same sandboxed frame.
