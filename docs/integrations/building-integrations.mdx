...
<Callout type="info">
  This whole section is for **builders**: people writing custom clients, extensions, marketplace
  adapters, or MCP servers on top of DorkOS. If you just want to connect a chat app or a service to
  your agents, like Telegram or Gmail, you want the [Connections](/docs/connections) page instead,
  no code required.
</Callout>
...
The browser, phone web app, and desktop renderer use HttpTransport. Tests can inject a mock through the same interface.
...
**HttpTransport** is used by the browser, phone web app, and desktop renderer. It communicates with the Express server over HTTP and WebSocket:

- Standard `fetch()` for CRUD operations
- `postMessage()` triggers a turn (`202` with the canonical session id)
- `subscribeSession()` consumes the durable WebSocket stream `GET /api/sessions/:id/events`; `subscribeSessionList()` consumes `GET /api/events`. The same routes also support SSE for external clients.
...
**Authentication.** By default, local login is off (`auth.enabled: false`), so a local DorkOS instance accepts these requests with no credentials, same as the web app talking to its own server. If you turn local login on, every `/api/*` and `/mcp` request needs either a Better Auth session cookie or a per-user API key sent as `Authorization: Bearer <key>`. The separate external MCP server at `/mcp` is the exception to the first sentence: even with login off it wants this instance's local MCP token for anything that changes state, and only the handshake, discovery calls, and read-only tools answer without one. In headless or CI deployments you can set a static `MCP_API_KEY` and hand that to every client instead.
