...
<Callout type="warn">
  **Docker operators:** the official image skips this check too, for the same reason it skips the
  bind check. `DORKOS_ALLOW_INSECURE_BIND=true` hands the network boundary to the container. If
  other people can reach that container, [enable login
  first](/docs/self-hosting/docker#with-a-reverse-proxy).
</Callout>
