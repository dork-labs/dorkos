...
<Callout type="warn">
  **`DORKOS_ALLOW_INSECURE_BIND=true` also turns this address check off.** That setting now does two
  jobs: it lets DorkOS listen beyond your own machine, and it stops DorkOS checking the address each
  request arrived under. So if you set it once to get past the startup error, you gave up the
  rebinding protection above as well, even on `localhost`.
