...
The email is a **local identifier only**: it is never verified and no email is ever sent. There is no SMTP anywhere in the local edition.
...
Exposing an instance — starting a tunnel, or binding to a non-loopback network interface — now **requires login**. This replaces the old tunnel passcode entirely.
...
- **Binding to a non-loopback host** (`DORKOS_HOST` set to something other than `localhost` / `127.0.0.1` / `::1`) with login not configured **refuses to start** with an actionable error. Enable login first, then restart.
...
<Callout type="info">
  Containers that own their own network boundary (e.g. the official Docker images binding `0.0.0.0`)
  can set `DORKOS_ALLOW_INSECURE_BIND=true` to override the bind gate. Real machines should enable
  login instead.
</Callout>
...
Send the key as `Authorization: Bearer <key>` to `/mcp` or any `/api/*` route.
...
<Callout type="info">
  **Headless deployments:** the `MCP_API_KEY` environment variable is still honored as a static
  override for the `/mcp` endpoint. It takes priority over per-user keys and cannot be revoked from
  the UI — ideal for CI and server deployments where no one is signing in.
</Callout>
...
There is no "forgot password" email — the local edition sends no email. Because machine access already implies owner-level trust, you recover from the command line:
...
<Callout type="error">
  ...
  is the owner, and further sign-ups are closed. A viewer/operator role model that safely admits
  more people is a planned fast-follow; until it exists, only give the owner login to people you
  trust with your machine and your Claude account.
</Callout>
