...
<Callout type="info">
  **Reference, for container setups:** containers that own their own network boundary (e.g. the
  official Docker images binding `0.0.0.0`) can set `DORKOS_ALLOW_INSECURE_BIND=true` to override
  the bind gate. Most people never need this. Real machines should enable login instead.
</Callout>
