...
Every published image includes a [SLSA](https://slsa.dev/) attestation (a signed record of exactly which source commit built it) proving it came from the `dork-labs/dorkos` repository, not a fork. Verify any image with the GitHub CLI:
