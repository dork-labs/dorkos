...
DorkOS deploys as a standalone server via the `dorkos` npm package. One process serves both the API and the web app, so nothing but your calls to Anthropic ever leaves the machine it's running on. Run it on your laptop, a spare desktop, or a cloud box, it's the same install either way.
...
Open `http://localhost:4242` in your browser. That's DorkOS, already running. (Don't have a key yet? The [Installation guide](/docs/getting-started/installation) shows you where to get one.)
...
<Cards>
  <Card title="Reverse Proxy" href="/docs/self-hosting/reverse-proxy">
    Configure nginx or Caddy for HTTPS, domain routing, and SSE (the live-update stream the app
    uses) compatibility.
  </Card>
