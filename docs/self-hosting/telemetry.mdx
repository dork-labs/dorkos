...
Set a value to `false` to turn a channel off, or `true` to turn it on. The three anonymous channels default to `true`; error reporting defaults to `false` and its flag is the only switch it needs.
