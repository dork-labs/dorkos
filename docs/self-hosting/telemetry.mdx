...
DorkOS sends us nothing unless you turn it on. If you choose to help, you can share a little anonymous data so we can see roughly how many people run it. It is anonymous, not personal: no prompts, no code, no file paths, no session content, ever. This page shows the exact data, word for word, and how to switch it on or off.

<Callout type="info">
  The three anonymous channels (a daily heartbeat, marketplace install counts, and a few named
  feature-usage events) are off until you turn them on. DorkOS prints a first-run notice explaining
  what it could share. Crash reporting is separate and also stays off until you turn it on. You can
  change any of this anytime.
</Callout>

## The short version

- Three anonymous channels are available, all off by default: a daily heartbeat, marketplace install counts, and a few named feature-usage events.
- On first run, DorkOS shows a notice explaining them. If you do nothing, nothing is ever sent.
- It is anonymous. No prompts, no code, no file paths, no session content, ever.
- Turn it on any time with `dorkos telemetry enable` or the Privacy & Data settings tab, and back off the same way. `DO_NOT_TRACK=1` keeps everything off no matter what the settings say.
...
The list above is the complete payload. It is enforced in code and in tests, so nothing sneaks in. It never includes your prompts, your code, file paths, your hostname or username, IP address, or anything from your sessions. This is what makes it anonymous rather than personal. If we ever want to add a field, we update this page first.

## We tell you before anything sends

The first time DorkOS starts, it prints a plain notice in its log: what it could share, a link to the payload, and how to turn it on. Nothing is sent on that run, or on any run, until you say yes. On top of that, the code holds every channel shut until the notice has actually been shown, so even switching a channel on cannot send before you have seen what it covers.

## Marketplace installs

The marketplace install-count channel is off by default too. Turning it on helps rank packages and spot broken installs. It follows the same no-PII rule and is documented on the [marketplace privacy page](https://dorkos.ai/marketplace/privacy).
...
Like the heartbeat and install counts, this channel is off until you turn it on. Two things still hold once you do: nothing sends until after DorkOS has shown you the first-run notice, and you can turn it back off three ways, the same as every other channel:
...
- It carries exactly what you chose to send: your message, your email if you gave one so we can tell you when it's fixed, and any picture you attached. If you pointed at part of the app, it names that part too. It also notes which screen you were on and your DorkOS version, so we have context.
...
Prefer a public thread? At the bottom of the feedback form, **Open one instead** opens a prefilled GitHub issue you review and edit before submitting.
...
## Linking analytics to your account (a separate opt-in)
...
## AI run metadata (a separate opt-in)
...
`dorkos --debug-trace` is a diagnostics tool, not part of the telemetry above, and it isn't covered by the opt-in dialog: it's a per-run flag you choose to add when you're troubleshooting something, most often because we asked you to when you report a bug.
...
`status` tells you each channel's state, whether a kill switch is overriding it, and whether you have already made a choice. `enable` and `disable` write your choice to `~/.dork/config.json` so the first-run dialog never asks again.
...
The first time you open DorkOS, a dialog asks whether you want to share anonymous data and shows you the payload above, with a **No thanks** and a **Share anonymously** button. Nothing is sent until you pick one. To change it later, open settings (there's a **Privacy & Data** tab with a switch for each channel), use `dorkos telemetry` above, or edit `~/.dork/config.json`:
...
Set a value to `true` to turn a channel on, or `false` to turn it off. Every channel defaults to `false`, and each one's flag is the only switch it needs.

<Callout type="info">
  Plenty of developer tools collect anonymous usage by default. We decided not to: nothing leaves
  your machine unless you say so. When you do turn a channel on, we keep it genuinely anonymous and
  show you the whole payload. Your code and your chats with the AI never leave your machine either
  way.
</Callout>
