...
# Sends "upgrade" only when the request is asking for one, and an empty value

# otherwise — the empty value is what keeps plain streaming connections from

# being closed early. Goes at the top level of nginx.conf, outside `server`.
...
<TypeTable type={{
  "Upgrade $http_upgrade": { type: "header", description: "Passes the client's upgrade request through, so live streams can open as WebSockets" },
  "Connection $connection_upgrade": { type: "header", description: "Says 'upgrade' for a WebSocket and stays empty otherwise — the empty case is what keeps plain streams from closing early" },
  "proxy_buffering off": { type: "directive", description: "Disables response buffering so events are sent immediately" },
  "proxy_cache off": { type: "directive", description: "Disables caching, live streams must never be cached" },
  "proxy_read_timeout 86400s": { type: "directive", description: "24-hour timeout for long-lived connections" },
  "chunked_transfer_encoding off": { type: "directive", description: "Disables chunked encoding, the stream uses its own framing" },
}} />
