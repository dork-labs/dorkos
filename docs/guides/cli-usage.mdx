...
That manifest holds three things, and nothing else: `harnesses`, the agents you want files shared with; `claudeOnlySkills`, the skills you are deliberately keeping in Claude Code's own folder; and `hookPolicies`, one line per agent saying what to do with your hooks. A policy is `generate` (DorkOS writes that agent's hooks file), `native` (the agent reads your hooks itself) or `none` (leave that agent's hooks alone). Say nothing about an agent and DorkOS does what it has always done for it. Four older keys — `skillWrappers`, `commandMappings`, `instructionProjections` and `skillBundles` — are no longer read by anything. Your file still works if it has them; DorkOS just names each one so you can delete it.
