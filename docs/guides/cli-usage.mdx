...
`--enable` is the only thing that ever writes to your manifest, and all it does is add the name to the list. Your own formatting, comments and key order stay exactly as you left them. You can pass it more than once. Nothing is turned on behind your back: without the flag, DorkOS only mentions it.
