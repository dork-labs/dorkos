...
**`dorkos capabilities`**: list everything this DorkOS can do.

```bash
dorkos capabilities                                  # Every capability as a table (id, title, tier)
dorkos capabilities --json                           # The full catalog as JSON
```

**`dorkos call`**: run any capability by its id. This is the one command that reaches every action, whether or not it has its own verb. List the ids with `dorkos capabilities` first.
