...
`--allow-hooks` saves your answer, so you only give it once. It is the same answer you would give on the approval card in DorkOS itself, kept in the same place. Held-back hooks are not a failure, so `sync` still finishes with a success code. Add `--strict` if you want a script to stop instead.
