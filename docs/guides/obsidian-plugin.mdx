...
- **Claude Code** — bundled with DorkOS, so no separate install is normally needed.

<Callout type="info">
  Claude Code ships with DorkOS as a per-platform binary. If the bundled binary
  is unavailable on your machine, install it manually as a fallback:
...
- **"Claude Code executable not found"** — the bundled Claude Code binary couldn't be resolved and no `claude` is on PATH. Install it manually as a fallback: `curl -fsSL https://claude.ai/install.sh | bash` (macOS/Linux) or `irm https://claude.ai/install.ps1 | iex` (Windows).
