...
<Callout type="warn">
  ...
  more setup than you want today, use the [standalone web app](/docs/getting-started/quickstart)
  instead. Everything here is built, but it hasn't been tested as widely as the web app — treat it
  as early and verify it yourself before relying on it.
</Callout>
...
This outputs to `apps/obsidian-plugin/dist/`:

- `main.js`: the plugin code
- `styles.css`: plugin styles
- `manifest.json`: plugin metadata
- `better_sqlite3-<platform>-<arch>-abi*.node`: SQLite, one build per Obsidian version the plugin
  supports, for the machine you built on. The build downloads these the first time and reuses them
  after. Build on the machine you run Obsidian on — a `dist/` copied from another kind of machine
  has the wrong binaries in it, and the plugin will tell you so rather than crashing.
...
- `sqliteAddon`: downloads SQLite builds for the Electron versions Obsidian ships, puts them beside
  the bundle, and points the bundle at them. Every download is checked against a SHA-256 pinned in
  `native-addons.lock.json`; a mismatch fails the build. Regenerate that lockfile with
  `pnpm --filter @dorkos/obsidian-plugin addons:lock` when the ABI list or `better-sqlite3` changes.
  Cached in `apps/obsidian-plugin/.native/`, so only the first build reaches the network, and skipped
  entirely under `CI`. If a download fails the build still succeeds — you get a plugin that cannot
  read your message history, and it says so in the console.
