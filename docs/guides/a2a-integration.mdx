...
```
[A2A] Gateway mounted (fleet card: /.well-known/agent-card.json, RPC: POST /a2a, auth: local MCP token (loopback))
```

The auth mode names the credentials a caller can send. `local MCP token (loopback)` means you've set no `MCP_API_KEY` and haven't turned on login, so calls are gated by the local MCP token (see [Authentication](#authentication)) and reachable from your own machine. Calls are never ungated.
...
Agent Cards advertise the spec-standard `http`/`bearer` security scheme, and they always say a bearer token is required — because it always is. The requirement is about **calling** an agent, not about reading the card: while login is off you can still fetch a card without a token, and the card itself tells you the call that follows needs one. Cards never say where your token lives; that stays between you and your own machine.
