...
**In the cockpit.** Open **Settings → Experiments** and flip "Let outside agents reach yours". DorkOS reads it once when it starts, so restart to pick up the change.
...
`tasks/cancel` asks the agent that's running the turn to stop, and answers with what really
happened. You get the task back marked `canceled` only when something confirms the turn is
ending. If nothing can be stopped — the turn already finished, or DorkOS restarted and lost
track of it — you get a `TaskNotCancelable` error instead, and the task keeps the state it
had. A "canceled" answer means the work stopped, not that the request was filed.
