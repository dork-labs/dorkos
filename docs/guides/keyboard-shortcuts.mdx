...
<TypeTable
  type={{
    ...
    '⌘⇧F / Ctrl+Shift+F': {
      description:
        'Search your messages by what was said in them — your channels and direct messages, and your Claude Code, Codex and OpenCode conversations.',
      type: 'key',
    },
    '⌘B / Ctrl+B': {
...
<TypeTable
  type={{
    ...
    '⌘Enter / Ctrl+Enter': {
      description:
        'While your agent is working, send the message straight into what it is doing so it changes course without starting over. If your agent cannot take a message mid-task, the message lines up to go next instead, and says so.',
      type: 'key',
    },
    '⌘⇧Enter / Ctrl+Shift+Enter': {
      description:
        'While your agent is working, add the message as background for its next reply, without interrupting the one in progress. If your agent cannot take added context, the message lines up to go next instead.',
      type: 'key',
    },
    '\\ then Enter': {
      description:
...
If you would rather see the plain markdown characters, turn it off in **Settings → Preferences → Format text as you type**.
