...
<Callout type="info">
  **These keys work in the desktop app.** In a web browser they all belong to the browser's own
  tabs, so none of them reach DorkOS. Use the tab strip instead: press `Tab` to reach it, arrow keys
  to move between tabs, `Home` and `End` to jump to either end, and `Delete` to close the one in
  focus. The `+` button opens a tab, and the `×` on a tab closes it, anywhere.
</Callout>
...
<TypeTable
  type={{
    ...
    '⌘Enter / Ctrl+Enter': {
      description: 'Open the selected agent in a new DorkOS tab (skips the sub-menu).',
      type: 'key',
    },
    Backspace: {
...
Pick an agent and its sub-menu asks where you want it: **Open Here**, **Open in New Tab**, or **Open in New Window**. In the desktop app, New Window gives you a second DorkOS window, which is what you want for a second screen. In a browser it opens a new browser tab.
