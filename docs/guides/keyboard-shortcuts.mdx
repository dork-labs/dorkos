...
<TypeTable
  type={{
    ...
    '⌘⇧A / Ctrl+Shift+A': {
      description:
        "Open the agent's profile in the right panel. Press it again to close the panel.",
      type: 'key',
    },
    '?': {
...
<Callout type="info">
  These keys work in the desktop app. In a web browser, `⌘T` and `⌘1` to `⌘9` belong to the
  browser's own tabs, so use the tab strip instead: press `Tab` to reach it, arrow keys to move
  between tabs, `Home` and `End` to jump to either end, and `Delete` to close the one in focus.
</Callout>
