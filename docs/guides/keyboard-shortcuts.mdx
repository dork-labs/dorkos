...
There is an optional setting that makes the message box show formatting while you write, instead of leaving the markdown characters on screen. It is **off unless you turn it on**, and it applies to chat for now.

Turn it on in **Settings → Advanced → Format text as you type**.
...
same either way — `Shift+Enter` is still a new line, Escape still stops your agent, and two
Escapes still clear the box.
