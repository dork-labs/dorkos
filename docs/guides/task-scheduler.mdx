...
Your approval is tied to what the task actually says. Change its prompt or its timing, the timezone included, and it parks again for a fresh look. (A change you make yourself in DorkOS counts as your approval, so it keeps running.) When an agent makes the change through DorkOS, the schedule stops in that same moment, before the new version can run, and the card says what the agent changed: what the schedule does, or when it runs. Approve the same content twice, and it stays approved through every later sync.
