...
A schedule that leaves its runtime, model or effort on "follow the agent" also depends on the agent's own settings. An agent can't change those through DorkOS without asking you first. It could still edit its settings file (`.dork/agent.json`) directly, and so could you, and DorkOS can't tell which of you did. So DorkOS checks before each run and every few minutes. When one of those settings was changed outside DorkOS, the change is recorded in Activity as **Changed outside DorkOS**, and every approved schedule that follows the agent for that setting waits for you again. Its card shows the change, for example **Agent's model: claude-sonnet-4 → claude-opus-4**. Changing the setting back doesn't restart the schedule: the card says it was changed, then changed back, and you approve it. A change you make in DorkOS, from **Runs on** or the agent's settings, never makes a schedule wait.
