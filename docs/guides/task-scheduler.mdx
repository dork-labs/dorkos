...
What the schedule does, its instructions and its name, still belongs to the package, because the package's next update would put its own version back. If you try to change them, the edit window says so and offers **Make my own copy**: it opens a new schedule for the same agent with your changes already filled in, named after the original with `-copy` on the end. Check it and choose **Create**. The package's schedule keeps running unless you switch it off.
...
- any new schedule for an agent whose package was installed by an older version of DorkOS, because DorkOS can't yet tell that package's files from yours. Update or reinstall the package once, then try again.
