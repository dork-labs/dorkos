...
You can lift it, one job at a time, as you approve it. If your own setting in **Control Center** is higher than the one the job was held to, the approval card offers a second button naming that level — "Approve at Full autonomy", say — beside the plain Approve. The card tells you what each one will run the job at. Plain Approve never changes the setting, and lifting one job never lifts the next one: the next schedule an agent proposes arrives held back exactly the same way.
