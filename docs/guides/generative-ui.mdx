...
Where that id comes from depends on the surface, and the two differ today. In a **room**, every verb that opens something (`open_canvas`, `open_file`, `open_diff`, `browser_navigate`) returns the new document's id, because the room's canvas is the server's and the row exists before the call answers. In a **one-on-one session** it does not: the call answers with `success` and the action it ran, and nothing else. That session canvas lives in your own browser rather than on the server, so there is no id to hand back until it moves there.
