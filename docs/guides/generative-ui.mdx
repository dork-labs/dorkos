...
Every runtime you can pick in DorkOS (Claude Code, Codex, or OpenCode) renders widgets the same way, so switching runtimes never changes what you see. Putting one on the canvas is the same set of tools on all three, too — nothing to turn on.
