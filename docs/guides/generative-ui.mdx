...
On desktop, you can also pop an interactive widget, like a tic-tac-toe board, out of the chat into a small floating window that stays visible while you work elsewhere. It follows the newest board the agent posts, so a game keeps going even after you switch to another session; the chat keeps its own copy too, and closing the window just dismisses it.
