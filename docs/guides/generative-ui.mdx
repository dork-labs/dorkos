...
The canvas is a tabbed, multi-document surface (each `open_canvas` call appends a tab and activates it, deduping by source so reopening the same file re-activates its existing tab instead of duplicating it), and `widget` is one of thirteen content types it can render: `widget`, `image`, `pdf`, `file` (CodeMirror, for arbitrary text/code), `markdown` (the rich Blintz editor), `model3d` (glTF/GLB/STL/OBJ/3MF/PLY/FBX/DAE), `audio` (HTML5 player), `video` (HTML5 player), `csv`, `browser` (an embedded, sandboxed page), `mcp_app` (a UI resource an MCP server hosts), `url`, and `json`. The two content-heavy ones worth knowing up front:
