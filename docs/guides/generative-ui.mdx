...
Agents don't have to describe data in prose — they can render it. DorkOS's generative UI system lets any agent emit a small, declarative JSON document that the client renders as a real, native widget: stat cards, tables, lists, charts, key/value pairs, badges, progress bars, images, and forms. There's no agent-authored code and no iframe — every widget is built from the same shadcn components the rest of DorkOS uses, so it looks and behaves like the surrounding UI.
...
| Type       | Fields                                                                                        | Renders as                                                                                           |
| ---------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `stat`     | `label`, `value`, `delta?: { value, direction: "up" \| "down" \| "flat" }`, `hint?`           | A labelled metric with an optional trend arrow and hint                                              |
...
| `list`     | `items: [{ title, subtitle?, icon?, badge?, actions? }]`                                      | A vertical list of rows, each with an optional icon, badge, and action buttons                       |
...
- `image.src` must start with `https://` or `data:` — no bare paths, no `http://`.
