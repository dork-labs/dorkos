...
Behind the scenes, the agent sends a small JSON document (a simple, structured text format) and DorkOS turns it into a native UI element: a stat card, a table, a list, a chart, a timeline, a checklist, a comparison matrix, a star rating, a key/value list, a badge, a progress bar, an image, or a form. There's no code the agent writes and no embedded webpage. Every widget uses the same building blocks as the rest of DorkOS, so it looks and feels like it belongs.
