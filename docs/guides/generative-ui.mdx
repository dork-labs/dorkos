...
`update_canvas` and `close_canvas` also take an optional `documentId`, and every verb that opens something (`open_canvas`, `open_file`, `open_diff`, `browser_navigate`) hands one back. Leave it out in a one-on-one session and the call acts on the active document, exactly as it always has. Leave it out in a **room**, where there is no shared active document on purpose, and it means "the last document you opened in this room" — so acting on a document somebody else put up takes naming it.
