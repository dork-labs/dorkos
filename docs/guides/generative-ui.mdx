...
send a note back to the agent, or that would open something in the canvas, the browser or a
terminal, are still shown but switched off, with a short note when you hover them. Buttons that
only touch the app itself, like opening Settings or a link, work exactly as they do in chat.
...
Two of those fourteen — `url` and `browser` — are pages, and pages live one tab along, in **Browser**. The other twelve open in **Canvas**. `mcp_app` stays in Canvas despite being served over HTTP: it's an app, not a page.
