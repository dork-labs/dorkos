...
Click an empty square and your mark draws itself instantly, before the agent even replies. Its next turn re-emits the same `board` widget with your move applied — trusting the `state` string in your click's payload over its own memory, so the board you see and the board it thinks it's playing never drift apart.
