...
- **Pinning.** Pin a workspace you want to keep. DorkOS does not reclaim old workspaces on a schedule yet; when retention cleanup does run, it never removes a pinned workspace. The pin persists and shows on the card and in the status-bar chip tooltip.
