...
`name` is the agent's permanent slug, not a label: it is what the agent's `@handle` in a room is derived from. A name with spaces or capitals is turned into a slug (`"Research Bot"` becomes `research-bot`) and kept as the display name. Pass `displayName`, `icon` (one emoji) and `color` (a hex colour) to choose the rest of its identity; leave the last two out and DorkOS picks a face.
