...
<TypeTable
  type={{
    ...
    behavior: {
      type: 'AgentBehavior',
      description:
        "Response mode: 'always', 'direct-only', 'mention-only', or 'silent'. This is the agent's own default and it only seeds a direct message; a channel seeds 'engaged', and every room can override it",
