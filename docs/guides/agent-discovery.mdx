...
<ProductShot
  id="agent-discovery"
  alt="The Mesh tab scanning for agent projects and registering them one by one into a growing network view"
/>
...
<TypeTable
  type={{
    ...
    behavior: {
      type: 'AgentBehavior',
      description:
        "Response mode: 'always', 'direct-only', 'mention-only', 'engaged', or 'silent'. This is the agent's own default, and it only sets what a new direct message starts as. A channel always starts 'engaged', and a room someone already set up keeps what it has",
...
Discovery strategies automatically populate the manifest with hints extracted from the project (name from `package.json` or directory name, capabilities from detected tooling). You can override any field during registration — but only for a project directory that has no `.dork/agent.json` of its own. Point registration at a directory that already has one and DorkOS adopts what's already there instead: the manifest on disk is left untouched, and your overrides are ignored.
...
`name` is the agent's permanent address, not a label: it is what the agent's `@handle` in a room is derived from. A name with a space in it is read as a label, turned into a slug (`"Research Bot"` becomes `research-bot`) and kept as the display name; a name with no whitespace — `144x.co`, `next_starter`, `日本語` — is stored exactly as you sent it. Pass `displayName`, `icon` (one emoji) and `color` (a hex colour) to choose the rest of its identity; leave the last two out and DorkOS picks a face.

When an agent creates a registration via MCP tools, the manifest is written to `.dork/agent.json` in the project directory (or adopted as-is if one is already there, per the note above) and the agent becomes available in the registry right away.
