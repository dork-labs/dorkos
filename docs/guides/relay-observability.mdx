...
Relay computes aggregate delivery metrics straight from the trace store, so they always reflect the current state of the database. These give you a summary of bus health without inspecting individual traces.
...
<TypeTable
  type={{
    ...
    p95DeliveryLatencyMs: {
      type: 'number | null',
      description: '95th percentile delivery latency. Currently returns null, not yet implemented.',
    },
    activeEndpoints: {
      type: 'number',
...
<Callout type="info">
  ...
  `p95DeliveryLatencyMs` currently returns `null` and `budgetRejections` counters return `0` in this
  endpoint. They're tracked at the RelayCore level but not yet aggregated into trace store metrics.
</Callout>
