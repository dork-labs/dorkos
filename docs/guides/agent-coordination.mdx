...
<Callout type="info">
  This is a shipped capability we are still hardening end to end. Multi-agent messaging works in the
  setups we've tested, but treat it as early: watch what an agent actually does with a handoff
  before you trust it with something you can't easily undo.
</Callout>
...
### Let all my agents talk to each other

Pairs add up fast: five namespaces need twenty rules for everyone to reach everyone. If you'd rather not think about walls at all, Team → Access has a single switch, **Let all my agents talk to each other**. Turn it on and every agent on this machine can message every other one, whatever project it lives in. Turn it off and you are back to the defaults, with any pairs you added still exactly where you left them.

It is off when you install DorkOS — agents stay in their own project until you say otherwise. Behind the scenes it is one more rule, at the same priority 50, using `*` on both sides:

```bash
# The switch, from the command line
curl -X PUT http://localhost:4242/api/mesh/topology/access \
  -H 'Content-Type: application/json' \
  -d '{"sourceNamespace": "*", "targetNamespace": "*", "action": "allow"}'
```

`GET /api/mesh/topology` reports it as `openMesh`, and so does an agent's own `mesh_query_topology` tool. `*` has to be on both sides or neither — a half-wildcard is rejected.
...
<Callout type="info">
  Every message carries a budget: a default limit of 5 hops, a 1-hour TTL (time to live, how long a
  message stays valid before it expires), and 10 calls. A reply inherits its budget from the message
  that started the turn, so a chain of agents can't hand itself a fresh allowance. On top of that,
  DorkOS caps how many agent turns can start in an hour — by agent and across the whole install —
  because every route into an agent (a message, a scheduled task, an outside system) ends at the
  same counted choke point. See [Relay Messaging](/docs/guides/relay-messaging#the-hourly-ceiling)
  for both sets of numbers and how to change them.
</Callout>
...
Creating a task from the Tasks panel (or via `POST /api/tasks` with `name`, `description`, `prompt`, `cron`, `timezone`, and a `target` agent) writes exactly this file to disk; DorkOS never asks you to hand-edit JSON. Set the cron for the middle of the night on a machine that stays on, and you come back to a report, not a fire.
