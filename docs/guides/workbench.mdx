...
- **Dev servers you're running** — Vite, Next, anything on a `localhost` address — render too, as long as you're using DorkOS on the same machine that runs them. Live reload keeps working. From another device (a phone, a tablet, a tunnel), "localhost" means _that_ device, so the page won't render in the canvas yet — use the "Open in system browser" button in the browser toolbar, which is always there.
