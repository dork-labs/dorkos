...
If the file changes underneath you — say the agent edits it again while you're reviewing — you get a calm "this file changed" notice with a Refresh button, never a silent overwrite.

Reviewing an image isn't available here yet — a changed image opens on its own instead. Diff review works in the web app and the Obsidian plugin; the image tools, when they arrive, will be web-only.
