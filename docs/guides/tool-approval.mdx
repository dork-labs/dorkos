...
<Callout type="warn">
  **In group chats, anyone can respond.** Approval buttons are not restricted to the person who
  started the conversation — any member of the bound group chat or channel can approve or deny a
  tool execution. If that matters to you, bind agents with approval-gated permissions to private
  chats or DMs instead of shared channels.
</Callout>
