...
<Callout type="info">
  **This covers what an agent does in a session: editing files, running commands, and working
  ...
  removing an agent. Those are a separate question with its own setting: turn on **Standing
  permissions** in Settings, under Access, and you can then answer "stop asking about this" straight
  from an approval card. See [Action approvals](/docs/guides/action-approvals). You will see the
  same reminder next to the permission picker whenever you choose a stop that skips prompts.
</Callout>
