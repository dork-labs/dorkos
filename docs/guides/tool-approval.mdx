...
- **Full autonomy**: every tool prompt inside the session is skipped. The first time you turn this on, DorkOS asks you to confirm what it means and writes down that you did; after that, the session runs without asking again.

<Callout type="warn">
  Full autonomy turns off every prompt inside the session, so the agent edits files and runs
  commands without stopping. Only use it in isolated environments where you trust the agent
  completely.
</Callout>
