...
<Callout type="info">
  A session running at Full autonomy shows no standing warning banner of its own — the signal lives
  where the setting does, in the picker's word and tint. The one app-wide autonomy banner is for
  **unattended** drivers: a relay binding or scheduled task set to run without asking anyone, which
  you can't see just by looking at an open chat.
</Callout>
