...
<Steps>
  ...
  <Step>
    **Click Always Allow** (when available) to approve this action and stop being asked about
    similar calls. The button says how far that reaches: this session, this project, or all your
    Claude sessions.
  </Step>
  <Step>
...
2. Tells Claude Code to stop asking about similar tool calls

How far that reaches depends on what Claude Code suggests, and the button names it: **this session**, **this project**, or **all your Claude sessions**. The last two are saved in Claude Code's own settings files, so they last beyond this conversation.

Not every approval card shows this button: it only appears when Claude Code offers a suggestion for the tool.

This is different from **Always allow** on a DorkOS request card, which remembers your yes for one agent and one DorkOS action. See [Permissions](/docs/guides/permissions). The keyboard shortcut is <kbd>Shift+Enter</kbd>.
...
<Callout type="info">
  **This covers what an agent does in a session: editing files, running commands, and working
  outside this project.** DorkOS's own risky actions still stop for you, like deleting a schedule or
  removing an agent. Those are a separate question with their own setting. To stop being asked about
  one of them, choose **Always allow** on the card when an agent asks, or change it in **Settings**,
  under **Permissions**. See [Permissions](/docs/guides/permissions) and [Action
  approvals](/docs/guides/action-approvals). You will see the same reminder next to the permission
  picker whenever you choose a stop that skips prompts.
</Callout>
...
- **Use Always Allow for repetitive approvals.** If Claude is performing many similar operations (e.g., editing multiple files), use Always Allow so similar calls stop asking. Check how far it reaches, named on the button, before you press it.
