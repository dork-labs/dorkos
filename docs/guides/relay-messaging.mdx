...
**`relay_send_async`** returns immediately with a dispatch inbox subject. The agent runs asynchronously; poll `relay_inbox` with `status: "unread"` and `ack: true` for progress events and the final result — each message's `payload` carries the content, and acknowledging marks it read so every poll only returns new messages. Use this for long-running tasks where blocking is not appropriate. Call `relay_unregister_endpoint` to clean up when you receive a payload with `done: true`.
