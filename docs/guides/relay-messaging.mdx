...
<Callout type="info">
  Relay is fully opt-in. When `DORKOS_RELAY_ENABLED` is not set or is `false`, all Relay routes
  return 404 and session messaging uses the direct Agent SDK call. The Maildir files and SQLite
