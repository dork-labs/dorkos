...
A message that arrives while every session is busy is not turned away. It waits, and it runs as soon as one of those sessions finishes. If the wait lasts more than ten seconds, the chat gets one line saying the message is waiting. If it waits longer than `defaultTimeoutMs` and no session ever frees up, the chat gets one more line saying it was not picked up. Holds live in memory only, so restarting the server drops any message still waiting.
