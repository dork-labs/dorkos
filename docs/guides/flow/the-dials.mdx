...
By default, `/flow` only runs when you type a command. Letting it work on its own schedule (a "tick" that claims one ready item and carries it forward) takes two steps: say you want it, then approve it.

First, open `.dork/plugins/flow/skills/flow-drain/SKILL.md` and find the `schedule:` block at the top. (flow 0.6.0 and later ships the `schedule:` block. If your copy lists `cron` and `enabled` flat at the top instead, it is an older install: update the plugin from the Marketplace.)
...
Then open the **Schedules** tab on Home, find `flow-drain`, and approve it. DorkOS never arms a schedule it finds in a file, so the flag alone fires nothing. The flag is your intent; the approval is your permission.
