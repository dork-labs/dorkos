...
By default, `/flow` only runs when you type a command. Letting it work on its own schedule (a "tick" that claims one ready item and carries it forward) takes one step: approve it.

Open the **Schedules** tab on Home, find `flow-drain`, and approve it. Approving is the only step that matters: it arms the schedule and switches it on together, in one action.

flow-drain ships with its `schedule:` block set to `enabled: false` in `.dork/plugins/flow/skills/flow-drain/SKILL.md`. That flag is just the file's starting point, not something you need to hand-edit: DorkOS never arms a schedule from the file alone, and approving it flips `enabled` to `true` for you regardless of what the file says. (flow 0.6.0 and later ships the `schedule:` block shown below. If your copy lists `cron` and `enabled` flat at the top instead, it is an older install: update the plugin from the Marketplace.)
...
| `.dork/plugins/flow/skills/flow-drain/SKILL.md` (`schedule:` block) | The Pulse schedule: when the autonomous tick fires, and whether it fires at all.    | The Pulse task loader                                |
...
Autonomous mode runs on DorkOS Pulse, a file-based scheduler. Its timing lives in the `schedule:` block of `.dork/plugins/flow/skills/flow-drain/SKILL.md`, not in `config.json`. That block is also what makes the file a scheduled task at all. Remove it and the same file is a plain skill.
...
| ---------------------- | ------------------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------- |
| `schedule.enabled`     | Whether the autonomous tick fires at all.                           | `false`                        | Set `true` to turn on the loop (it's off until you opt in).   |
| `schedule.cron`        | When each tick fires. One tick claims and carries one item forward. | `0 * * * *` (top of each hour) | Tighten for a busier queue; loosen to ease up on the tracker. |
| `schedule.timezone`    | The timezone the schedule is evaluated in.                          | `America/Los_Angeles`          | Match your working hours.                                     |
| `schedule.max-runtime` | Hard ceiling on how long a single tick may run.                     | `2h`                           | Raise for large items; lower to fail fast.                    |
| `schedule.permissions` | How much the unattended session is allowed to do without asking.    | `acceptEdits`                  | Tighten for a more cautious unattended posture.               |

<Callout type="warn">
  The schedule ships `schedule.enabled: false`, and that flag is your intent, not your permission. A
  schedule DorkOS finds in a file still waits on the **Schedules** page for you to approve it,
  `true` or not. Autonomous mode also needs a **running DorkOS server** (it hosts the scheduler);
  manual `/flow` doesn't. `/flow:pause` flips this same flag to halt the whole schedule from one
  place.
</Callout>
...
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Turn the autonomous loop on                  | `.dork/plugins/flow/skills/flow-drain/SKILL.md` `schedule.enabled: true`, then approve it on the Schedules page (server required) |
| Make the autonomous tick fire more often     | Same file's `schedule.cron` field                                                                                                 |
