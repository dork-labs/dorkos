...
- **Skills**: every skill this agent's folder holds — the ones you wrote, the ones a package brought, and the ones that only live in one tool's own folder — and, beside each, what every coding tool you run does with it: reads it, has a shared copy, is out of date, cannot see it, or needs a decision from you. When some of those files are out of date, a line at the top of the page says so and offers a **Sync now** button that writes them. If syncing would also remove anything — a link to a skill you deleted, the files a package left behind when you uninstalled it — the page names every one of those files, and why each is going, _before_ you click, and lists them again afterwards. Below the list, a panel per tool listing every agent file it cannot see — skills, rules, commands and more, which is why that count is usually bigger than the number of skills. Where a tool cannot see one, the reason is the same sentence `dorkos harness sync` prints in your terminal, so the two can never tell you different things. There is a link to browse for more. If DorkOS finds files for a tool in the folder that it is not sharing to, the page says so and gives you the one command that turns it on. The row above says how many skills there are once you have opened the page — before that it says nothing, because counting them means reading your folder and it would rather not do that every time you open a profile.
- **Tools & MCP**: two different settings live here. The four tool groups — Tasks, Relay, Mesh, and Adapter — choose what this agent is **told about**. (These are MCP tools, the protocol that lets Claude call into DorkOS.) Each has three states: inherited (follows global config), on, or off. Turning one off leaves it out of the agent's instructions, so the agent stops reaching for those tools — but it is guidance, not a lock: an agent that asks for one anyway still gets it. Changes take effect on the next session, and one agent's settings never affect another agent. This page also lists the MCP servers this agent runs.
- **Permissions**: what this agent may do on its own, what waits for your yes, and what it cannot do at all — opening and arranging rooms, for a start. A row says "Same as everyone" until you set this agent differently. It changes only through this page or Settings; an edit to the agent's settings file made some other way still takes effect, and the history records it as changed outside DorkOS. See the [Permissions guide](/docs/guides/permissions).
