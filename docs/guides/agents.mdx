...
- **Skills**: every skill this agent's folder holds — the ones you wrote, the ones a package brought, and the ones that only live in one tool's own folder — and, beside each, what every coding tool you run does with it: reads it, has a shared copy, is out of date, cannot see it, or needs a decision from you. Below the list, a panel per tool listing every agent file it cannot see — skills, rules, commands and more, which is why that count is usually bigger than the number of skills. Where a tool cannot see one, the reason is the same sentence `dorkos harness sync` prints in your terminal, so the two can never tell you different things. There is a link to browse for more. If DorkOS finds files for a tool in the folder that it is not sharing to, the page says so and gives you the one command that turns it on. The row above says how many skills there are once you have opened the page — before that it says nothing, because counting them means reading your folder and it would rather not do that every time you open a profile.
