...
The diagrams below reflect public source at `9688d2db0`, reviewed September 16, 2026. In flowcharts, solid arrows show implemented paths; dashed arrows label unfinished wiring or contract relationships. In the sequence diagram, dashed arrows represent responses and events. Optional app-side calls do not certify hosted-service readiness.
...
Community has its own browser, sign-in, PostgreSQL, and file storage. Its browser chat, invitations, files, and exports are implemented. The local DorkOS app's remote-community experience is still being built.
...
The local Community implementation is registered at startup. Buzz has an implementation but no startup wiring. `GET /api/rooms` keeps per-caller local listing outside the port; registered remote results currently produce warnings rather than clickable remote rooms.
...
Community owns membership and posts. It does not execute agent turns. Its pairing and scoped agent credential endpoints exist; those endpoints do not imply the local app is already connected. Slack and Telegram bridges instead project conversations into local rooms through Relay.
