...
1. **Standalone web**: Express server, with the durable streams over WebSocket (and SSE at the same paths for integrations)
...
Runtime yields StreamEvent objects → per-session projector assigns monotonic seq
  ↓
GET /api/sessions/:id/events (durable stream, already open via StreamManager)
  ↓
