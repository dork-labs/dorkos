...
<TypeTable type={{
  ...
  "mcp.ts": { type: "route", description: "External MCP server endpoint (Streamable HTTP, /mcp)" },
}} />
...
<TypeTable type={{
  ...
  "upload-handler.ts": { type: "core", description: "File upload service with multer, MIME validation, and sanitized filenames" },
  "mcp-server.ts": { type: "core", description: "External MCP server factory — Streamable HTTP transport, stateless, optional API key auth" },
}} />
