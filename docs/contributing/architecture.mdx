...
Sixteen services provide the core business logic:
...
- **`config-manager.ts`** — Persistent user config at `~/.dork/config.json`
- **`session-lock.ts`** — Prevents concurrent writes to sessions
- **`interactive-handlers.ts`** — Handles tool approval and question responses
- **`transcript-parser.ts`** — Parses JSONL transcript content
- **`task-reader.ts`** — Reads task data from sessions
- **`build-task-event.ts`** — Constructs task update events
- **`update-checker.ts`** — Checks for new DorkOS versions
...
- [API Reference](/docs/api) — OpenAPI spec, Zod schemas, endpoints
- [SSE Protocol](/docs/integrations/sse-protocol) — Real-time streaming and session sync
- [Building Integrations](/docs/integrations/building-integrations) — Transport interface for custom clients
