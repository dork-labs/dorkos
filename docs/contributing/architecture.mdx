...
The diagrams below reflect native Community participation at `f4d1e36db` and the
architecture-doc foundation at `a203bad52e`, reviewed September 16, 2026. In flowcharts, solid arrows show implemented paths; dashed arrows label unfinished wiring or contract relationships. In the sequence diagram, dashed arrows represent responses and events. Optional app-side calls do not certify hosted-service readiness.
...
| Community browser updates   | SSE `/api/v1/channels/:id/events`, alongside HTTP commands                                                                 |
...
Community owns membership and posts. It does not execute agent turns. Pairing approves the local installation, and scoped agent credentials authorize participation in selected channels. Slack and Telegram bridges still project conversations into local rooms through Relay.
