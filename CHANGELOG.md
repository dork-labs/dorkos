...
- Messages you send while an agent is busy now wait in a queue every window can
  see. Open the same session in a second tab or on another device and the queue
  is there too — and if the server restarts, your queued messages come back
  instead of vanishing (DOR-1132)
