...
- `AGENTS.md`, sections Vision and Quality Standard — the personas (Kai, Priya, Ikechi, Lil),
  the retired vocabulary enforced by `scripts/check-banned-words.sh`, and the
...
1. **Tokens & consistency** — fonts, colors, spacing, radii, shadows. Raw hex or arbitrary
...
2. **Composition & CVA** — are variants expressed through `class-variance-authority` wherever a
...
3. **DRY** — duplicate or near-duplicate components, hooks, and utilities; parallel
...
4. **Organization & naming** — components in the wrong layer or slice; misleading or
...
5. **DX** — is each shared primitive easy to use correctly and hard to misuse? Missing or wrong
...
6. **Playground organization & coverage** — are the playground's pages coherent? Pages with too
   ...
   mock-data drift.
7. **Copy (ELI5)** — every user-facing string. Shorten sentences, use simple friendly words,
   ...
   is a finding.
8. **Responsiveness** — phone, tablet, and desktop behavior; touch targets under 44px; text,
...
9. **UI states** — hover, active, focus-visible, disabled, loading, empty, error, skeleton.
...
10. **Motion & micro-interactions** — what a world-class motion designer would add or fix inside
...
11. **Clutter, simplification & progressive disclosure** — surfaces doing too much at once;
...
12. **Componentization** — repeated inline JSX that should become a shared component; ad-hoc
...
- **Surface-local lenses** (7, 8, 9, 10, and the surface-scoped half of 1 and 11) can be
  answered about a single route or a single diff.
- **Whole-tree lenses** (2, 3, 4, 5, 6, 12) are structurally incapable of it: duplication,
  placement, and API consistency are properties of the whole tree, and a diff-scoped run of them
  produces confident nonsense.

A scoped or incremental audit runs the surface-local lenses over the change and reaches the
whole-tree lenses only through rotation. See the `auditing-ui` skill.
...
One markdown file per lens, plus a structured summary. Each finding:
`{lens, severity, effort, title, files[], evidence, recommendation}`, preceded by a `coverage`
section stating what was examined and what was skipped.
