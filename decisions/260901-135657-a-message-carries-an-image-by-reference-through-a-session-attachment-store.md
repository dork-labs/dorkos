...
- A picture that cannot be kept usually says so. Every refusal path on the live capture — too large, wrong type, unreadable source, no store wired — emits a typed `error` with a sentence, and a history read whose bytes cannot be re-materialized still projects a part, so the reader gets an honest "not available" row instead of a missing turn. One narrow silence survives: `urlFor` answers `null` for a media type the store could never have held, so a turn whose ONLY output was a generated SVG, AVIF, BMP or HEIC would still be dropped from history. Nothing produces that today (a tool attachment always contributes its `tool_call` part alongside, and OpenCode discards generated images upstream), and it is the case to close when the allowlist widens.
...
in Decision's last sentence ("declare `'none'` and are visibly non-compliant
until a follow-up teaches them"), in Positive ("reports claude-code and codex as
known-non-compliant by name") and in the final Negative bullet is closed. Both drop sites named there now call into
