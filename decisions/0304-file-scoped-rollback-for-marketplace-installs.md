...
**Amended 2026-09-23 by ADR 260923-163513 (DOR-2245).** A transaction given `ownership` stages beside its target, writes the installed-files record into the staged tree, clones the person's files from the live target into it before the backup is taken, and runs a late-write pass before the commit. Two more records share the recovery table: a `.dorkos-stage-` directory (deleted) and an in-place uninstall's `.dorkos-uninstall-` sibling (settled by its journal). DOR-2273's note that the update path's temp-dir uninstall stays outside recovery no longer applies.

**Amended 2026-09-23 (DOR-2273).** The backup is now also the transaction's crash record, because the "janitor sweep" follow-up named under Consequences did the wrong thing: it deleted any backup older than a day, which after a crash that left the target missing or half-written was the only good copy. So a fresh install writes a `.absent` marker where a reinstall moves the target aside, the transaction commits by renaming the backup to `.committed` (or deleting the marker) only after `activate` returns, and recovery rolls back every uncommitted record and deletes only committed ones. A backup from before commit records existed is restored only when the target is missing, and otherwise kept until the next finished change to that target. Recovery runs before every install (and again right before its record is written) and uninstall of a target, at startup for the global install and skills roots, and for registered projects once Mesh is up. Each record's name carries its writer (pid, start time, host hash), so recovery never undoes another running server's install; unprovable writers wait out a 10-minute floor. The update path's temp-dir uninstall stays outside this and is DOR-2245's. `contributing/marketplace-installs.md` §5.0 has the rules; `services/marketplace/install-recovery.ts` has the grammar.
