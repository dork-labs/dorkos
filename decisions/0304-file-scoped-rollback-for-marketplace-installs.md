...
**Amended 2026-09-23 (DOR-2273).** The backup is now also the transaction's crash record, because the "janitor sweep" follow-up named under Consequences did the wrong thing: it deleted any backup older than a day, which after a crash that left the target missing or half-written was the only good copy. So a fresh install writes a `.absent` marker where a reinstall moves the target aside, the transaction commits by renaming the backup to `.committed` (or deleting the marker) only after `activate` returns, and recovery rolls back every uncommitted record and deletes only committed ones. It runs before every install and uninstall of a target, at startup for the global roots, and for registered projects once Mesh is up. Each record's name carries its writer (pid, start time, host hash), so recovery never undoes another running server's install; unprovable writers wait out a 10-minute floor. `contributing/marketplace-installs.md` §5.0 has the rules; `services/marketplace/install-recovery.ts` has the grammar.
