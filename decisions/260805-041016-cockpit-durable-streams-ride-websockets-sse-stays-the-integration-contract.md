...
So the pairing stands down for exactly one case: a credential-gated route with login on, where an origin-scoped cookie means a rebound origin cannot present a credential. Not for the container flag, and not for `bearer-of-id` routes under login either, since those skip the credential check the exemption reasons about. The common Docker shape (`-p 4242:4242`, browsed at localhost) needs no configuration: `isHostAllowed` accepts loopback.
