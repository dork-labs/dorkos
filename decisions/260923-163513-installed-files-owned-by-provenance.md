...
The two new install siblings (`.dorkos-stage-`, `.dorkos-uninstall-`) join DOR-2273's `MARKETPLACE_INSTALL_SIBLING_MARKERS`, which every reader of an install root skips through `isInstallSiblingName`. Crash recovery goes through DOR-2273's recovery policy table, where they are two rows: a stage is discarded, and an uninstall is rolled back or finished by its journal. They use DOR-2273's in-name owner stamp (`<createdAt>-<pid>-<uuid>`), so a record is settled only when its writer is this process or provably gone (`assessProcessLiveness`), or it is older than the 10-minute floor. That matters because the install lock is per process and two servers can share a project.
