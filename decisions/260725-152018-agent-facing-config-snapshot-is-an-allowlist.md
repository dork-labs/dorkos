...
1. **The walk descends.** It follows array `items` (adding a `[]` segment to the path) and every non-null `anyOf` / `oneOf` branch, so the nine properties of a sidebar group, and anything behind a nullable object or a discriminated union, are enumerated individually. `ui.sidebar.groups` is no longer one verdict; it is thirteen, because the ninth property (`rules`) supplies five of them.
...
3. **The projection matches the model.** It compiles the `expose` paths into a plan and rebuilds arrays of objects element by element, copying each leaf through a sanitizer that refuses nested structure. So an unclassified field one level down is absent by construction, not merely absent because a test was watching. Recursion is now bounded by the plan rather than by the data, which also makes the projection terminate on inputs that would have defeated the previous `structuredClone`.

The sanitizer is defense in depth rather than a live code path, and it is worth being precise about why, because the tempting justification is false. `ConfigManager` builds `conf` with `clearInvalidConfig: false`, and `conf` re-validates the entire store on every read, so `getAll()` throws on a schema-violating value instead of handing it to the projection. A hand-edited `config.json` therefore cannot reach the sanitizer; only an unknown **top-level** key can, because `conf`'s root wrapper sets no `additionalProperties`, and the plan drops those anyway. The sanitizer earns its place by keeping the guarantee true if `conf`'s validation is ever relaxed, not by catching something today.

Nothing that reached the snapshot before reaches less of it now: the same values are disclosed, argued field by field instead of subtree by subtree. One schema-legal input diverges, checked over 107 systematic variants and 686 random mutations: a sidebar group carrying `rules: {}` loses the empty object, because the projection collapses a node with no emitted children while `structuredClone` copied it verbatim. The client never writes that shape (convert-to-manual sets `rules: undefined`), and an empty rule set carries no information, so the divergence is accepted rather than papered over.
