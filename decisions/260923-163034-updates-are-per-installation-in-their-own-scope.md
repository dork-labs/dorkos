...
Accepted (extracted from spec: marketplace-update-all)
...
- An apply reinstalls exactly the installation that was checked — its install root is passed to the installer — in the scope it was found in. A global installation is reinstalled with no `projectPath`, whatever scope the request named. This holds for the per-package door and the all-packages door alike.
