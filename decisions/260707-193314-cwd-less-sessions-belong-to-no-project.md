...
**Read side — "no project", not "every project".** A tracked session without a cwd is returned by `list(projectDir)` for **no** projectDir (exact match only), in both `CodexSessionRegistry` and `TestModeSessionRegistry`. It stays reachable by id (`get`/`getSession`), so it is hidden, not lost.

**Write side — every runtime turn resolves a real cwd.** `CodexRuntime` takes a `defaultCwd` (defaulting to the server's resolved root, `DEFAULT_CWD`) as the floor of its turn-cwd resolution chain, mirroring the Claude adapter. Every thread bind now persists a real cwd; NULL-cwd rows can no longer be minted. Legacy NULL-cwd rows resume in the default root and gain a cwd in the registry on their next turn.
...
- Cwd-less durable rows written by older builds no longer appear in any list; blank orphans can be deleted (`DELETE FROM codex_threads WHERE cwd IS NULL AND title IS NULL`), and rows with real content resurface under the project of their next turn.
