...
- `SessionEventStore` persists only for log-backed runtimes (codex, opencode, test-mode). **Claude Code — the default runtime — never writes to `session_events`**; its history is SDK JSONL and its `EventLog` is gap-replay overflow only (`apps/server/src/services/session/session-event-store.ts:14-16`, ADR-0309).
