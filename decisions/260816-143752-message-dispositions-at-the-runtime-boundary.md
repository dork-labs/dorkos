...
The single exception is `session-idle`, and the exception is in the telling rather than the reporting. The server still stamps `degradedBecause: 'session-idle'` on the outcome and still puts it on the wire; what differs is what a consumer is expected to do with it, which is nothing. A steer on an idle session simply ran now, and "it ran immediately" is not a degradation anybody needs told about — so the cockpit maps that one reason to no chip at all (`queue-chips.ts`), while `unsupported`, `no-open-turn`, and `pending-interaction` each get a line. The wire stays complete; the UI stays quiet.
