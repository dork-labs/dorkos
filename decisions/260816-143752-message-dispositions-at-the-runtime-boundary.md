...
The single exception is `session-idle`: a steer on an idle session simply runs now, and "it ran immediately" is not a degradation anybody needs told about. Nothing was lost, so nothing is announced. `unsupported`, `no-open-turn`, and `pending-interaction` are all surfaced.
...
A runtime that cannot append to its own transcript still accepts a stage: the text is held and folded into the next dispatch as an additional-context entry (ADR-0273's neutral context bag), and the same `context_staged` receipt is emitted either way. To the person a stage landed; `degradedBecause: 'unsupported'` is only _how_. The alternative — refusing a stage on a runtime that lacks the native path — would have made the newest disposition the least reliable one.

### Steer and stage are writes, with the same authorization as sending

Both are gated by the runtime's real write-lock (`AgentRuntime.isLocked`), asked under the canonical id and held under the dispatch mutex, so the lock check and the delivery cannot straddle a turn ending or a warm-process reap. It is deliberately not the dispatcher's `inFlight` mirror, which is lossy: a budget-exhausted launch runs its turn holding the real lock without ever claiming `inFlight`, so gating on the mirror would let any client steer that turn. The spec's invariant is that **there is no path where a caller that may not send may steer** (§2.3), and `deliverSteer` is the single server path to `deliverIntoTurn`, held there by the single-ingress audit test exactly as `dispatchMessage` is the single path to `sendMessage`.
...
- Conformance holds every runtime to it. `runtimeConformance` cases C1 (a disposition it does not declare is absent or refused as `unsupported`, never thrown; a declared one reaches the model inside the open turn and mints no second `turn_start`), C2 (one `turn_end` per window however many native results it carried), C3 (queue durability behind a failed turn and never into an open interaction), and C6 (the three flags are real booleans) make a false declaration a test failure rather than a mystery in production.

### Negative

- **Two-thirds of the vocabulary is unavailable on two of the four runtimes.** Codex and opencode declare both flags `false`, so a person who learns to steer on claude-code will find the same button degrade to a queue on a codex session. The receipt says so, but the experience is genuinely uneven and stays that way until those adapters gain the capability.
...
- **A stage that folded is not the stage that was asked for.** The receipt says `applied: 'stage'` with `degradedBecause: 'unsupported'`, which is honest, but a folded stage reaches the agent one turn later than a native one. Somebody who staged a correction expecting the current turn to see it will find it arrived for the next.
