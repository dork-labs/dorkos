...
**`no-open-turn` was replaced by `turn-owned-elsewhere` on 2026-08-23 (DOR-1315) because one reason was carrying three unrelated facts, and the chip had to pick a sentence for all of them.** The wire reason covered "the turn ended", "the input stream closed", and "a DIFFERENT caller owns the live turn" — and the cockpit rendered the lot as "Queued. The task had already finished." A steer sent from a second window five seconds into a visibly running turn hit the third case every time (a steer is a write, and the live turn's lock belongs to whoever started it), so a person watching their task run was told it was over. Ownership now has its own reason and its own sentence, and — the part that matters beyond the rename — **no rung of the ladder asserts a turn state it has not probed**: both the ownership refusal and the leftover runtime refusals ask the session's own projection before choosing between `turn-owned-elsewhere`/`not-steerable` and `session-idle`, because a write-lock outlives its turn by up to `SESSIONS.LOCK_TTL_MS`. `no-open-turn` leaves the wire vocabulary entirely; it stays where it is true, as the runtime's own `RuntimeDeliveryResult.reason` to the dispatcher. The chip says "something else", never "another window": the holder is a client id, and rooms, MCP clients and the Obsidian transport all hold turns with no window anywhere.
...
- **The default install's staged words were held in memory under a durable receipt — NARROWED 2026-08-22 (DOR-1324), not closed.** The native path writes to the runtime's own transcript (claude-code's SDK JSONL), which survives a restart; the fold used to hold the text in a process-local `Map`, which did not, while `emitContextStaged` put the "Added context for the next reply" receipt on the DURABLE session stream the moment the stage landed — so a restart in the gap left a permanent receipt over words that no longer existed. `staged-context-store.ts` now writes the hold to the `session_staged_context` table beside the message queue: it is keyed the same way, moves with the same rekey, is swept with the same sweep, and the take reads and deletes in one transaction so a crash cannot make a note ride a second turn. The receipt is unchanged and stays immediate. What survives is two named residuals, both narrower than what they replace. **The take-to-turn-start window:** the take fires while the next dispatch assembles its context bag, one step BEFORE the durable queue deletes its own row (at `turn_start`), so a throw in between re-parks the message (`returnToQueue`) and retries it WITHOUT its staged words while the receipt stands. That asymmetry predates this change — the in-memory take was destructive in the same place — and closing it means moving the take to `turn_start` or restoring the hold on `returnToQueue`, which is turn-path work. **The write-refusal fallback:** if SQLite refuses the insert, the words are kept in process memory rather than dropped under a receipt already sent, so that one hold is only as durable as the process; it is logged at `error`, so it is a reported downgrade rather than a silent one.
- **A reap can turn the routing gate's answer stale, and the fallback has to re-ask.** The router reads `canStageSession` before `deliverStage` takes the dispatch mutex, and the warm-process reaper takes nothing from that module — so a reap landing in the window makes a designed fold look like an adapter that declared ahead of its mechanism. It was reported as one until DOR-1325, which has the fallback re-ask the session AFTER the runtime's refusal rather than trusting the earlier read. The general shape survives the fix: the flag and the per-session answer are both snapshots, and any code that acts on one has to tolerate it having moved.
