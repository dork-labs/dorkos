...
- **The unit is a message, not a turn.** The count is `COUNT(*)` over the agent's entries in the cascade, so a turn that posts several times through the rooms tool spends several of its ten. That is the honest reading of what the row count means, it is inherited from the ancestry set this replaces (which had the same scope, at a ceiling of one), and it errs toward stopping a noisy agent sooner. A dispatch-scoped count is a follow-up, not a silent re-interpretation of this number.
