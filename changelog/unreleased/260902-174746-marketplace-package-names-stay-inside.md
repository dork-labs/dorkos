...
covers:
  ...
  - 'fix(server): the preview route bounds projectPath, and git stops parsing options before a subpath'
  - 'fix(server): close three review findings on the marketplace path guards'
---
...
- Installing a package from a folder on your own disk now has to stay inside the folder DorkOS is allowed to reach, the same limit every other file feature already respects. Nothing changes for the usual case; a path outside that limit is refused rather than quietly read. Previewing a package is held to the same limit, so a preview can no longer be used to ask what is in a folder elsewhere on the machine
