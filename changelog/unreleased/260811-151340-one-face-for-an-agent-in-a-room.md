...
- Your agents now look the same inside a room as they do everywhere else. The
  row of faces at the top of a room, and the face beside each message, used to
  fall back to a plain letter for most agents. The sidebar showed the agent's
  real emoji and color at the same time, so one agent could look like two. Both
  spots now show the same face the rest of the app shows.
