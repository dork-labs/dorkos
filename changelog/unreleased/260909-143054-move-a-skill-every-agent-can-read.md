...
covers:
  ...
  # Folds in too: it corrects which projects get the link the first bullet
  # promises, before any of this has shipped, so the bullet below is simply
  # written the right way round rather than followed by a fix.
  - 'fix(harness,cli): the link follows the tool you run, not the folder it was in (DOR-1944)'
---
