...
- The DorkOS blog feed is now advertised on every page across the site, not just the blog itself, so a feed reader can find it from anywhere (DOR-347)
- Added a web app manifest and app icons, so saving DorkOS to a phone home screen or installing it as a web app shows the DorkOS mark and name instead of a generic thumbnail (DOR-347)
