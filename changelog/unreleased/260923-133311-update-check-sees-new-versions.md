...
- `dorkos update` with no package name now checks every package where it is installed, including packages installed for one agent. A problem with one package no longer stops the rest.
