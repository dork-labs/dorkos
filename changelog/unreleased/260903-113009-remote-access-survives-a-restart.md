...
covers:
  ...
  - 'fix(server,shared,client): say whether the tunnel is reconnecting or off (DOR-1738)'
---
...
- If you have Require login turned on, turning Remote Access on now takes a person signed in to DorkOS. A script holding one of your API keys is refused, because a key proves the account and not the person. This is a deliberate narrowing: a script of your own that used to turn the tunnel on will now be turned down, and you turn it on from the app instead (DOR-1738)
