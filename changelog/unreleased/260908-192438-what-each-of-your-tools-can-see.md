...
covers:
  ...
  - 'fix(server): a relative projectPath is refused, not resolved (DOR-1892)'
---

### Added

- DorkOS can now be asked, for any folder, what each of your coding tools can actually see there — every skill, command, rule and hook, and whether Claude Code, Codex, Cursor, Gemini, Copilot and OpenCode get it, miss it, or are out of date. Asking only looks: it never creates a file, never turns a tool on, and never changes a setting of yours. The folder has to be given as a full path, so the answer is always about the folder you asked about and never about wherever DorkOS happens to be running. A folder you have not set up yet says so plainly rather than failing. This is the answer the Skills page will draw; nothing in the app asks for it yet, so there is nothing new to click (DOR-1892)
