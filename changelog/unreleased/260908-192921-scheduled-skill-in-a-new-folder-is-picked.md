...
- Folder-watching can also go quiet in two other ways: a busy machine running several agents at once can use up the operating system's supply of folder watches, and a schedule saved in the first moment after DorkOS starts watching can slip past. In both cases DorkOS now checks the folder itself every ten seconds until watching is working again, so the schedule still starts on time. When everything is working normally, nothing changes (DOR-1908)
