...
- An agent that has been removed from your machine stops answering to its `@name`, so a room-mate with the same name is reachable again. If someone messages it anyway, the room now says it isn't set up here any more instead of going quiet. An agent that is merely offline — a closed laptop — keeps its name. (DOR-790)
