...
covers:
  ...
  - 'fix(client): a screen reader hears the same word for the header block that everyone else does (P2.4, DOR-1069)'
  - 'fix(client,e2e): the New menu stops blurring the editor it just opened, and the page objects follow the new path (P2.4, DOR-1069)'
  - 'fix(client): the sidebar-chrome showcases render instead of crashing, and two guards stop matching only the spellings they were written against (P2.4, DOR-1069)'
  - 'refactor(client): the settings dialog and your profile each get one door in the sidebar, not two (P2.4, DOR-1069)'
---
