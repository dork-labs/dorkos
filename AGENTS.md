...
**Never write "mission control" or "cockpit" in user-facing prose** (site, docs, READMEs, UI copy, error messages, release notes) — the operator retired both words, and the category phrase is "one place" ("All your agents. One place." / "one place for every AI agent you run"). Say "the DorkOS app", "the app", "one place" or "one window" instead. Two carve-outs: GitHub ships a product literally named "Mission Control", and the compiled changelog keeps its historical wording. Identifiers and code comments are out of scope and no sweep is owed for them — but they are where the habit starts, so rename one when you are already editing its file. Enforced by `scripts/check-banned-words.sh` in the `typecheck` workflow, which scans this file too. This paragraph is deliberately its own paragraph: the guard matches line by line, so the marker below exempts exactly the prose that has to name the two retired words and nothing else in the file. <!-- vocab-allow: states the prohibition, so it has to name both retired words -->

**"Integration", "connector", "adapter" and "provider" are not user-facing nouns** — ADR `260804-021140` retired all four and settled on **Connections** (the umbrella and the page), with **Messaging** and **Accounts** as its two regions. In app copy, error messages, toasts and marketing prose, a Telegram or Slack hookup is a _connection_; a service an agent acts on is a _service_ under Composio & Nango. The architecture keeps its own names on purpose and is not swept: `RelayAdapter`, `ConnectorProvider`, `/api/connectors/providers/*`, the marketplace package types an author writes (the Adapters and Connectors facets), and the model-provider sense OpenCode uses — that split, with the viewport test that decides it, is the ADR's own table. Enforced by `scripts/check-vocab-gate.ts` in the `typecheck` workflow, which parses the source so it never fires on an identifier; every legitimate technical use is listed per file, with its reason, in `scripts/vocab-gate/allowlist.json`. Add a wave to `scripts/vocab-gate/banned-terms.json` to retire a word; add a scoped, reasoned entry to the allowlist to keep one.

**Product state (pre-launch alpha, 2026-08):** the web app via CLI install is the primary, launch-critical surface. The macOS desktop app is shipped and verified — signed, notarized, downloadable at `dorkos.ai/download/mac`, riding the unified `vX.Y.Z` releases (Apple Silicon), and matured by the desktop-resilience programme. The phone surface is an installable web app with push notifications, reached over the built-in tunnel. A **Windows x64 build rides the same release train** — unsigned NSIS installer, downloadable at `dorkos.ai/download/windows` — but it is an **early alpha: built and code-reviewed, not yet confirmed by a real end-user install on Windows**, so it stays behind the demo-claim gate (labeled "alpha" in site copy). There is no Linux desktop build yet. The Obsidian plugin is retired; migration and source recovery are documented in `docs/guides/obsidian-plugin.mdx`. Multi-agent Mesh+Relay coordination now carries end-to-end coverage (rooms e2e specs plus the shared conformance suites) and has come off the gate: Rooms ships GA, and the reply-limit dials that make agent-to-agent traffic safe to leave on landed with it. The marketplace's install path is covered end to end, but its **Claude-Code-superset compatibility** is still the unverified part of that pillar. In user-facing copy, docs, and release notes, never state that a still-unverified surface or pillar — the Windows desktop alpha, the marketplace's Claude-Code-superset compatibility — works (the demo-claim gate: `meta/positioning-202607/09-gtm-plan.md` §2.0).
...
**Evals** (`packages/evals`, README there): `pnpm evals:local` runs the `core` suite on a real model through the `claude` sign-in on your own machine, so it spends against your own Claude subscription. The `real-provider` tier is the exception to every sentence in this paragraph: it runs the cross-runtime `chat` suite through OpenCode on OpenRouter, bills an OpenRouter account rather than a Claude plan, needs `DORKOS_EVALS_PAID_PROVIDER=1` beside `OPENROUTER_API_KEY`, and refuses `--isolation docker` because eval containers have no network. A `--runtime codex` leg is the same shape on a different bill: `DORKOS_EVALS_PAID_CODEX=1` beside `CODEX_API_KEY`, and no docker either. Credentials resolve in a fixed order: `ANTHROPIC_API_KEY`, then `CLAUDE_CODE_OAUTH_TOKEN`, then your local sign-in; the run prints which one it used. No credential at all is a runner error, never a pass. The `docker` isolation tier is the exception: its container is sealed off from your home directory, so it needs one of the two variables and says so.
...
**Seven paths in the repo spend real money, and each needs its OWN flag beside its OWN key.** The shape is always the same — the flag is the decision, the key is the instrument, and a key alone arms nothing, because plenty of people leave one exported and having a key is not the same as deciding to spend.

| Path                                                                                                | Flag                                                                                        | Key                                                                                            | What it does                                                                                                                                                                                                                                                         |
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/evals/src/runner/__tests__/harness-server.test.ts`                                        | `DORKOS_EVALS_CREDENTIALED=1`                                                               | `ANTHROPIC_API_KEY`                                                                            | boots a server and drives one live turn                                                                                                                                                                                                                              |
| `apps/server/src/services/runtimes/opencode/__tests__/conformance.test.ts`                          | `DORKOS_OPENCODE_LIVE_PAID=1`                                                               | `OPENROUTER_API_KEY`                                                                           | runs the whole OpenCode conformance suite against a real sidecar on OpenRouter (its sibling `DORKOS_OPENCODE_LIVE=1` is the FREE local-model smoke and can never arm the paid one)                                                                                   |
| `pnpm evals -- --tier real-provider`                                                                | `DORKOS_EVALS_PAID_PROVIDER=1`                                                              | `OPENROUTER_API_KEY`                                                                           | the cross-runtime `chat` suite through OpenCode on OpenRouter, default ceiling $0.50                                                                                                                                                                                 |
| `scripts/harness-smoke/run.sh <claude\|codex\|opencode>`                                            | `DORKOS_HARNESS_SMOKE=1`                                                                    | one of `ANTHROPIC_API_KEY` (claude), `OPENAI_API_KEY` (codex), `OPENROUTER_API_KEY` (opencode) | the H tier: a real harness binary reads a projected tree and reports what it found, one turn, `--max-usd` default $0.50                                                                                                                                              |
| DorkOS credits as the inference source (`apps/server/src/services/core/cloud/credits-inference.ts`) | `DORKOS_CLOUD_CREDITS=1`                                                                    | the cloud-link instance credential (`cloud.instanceToken`)                                     | points a runtime at the DorkOS inference endpoint, so turns draw on the account's credits                                                                                                                                                                            |
| `pnpm evals -- --runtime codex` (any eval tier)                                                     | `DORKOS_EVALS_PAID_CODEX=1`                                                                 | `CODEX_API_KEY`                                                                                | a Codex eval leg, billed to an OpenAI account: the sandbox pins `CODEX_HOME` to an empty dir, so a ChatGPT login is invisible and the forwarded key is the only way in                                                                                               |
| `pnpm --filter dorkos test:community-live` (`packages/cli/scripts/test-community-deploy-live.ts`)   | six `DORKOS_COMMUNITY_LIVE_*` arms: five `=1`, plus the exact charge-acknowledgement phrase | your signed-in `fly` and `neonctl` profiles (no key variable)                                  | installs the published `dorkos` at an exact version, deploys a throwaway Community on Fly, Neon and Tigris through the real launcher, proves owner setup, then deletes it; `DORKOS_COMMUNITY_LIVE_BUDGET_USD` (at most $25) is recorded in its receipt, not enforced |

Every one of them reads its flag at MODULE scope, so no other file's `vi.stubEnv` can blank it; the Community gate is a standalone executable no test imports, and checks every arm before its first process, profile or network call. **None of these seventeen names reaches any task `pnpm test`, `pnpm verify`, pre-push or CI runs**, and none may: turbo runs strict and strips whatever a task was not told to pass, which is the only reason those commands have never been exposed. There is exactly one deliberate exception — `ANTHROPIC_API_KEY` on the `e2e` task, which none of those commands runs — and `packages/evals/src/runner/__tests__/paid-provider.test.ts` pins it by path so the carve-out cannot grow, while walking the WHOLE parsed `turbo.json` for the other names (`globalEnv`, a package-scoped `pkg#task` override and `test:watch` expose a variable just as effectively as the three keys an earlier version of that test checked). Bare vitest strips nothing, so on those paths the flag is the whole protection. And a fake key is not a safe way to run the Anthropic one: the test server inherits your `claude` sign-in and bills that instead, measured with a deliberately invalid key. The harness smoke is the one path that can PROVE which credential paid — Claude Code reports `apiKeySource` on its session-init message, and a turn served by anything but the named key fails the run.

The gate follows the **money**, not the tier name, and it answers WHICH bill a run reaches so the refusal names the right key. `--tier claude-code-cheap --runtime opencode --model openrouter/…` spends on OpenRouter, so it demands `DORKOS_EVALS_PAID_PROVIDER` exactly as `--tier real-provider` does; `--runtime codex` on any tier spends at OpenAI, so it demands `DORKOS_EVALS_PAID_CODEX` instead (`paidPathFor` in `packages/evals/src/runner/credentials.ts`).
...
**Hexagonal architecture** with a `Transport` interface (`packages/shared/src/transport.ts`) decoupling the React client from its backend: `HttpTransport` (browser, phone, and desktop), with mock transports in tests. See `contributing/architecture.md`.
...
`CommunityAdapter` (`packages/shared/src/community-adapter.ts`) is the **fourth swappable seam** beside `AgentRuntime`, `Transport` and `ConnectorProvider` — one port for rooms in more than one place, gated by `communityConformance`. This machine's own SQLite rooms are the first backend behind it (`services/communities/local/`), registered as `LOCAL_COMMUNITY` at startup; it wraps `RoomService` rather than replacing it. `GET /api/rooms` keeps caller-filtered local rooms outside the single-identity port; any other registry entries contribute warnings, not clickable remote rooms. Native Community participation uses lazily cached, owner-qualified `RemoteCommunityAdapter` instances outside `CommunityRegistry`, reached through qualified `/api/communities/:ref/rooms/:roomId` routes and a dedicated SSE stream. Community credentials stay on the local server. Telegram/Slack bridged rooms are **projections into local rooms, not community backends** (ADR `260814-024525`); the port is reserved for communities whose truth is remote.
...
**One ref namespace, many readers.** That rule covers writers; the reader half is that a worktree isolates the working tree but **not** `origin/*` or the object store, which live in the shared common git dir. Another session's `git fetch` advances `origin/main` between two commands of your own reasoning, every command still exits `0`, and the only symptom is a conclusion that does not match the code. **Pin the base once — `BASE=$(git rev-parse origin/main)` — and compare against `$BASE` for the whole investigation; never name `origin/main` twice.** Prefer `git merge-base "$BASE" HEAD`, and treat a surprising file list as evidence the ref moved rather than as data.
...
3. **`os.homedir()` banned in `apps/server/src`** — use `lib/dork-home.ts`. Six carve-outs beyond tests, enumerated in `.claude/rules/dork-home.md`: `lib/dork-home.ts`, `lib/boundary.ts` (two inline-disabled call sites), `claude-code/claude-config-dir.ts`, `codex/codex-home.ts`, `opencode/opencode-data-dir.ts`, `harness/agents-user-home.ts` — the last four mirror another program's resolution of its own directory 1:1, never DorkOS's (the sixth mirrors five vendors at once: `$HOME/.agents/skills`)
4. **TSDoc on exports** — enforced by `eslint-plugin-jsdoc`. Every jsdoc rule is `error` (DOR-627); they were all `warn` until then, and since every package's lint script is a bare `eslint .`, which exits 0 on warnings, this rule used to fail nothing anywhere. A missing block description counts: `@param`/`@returns`/`@internal` tags alone are not a TSDoc comment
5. **Prettier + Tailwind class sorting** are automatic — never hand-sort. Formatting runs at **turn end** (`Stop` hook) and again at `pre-commit`, deliberately **not** after each edit: rewriting a file the moment you edit it makes your in-context copy stale and breaks your next string-replace (`.claude/hooks/format-changed.sh` header). So a file you just wrote is not formatted yet, and that is fine — do not chase it. `pre-push` then **checks** the changed set rather than rewriting it (a hook that edits files mid-push leaves the tree disagreeing with what just went out), and refuses the push naming the files and the exact `prettier --write` command — DOR-1839, after seven PRs in one week went red on CI's formatting gate
...
- **Every required check comes from ruleset 19893973** (classic branch protection is gone); the list in `contributing/ci.md` is generated from `ci/required-checks.json`, so never restate the count. **Every required check must report on `pull_request` and `merge_group`**, with no `paths:` filter and no job-level `if:` that can skip it, except `no-fragment-under-skip-label`, which skips on `merge_group` by design and is allowlisted in `ci/census-allowlist.yaml`; the CI Steward census step in `typecheck` enforces it.
...
- **A red is not a reason to add load** (Team plan: 60 concurrent jobs, 19-25 per push). First failed-checks ejection from the queue: don't push or rerun; read the failing job, and once it is plainly not yours re-arm once with `gh pr merge --auto <n>` (85% re-pass unchanged). The same check failing again on the same head is a regression only the queue can see (browser tests run only there): stop re-arming, read the log, push a fix. A PR check that is not yours: `gh run rerun <run-id> --failed`, once. Never an empty commit.
