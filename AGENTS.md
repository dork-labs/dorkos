...
`/flow` is the PM-agnostic workflow engine (stage spine `CAPTURE → … → DONE`) — it lives entirely in the external marketplace plugin (`dork-labs/marketplace`, `plugins/flow/`; ADR-0297), so `/flow:*` commands exist only when that plugin is loaded (`claude --plugin-dir <marketplace-checkout>/plugins/flow`). All tracker I/O goes through its `linear-adapter` skill; Linear team key `DOR`. Reach Linear via Linear MCP or `composio execute LINEAR_* --account personal` — **never the `artblocks` work account**. Reference: `contributing/flow-engine.md`.
