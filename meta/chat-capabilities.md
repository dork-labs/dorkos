...
10. **Rooms above the unit layer — half-closed.** `/chat:self-test` and `/chat:session-switch-test` are session-only, written before channels matured, and this run grew rooms the most (collect/steer/halt, 3-way DMs, reactions, tool hand, DM notifications). `/chat:rooms-test` now exists (DOR-1218) and covers the agentic half — live browser, two agents in a channel, burst → one reply, mid-turn fold, halt, reaction, thread, three-way rule. What is still missing is the deterministic counterpart. `apps/e2e/tests/rooms/` already holds 18 specs (conversation, reactions, entry actions, DMs, presence, attachments, mobile sheet, palette), so the gap is narrower and sharper than "rooms have no e2e": **nothing deterministic drives the halt button or the collect window**, the two mechanisms this landing run added. Those, plus the `rooms` eval suite (§7's probes + the gate's tuning loop, shapes specced in `specs/engaged-response-gate`), are what would close it.
...
- Agent evals — `pnpm evals:local` (`packages/evals`). `--suite` resolves against **tags** — `smoke`, `core`, `connector`, `experimental` — or a single case id, so "suite" names like `governance` or `operate` are groups of cases in the registry (`packages/evals/src/suite/index.ts`), not selectable suite names; `--suite all` runs everything. Cases today cover governance/approvals, agent self-edit, activity reads, config toggles, marketplace install, capability discovery, connectors, the widget round-trip, and the harness self-test. Oracle-judged, budget-capped. **No chat/rooms cases exist yet** — its intended shape (tuning + gating loops, drive-a-room-post harness gap) is specced in `specs/engaged-response-gate/01-ideation.md`.
