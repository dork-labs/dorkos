...
3. **Add a security stance to "We Believe" and the Honesty note** (sections 7, 9). Post-OpenClaw, "self-hosted" with no security claim beside it reads as risk. Add the claims that are true: localhost-only defaults, sign-in required to expose it, permission gates on the actions you cannot take back, a published threat model, no silent network exposure. Do **not** add the phrase "secure by default" (DOR-509): every protection here depends on an auth posture, and the default posture trusts every program running as you. State the protection and its posture, never an unqualified adjective. Trust language exists in the doc; harden it into commitments.
...
7. **Taglines bank** (section 12): add the fleet-era lines: "All your agents. One place." / "Every agent you run. One window." / "Your agents, any vendor." _(Rewritten 2026-08-24 — this item originally prescribed "Mission control for your agents." and "One cockpit. Any agent."; both words are now banned, see §9.)_ Keep the existing bank; nothing there needs deleting except confirming "You slept. They shipped." stays marked as awareness-noted secondary.
...
3. **Update the Message House to the v2 draft** in `02-positioning.md` §5 (same roof, pillars become: All your agents one place / A team not tabs / Yours and safe to run). The current pillars (Autonomy/Communication/Control) map cleanly onto the new ones; this is a re-lead, not a rewrite.
...
3. **Personas**: added **Ikechi — The AI-Native Founder** (secondary, grounded on a real user) and **Lil — The Private Professional** (horizon, staged, grounded on a real user); both carry close-contact validation caveats and explicit boundaries (Kai remains the beachhead; launch messaging unchanged). Anti-persona (`the-prompt-dabbler.md`) boundary redrawn: **operator mentality, not technical skill**. `manifest.json` updated. This satisfies §5.3's evidence rule — these are observed users, not imagined ones; the outstanding validation is _unassisted_ use.
4. **customer-voice.md**: `[aspiration]` quote type added; **Theme 9: The Non-Developer Operator** opened (no quotes yet; interview the users behind the Ikechi/Lil personas with a close-contact disclosure; milestone is the first organic quote).
5. **Downstream surfaces owed** (not yet done, gate on founder go): site hero, README opener, GitHub description re-lead ("You, multiplied. One place for every AI agent you run."), OG image copy, and the `06-marketing-tactics.md` tactic-1 copy which the 2026-07-09 GitHub description used pre-reframe. _(Wording updated 2026-08-24, §9.)_

## 9. Addendum: category-phrase ruling (2026-08-24, DOR-1517, applied)

The founder retired **"mission control"** and **"cockpit"** permanently. "Mission control" borrows NASA swagger a laptop tool has not earned; "cockpit" names a place most readers have never sat in and reads as strange rather than confident. Neither appears on any surface a user can read.

The category phrase is **"one place"**: "All your agents. One place." on hero surfaces, "DorkOS is one place for every AI agent you run." as the long form. Supporting variants: "one window", "one home", "your team". For prose about the product itself, use "the DorkOS app" or "the app".

Applied in one sweep to `AGENTS.md`, both READMEs, the marketing site, `brand-foundation.md` (§10 "Terms to Avoid" now carries the ban as its canonical home; §19's "Home Base" candidate is superseded), `02-positioning.md` (§2 category, positioning statement, word-of-mouth test, §5 Pillar 1), the rest of `positioning-202607/`, `value-architecture-applied.md`, `dorkos-litepaper.md`, `chat-capabilities.md`, `agent-etiquette.md`, `INDEX.md`, `website-copy/decisions.md`, and the two affected personas. Deliberately not swept: `website-copy/rounds/**` and `archive/**` (frozen records), `research/`, `decisions/`, `specs/`, and the changelog — those record what was said at the time.

One exception is not ours to rename: GitHub ships a product literally called "Mission Control", so competitive analysis may name it.
