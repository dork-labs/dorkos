...
| `08-demo-video-scripts.md` | Three flagship demo beat sheets ("2:47 AM", "All Your Agents, One Place", "The Org"), each with a real-vs-aspirational ledger                                                      |
...
4. **Multi-runtime is the headline and it is currently a secret.** The site, README, and GitHub description still tell a Claude-Code-only story. The single cheapest positioning win available is saying, everywhere, "Claude Code, Codex, OpenCode: one place."

5. **Autonomy demotes from hero to proof.** "Runs while you sleep" is being commoditized by first parties for their own agents. The unsolved, growing pain is fleet chaos and vendor sprawl. New door: one place for every agent you already run. Same thesis underneath: intelligence doesn't scale, coordination does.
...
- **OpenCode brings local models**, which turns the privacy claim into a demo: a session that provably never leaves the machine, in the same window as the cloud runtimes.
...
Fix the funnel (site + README + security page + 5-minute path), reframe every surface around one place for every agent you run, then fire the launch ladder in order: Obsidian community → Show HN → Reddit → Product Hunt, each armed with a 60-second product video and honest founder story. Behind it, run the agent-powered engines: release-to-everywhere pipeline, weekly fleet report, comparison/intent SEO, marketplace seeding. Measure by stars/week, installs/week, and newsletter growth. Founder time converges to: replies, one weekly fleet report, one monthly essay; agents do the rest.
