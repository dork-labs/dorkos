...
- **Anthropic**: Claude Code for web (async cloud execution, teleport to local), **Claude Managed Agents** (lead agent delegating to parallel specialists), "Dreaming" (scheduled memory curation), Cowork. Single-vendor orchestration is being absorbed into the platform itself.
  - **Cloud scheduling shipped (verified 2026-08-23).** Cowork scheduled tasks run on Anthropic's servers "even when your computer is asleep or the Claude Desktop app is closed." Claude Code has cloud **Routines** (research preview) that run on Anthropic-managed infrastructure "when your laptop is closed," triggered by a schedule, an API call, or a GitHub event. **Treat any "their scheduling needs your machine awake" claim as retired.**
  - **Maturity:** Cowork on web and mobile is **in beta** (Pro, Max, Team; Enterprise where an admin enables it), announced 2026-07-07 with access "rolling out over the next several weeks starting with Max users." Routines are a research preview.
  - **The line that still holds:** every Anthropic path that reaches files on your disk still needs the desktop app open and the machine awake — a Cowork scheduled task that "requires local files or apps ... will only run locally," and a cloud session "can read and write files in folders you've connected on your computer only while the desktop app is open on that computer." What runs with the device off is the cloud path, and that one works from files in your Claude account or a fresh GitHub clone, not your working copy. Routines are also one-vendor, GitHub-bound, one-hour minimum interval, daily run cap.
  - Sources: [blog: Cowork on web and mobile](https://www.claude.com/blog/cowork-web-mobile) (2026-07-07) · [support 13854387](https://support.claude.com/en/articles/13854387-schedule-recurring-tasks-in-claude-cowork) · [support 15520349](https://support.claude.com/en/articles/15520349-use-claude-cowork-on-web-desktop-and-mobile) · [Routines](https://code.claude.com/docs/en/routines) · [Desktop scheduled tasks](https://code.claude.com/docs/en/desktop-scheduled-tasks)
