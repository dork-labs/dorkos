...
**The two words that are now banned, and why.** "Mission control" (Decision 17's entry point) borrows NASA swagger a laptop tool has not earned, and the founder simply dislikes it. "Cockpit" (the fleet-era tagline bank, `brand-foundation.md` §12) names a place most readers have never sat in; it reads as odd rather than confident, and it fails the plain-language bar every user-facing surface is held to. Neither word appears in any prose we still write: site, README, docs, UI copy, error messages, release notes, decks, video scripts. The one carve-out is the published changelog (`/docs/changelog`, compiled from `CHANGELOG.md`, which repo rule forbids editing) — it keeps its historical occurrences, because it records what was said at the time. An earlier candidate replacement, "Home Base" (`brand-foundation.md` §19, 2026-07-17), is also retired — "one place" carries the same hero property (the place is the customer's, not the product's) in plainer words.

**Rationale:** the phrase has to survive a person saying it out loud to a friend who does not code. "One place for every AI agent you run" passes; the retired words do not. The reasoning behind Decisions 17 and the §19 naming addendum is preserved above so nobody re-proposes them from first principles.

**"AI agent", not "coding agent" (2026-08-24).** The canonical long form is "one place for every **AI** agent you run", authorized by the operator's beyond-coding directive of the same day: the product is being repositioned past coding agents, so the category line must not narrow it back down. Breadth is still **demonstrated, never claimed** — show the agent sending the email and booking the call next to the code, rather than asserting the product does more. Surfaces standardized to the canonical form: `docs/index.mdx` (page copy and its meta description), `packages/cli/package.json` (the npm listing), `apps/client/public/manifest.webmanifest`, and `context7.json`. Deliberately unchanged: the line in both READMEs that reads "You run AI coding agents like Claude Code, Codex, and OpenCode" — that sentence is about three specific products the reader already runs, all of which genuinely are coding agents, not a claim about DorkOS's category.

**Where the rule lives:** `brand-foundation.md` §10 "Terms to Avoid" is the canonical home. The deliberation is in `positioning-202607/02-positioning.md` §2 and `positioning-202607/04-brand-doc-changes.md` §9. The automated guard is `scripts/check-banned-words.sh`, run by the `typecheck` workflow.
