...
| **Autonomous Agents**    | VALIDATED | Strong               | RedMonk #1 developer wish. Since mid-2026 Anthropic schedules in its own cloud too (Cowork tasks, Claude Code Routines), so "their scheduling stops when your Mac sleeps" is retired. Pulse's remaining edge: every runtime, not one vendor's, and it works on your local repos. |
...
| **One Scheduler for Every Runtime**   | High (concrete) | Anthropic schedules Claude in Anthropic's cloud. Pulse schedules Claude Code, Codex, and OpenCode from one place, on your own machine, against your real working directory. (Replaced "The Mac Sleep Problem" on 2026-08-23 — Anthropic fixed that.) | VL-01      |
...
# Layer 2: Mechanism
mechanism:
  alternative: 'Manually invoke your agent from the terminal every time you want work done. Or: cobble together shell scripts and cron. Anthropic will schedule Claude for you, but you have to pick one of two halves — a cloud run (Cowork task or Claude Code Routine) that never sees your machine and starts from a fresh GitHub clone, or a Claude Code Desktop local task that reaches your files but only fires while the app is open and your computer is awake. Either way it schedules Claude, and only Claude.'
...
proof:
  type: 'contrast'
  spec: "One cron scheduler across Claude Code, Codex, and OpenCode, running on your own machine against your real working directory — vs. per-vendor scheduling that either runs in that vendor's cloud on a fresh clone or needs your computer awake"
  translation: 'Anthropic can schedule Claude. Pulse schedules whichever agent you picked, on the code sitting on your own disk.'
...
Claude Code, Codex, or OpenCode, on one schedule, on your own machine.
...
numeric_proofs:
  ...
  - 'One scheduler for Claude Code, Codex, and OpenCode — running on your own machine, on your own repos.'
...
| ~~Cowork scheduling only runs while Mac is awake~~ **RETIRED 2026-08-23** — Anthropic now runs Cowork tasks and Claude Code Routines in its own cloud with your device off. What survives: a cloud run never sees your machine (fresh GitHub clone; a Cowork cloud task "can't be tied to a folder on your computer"), and the local Claude Code Desktop task that does see your files still needs the app open and the computer awake. | [support.claude.com/13854387](https://support.claude.com/en/articles/13854387-schedule-recurring-tasks-in-claude-cowork), [code.claude.com/docs/en/routines](https://code.claude.com/docs/en/routines), [code.claude.com/docs/en/desktop-scheduled-tasks](https://code.claude.com/docs/en/desktop-scheduled-tasks) | VL-01, Proof Anchor         |
