...
`isPackageOwnedInRoot` (the watcher and the reconciler) goes through the same function, so a person's schedule under a recorded package agent is reported `packageOwned: false` and syncs like any other file (its `enabled` and timing come from the file), while a shipped one keeps the FB-26 and DOR-2302 row-only behaviour. No code change there; covered by tests.
