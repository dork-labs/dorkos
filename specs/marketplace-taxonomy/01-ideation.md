...
- `packages/harness/src/sources/installed.ts:117` (`readPluginManifest`): Harness projection safeParses every installed package's on-disk `.dork/manifest.json` with `MarketplacePackageManifestSchema` and **returns `undefined` on failure** — an unparseable manifest makes the package invisible and Harness Sync silently projects zero files (the DOR-264 class the file's own docstring records). This is why the singular `category` must stay a lenient string (assumption 2, D6).
- Real registry (cached `apps/server/.temp/.dork/cache/marketplace/marketplaces/dorkos-community/marketplace.json`): 11 packages with free-string singular categories — see §5 table. **No `dorkos.json` sidecar exists in the registry yet** → the `categories[]` backfill is greenfield.
...
- **Schema surfaces:** `dorkos-sidecar-schema.ts` (`DorkosEntrySchema.categories`), `manifest-schema.ts` (`BasePackageManifestSchema.categories` — enum-typed; singular `category` stays a lenient `z.string()` — plus the coherence refine), `packages/shared/src/marketplace-schemas.ts` (`AggregatedPackage.categories`).
- **Schema consumers (blast radius):** beyond the validator, `MarketplacePackageManifestSchema` is parsed by `packages/harness/src/sources/installed.ts:117` over installed on-disk manifests (drops failures → package invisibility, DOR-264 class) — the consumer that forbids tightening the singular `category`.
...
- **Registry backfill (external):** `dork-labs/marketplace` → new `.claude-plugin/dorkos.json` with `categories[]` for all 11 packages + canonicalized inline `category`.
- **Feature-flags/config:** none — the taxonomy is code-shipped, not user-config.
- **Blast radius:** additive. Every schema field is optional; `category` (singular) is preserved for CC-interop **and stays lenient** so already-installed manifests with legacy free-string categories keep parsing in the harness (`installed.ts:117`); a package with no `categories[]` still browses (falls back to singular `category`, then to "Uncategorized").
...
| D6  | Enum scope                           | Enum binds **`categories[]` only** (sidecar + `.dork/manifest.json`) plus our CI check — **never** the inbound-lenient `marketplace.json` parser and **never** the singular `category`, which stays `z.string()` | Preserves ADR-0236's inbound invariant (foreign CC `category: "anything"` must parse) **and** keeps installed legacy manifests parseable: `packages/harness/src/sources/installed.ts:117` safeParses on-disk manifests and drops failures (DOR-264 class). Effective constraint comes from the coherence refine — `categories[0]` is enum-typed. |
