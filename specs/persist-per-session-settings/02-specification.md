...
- **Cross-runtime**: a runtime that does not implement `setSessionSettings` still functions (the port is optional).
...
- No new external surface; settings are local SQLite operational metadata. `permissionMode` is written only via the schema-gated PATCH (`PermissionModeSchema` at the API boundary, ADR-0240), and a session's runtime ownership is immutable (ADR-0255), so a persisted mode is always a valid mode for its session's runtime; it is passed through on read (consistent with `model`). The only way an invalid value could enter is a direct DB edit, which is out of scope. Persisting `bypassPermissions` is intended user state, not a new capability.
