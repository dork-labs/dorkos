...
- **Bind to the install event.** After a successful install or update, the installer records the package's content hash in the install metadata (taken over the staged package before the npm step; see round 3) (`InstallMetadata.contentHash`, `lib/content-hash.ts` `packageContentHash`, the same function the preview, the update check and the update target use). Activation compares the stored approval against that recorded hash plus the declarations read at refresh. The per-turn re-check, the `lstat` fingerprint cache (`TreeHashCache`) and `installedContentHash` are removed. The runtime refreshes at boot and after every marketplace change. The preview (`POST /packages/:name/preview`) and the detail (`GET /packages/:name`) now return `contentHash`, which the app and CLI send back as `approvedContentHash`.
...
## Review delta (round 3, 2026-09-24): hash what came through the channel

The final delta review found one gap: the hash skipped a shipped root `node_modules` and `package-lock.json`, yet both land untouched and npm obeys them, so a source that moved could land different code under the approved hash. Chosen fix (option b):

- `packageContentHash` is taken over the STAGED package exactly as shipped, before the flow copies it and npm writes into the copy. It covers a shipped `node_modules` and lockfile, and leaves out only what never lands (the root `.npmrc` and symbolic links, both stripped at staging) and DorkOS's runtime state (refused at staging). Packages that vendor their dependencies keep working; nothing is stripped or refused for it.
- The installer computes that hash in `installStaged` before `dispatchFlow` and writes it into the install metadata; it never re-hashes the landed folder. The preview, the update check, the approval card and the metadata carry the one hash of the same kind of tree, which the real-installer parity test pins, including a source that moves between preview and install (held back).
- The declaration check for runtime-state paths ignores case (on a case-insensitive disk `.DORK/Data` is the skipped folder); the hash's own skip stays exact, so on a case-sensitive disk such a folder is simply hashed.
- A Changed row's "Installed now" shows just the old command.
- **Alongside DOR-2245.** DOR-2245's package validator now refuses a shipped FILE at any reserved path (`isReservedPackagePath`, case-folded, plus `.dork-old` / `.dork-new` copies), so those are refused before `assertShipsNoRuntimeState` runs. The guard stays for what the validator does not match: a folder where a runtime-state file belongs (`.dork/secrets.json/run.sh`), whose contents the hash's prefix skip would otherwise leave out. Files a person keeps in an install root under DOR-2245 never enter the hash, which is of the staged package.
