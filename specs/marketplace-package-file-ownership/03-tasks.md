...
Spec: [`02-specification.md`](./02-specification.md). Generated 2026-09-23T17:03:43Z. Canonical source: `03-tasks.json`.
...
Extra round-2 tests: an edit to a file INSIDE a unit-carried directory after the snapshot reaches the target; an edit to a SHIPPED file in the backup reaches the target (or .dork-old when the target now holds a different package file); a file deleted from the backup after the snapshot is removed from the target when its clone is untouched; carried files keep mtimes.
...
2. New in-place, JOURNALED algorithm inside withInstallTargetLock (round-2 N2):
   ...
   e. failure in b or c (not committed): replay the journal in reverse, identity files FIRST, rename everything back; remove the sibling; rethrow; the result/error says which side effects already ran (they stay run, as today);
