...
Spec: [`02-specification.md`](./02-specification.md). Generated 2026-09-23T17:10:41Z. Canonical source: `03-tasks.json`.
...
3. When ownership is set, runTransactionUnlocked (lock already held) does, in order:
   a. stage into a SIBLING named with DOR-2273's record stamp, `<target>${MARKETPLACE_STAGE_DIR_MARKER}<createdAt>-<pid>-<uuid>` (use DOR-2273's stamp/record helper from install-recovery.ts — e.g. extend beginInstallRecord or its stamp builder — never a second stamp format), instead of mkdtemp(tmpdir()): same filesystem, no RAM-backed /tmp, activation is a true rename. No stamp file, no sidecar: the owner is in the name (round 4);
...
1. Markers: task 1.3 appends `.dorkos-stage-` and `.dorkos-uninstall-` to MARKETPLACE_INSTALL_SIBLING_MARKERS. Add a guard test that scans the source of the server marketplace service, @dorkos/harness and @dorkos/mesh discovery for readdir over an install root (plugins/agents/shapes) and fails if one does not route through isInstallSiblingName (prove it with a fixture that bypasses).
2. INSTALL_RECORD_POLICIES rows: `.dorkos-stage-` -> discard; `.dorkos-uninstall-` -> roll-back whose `undo` is `recoverUninstallSibling` (task 2.5: not committed -> roll back identity files first, tolerating logged-but-unmoved entries; committed -> finish; unreadable journal -> leave in place + report, never delete). Ownership, the 10-minute floor and 'this process' handling come from DOR-2273's shared check unchanged.
3. Startup sweep scope: extend DOR-2273's startup sweep (backup-janitor.ts) to also walk plugins/, agents/, shapes/ under every registered agent's project `.dork/` via listAgentScopes() (the input scanInstallationsAcrossScopes already gets in index.ts). Under-lock recovery on each transaction/uninstall target is DOR-2273's and needs no change.
4. REQUIREMENTS ON DOR-2273'S STAMP (round 4, #2) — verify in its merged code and REPORT to the orchestrator if missing; do not add a second stamp: (a) the stamp carries the host, and a record written on a different host settles by the age floor only (the workspace can live in a synced folder); (b) on Windows the start time is unreadable (no `ps`), so records settle by the floor only — confirm assessProcessLiveness returns 'live-unconfirmed' there and a test pins it.

TESTS: a staged install's sibling is hidden from every reader (installed scanner, conflict detector, harness scanPluginsRoot, mesh scanner, update enumeration); the bypass guard fails on a fixture; through recoverInterruptedInstall: a stage record of a dead writer is discarded, an uncommitted uninstall record is rolled back (identity files first, inodes intact), a committed one is finished, an unreadable journal is left + reported, a live writer's record younger than the floor is left alone; the startup sweep reaches a registered project's scope.
