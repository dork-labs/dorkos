...
Spec: [`02-specification.md`](./02-specification.md). Generated 2026-09-23T16:56:12Z. Canonical source: `03-tasks.json`.

All tasks land in the **dorkos** repo, in one PR with the spec docs. No dork-labs/marketplace change is part of this item.

## Critical path

1.1 → 2.1 → 2.2 → 2.3 → 2.5 → 2.6 → 4.1 → 4.2. 2.8 and 4.2 need DOR-2248 merged, including its fetch of a named past commit. 1.2, 1.3, 1.4, 2.4, 2.10 and 3.1 run in parallel.
...
1. Schema (zod): `{ version: 1, package: { name, type, source? }, ownedPaths: string[], files: Record<posixPath, 'sha256:<hex>'>, pendingDefaults: Record<dorkNewPath, shadowedPath>, userEditable: string[], uninstalledAt?: string, inferred?: true }`.
...
3. When ownership is set, runTransactionUnlocked (lock already held) does, in order:
   a. stage into a SIBLING `<target>${MARKETPLACE_STAGE_DIR_MARKER}<Date.now()>-<uuid>` (mkdir parent -p) instead of mkdtemp(tmpdir()) — same filesystem, no RAM-backed /tmp, activation is a true rename;
   b. after opts.stage: computeInstalledFiles(staging, { npmRan: <node_modules present in staging> }) and write it;
   c. if the target exists: read R_old from the LIVE target; if null and hasPackageIdentity(target) -> rebuildLegacy (network allowed here, before anything moves; review 6); build planCarryOver input from the live root with lstatChain; apply it to STAGING by cloning from the live root (`copyFile(src, dst, fs.constants.COPYFILE_FICLONE)` per file, mkdir for dirs, symlinks recreated verbatim with readlink/symlink, special files skipped + noticed; carryDirs cloned recursively the same way). The live root is only read. Note each carried entry's (size, mtimeMs, ino) snapshot. Add pendingDefaults to the staged record and rewrite it;
   d. moveTargetAside (unchanged) -> activate({ path, fileNotices, warnings });
   e. LATE-WRITE PASS before runSuccessCleanup deletes the backup (review 14): walk the backup's person-owned entries (per R_old + plan); any entry whose (size, mtimeMs, ino) differs from the snapshot or that is new is cloned into the new target over the carried copy; if it collides with a package file (lstat), save it under a free .dork-old name; notice `late-write`. Then delete the backup.
...
5. backup-janitor.ts sweeps stale `.dorkos-stage-*` (delete) and `.dorkos-uninstall-*` (delete: holds only record-proven package files) siblings older than 24h, with the same name-embedded timestamp logic; packages/mesh/src/discovery/unified-scanner.ts (~L112) excludes both new markers exactly like `.dorkos-bak-` (DOR-175).
...
TESTS transaction.test.ts (+ backup-janitor, unified-scanner tests): staging dir is a sibling (assert its parent); record activates with the package; reinstall keeps `config/mine.json`; rebuildLegacy is called BEFORE the target moves (fake records call order); activate failure leaves target byte-for-byte original incl. person files and no stage/backup siblings; a file written into the backup after the carry snapshot appears in the new target with a late-write notice; a relative symlink in the live root stays relative; a socket (net.createServer().listen(path)) is skipped with a notice and never read; janitor sweeps old stage/uninstall siblings and not young ones; scanner ignores both markers. Mutation-check: remove the carry call -> person-file test fails; remove the late pass -> late-write test fails.

VERIFY: pnpm vitest run apps/server/src/services/marketplace/**tests**/transaction.test.ts apps/server/src/services/marketplace/**tests**/transaction-concurrency.test.ts apps/server/src/services/marketplace/**tests**/backup-janitor.test.ts apps/server/src/services/marketplace/lib/**tests**/atomic-move.test.ts packages/mesh/src/discovery/**tests**/unified-scanner.test.ts (adjust to the real paths); typecheck + lint server and mesh.
...
5. Source-changed warning (§12) produced in the transaction when R_old.package.source exists and differs: "Files kept from the earlier <name> (from <old>) are now available to this one (from <new>)."
...
2. New in-place algorithm inside withInstallTargetLock:
   a. read the record (legacy: rebuild via the injected rebuildLegacy, task 2.8; fallback per 2.8);
   b. create sibling `<root>${MARKETPLACE_UNINSTALL_DIR_MARKER}<Date.now()>-<uuid>`; rename into it (same relative paths) every PACKAGE-OWNED entry: isProvenPackageFile files (lstatChain, never through a symlink), ownedPaths, `.dork/install-metadata.json`, `.dork/manifest.json`, `.claude-plugin/plugin.json` when recorded. A directory whose whole contents are package-owned moves as one unit. Person files NEVER move. Keep an ordered move log;
   c. run side effects against the sibling (disableBundledExtensions reads `<sibling>/.dork/extensions`; removeGeneratedSchedules reads `<sibling>/.dork/install-metadata.json`; adapter removal; Shape teardown unless `replacing`), and for type 'agent' and not `replacing`, LAST: copy `.dork/agent.json` to `.dork/uninstalled-agent.json`, then `deps.agentRegistry?.unregisterAtPath(root)`;
   d. success: prune the record to entries still present (edited ones), set uninstalledAt, write it; delete the sibling; prune empty directories bottom-up (never count a directory, including an empty `.dork/data`, as kept); if nothing but the record remains, remove it and the root;
   e. failure in c: undo the move log in reverse (rename back), remove the sibling, rethrow. (If unregister already ran, re-register is not attempted: unregister is last, so a failure after it can only be the sibling delete, which is janitorial and logged, not thrown.)
   f. purge: after c remove the whole root (and the sibling).
...
1. agent-creator.ts: `createAgentWorkspace(input, meshCore?, internal?: { marketplace?: true })` — a THIRD parameter, server-internal, never on CreateAgentOptionsSchema (packages/shared/src/mesh-schemas.ts:950). Requires `skipTemplateDownload: true` (throw otherwise). In marketplace mode, for EVERY marketplace install:
   ...
   e. notifyAgentCreated with `origin: 'registered'` when adopted (existing AgentArrival value, moment-detectors.ts:254: DorkOS registering a directory already on disk — seat + re-bind, not announced), `'created'` when minted;
...
3. stagePackageContents(tree, tmp) (strips symlinks, root .npmrc, reserved paths — task 2.10), computeInstalledFiles(tmp, { identity from metadata, userEditable from that tree's manifest, npmRan: <root has node_modules> }), write to the root, log info, remove tmp;
4. FALLBACK (no original tree: local-path install, commit gone, offline): build a record marked `inferred: true` listing exactly the live files whose bytes equal the file at the same path in ANY obtainable tree — the new version's staged tree (pass it in when called from the transaction), every cached `<name>@*` entry, and the recorded local source path if it still exists (review 8). Everything else is the person's. One grouped warning names the kept files the new version does not ship. This removes an old version's unchanged skills (Harness Sync stops projecting them) without deleting a byte no tree vouches for.
...
TESTS: manifest + no record + commit in a fake cache -> exact record; cache miss + fake pinned fetch -> same; then update keeps config/config.json (row 9) and replaces shipped files silently. Fallback: an old skill identical to a cached older version and dropped by the new one is removed; a file no tree has is kept and warned; a local-path install uses the fallback; a tampered record (`../x`) -> treated absent -> rebuilt.
...
- apps/server/src/services/marketplace-mcp/tool-uninstall.ts:43; marketplace-capabilities.ts:237 (also: uninstalling an agent package removes it from the team); confirmation-provider.ts:225
- packages/shared/src/marketplace-schemas.ts:489 (if not done in 1.3)
- packages/cli/src/commands/uninstall.ts:5-6 + help text; print `agentRemoved` ("Removed <name> from your team." + "Its folder is blocked from your team because git tracks its settings file." when denied)
