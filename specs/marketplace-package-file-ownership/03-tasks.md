...
Spec: [`02-specification.md`](./02-specification.md). Generated 2026-09-23T20:51:25Z. Canonical source: `03-tasks.json`.
...
3. When ownership is set, runTransactionUnlocked (lock already held) does, in order:
   a. stage into a SIBLING named with DOR-2273's record stamp, `<target>${MARKETPLACE_STAGE_DIR_MARKER}<createdAt>-${formatRecordOwner(currentRecordOwner())}-<uuid>` (lib/record-owner.ts, merged in DOR-2273; the same stamp grammar parseInstallRecordName reads — never a second format), instead of mkdtemp(tmpdir()): same filesystem, no RAM-backed /tmp, activation is a true rename. No stamp file, no sidecar: the owner is in the name (round 4);
...
2. New in-place, JOURNALED algorithm inside withInstallTargetLock (round-2 N2):
   ...
   b. create sibling `<root>${MARKETPLACE_UNINSTALL_DIR_MARKER}<createdAt>-<pid>-<uuid>` stamped `<createdAt>-${formatRecordOwner(currentRecordOwner())}-<uuid>` (lib/record-owner.ts; owner in the name; no stamp file), and write `<sibling>/.dorkos-journal.json` = { version: 1, root, package, sideEffects: <captured inputs>, moves: [], phase: 'moving' } (atomic write + fsync). Rename into the sibling every PACKAGE-OWNED entry (isProvenPackageFile via lstatChain, ownedPaths, `.dork/install-metadata.json`); whole-package-owned dirs as one unit; the identity files `.dork/manifest.json` / `.claude-plugin/plugin.json` LAST. Append each move to the journal (fsync) BEFORE performing it; for a directory moved as one unit, the journal entry also lists the files step (a)'s classification found under it. Person files NEVER move;
   ...
   c2. STRAY SWEEP (round-3 N9, defined in round 4): a STRAY is an entry in the sibling that is neither the target of a journaled move nor a file listed under a unit-moved directory in that move's journal entry. ALWAYS skip `.dorkos-journal.json`. Journaled moves (install-metadata.json, identity files, node_modules, lockfile) are never strays. Each stray moves back to the root (free .dork-old name if its path is occupied again), journaled;
   ...
   Export the journal schema + `recoverUninstallSibling(siblingPath, deps)` (rollback if not committed, finish if committed, leave + report if the journal is unreadable): it is the `undo` of the uninstall row task 2.11 adds to DOR-2273's INSTALL_RECORD_POLICIES.
...
DEPENDENCY MET: DOR-2248 merged (2f901e390) with fetchAtCommit.
...
2. original tree via DOR-2248's `PackageFetcher.fetchAtCommit({ packageName: metadata.name, sourceKey, commitSha })` (package-fetcher.ts:223; refuses a non-full SHA via isFullCommitSha; reuses the trees/ cache ONLY under the install-time name — metadata.name is that name, incl. the typed name for direct name@url installs; any miss just fetches);
...
### Task 2.11: Plug the two new siblings into DOR-2273's recovery table and extend its startup sweep to project scopes

- **Size:** medium · **Priority:** high · **Depends on:** 1.3, 2.5, 2.2 · **Parallel with:** 2.6, 2.7, 2.8, 3.1

WHY (round-2 N1/N2, round-3 N7, round-4 #2/#3): DOR-2273 owns install-sibling handling: `isInstallSiblingName` + `MARKETPLACE_INSTALL_SIBLING_MARKERS` (@dorkos/shared, already used by the installed scanner, conflict detector, harness scanPluginsRoot, mesh unified scanner and update enumeration), records named `<target><marker><createdAt>-<pid>-<uuid>` with the owner IN THE NAME, `isRecordSettleable` -> `assessProcessLiveness(pid, createdAt)` (packages/shared/src/process-liveness.ts: `ps -o lstart=`, unreadable = live), `IN_FLIGHT_FLOOR_MS` (10 min) and a per-kind `INSTALL_RECORD_POLICIES` table in apps/server/src/services/marketplace/install-recovery.ts. This spec CONSUMES all of it and defines no second predicate, stamp, sidecar or janitor (spec §14).

PRECONDITION: DOR-2273 merged. Read its merged install-recovery.ts first; names above are from its in-progress worktree and may have shifted — follow what shipped. If it exposes no per-kind policy table, STOP and report BLOCKED.

CHANGES (DOR-2273 merged, 16e11be7b — read install-recovery.ts and lib/record-owner.ts first):

1. Markers: task 1.3 appends `.dorkos-stage-` and `.dorkos-uninstall-` to MARKETPLACE_INSTALL_SIBLING_MARKERS. The shipped guard `install-sibling-readers.test.ts` covers readers; extend its fixtures if it enumerates markers.
2. InstallRecordKind gains 'stage' and 'uninstall'; INSTALL_RECORD_POLICIES gains `{ kind: 'stage', marker: MARKETPLACE_STAGE_DIR_MARKER, owned: true, suffix: '', recovery: { phase: 'finished' } }` and `{ kind: 'uninstall', marker: MARKETPLACE_UNINSTALL_DIR_MARKER, owned: true, suffix: '', recovery: { phase: 'unfinished', recover: recoverUninstallSibling } }` where recoverUninstallSibling (task 2.5) returns 'rolled-back' (not committed), 'rolled-forward' (committed: finished), 'kept' (unreadable journal), and leaves the sibling on disk until the root is settled. Update the module header table.
3. "Whole" hook (DOR-2273 header): in recoverInterruptedInstall, before discarding a 'committed' backup, if the live target has an installed-files record and `isInstallWhole(target)` (lib/installed-files.ts: every recorded file present, regular, not through a symlink, matching hash) is false, KEEP the leftover and report it in `kept` with a reason; never restore it over the target.
4. Startup scope: confirm the shipped sweep already covers registered projects ("for registered projects once Mesh has reconciled" per DOR-2273's commit); if it does, no change.
5. Pin the round-4 requirements with tests only: a record whose owner host hash differs is 'maybe-running' -> not settleable before the floor; a startedAt of 0 (Windows / unreadable) likewise.

TESTS: a staged install's sibling is hidden from every reader (via the shipped guard); through recoverInterruptedInstall: a stage record of a dead writer is discarded, an uncommitted uninstall record is rolled back (identity files first, inodes intact), a committed one is finished, an unreadable journal is left + reported, a live writer's record younger than the floor is left alone; a committed backup beside a target that fails isInstallWhole is kept and reported; host-hash and startedAt=0 records wait for the floor.

VERIFY: targeted vitest on install-recovery, backup-janitor, installed-scanner, conflict-detector, harness sources, mesh unified-scanner and the guard test; typecheck + lint server, harness, mesh, shared.
