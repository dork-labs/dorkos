...
**Revision:** 3 (after two independent design reviews; see the Review log).
...
6. **Late-write pass** (before the backup is deleted). Walk **every** entry of the backup except owned paths (`node_modules`, the npm lockfile). That includes files the package shipped: an agent editing a shipped skill in the middle of its own update is still a person's write. Compare each against the step-4 snapshot:
...
5. **Failure in step 2 or 3** (before `committed`): replay the journal in reverse and rename every moved entry back, **identity files first**, so the root regains its manifest before anything else. Then remove the sibling and rethrow. Side effects that already ran stay run. That matches today's behaviour: its rollback never re-enabled an extension either. The result says which ones ran.
...
- **Is it the same package?** Compare the live root's record `package.source` with the incoming source. When they differ (a different package that happens to share the name), nothing of the old identity is adopted. `agent.json`, `uninstalled-agent.json`, SOUL.md, NOPE.md and MEMORY.md are each saved under a free `.dork-old` name, a fresh id is minted, and the result warns that the earlier agent's files were set aside.
...
10. ~~Where does the janitor look for project-scope leftovers?~~ (RESOLVED, round 2) **Answer:** At startup, every registered agent's project scope as well as `dorkHome`. And under the lock of every install, update or uninstall, that target's own siblings. **Rationale:** startup alone misses unregistered projects, and under-lock alone leaves a project nobody touches again; together they cover both, and the lock makes immediate recovery safe.
11. ~~What proves a rebuilt legacy record describes the installed tree?~~ (RESOLVED, round 2) **Answer:** No more than 10% (and fewer than 3) of the recorded paths present in the live root may differ; otherwise use the byte-match fallback. **Rationale:** the pre-DOR-2248 fetcher could record a SHA it never fetched.
