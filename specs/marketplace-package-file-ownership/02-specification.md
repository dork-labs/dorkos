...
**Revision:** 4 (after three independent design reviews; see the Review log).
...
3. Set `phase: 'side-effects'` and run the side effects from the inputs captured in step 1 (and the sibling's copy of `install-metadata.json`): extension disable plus forget run approvals, adapter removal, Shape teardown (unless `replacing`), generated-schedule removal, and, for an agent package not `replacing`, agent unregistration, last (below).
   3a. **Before committing, sweep the sibling for strays.** Walk the sibling and move back to the root any entry that is not record-proven (a file some process wrote into a unit-moved directory while it sat in the sibling). A stray whose path in the root is occupied again gets a free `.dork-old` name. Otherwise the commit would delete it.
...
**One name rule.** `@dorkos/shared` exports `isInstallSiblingName(name)`, true for any of the three markers, next to the marker constants. Every reader of an install root uses it and nothing else: the installed scanner, the conflict detector, Harness Sync's `scanPluginsRoot` (and any other `readdir` of `plugins/`, `agents/` or `shapes/` in `@dorkos/harness`), the mesh unified scanner (`unified-scanner.ts:112`), the update enumeration and the janitor. A test enumerates every `readdir` of an install root in those packages and fails if one does not go through the predicate.

**One janitor, three policies.** DOR-2273 (in progress) is rebuilding the janitor around "restore when the live install is missing or incomplete, delete only when it is verified whole", and is exposing a per-kind policy seam. This spec consumes that seam; it does not build a second janitor:

- `.dorkos-bak-`: DOR-2273's policy. The live install's installed-files record is the proof of "whole": every recorded file present with matching bytes.
- `.dorkos-stage-`: always delete. A staging dir holds only clones and new package files; the live root was only ever read.
- `.dorkos-uninstall-`: by journal. Not `committed`: roll back (rename every entry back into the root, identity files first; a path that is occupied again gets a free `.dork-old` name). `committed`: finish (prune the record, delete the sibling). A sibling with no readable journal is left in place and reported, never deleted, because nothing proves what it holds.

**Where the janitor looks.** At server startup: every install root under `dorkHome`, plus the `plugins/`, `agents/` and `shapes/` of every registered agent's project `.dork/` (the same `listAgentScopes()` the cross-scope installed scan walks). And on every install, update or uninstall, under the target's lock and before anything else: the siblings of that one target. So a project nobody has registered is still cleaned the next time anything is installed there. The age threshold (24 h) applies only to the startup sweep; the under-lock sweep may recover sooner, but not blindly. The target lock is **per process** (`transaction.ts:70-88` names the residual: two servers with different data directories on one project, and this machine runs two, dev on :6242 and the built app on :4242), so holding it proves nothing about the other server. So **every sibling carries an owner stamp**, written when it is created: `.dorkos-owner.json` = `{ pid, dataDir, processStartedAt, createdAt }`, where `dataDir` is that server's `dorkHome` and `processStartedAt` is that process's start time. Under the lock, a sibling is recovered only if its owner is provably gone: no process with that `pid` exists, or one does but its start time differs from `processStartedAt` (the pid was reused). Otherwise, whenever the owner may still be alive (the same machine can't tell, or another server owns it), the sweep waits for an **age floor of 10 minutes** from `createdAt`, longer than any transaction can run (npm is bounded at 120 s). The startup sweep keeps its 24 h threshold and applies the same owner check. A sibling without a readable stamp is treated as possibly alive and gets only the age floor. A staging or uninstall sibling holds its stamp inside itself. A backup is the renamed target, so a stamp inside it would land in the person's tree: its stamp is a sidecar file beside it, `<backup name>.owner.json`. That name carries the `.dorkos-bak-` marker, so readers skip it, and it is removed together with the backup.
...
10. ~~Where does the janitor look for project-scope leftovers?~~ (RESOLVED, round 2) **Answer:** At startup, every registered agent's project scope as well as `dorkHome`. And under the lock of every install, update or uninstall, that target's own siblings. **Rationale:** startup alone misses unregistered projects, and under-lock alone leaves a project nobody touches again; together they cover both. The lock is per process, so recovery also needs the sibling's owner stamp to show a dead owner, or else the 10-minute age floor (round 3).
...
| N7  | Immediate under-lock recovery is unsafe with two servers on one machine, because the lock is per process                                                                   | `transaction.ts:70-88` names the two-server residual; this machine runs :6242 and :4242 | §14: every sibling carries an owner stamp (`pid`, `dataDir`, process start time, `createdAt`); under the lock, recover only a provably dead owner, else wait for a 10-minute age floor; an unstamped sibling gets only the floor |
