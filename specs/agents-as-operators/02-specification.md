...
> **Errata (2026-07-25).** "The per-instance local token" was wrong, and there was no auth pattern in the marketplace commands to reuse: `apiCall` sent no credential at all, so every operator verb `401`d on an instance with login on. The local MCP token cannot fill that gap by construction: `sessionGate` only accepts a Better Auth session cookie or a per-user API key on `/api/*`, and ADR-0320 makes the local token inactive whenever login is on. **What ships:** `apiCall` presents a per-user API key as `Authorization: Bearer <key>`, resolved `DORKOS_API_KEY` env var → `<dork home>/api-key` (a `0600` file the operator saves their key into, so agent subprocesses inherit it) → nothing. The branch is on key **presence**, not server state, which the CLI cannot know before it calls: with no key set up nothing is sent and behavior is unchanged, and a key that is set up rides along even to a login-off server, where `sessionGate` short-circuits before reading it. A `401` no longer surfaces as a bare `Unauthorized`: the CLI rewrites it to name the missing credential, both places it can live, and the Settings → Security → API keys screen that mints one.
...
> **Errata (2026-07-25), two corrections.**
>
> 1. **The CLI does not use the per-instance local token.** See the §1.6 errata: it presents a per-user API key (`DORKOS_API_KEY`, then `<dork home>/api-key`), which is the only credential `sessionGate` accepts on `/api/*`.
> 2. **`config_get` disclosed more than intended.** It carries `readOnlyCarveOut: true`, so it answers with no credential in the default login-off posture, and its snapshot was the whole raw config minus the four `SENSITIVE_CONFIG_KEYS`. That leaked the `providers` credential-reference map and `runtimes.codex.credentialRef` (a `file:` reference names the plaintext key file on disk) plus `cloud.linkedAccountLabel`, none of which the curated `GET /api/config` ever emits. The snapshot is now a **classification allowlist** (`apps/server/src/services/core/operator/config-disclosure.ts`): every leaf of `UserConfigSchema` is explicitly `expose` or `withhold`, only `expose` paths are copied, and a drift guard fails the build when a new config field is added without a verdict. Secrets and credential references are replaced by boolean `…Configured` flags plus `providersConfigured`. Absolute paths stay exposed on purpose (`update_agent` addresses agents by `cwd`, and the same values already ride the equally tokenless `GET /api/config` in that posture); the line held is "nothing that is a credential or points at one".
