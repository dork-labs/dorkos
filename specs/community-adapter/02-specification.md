...
- **G1** — A Zod-first `CommunityAdapter` port in `packages/shared`, with `CommunityCapabilities`, two typed errors, and DTOs that carry no credential and no filesystem path. **[Amended 2026-07-30 (DOR-754): three typed errors — `subscribeRoom` gained `CommunityRoomNotFoundError`; see Amendment 3.]** **[Amended 2026-07-31 (DOR-770): four typed errors — `setResponseMode` and `publishSignal` gained `CommunityMemberNotFoundError`, so a member that this community has nobody for is refused in the port's own vocabulary rather than each backend's. It carries no reason, for `CommunityRoomNotFoundError`'s argument: unknown and not-in-this-room must be indistinguishable. `removeMember` deliberately keeps its idempotence and does not throw it.]**
...
**Mismatch 1 has no flag, and its absence is the load-bearing one here.** The obvious design gives history a `resume: 'gap-free' | 'none'` capability; §Decisions resolved after SPECIFY, OQ6 cuts it, because all three backends reach gap-free and a one-valued enum is a flag that describes nothing. The guarantee did not go anywhere — it moved from a per-adapter declaration to a **property of the port**: a resume is gap-free or it refuses before serving an event, for every adapter, always (§4 rule 3, conformance U6). Mismatch 1 is still absorbed; it is absorbed by the cursor's contract instead of by a flag a consumer would have to branch on.
...
export interface CommunityAdapter {
   ...
   *
   * Throws `StaleCommunityCursorError` before a snapshot or entry when
   * `sinceCursor` cannot be served gap-free — it belongs to a different room
   * or community, or it is from a superseded epoch. An in-process adapter may
   * throw at call time; an HTTP adapter may surface the same refusal on its
   * first pull after server validation. Callers MUST catch it and fall back to
   * a cold snapshot. This is
   ...
   * before a snapshot or entry when `roomId` is not a room this identity can
   * stream — unknown and invisible alike, indistinguishably. See Amendment 3 at the end of this
   * document.]
   */
  subscribeRoom(
    roomId: string,
...
3. **Resume is gap-free or it throws before serving an event.** An in-process adapter normally validates at call time; an HTTP adapter may surface the same refusal on the first pull after its authoritative server validates the opaque cursor. There is no best-effort, and — since §Decisions resolved after SPECIFY, OQ6 cut the `resume` flag — no per-adapter opt-out either: this is a property of the port that every adapter owes, not a capability one may decline. A backend that cannot serve a given cursor gap-free throws `StaleCommunityCursorError` for that cursor; one that could never serve any cursor gap-free does not belong behind this port until it can, and §6 shows what "with effort" looks like for the hardest of the three.
...
```typescript
/** Thrown by `subscribeRoom` before it serves an event when a cursor cannot be served gap-free. Callers fall back to a cold snapshot. */
...
| **U6**  | Resuming from that snapshot cursor is **gap-free or throws before the first emitted event**. An in-process adapter may throw at construction; an HTTP adapter may throw on the first pull after authoritative validation. There is no third outcome; a resume that returns a partial stream fails. This is universal because §Decisions resolved after SPECIFY, OQ6 cut the `resume` flag: it absorbs what was a capability-branched row, and no adapter can declare its way out of it. |
...
| **U16** | **[Added 2026-07-30 (DOR-754) — see Amendment 3.]** `subscribeRoom` on a room this identity cannot stream throws `CommunityRoomNotFoundError` before its first emitted event. An in-process adapter may throw at construction; an HTTP adapter may throw on the first pull after authorization. Universal and un-gated: every backend can be handed a fabricated room id, so nothing needs arranging and no capability governs it.                                                      |
...
### A. §3 — a third typed error, identical for unknown and invisible

`subscribeRoom(roomId, …)` on a room this identity cannot stream throws **`CommunityRoomNotFoundError`** before a snapshot or entry. An in-process adapter may do so synchronously at call time; an HTTP adapter may surface the same typed refusal on the first pull after remote authorization. The room check runs first, before the cursor is looked at.
...
- **a stream that simply never yields** is indistinguishable from a quiet room, so the caller parks on it forever — the exact failure the before-first-event refusal discipline exists to prevent, one method along;
