...
**Seam.** `packages/mesh/src/agent-registry.ts` is the one place every agent identity write lands (`upsert`, `update`, `remove`, `relocate`). Add an observer there, surfaced on `MeshCore` as `onAgentsChanged(cb)` next to `onUnregister` / `onLivenessChange` / `onAgentAdopted` (`packages/mesh/src/mesh-core.ts:275-312`). Fire it **after** the write commits, once per write, with `{ kind: 'registered' | 'updated' | 'removed', agentId, projectPath?, name?, displayName? }`. The wire broadcast drops `projectPath` (nothing reads it, and the event is global). The observer is **suppressed when the row is unchanged**: the reconciler's discovery scan re-upserts every registered agent every five minutes, and without suppression `agents_changed` would be a per-agent polling timer (as built, DOR-2052).
...
In `apps/server/src/services/runtimes/claude-code/mcp-tools/mesh-tools.ts` (and the external server's projection of the same definition): add optional `displayName`, `icon` (single emoji), `color` (hex) and pass them through `registerByPath`'s `partial`. Fix the `name` description: it is the immutable slug that the @-mention handle is derived from (`deriveHandle(name) ?? deriveHandle(displayName)`); it is **not** the relay subject, which is `relay.agent.{namespace}.{agentId}` (an earlier draft of this line said otherwise; corrected in the DOR-2054 review). Rule as built: a name containing whitespace is a label — slugify it with `slugifyAgentName` and put the original in `displayName`; anything else is stored byte-for-byte, so existing addresses (`144x.co`, `next_starter`, non-Latin folder names) and the Discovery UI, which sends the directory basename as `name`, are unchanged. One shared gate (`normalize-agent-identity.ts`) serves the tool, `POST /api/mesh/agents` and `POST /api/agents`. The HTTP `RegisterAgentRequestSchema.overrides` already accepts these — no shared-schema change. Skill pack: mention the three fields.
...
- `operator.sidebar_add_to_group { group: string (name or id), items: SidebarItemRefInput[], createIfMissing?: boolean }` — the **input** is wider than the stored `SidebarItemRef`: an agent may be named by `path`, `agentId` or `name` (slug or display name, case-insensitive) and a room by `roomId` or name; every ref is resolved against the roster before it is stored as the canonical `{kind:'agent', path}` / `{kind:'room', roomId}`, and any miss refuses the whole call (`SIDEBAR_ITEM_NOT_FOUND`) so the tool can never report success for a row the sidebar will not draw. Rooms resolve **as the caller** through the rooms domain's own visibility (`services/rooms/visible-rooms-for-caller.ts`): a room the caller cannot see answers with the same bytes as a room that does not exist, keeping the existence-oracle rule in `room-visibility.ts`; so DorkBot can file `#team` and rooms it is seated in, not the operator's DM with another agent, while the operator's own path sees everything. Reads config fresh, finds the group by id then case-insensitive name, appends items not already present (`sameSidebarItem` from `@dorkos/shared/config-schema`), writes back the **whole `ui.sidebar` section** (the write contract). Membership is single-parent, so an item already in another manual group is **moved**: it is lifted out of every other group exactly as the client's `moveToGroup` does, and the groups it left come back as `movedFrom: [{ groupId, name }]`. Apart from those, every other group is untouched. Returns the group as saved.
...
Shipped 2026-09-15/16 as PRs #1874 (DOR-2053), #1878 (DOR-2055), #1879 (DOR-2054), #1880 (DOR-2052), #1882 (DOR-2056). Filed from the reviews: DOR-2066 (marketplace agent uninstall bypasses Mesh, so no `agents_changed` fires until the grace sweep), DOR-2067 (register capability tools with a full `ZodObject` so the DOR-2053 workaround can go), DOR-2068 (vocab gates do not scan the operating-skills prose), DOR-2069 (`configuredRuntimes` in three copies; allowlist guard should enumerate from the schemas), DOR-2070 (direct tests for `useCoalescedInvalidation`'s veto edges), DOR-2071 (backfill three stranded fragments on main).

A fresh reader must not "improve" these back into bugs: the `agents_changed` suppression on unchanged rows is what stops a five-minute broadcast storm; the room resolution is caller-scoped on purpose, not an oversight; `name` is left byte-for-byte when it has no whitespace on purpose, because the Discovery UI sends directory basenames as `name` and handles derive from it; the `feedback_draft` environment block renders above the prose on purpose, so a body cannot forge it.
