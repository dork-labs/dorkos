...
| 3.3  | Build the out-of-usage banner and its transcript marker, and run the accessibility check over every new showcase | xl     | high     | 3.2, 3.1, 2.2, 2.3      | dorkos      |
...
| 5.1  | Document using several Claude accounts and what happens when usage runs out                                      | small  | medium   | 2.1, 2.3, 3.2, 3.3, 4.3 | dorkos      |
...
- **D**: 2.2, 3.2. After 2.1 (2.2), and 2.1 + 3.1 (3.2).
- **E**: 3.3. After 3.2, 3.1, 2.2, 2.3; its axe spec covers every new showcase.
...
- New use-account-identity-gate.ts `useAccountIdentityGate(runtime: string | null | undefined): boolean`: `true` when `runtimeCapabilities[runtime].supportsAccounts` (read the capabilities from the same source the client already uses for other runtime capability flags) and that runtime's registered account count is 2 or more (Claude Code: `useClaudeAccounts().accounts.length`; every other runtime: 0, no registry today). THE one gate for every account-identity surface (invariant 1).
...
- New use-account-usage.ts `useAccountUsage(runtime: string | null | undefined, opts: { enabled: boolean }): { byId: Map<string, AccountUsage>; byPath: Map<string, AccountUsage>; isLoading: boolean }` = `useQuery({ queryKey: accountKeys.usage(runtime), queryFn: () => transport.getAccountUsage(runtime), staleTime: 60_000, enabled: !!runtime && opts.enabled })`. The CALLER decides `enabled`: it is true only while a surface that shows usage is mounted (the chip, gated by the identity gate; the Settings → Runtimes tab; the out-of-usage banner while a limit is showing), so a one-account app makes no request on the hot path. `byId` keyed by `accountId` (skip null), `byPath` by `path`.
- Seed from the session list: where the session-list query receives `GET /api/sessions` data, when the envelope carries `accountUsage` (S4 D7), merge it per `runtime` into any cached `accountKeys.usage(runtime)` with `queryClient.setQueryData` (never create a cache entry for a runtime nothing asked for, so the seed makes no request and costs nothing with one account). Export the pure merge `mergeAccountUsage(prev: AccountUsage[], incoming: AccountUsage[]): AccountUsage[]`: upsert by `(runtime, accountId ?? path)` and never replace a record whose `updatedAt` is newer than the incoming one (compare as dates; null loses).
...
- useAccountIdentityGate: false with 0 and 1 Claude accounts, true with 2 on `claude-code`, false for `codex` with `supportsAccounts: false`, false while capabilities are loading.
- useAccountUsage: `getAccountUsage` is NOT called when `enabled` is false (spy); called with the runtime when true; byId/byPath mapping.
...
- The session-list seed fills an existing `accountKeys.usage('claude-code')` entry and creates none when nothing is cached.
...
4. POPOVER: apps/client/src/layers/features/status/ui/AccountPopover.tsx (house `ResponsivePopover`), top to bottom:
   ...
   4. "Continue on another account →" (primary `Button size="sm"`). Hidden while the session has a live turn (`lifecycle` running) and before launch (`pending`). In THIS task it renders only when an `onContinue` prop is provided (task 3.2 wires it to open the picker), so nothing dead ships.
...
- Gate: chip absent with 0 and 1 Claude account and for a Codex session (`supportsAccounts: false`), present with 2 on a Claude session; `getAccountUsage` not called for a one-account chat view or a Codex session (spy).
- Text and accessible name for ok / unknown / near (week and 5h) / out on a 7-day window ("out until Tue 3pm") / out on the 5-hour window ("back in 47 min", ticking to "back in 46 min" after a minute with fake timers) / out without reset ("out"); a `moved` limit with an ok account renders ok; bars' aria-label.
- Popover: rows per window with local reset times (fixed `now`, `TZ`), "5-hour" / "This week" labels, an extra Opus row keeps the server label, plan shown and omitted; tracker line only with `trackerItem`; the action hidden during a live turn and before launch, and absent without `onContinue`.
...
- the trailing position (where the relative time sits) shows state text, and the time is hidden while it shows:
  ...
  - `out · waiting for reset` for every other limited state (`limited`, `wait-only`, `model-limited`, `all-accounts-out`, `waiting-reset`, `reset-ready`);
...
- apps/client/src/layers/entities/session/model/status/use-session-border-state.ts: `SessionBorderKind` gains `'limited'`, labelled "Out of usage" for the tooltip, error-tone border; map `sessionDisplayState(status) === 'limited'` (S4) to it for every state except `moved`. Update its test.

3. HEADER BADGE: new apps/client/src/layers/features/status/ui/AccountBadge.tsx (export via the features/status barrel) reading `useSessionAccount(sessionId)`: a small outlined pill (`rounded-full border text-2xs`) with the `AccountDot` and the name, `● Acct 2`; when `chipState === 'out'` it uses the error surface (`STATUS_TONE_SURFACE.error`) and reads `● Acct 4 · out`. Returns null unless `visible`. apps/client/src/layers/widgets/one-bar/ui/SessionHeader.tsx passes it into `OneBar`'s `chips` after the origin chip; update SessionHeader.test.tsx.
...
- Limited row: `createMockSessionLimit('handing-off')` -> "out · handing off"; each of `limited`, `wait-only`, `model-limited`, `all-accounts-out`, `waiting-reset`, `reset-ready` -> "out · waiting for reset"; `moved` -> no tint and no text; `status` absent -> nothing; the relative time is hidden while text shows; the accessible name includes the text; border kind `limited` labelled "Out of usage".
- Badge: name; out variant text `● Acct 4 · out`; absent at 0/1 accounts and for Codex.
...
- Depends on: 1.2, 3.1
- Parallel with: 2.1, 2.2, 3.2, 4.2, 4.3
...
Tracker: DOR-2379's UI part (color, usage bars), riding DOR-2387. Spec §6.5, §9 Settings rows, §12 Settings and Gate rows, §13 Settings row, §14 Q8 and Q15.

1. USAGE FOR EVERY RUNTIME (R7, operator-confirmed): each `RuntimeCard` (apps/client/src/layers/features/settings/ui/runtimes/) whose runtime has usage windows shows its account's 5-hour and weekly `UsageBar`s, EVEN WITH ONE ACCOUNT. The Settings → Runtimes tab calls `useAccountUsage(runtime, { enabled: true })` for each runtime card it renders (only while the tab is mounted).
...
Tracker: DOR-2382's UI part (the history the transcript marker reads) and DOR-2388 (the `advised` flag the picker needs). Spec §7.1, §7.3, §4 invariant 4, §12 History and Invariant 4 rows, §15 ("Limit history is a table owned here … if #2147 adds its own history, this section is dropped in favor of it").

FIRST, check #2147 as merged: if S4 already keeps a limit history, `advised`, or a per-runtime usage route, use S4's and skip that part here (say so in the PR). N3 (cross-runtime continue) and N4 (accounts of every runtime for the extension) are NOT built here or anywhere in this spec: the UI feature-detects them.
...
OPENED FROM: the account popover's "Continue on another account →" (wire task 2.1's `onContinue` prop in AccountItem to open this dialog; composing a sibling feature's UI is allowed) and the banner's "Continue on another account…" / "Choose account…" (task 3.3). Only when the identity gate is on (one account has nowhere to go; S4 then reports `wait-only`). Export the dialog as a controlled component (`open` / `onOpenChange` / `sessionId`) from the slice barrel for task 3.3.
...
DATA: `useLimitBanner(sessionId)` reads `limit` from `useSessionAccount(sessionId)` (live on the session stream, falling back to `Session.status`); names and colors from `useClaudeAccounts()` (`nameFor`/`colorFor`, config already cached) and reset times from the limit itself (`resetsAt`, `plan.fireAt`, `plan.resumeAt`, `allOut.resetsAt`), so the banner makes no usage request; `continue-options` only when a state needs the target's name and it is not in `useClaudeAccounts`; `getLimitHistory(sessionId)` (`accountKeys.limitHistory`) for markers, refetched when the stream's `status_change` clears the limit. `Name` below is task 1.1's `limitSubject({ runtime, accountLabel, identityGate })`: the account label with 2+ Claude accounts, else the runtime's name ("Codex is out of usage until Tue 3pm"). Model display names ("Opus", "Sonnet") come from the runtime's model list the client already has (the model whose id or family matches `limit.window`'s model bucket, and `limit.modelFallback`), never hard-coded.
...
| `handing-off` (flow auto) | The same first sentence + "Moving this task to ● Acct 2 in 10s…", counting down to `plan.fireAt` (`plan.target`'s name and dot). **Move now** (primary: `POST …/continue { account: plan.target }`), **Choose account…** (opens the picker), **Wait for reset**.                                                                                                                                                                                                                                      | paused: "Paused until the task moves or the account resets"                      |
...
| `model-limited`           | **"Opus is out on Acct 3 for this week."** + primary **Keep going on Sonnet, same account** (`POST …/continue { model: limit.modelFallback }`), then **Continue on another account…**, **Wait for reset**.                                                                                                                                                                                                                                                                                            | paused                                                                           |
...
- In the Continue-on / Choose-account buttons, the picker is reachable only when the identity gate is on; with the gate off, S4 reports `wait-only` and those buttons do not appear.
...
- Note on the checkbox: S4's `carryOver: false` lives on the `ask` plan, and once the plan becomes `waiting` it is no longer on the plan. Before building, check whether #2147 exposes it on the `waiting` plan or the limit; if it does, hide the checkbox from that field; if not, hide it when the episode was `wait-only` before the wait (keep that in `use-limit-banner.ts`), and tell the orchestrator S4 should carry it.
...
Tracker: DOR-2388 (the advisor) and the server half of the Flow tab (settings decision 2, option A). Spec §8.1, §8.2, §8.4, §7.3 (N3, N4 and the carried-over-run risk), §12 Flow extension row, §15.
...
- `GET /fleet` -> `{ handoff: 'auto' | 'ask', crossRuntimeFallback: 'off' | 'on', groups: Array<{ runtime: string, label: string, accounts: Array<{ key: string, id: string, label: string, color: string, implicit: boolean, role: 'main' | 'rotation' | 'kept-out', reservePct: number, spendDownWindowHours: number, repos: string[], effectiveReservePct: number }> }>, warnings: string[] }`.
  ...
  - Group labels "Claude Code", "Codex", "OpenCode"; an implicit account's label is "<Runtime> (this computer's sign-in)" and its color the stone palette value `#78716c`.
...
- Depends on: 4.2
- Parallel with: 2.1, 2.2, 2.3, 3.2, 3.3
...
Tracker: DOR-2387 (settings decision 2, option A: the Flow tab). Spec §8.1 (client rules), §8.3, §10 (Flow tab accessibility), §11 Flow tab row, §12 Flow extension row, §14 Q9 and Q16.
...
- Depends on: 2.1, 2.3, 3.2, 3.3, 4.3
- Parallel with: none

Depends on (outside this file): S4 (#2147) shipped (the behaviour described here). In this file: 2.1, 2.3, 3.2, 3.3, 4.3 merged (describe only what shipped).
