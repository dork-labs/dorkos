...
This specification defines a CSS custom property scale multiplier system for the LifeOS Gateway client that makes text, icons, and interactive elements proportionally larger on mobile devices. The system provides a single centralized "dial" (`--mobile-scale`) that controls how much bigger UI elements render on screens narrower than 768px, with optional per-category overrides for fine-tuning text, icon, and interactive element scales independently.
...
The LifeOS Gateway client was designed desktop-first. On mobile devices:
