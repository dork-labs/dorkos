...
- Resume failures during release verification, CLI checks, preflight, or plan comparison retain the same recovery report. Unresolved creation intents show provider-specific read-only reconciliation commands and explicitly forbid adopting a name match.
- Preflight proves a local non-printing clipboard and browser handoff before consent, gives official install or sign-in guidance for Fly and Neon failures, labels billing and quota checks as unknown where provider APIs cannot prove them, and names the accounts that may incur charges. Provisioning prints secret-free progress at each bounded phase.
...
- 145 focused launcher and shared release-contract tests pass.
