...
- Preflight proves a local non-printing desktop clipboard session before consent, gives official install, compatibility, status, and sign-in guidance without claiming an ambiguous process exit proves expired authentication, labels billing and quota checks as unknown where provider APIs cannot prove them, and names the accounts that may incur charges. The browser opener is optional because the non-secret owner URL is always printed. Provisioning prints secret-free progress at each bounded phase.
