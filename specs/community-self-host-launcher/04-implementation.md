...
- Resume failures during release verification, CLI checks, preflight, or plan comparison render only the immutable non-secret recovery context saved with the consented plan. Unresolved creation intents show provider-specific read-only reconciliation commands and explicitly forbid adopting a name match.
- Preflight checks for the platform clipboard command and desktop session without reading or changing private clipboard contents. After typed launch consent and before the first service write, the operator separately approves replacing the current clipboard with harmless test text and the launcher exercises the exact copy command. It also gives official install, compatibility, status, and sign-in guidance without claiming an ambiguous process exit proves expired authentication, labels billing and quota checks as unknown where service APIs cannot prove them, and names the accounts that may incur charges. The browser opener is optional because the non-secret owner URL is always printed. Provisioning prints secret-free progress at each bounded phase.
...
- 148 focused launcher and shared release-contract tests pass. One loaded full-directory run reached 145 passes and three unchanged one-second fake-process timeouts; the exact two affected files then passed 14/14 in isolation.
