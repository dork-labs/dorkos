...
Its label names the whole scope, so nobody learns what they granted afterwards: **"Allow, and stop asking about this for 8 hours"** (the number comes from the setting). One line under it names the exact pair in plain words: which agent, which action. Styled below "Allow" and less prominently, so the plain one-time answer stays the obvious default. "Don't allow" stays first and unstyled, keeping the property the guide calls out, that neither answer is dressed up as the safe one (`docs/guides/action-approvals.mdx`).

**When it cannot be offered, the card says so rather than drawing nothing** (DOR-2102). Omitting the button silently taught the wrong lesson: somebody who has never seen it concludes there is no way to stop being asked. Where the settings are the only thing missing, the card names the one place that fixes it — _"Want to stop being asked about this? Turn on Standing permissions in Settings, under Access."_ — gated on the config having actually been read, so it never states a reason that is not true yet. On a request carrying no agent path it stays silent, because permissions key on that path and no setting would help.
...
- Settings, under **Access** (`apps/client/src/layers/features/auth/ui/SecurityPanel.tsx`, composed into `AccessTab`), holds the master switch, the window picker, and the same list. This is where the approvals guide already points people (`docs/guides/action-approvals.mdx`). The tab was called Security when this spec was written; DOR-1758 merged Security and Account into one **Access** tab, and DOR-2102 corrected the copy that still named the old one.
...
**Making the two settings legible together.** The product owner's intent was that a user never discovers by surprise that "don't ask" did not mean don't ask. There is no per-agent permission mode to sit beside, so this is done at each place a permission mode is actually chosen. When a mode that stops asking is selected, one line appears (`apps/client/src/layers/shared/ui/permission-mode-scope-note.tsx`):

> This covers what an agent does in a session: editing files, running commands, and working outside this project. DorkOS's own risky actions still stop for you, like deleting a schedule or removing an agent. The setting for that is Standing permissions, in Settings under Access.

On a surface that renders the Standing permissions control itself, the last sentence names that control instead of sending the reader past it (`standingPermissionsAt="below"`, used by the Control Center's dial):

> The setting for that is the Standing permissions switch below.

**DOR-2102 rewrote that line and widened where it appears.** As shipped here it said only the half that does not happen ("Actions on DorkOS itself, like removing packages, still ask. Change that in Settings, under Security"), which never said what the mode _does_ cover, named the rarest member of the destructive class, and pointed at a tab that no longer exists. It now carries three facts: what the stop covers, what it does not, and where the setting for the rest lives. The two examples are the destructive MCP tools that really do park a card, `tasks_delete` and `mesh_unregister` (`apps/server/src/services/core/mcp-tool-tiers.ts`). The clause names a place rather than giving an instruction, because the component sits in `shared`, reads no config, and therefore cannot know that the setting may already be on or that Require login is the real first step on a login-less install. The one surface that _has_ read the config and knows the feature is off — the approval card — keeps the imperative.

The three sites this spec named are the session picker (`apps/client/src/layers/features/status/ui/PermissionModeItem.tsx`), the channel binding (`apps/client/src/layers/entities/binding/ui/BindingAdvancedSection.tsx`), and the scheduled task form (`apps/client/src/layers/features/tasks/ui/TaskFormInner.tsx`). DOR-2102 added the three that set where sessions _start_: both dials on Settings → Runtimes (`GlobalTrustRow.tsx`, `rows/TrustRow.tsx`) and the Control Center's Power dial (`ControlCenterDial.tsx`). Those mattered because a person who has recorded a standing autonomy acknowledgement never opens the consent dialog again, so nothing else on those routes carried the sentence. The frozen picker list in `apps/client/src/layers/shared/ui/__tests__/permission-mode-scope-note.test.tsx` is what notices a new picker arriving without it. Because that guard greps source text, it proves a mention rather than a rendering, so each of the three new sites also carries its own rendering assertion, and where the sentence lands across the Runtimes tab as a whole is counted in `apps/client/src/layers/features/settings/ui/runtimes/__tests__/scope-note-placement.test.tsx`.

The condition is no longer the name-based `isBypassPermissionMode` alone. DOR-816 moved it to `needsConsentRitual` (`packages/shared/src/permission-semantics.ts`) wherever the runtime's declared profile is at hand, so a mode that never asks but is filed at the middle stop — Codex's `workspace-write` — is covered too; the name check remains only as the fallback for a site with no descriptor.
...
**The rationale this spec gave has since expired, and the firewall was re-decided on its merits.** As written, the argument was that an agent could already set `bypassPermissions` on a scheduled task through the `tasks_update` MCP tool, so a gate that honored permission mode would be an ungated path for an agent to disarm its own destructive gate. DOR-504 closed that door: `apps/server/src/services/tasks/task-write-policy.ts` now classifies `permissionMode` as `operator-only` and refuses the field outright, whoever sends it.

The operator re-examined the firewall on **2026-09-18** and kept it (DOR-2102), on three grounds:

1. **It costs this operator nothing.** What a merge would buy is fewer interruptions, and the tier gate is not where they come from. Of 43 recorded since 2026-09-10 on the reporting machine, 35 were the runtime's own permission prompts and 8 were schedule cards; the capability card gate had fired zero times. Full autonomy already removes the 35, and the 8 are a parked `Task` rather than a tier-gated hold, so merging permission mode into this gate would not remove them either.
2. **The two gates have different blast radii.** A runtime prompt guards a recoverable "did you mean to run this command" in a tree with a git history behind it. The tier gate guards permanent, outward actions — `mesh_unregister` deletes an agent's file and tears down its relay endpoint, `marketplace.uninstall` removes a package. One switch for both means a dial flipped for convenience also disarms the irreversible half, and the person flipping it is thinking about the first kind.
3. **The reversal cost is asymmetric.** Keeping the firewall and opening it later is cheap, and the evidence for wanting it would be a card count that is no longer zero. Opening it and then discovering what it was for is not, because that discovery looks like a deleted agent nobody approved.

So the supported way to stop being asked at this gate remains a standing permission, which needs a signed-in person and a clock — and DOR-2102 made the approval card offer one in place, or name the one door when it cannot. The guards above are unchanged; only the reason written above them is. The live version of this argument travels with the test, in the header of `apps/server/src/services/core/capabilities/__tests__/permission-mode-firewall.test.ts`.
...
3. **Surfaces**: the third card button, the login-off disabled state, the header indicator section, the Security panel, the permission-mode clarifiers (three here, six since DOR-2102), the SSE event and its allowlist entry.
...
- ~~Where is the setting surfaced, given permission mode has no per-agent home?~~ **(RESOLVED)** Canonical home is Settings under Access (Security when this was written; see §3.7), with the live list mirrored on the header indicator, plus a one-line clarifier at each place a permission mode or a trust stop is actually chosen — three at the time, six since DOR-2102.
