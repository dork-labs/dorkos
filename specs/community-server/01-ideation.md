...
**The room primitive shipped** (DOR-521 → DOR-526, PRs #502/#504/#509/#510/#515/#519/#520). Rooms are membership-scoped durable streams; the read cursor is keyed `(member, room)`; agents reply in rooms with addressing and a cascade guard. (Threads were child rooms when this was written; ADR `260728-022013` made a thread a relation between entries and DOR-634 shipped it. The Buzz comparison below describes a difference rather than arguing for a shape — what stays foreign there is the depth, not the placement.) Three ADRs cover it: `260726-170125`, `260726-170127`, `260726-193526`.
...
## 9) Amendment — 2026-09-16: independently working community v1

The 2026-07 plan was written before the remote port, local room dispatch, Cloud boundary, and deployment requirements settled. `02-specification.md` is the implementation source of truth for this programme. The earlier decisions D1 (Postgres remote/SQLite local), D2 (server-side port), D6 (single-owner local install), D7–D9 (roles, owner-vouched agents, no hosted execution) survive. The following statements are superseded:

1. **Build order D5 and Track B.** Buzz read-only and the local adapter already ship. V1 begins with an independently usable `apps/community`, followed by native local connection and actual agent participation. Local invites/second signup from the old Track B never land. The obsolete B1→C1 tracker dependency must be removed.
2. **Cloud identity and the front-door buttons.** Cloud sign-in/keypair portability is optional future work. Better Auth is configured per community, with email/password by default and Google/GitHub only when the operator configures them. No SMTP or Cloud service is mandatory. `apps/site` auth is copied only as a design precedent, not imported or extracted; Cloud's private control plane is untouched.
3. **Agent credential premise.** The adapter spec's earlier assumption that agents need no community credential is superseded for this concrete backend: an agent has a distinct server identity and token, privately resolved by the local adapter when `actingMemberId` is selected. Owner revocation invalidates all owned agents on every request. The shared port still carries no secret.
4. **Remote dispatcher premise.** `specs/community-remote-trigger/01-ideation.md` proposed a small dispatcher beside the local one when the latter was much smaller. Current `RoomTriggerDispatcher` owns complex Stop/tool-only/held-turn semantics and the persisted install-wide budget. V1 mirrors remote truth into a local cache and delegates eligible live entries to that existing dispatcher and the same budget instance. Remote humans retain external origin. Snapshots/restarts import history without executing mentions.
5. **Reply path.** A synchronous local room writer cannot await an HTTP post. V1 writes an outbox row transactionally with local agent output, sends asynchronously with idempotency, and treats only a remote receipt/echo as shared history. Pending and failed states remain visible to the owner; notices and human mirrors are never forwarded.
6. **Host and files.** Hono on persistent Node with React/Vite and Postgres is the selected app. Docker Compose with persistent filesystem blobs is the portable reference; a paid Render web service with managed Postgres and S3-compatible blobs is an optional hosted recipe. Prior serverless-first or no-filesystem absolute statements are superseded. Message attachments use an actual `BlobStore` in v1; neither deployment depends on DorkOS Cloud.

This amendment resolves the old section 7 questions for v1: the shipped `CommunityAdapter` defines capabilities and `(communityRef, roomId)` addressing; remote room workspace IDs have no meaning on the wire; the server has owner-vouched agent identity with its own credential; community roles are owner/admin/member; and the local install never gains a second human account. No unresolved choice in the older list should be treated as a blocker for implementation.
