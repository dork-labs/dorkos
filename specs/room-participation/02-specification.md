...
- _"Adds a way for it to fail"_ remains **true**, and it is the price. The mitigation is not two paths: it is `E1` stated in the room context flag-ON ("in a direct message with a person, answering is not optional"), the `agent_declined` floor of §10.2.2, and an eval that measures DM answer-rate with a mutation drill proving it can go red.
  ...
  **Amended 2026-09-02 (DOR-1643) — the eval collected on that promise, and the mitigation it forced is bigger than the sentence.** Measured with the flip on, the failure was not the one this bullet anticipated: the agent did not go silent for want of an instruction, it wrote a complete answer as narration the mode drops and then spent the posting tool on a pleasantry — answer-rate and restraint failing in the same direction. Stating `E1` more forcefully did not move it; **spelling the posting CALL, room id included, as the last line before the person's message did**. So the mitigation now reads: `E1` in the room context, the `agent_declined` floor, a closing directive that leaves nothing to assemble, and the drill — which seeds that directive rather than the `E1` line. Detail in `specs/tool-only-room-replies/02-specification.md` §D11 and §D13.
