...
| T13  | Stop on a runtime turn interrupts it. **Amended in slice 3a: asserted at the ADAPTER — the interrupt reaches the warm process while a runtime window is open, which is the only part this slice makes new. "Returns queued chat" and "helpers keep running" are pre-existing behaviours of the shared `interruptQuery` path, unchanged here, and the row no longer claims tests for them** | `runtime-turn.test.ts`                                                   | 3a      | no runtime turn                                    |
