...
5. **Guided key retrieval:** "Open your Composio dashboard and go to API keys." (link out) → "Copy the project API key. A key that starts with `uak_` is your CLI key, and the API will refuse it." → "Paste it below."
...
- **OQ1 — Composio's API-keys deep link and exact key naming.** _Recommendation:_ verify the URL and the dashboard's own label live at EXECUTE; if the deep link 404s, link the dashboard root and name the page in the sentence. The `uak_` warning stands either way (it is the founder's observed failure).
