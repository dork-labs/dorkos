...
- ~~**No "Leave".** With a single human author, leaving a room you created makes it invisible with no route back. Archive is the honest verb for that intent, and it is reversible.~~ **Reversed by DOR-1233.** The premise did not hold: the owner sees every room on the install whether or not they are a member of it (`seesEveryRoom`), so leaving never makes a room invisible — only unpostable, until somebody adds the owner back. The sidebar row's menu now offers **Leave**, next to **Archive**, and they are not the same verb: leaving takes the owner off the roster and nothing else, while archiving stops the room being triggered at all. The three-way rule already refused the owner's own removal from a two-agent room before this shipped (`RoomService.removeMember`, `OWNER_MUST_BE_PRESENT`) — this only gave that refusal, and the room's own leave, a door in the UI.
