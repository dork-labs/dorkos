...
- **Task 5.4 (status-aware loading affordance)**: **Tracked in follow-up spec [`specs/status-aware-loading-affordance/`](../status-aware-loading-affordance/) (spec #248) — implemented 2026-04-17.** Widened the client's `systemStatus` store slot to `SystemStatusState | null` (`{message, status}`), threaded the SDK `status` discriminator through `stream-event-handler.ts`, and added a pure `deriveStatusCopy()` helper in `ChatStatusStrip.tsx` that maps `'requesting' → 'Thinking…'` and `'compacting' → 'Compacting context…'`. When `status` is absent (legacy path, `api_retry`), the renderer falls back to the raw `message` — no regression. See the follow-up spec's `04-implementation.md` for the full change log.
