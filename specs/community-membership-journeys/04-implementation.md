...
- Cloud unavailable (`apps/server/src/services/communities/remote/__tests__/cloud-unavailable.test.ts`): the spec asks that "the full journey works with DorkOS Cloud egress blocked". The Community server's half is `tenancy-egress.integration.test.ts`; this file proves the DorkOS half. Every non-loopback connect and DNS lookup in the process is refused and recorded, the guard is first shown to catch a real `fetch` to dorkos.ai, and then pairing, approval polling, listing (with attention), channel and history reads, and disconnect with grant revocation run through the real routes and services against a loopback Community with nothing refused. Mutation: a `fetch` to Cloud added to `RemoteCommunityPairingService.start` or to `RemoteCommunityAdapter.attention` turns it red.
