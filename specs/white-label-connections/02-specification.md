...
- trigger capability discovery and create/update/pause/delete subscription are optional and declared honestly;
...
- `connector_provider_instances(id, type, mode, display_name, custody, capability_json, credential_ref, status, created_at, updated_at)`;
- `connections(id, provider_instance_id, external_account_ref, toolkit, label, identity_hint, status, auth_config_ref, created_at, updated_at, last_verified_at)`;
- `connector_operation_revisions(id, provider_instance_id, toolkit, operation_slug, toolkit_version, schema_hash, capability_classification, input_schema_json, discovered_at)`; rows are immutable and unique for the provider instance, operation, version, and schema hash;
...
2. In one SQLite transaction, claim the migration-ledger version, seed one default provider-instance row per configured legacy provider type, copy every `connected_accounts` row to a generated stable `connections.id`, and retain the external id only in the private binding.
3. Rewrite agent/session attachment references through that mapping, insert immutable operation revisions and grants only for the resolved operation set, preserve every `attached|detached` override exactly, verify counts and foreign references, and mark the ledger complete.
...
The DorkOS MCP surface adds `connector_list_available`, `connector_list_operations`, `connector_execute`, and request-status capabilities. They expose only the invoking agent’s granted connections and exact operations. `connector_execute` requires an exact `connectionId`; no “first Gmail account” fallback exists when several are connected. Connector management capabilities are operator-only and are not offered to an agent as a self-grant path.
...
For managed subscriptions, the hosted server validates that the target instance and connection belong to the same tenant. For BYO subscriptions, the local server validates provider-instance ownership plus agent/destination existence. A canonical `filterHash` plus the owning scope, connection, event type, target, agent, and destination owns one subscription; retries and edits update it instead of creating duplicate upstream triggers. The connection page is the owner. Relay retains ownership of ordinary Slack/Telegram chat ingestion; a provider event subscription is created only for explicit account-event intent.
...
- Regenerate OpenAPI and update architecture/custody ADR references. Review proposed ADR `260905-205123`, which amends the platform-key/default-lane clauses in ADR `260729-234626` and the provider-MCP execution assumption in ADR `260718-045630`; keep both parent ADRs accepted for their surviving decisions and preserve ADR `260804-021140`.
...
- Proposed ADR `260905-205123` — DorkOS-brokered SDK/API execution plus a tenant-scoped managed provider; partially amends the next two accepted ADRs.
