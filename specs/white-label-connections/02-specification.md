...
Managed OAuth account linking uses an HTTPS callback verifier route in `apps/site`. Before redirect, the site creates a single-use, expiring flow bound to the signed-in Better Auth owner, provider instance, server-derived provider user ID, auth config, browser state, and PKCE/CSRF verifier where the upstream supports it. Completion requires the same signed-in owner, matching state/verifier, and the one unused `session_uri`; it ignores a caller-supplied callback URL whenever project callback verification is enabled. Success consumes the flow atomically before the connection becomes visible. Replays, signed-out callbacks, another user/browser, changed provider user ID/auth config, expired state, and callback URL substitution all fail without revealing account metadata. The implementation verifies these fields against the selected SDK/API version rather than copying an older documentation example.

Deployment configuration is Zod-validated and fail-closed. It includes an enable flag, a DorkOS-owned Composio project API key, an optional toolkit→custom-auth-config override map, public callback origin, webhook subscription/signing secret, and any SDK-required project/version configuration. Existing explicit overrides take precedence and must match the exact service; a broken override does not silently fall back. Otherwise, supported Composio-managed OAuth is resolved automatically, followed by supported declared account-field/no-auth schemes. The full paginated catalog stays visible with specific unsupported reasons instead of requiring a manual map or per-service live certification. Automatic config resolution does not change existing account/grant material identity. Secrets stay in hosted environment/secret storage; auth-config IDs may be configuration. Availability remains per capability: account operations may stay available while events lack webhook configuration; only the affected capability reports `unavailable` with a plain reason.

Non-OAuth completion follows ADR `260910-001730`: credential values are collected only on the signed-in same-origin `dorkos.ai` owner page and relayed transiently to a fixed Composio endpoint. Exact session, Origin, CSRF, cookie, expiry, descriptor and material checks precede single-use consumption. Values never enter local transports, agents, persistent flow rows, browser storage or telemetry. NO_AUTH requires explicit owner confirmation without credentials. The returned account ID is retained for reconciliation, but a hosted connection binding requires exact user/toolkit/config/ACTIVE verification. A generic callback ID or polling cannot replace the OAuth deferred verifier.

The code workstream is incomplete until deployable routes, migrations, tests, and operational docs exist. Product availability is a separate external gate: provision the real Composio project/key, verify representative supported authentication and any existing custom override, register/rotate the webhook secret, deploy migrations/routes, link a real instance, complete one real grant, execute a harmless read through the production path, verify its hosted and mirrored usage receipts, revoke it, and prove the next call is denied. Keep the temporary custom Gmail verification fixture distinct from normal managed defaults. Future DorkOS-branded OAuth migration is separate work, not a catalog launch requirement. If required credentials, consent or deploy access is unavailable, record the exact blocker and keep the affected managed capability unavailable; never substitute mocks as production evidence.
