...
**Revised after review (2026-09-23, coordinator decisions; the specification is authoritative):** `child_safety` notices default off; `child_safety` and `legal_order` bytes are kept unreachable on primary storage without a store until an operator releases them; the community reversal window has a 24-hour floor and community takedowns are rate limited per actor; member erasure task 2.1 (the redaction feed) is a hosted-launch blocker; an `icon` target is added.
