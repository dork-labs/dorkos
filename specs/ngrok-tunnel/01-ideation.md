...
**Task brief:** Integrate ngrok tunnel support directly into the LifeOS Gateway server so users can access the gateway externally without manual network configuration. The tunnel should optionally auto-start when the server boots, be controlled via environment variables, support HTTP basic auth, and work in both production and dev mode.
