...
- **Fly reads today cannot see a network.** fly-go's `GetApp` and `getAppsPage` do not select `network`, and the fixtures show `"Network": ""`. `App.id` is the app name (fixture `"ID": "community-fixture-app"`). The GraphQL `App` type exposes `network`, `createdAt` and `internalNumericId`, and the top-level `Query.appNameAvailable(name: String!): Boolean!` reports whether a name is free (fly-go v0.9.15 `schema.graphql`).
...
- **Tigris removal leaves secrets.** flyctl v0.4.104 `ext tigris destroy` only calls `DeleteAddOn`, and the client never unsets `AWS_ACCESS_KEY_ID` or `AWS_SECRET_ACCESS_KEY` on the app. flyctl's own `setSecretsFromExtension` (`extensions/core/core.go`) deliberately does not overwrite a secret name that already exists. The launcher relies on `createAddOn(appId)` to set the `AWS_*` secrets on the service side, and nobody has checked whether that overwrites existing names.
- **Neon creation time is not read yet.** `readNeonProjects` does not parse `created_at`, and `fixtures/neon/projects.json` does not include it.
...
| **Neon project**  | Role `community_<marker>` instead of the fixed `community_owner` (`neonctl projects create … --role community_<marker>`) | `readNeonProjects(org)`, filtered by name (the implementation adds `created_at` to its schema and to `fixtures/neon/projects.json`; a missing value means `unproved`), then the role list from `readNeonBranchTopology` on each candidate's default branch               | Exactly one project in the organization and the plan's region, named `resourceName`, with `created_at` inside the window, whose default branch has the role `community_<marker>`. |
...
- **A Tigris removal also removes the bucket's secrets.** Otherwise a re-created bucket could inherit stale credentials, or, if the service does not overwrite existing names, leave the run stuck: the re-created bucket's `inspect()` would fail, giving shape B, where `--resume` fails every time and `--remove-uncertain` answers `resume-first`. So:
  - Before deleting, the command records the non-secret digests of `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` from the app's secret list (`priorSecretDigests`).
  - After the add-on is confirmed gone, it unsets both names on the app with `fly secrets unset AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY --app <app> --stage`. No Machine exists at the Tigris step, so nothing needs restarting, and `--stage` avoids a deploy. It then confirms through the existing non-secret secret list (`readFlySecretInventory`) that neither name is present. Only then is the removal recorded as complete (§4 step 5).
  - When a resumed run creates the bucket again, its Tigris `inspect()` requires both secrets to **exist** and both digests to **differ** from `priorSecretDigests`, the same rule `provesFreshStage` applies to runtime secrets. Otherwise the step stops with `INVALID_RESPONSE` and never deploys with stale credentials. Because the names were removed first, this rule holds whether or not the service overwrites existing names.
...
4. **Confirm absence** with the same query used for the verdict. Poll with capped backoff for up to 60 seconds, because Fly and Neon deletions finish asynchronously. For Fly, also call `appNameAvailable` once, and say in the result whether the name is free yet. For Tigris, once the add-on is gone, unset the two `AWS_*` secrets and confirm by readback that they are gone (§1). A failed unset or readback counts as "still present" below.
...
- The resource is gone: for Tigris, first finish the secret unset and readback from step 4. Then finish step 5 as **Gone** with no prompt.
...
- **Stale Tigris credentials after removal:** the removal unsets both `AWS_*` names, and the re-created bucket must set new ones (§1).
...
- **Create path:** `execute.test.ts` asserts that the intent revision carrying `provenanceMarker` and `requestedAt` is persisted before `create(marker)` is called. `default-services` tests assert that the #2012 fallback rejects an app whose provenance `network` is empty or different, and that `provenance.flyNetwork` equals the value read back from the service, even when a fake returns a different value from the intent. The Tigris removal test does not record completion until the fake secret list shows both `AWS_*` names gone, and it keeps `pendingRemoval` when the unset fails. The Tigris re-create test rejects missing or unchanged `AWS_*` digests after a recorded removal. The Neon probe returns `unproved` when `created_at` is missing. The `prepare()` test stops before the intent while `appNameAvailable` is false.
