...
- Integration onto merged Task 1.1 exposed a lock inversion between its managed-inventory writes and Task 1.2's reconciliation-generation triggers. Attachment reservation and commit now lock channel, authority, and quota state before dirtying inventory; export completion locks live membership and current-channel access first. Two deterministic PostgreSQL regressions use `pg_blocking_pids` to hold the domain lock while a competing generation-triggering write completes, covering the upload/cursor and export/quota cycles. The attachment suite passes 18/18, and the exact packaged-browser scenario that exposed the deadlock passes 1/1.
