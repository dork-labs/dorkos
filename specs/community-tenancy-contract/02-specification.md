...
`pending_blob_deletions` may stay host-operational because its key is a globally unique opaque blob key and it grants no read path. Cleanup candidates must derive and log only redacted tenant context before mutation.
...
1. **Expand:** add nullable tenant columns, host-operator and lifecycle structures, composite candidate keys, and indexes without dropping current constraints.
2. **Backfill:** when one community exists, copy its UUID through every tenant-owned relation by its current foreign-key chain, normalize each mention and export-channel array element into its ordered tenant-qualified child row, promote its active owner account to host operator, and bind viable pairing rows to it. Mention backfill resolves each UUID to exactly one human or agent in the entry's community and rejects missing or human/agent-ambiguous targets rather than dropping them. With zero communities, perform no tenant backfill. Invalidate outstanding legacy bootstrap grants so the new purpose-bound preflight reissues them safely; reject migration if any other row is ambiguous or orphaned.
3. **Validate:** accept zero communities with no tenant-owned rows, or one existing community with one active owner; assert no cross-tenant relation, no unexpected null tenant key, and unchanged row counts and stable IDs. Add and validate composite constraints.
