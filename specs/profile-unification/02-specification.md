...
- **Status sentence** (`profileStatusText(member)`), from `member.agent.activity` (new, §3.1) or person facts: agent — `working` → "Replying to you in #team · 2 min" / "Working in #team · 5 min" (room label when known, else "Working · 5 min"); else `lastActiveAt` → "Last active 3 h ago"; else "Hasn't run yet". Person — self "On this machine"; other `lastSeenAt` → "Last seen 3 h ago" else "On this machine"; bridged "On Telegram" (`platformLabel`). Words only, never a coloured ring on the face (identity-micro-interactions rule); a 7 px dot before the sentence is allowed and is the only live indicator (green = working, muted = idle).
...
| rooms                     | Rooms this member is in (name · kind glyph · members) → row navigates to `/channels?room=`                        | new `useMemberRooms`                   |
