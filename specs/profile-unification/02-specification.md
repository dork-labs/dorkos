...
- **Status sentence** (`profileStatusText(member)`), from `member.agent.activity` (new, §3.1) or person facts: agent — `working` → "Working in #team · 5 min" (room label when known, else "Working · 5 min"); else `lastActiveAt` → "Last active 3 h ago"; else "Hasn't run yet". There is no "Replying to you" variant: a claim says which room a turn is held in, never whom it answers. Person — self "On this machine"; other `lastSeenAt` → "Last seen 3 h ago" else "On this machine"; bridged "On Telegram" (`platformLabel`). Words only, never a coloured ring on the face (identity-micro-interactions rule); a 7 px dot before the sentence is allowed and is the only live indicator (green = working, muted = idle).
