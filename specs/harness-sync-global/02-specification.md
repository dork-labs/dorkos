...
- **Containment is a path-segment test**, `p === root || p.startsWith(root + path.sep)`. A bare
  `startsWith` matches `<dorkHome>/plugins-of-someone-else`. **It is the second of two guards, not the
  only one** (Deviation 33): the clause also rejects a first relative segment of `..`, and that rejection
  is what actually kills the mutation — a neighbour of the plugins root always relativises to `../…`
  whatever `isInside` answered. Both are kept, and the case that pins the outcome says which one holds
  it.
...
**Measured on 2026-09-09 (DOR-1924): ONE entry. The design stands, and slice A3 does not take the
fallback.** The free H-tier user-tier run
([`meta/harness-smoke/20260909-103643.543-claude-user-tier.md`](../../meta/harness-smoke/20260909-103643.543-claude-user-tier.md))
staged one package reachable BOTH through `<claudeRoot>/skills/bothpkg__bothskill` and through
`--plugin-dir` — the flag the Claude Agent SDK 0.3.224 appends for each `{ type: 'local', path }` entry
`plugin-activation.ts` builds — plus a second package injected and never linked, as the control. A real
`claude` 2.1.266 listed `bothpkg__bothskill` **once** and never a second entry for it, while the control
`injpkg:injskill` appeared, so the injection route demonstrably loaded and the two routes collapsed to one
target rather than one of them having been dead. The plugin loader and the personal-skills loader do count
as "more than one location" in the vendor sentence's sense.

So this joined the H tier (DOR-1856, answered by DOR-1924) as one more question the run answers: **stage a
global package that is both SDK-injected and user-tier linked, start a DorkOS-driven session, and ask
Claude Code to list its skills.** One entry means the design stands. Two entries flip a single switch: the Claude Code user-tier
...
| 4   | **A link whose text resolves to `<dorkHome>/plugins-elsewhere` is not swept.**                                                                                                                                                                                   | **No single mutation reds this** (Deviation 33): `startsWith` without the separator leaves it green, because the `..` first-segment rejection catches the neighbour anyway; removing that rejection instead leaves it green too, because clause 4 keeps a package no plan enumerated. The case asserts the OUTCOME — the neighbour's link survives the sweep and the re-check — and names the three guards that hold it. |
...
**Measured 2026-09-09 (DOR-1924), free, and the three answers are in the two committed reports**
([claude](../../meta/harness-smoke/20260909-103643.543-claude-user-tier.md),
[codex](../../meta/harness-smoke/20260909-103636.210-codex-user-tier.md)), produced by
`bash scripts/harness-smoke/run.sh <harness> --free --scenario user-tier`:

1. **Claude Code reads `<claudeRoot>/skills`.** A package linked there, in an EMPTY project inside a
   throwaway HOME, was listed. SRC-04's listing half is measured.
2. **Claude Code does NOT read `~/.agents/skills`.** The same package linked there instead, with nothing
   under `<claudeRoot>/skills`, was not listed. Decision 3's cell closes the other way: the Claude Code link
   is **not** redundant and slice A3 does **not** get smaller. A real `codex` 0.145.0 DID list it from the
   same directory, so the shared-directory half of the plan is measured too — the hand measurement DOR-1856
   made against the operator's own home is now a committed verdict from a sandbox.
3. **One entry, not two** (§2.9). Slice A3 does not take the `sdkInjected` fallback.

**The run also produced a FINDING, which is what this tier is for.** Codex reads `$CODEX_HOME/skills` — a
package linked there was listed beside the five skills Codex ships in that directory's `.system/` folder —
and the compiled `codex` row does not carry that path. Nothing in slice A3 changes: it writes
`~/.agents/skills`, which is documented and measured. The read path is not added to
`packages/harness/src/vendor-facts/index.ts` because no vendor page states it, and that module forbids a
cell without one; the row's notes record the measurement and what a refresh has to cite.

**What still needs the paid turn.** Every one of the three is a **listing** answer — the harness saying what
it found. Whether a harness **injects** a user-tier skill into a model's context, rather than merely
enumerating it, is the paid half, and it is the same half the project-scenario run leaves open (SK-08,
SK-09). The bar for the remaining five tools is unchanged: OpenCode is not installed on the machine that
ran this, and Cursor, Gemini CLI and Copilot have no smoke descriptor yet, so their `~/.agents/skills`
cells are still the vendor pages' and slice A3 records a dated drop for each, exactly as this section
already says.
...
| 31  | This spec's own §2.11 and Deviation 23, which both left "does Codex read `~/.agents/skills`?" as a paper claim from the vendor page                                             | **Measured, and the page is right.** DOR-1856's free listing probe ran `codex debug prompt-input` (codex-cli 0.145.0) over a staged fixture whose `CODEX_HOME` was an empty temp directory, and two skills of the operator's own in `~/.agents/skills` appeared in the resolved skills list beside the fixture's. So the user tier slice A3 writes into IS read by a bare `codex`, and A3's premise holds. The same run settled five cells of `vendor-facts`' codex row — `readPaths.user`, plus: identity is the frontmatter `name` (a `pkg__x` directory listed as `x`), the directory need not match it, duplicates are not merged (two entries named `x`), and symlinks are followed. It settled `walk` for nobody: the probe ran AT the repository root, where an ascending walk and a fixed one are indistinguishable | Nothing in the design changes; the claim stops being a citation and starts being a measurement. Report: `meta/harness-smoke/20260909-073603.110-codex.md`. What is still open is §2.9's question — whether a DorkOS-driven Claude Code session sees a doubly-reachable GLOBAL package skill once or twice — which needs a global-scope fixture and is out of the project-scope probe's reach |
| 32  | This spec's §2.12 and the contract's §1.1 model Codex as keying a skill by its frontmatter `name` alone, so a `~/.agents/skills/<pkg>__<name>` link would be listed as `<name>` | **Measured, and it is listed as `<pkg>:<name>`.** codex-cli 0.145.0 namespaces a skill whose resolved directory sits inside a package carrying a `.claude-plugin/plugin.json` — which every package under `<dorkHome>/plugins/` does (`plugin`, `skill-pack`, `adapter`; `requiresClaudePlugin`, `packages/marketplace/src/package-types.ts`), and those are the only ones the global projector links — so the entry a person sees for a linked global package is `<pkg>:<name>`, neither the link's directory name nor the bare frontmatter name (DOR-1924, [report](../../meta/harness-smoke/20260909-103636.210-codex-user-tier.md))                                                                                                                                                                                     | Nothing in the plan changes: the link A3 writes is still `<pkg>__<name>` and the display key is Codex's business. Recorded on the `codex` row's `observed` block in `packages/harness/src/vendor-facts/index.ts`, which is where a claim about another company's software has to be traceable from                                                                                           |

| 33 | This spec's §2.4 third bullet and §Testing case 4 present the path-segment containment test as the guard a neighbour's link survives by, with "use `startsWith` without the separator" as the mutation that kills it | **The containment is doubly guarded, and the separator is not the guard that holds it.** Measured (DOR-1924): removing the separator leaves the case green, because `relative(pluginsRoot, resolved)` yields `../plugins-of-someone-else/…` for any neighbour and `globalLinkPackage` rejects a first segment of `..`. Removing the `..` rejection alone leaves it green too, because the package name is then `..`, which no plan ever enumerates, so clause 4 keeps the link. Three independent clauses hold one promise and no single removal reds it | The code is unchanged: each guard is cheap and the failure it prevents is deleting somebody else's file. What changed is the TEST, which now asserts the OUTCOME — a neighbour's link survives a sweep and a re-check — and says in so many words which mutations do not red it, rather than claiming a discriminating defect that does not exist |
| 34 | This spec's §4a freezes ONE lead sentence for the global-install drop, `installed for all your projects. Only the Claude Code sessions DorkOS runs can see it.`, and says later slices APPEND to it rather than rewrite it | **Slice A3 REPLACES that lead when the package is shared** (`GLOBAL_INSTALL_LEAD_SHARED`, "installed for all your projects, and shared with the agent tools you chose."). The append rule exists so a sentence can never be false at the moment it is printed; here the lead ITSELF becomes false the moment `dorkos harness global --enable <tool>` has run, and a false sentence cannot be repaired by adding a true one under it. Measured on the status payload, which is where a person reads it: the cell for the skill Codex was reading said only DorkOS could see it | Two forms of the lead, chosen by one injected boolean, and the A3 offer (`Run dorkos harness global --enable <tool> …`) is appended only to the unshared form — telling somebody to share what they already shared is the same defect from the other side. The append rule still governs everything after the lead |
