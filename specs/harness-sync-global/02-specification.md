...
([`meta/harness-smoke/20260909-095332.049-claude-user-tier.md`](../../meta/harness-smoke/20260909-095332.049-claude-user-tier.md))
...
([claude](../../meta/harness-smoke/20260909-095332.049-claude-user-tier.md),
[codex](../../meta/harness-smoke/20260909-095328.829-codex-user-tier.md)), produced by
...
| 32  | This spec's §2.12 and the contract's §1.1 model Codex as keying a skill by its frontmatter `name` alone, so a `~/.agents/skills/<pkg>__<name>` link would be listed as `<name>` | **Measured, and it is listed as `<pkg>:<name>`.** codex-cli 0.145.0 namespaces a skill whose resolved directory sits inside a package carrying a `.claude-plugin/plugin.json` — which every marketplace-installed package does — so the entry a person sees for a linked global package is `<pkg>:<name>`, neither the link's directory name nor the bare frontmatter name (DOR-1924, [report](../../meta/harness-smoke/20260909-095328.829-codex-user-tier.md))                                                                                                                                                                                                                                                                                                                                                            | Nothing in the plan changes: the link A3 writes is still `<pkg>__<name>` and the display key is Codex's business. Recorded on the `codex` row's `observed` block in `packages/harness/src/vendor-facts/index.ts`, which is where a claim about another company's software has to be traceable from                                                                                           |
