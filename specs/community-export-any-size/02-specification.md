...
**Central directory.** Each segment's central-directory records are computed with offsets relative to the segment start and stored with the segment row (`central_directory bytea`). The tail writer concatenates them, adding each segment's starting offset in the logical archive, so rebuilding one segment (which can change its size) only means recomputing the tail.
...
export const CommunityExportManifestV2Schema = z.strictObject({
  ...
  community: z.strictObject({
    ...
    lifecycle: z.enum(['active', 'archived']), // held reads as archived, as in version 1
...
**Rebuild after removals.** Every change to what a message shows writes an `entry_redactions` row and bumps the content version in one transaction (the invariant in `specs/community-single-item-delete/`, followed by erasure, removal, and takedown). Before writing the tail, the worker:
...
3. stores `verified_content_version = v` and the new `last_checked_redaction_id`, and repeats from 1 at most 5 times, then fails with `EXPORT_CONTENT_CHANGING`.
...
export const CommunityWireExportSchema = z.strictObject({
  ...
  progress: z.strictObject({
    entriesWritten: z.int().nonnegative(),
    entriesTotal: z.int().nonnegative().nullable(), // known after planning
    filesWritten: z.int().nonnegative(),
    filesTotal: z.int().nonnegative().nullable(),
    bytesWritten: z.int().nonnegative(),
  }),
...
- **Erasure** (`erasure.ts`, `deleteExports`): deletes every `ready` export in the community and queues every segment blob (and any version 1 `blob_key`), as it deletes every live export today. Jobs in progress are left to the rebuild, which will see erasure's redaction rows and version bumps; the husk and other small collections are read in the tail. AC-4 of the erasure spec (an export snapshotted before a change cannot commit after it) is re-proved against the job model (AC-7 below).
...
- **Parts upload.** Beside P3's single `PUT /api/v1/imports/:importId/archive` (unchanged, at most 1 GiB), the upload token or host authority with `communities:import` may use:
  - `PUT /api/v1/imports/:importId/archive/parts/:partNumber` (1 to 10,000), body at most `COMMUNITY_EXPORT_SEGMENT_BYTES`, headers `Content-Length` and `X-Part-SHA256`. Each part is a managed blob (`purpose='import_staging'`). The same part again with the same hash is `200` with no write; a different hash replaces it.
...
| `apps/community/src/config.ts`                                                                                                                                 | the six settings                                                                    |
...
- `apps/community/OPERATIONS.md`: the six settings, disk sizing (one segment per concurrent job), storage used by exports and their lifetime.
