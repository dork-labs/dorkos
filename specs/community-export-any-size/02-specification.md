...
channels/000001.ndjson          ┐
members/000001.ndjson …         │ collection segments, after the last data segment
agents/000001.ndjson            │ (each file at most 100,000 rows)
channel-members/000001.ndjson   │
agent-channel-members/…         │
audit-events/000001.ndjson      ┘ owner and evidence scope only
community/icon                  ┐ the tail: small, fixed-size entries only
...
Row counts are unbounded; each `entries/` and `attachments/` file holds one data segment's rows, and collection files are split at 100,000 rows.
...
- **Deadline.** `COMMUNITY_EXPORT_MAX_HOURS` (default 24, range 1 to 168) of running time, summed across claims; time waiting for a turn does not count, and an erasure restart resets it. Past it: `failed` with `EXPORT_TIMED_OUT`.
...
- **One reader.** `src/archive/zip-reader.ts` reads any archive through its central directory over ranged reads of a segmented source (the uploaded parts): find the end records in the last 64 KiB plus 22 bytes, follow the ZIP64 locator, stream the central directory, and read each entry by offset. It refuses: more entries than a cap checked before the directory is read, encrypted entries, methods other than 0 and 8, unsafe names (absolute, `..`, backslash, colon, control, bidirectional and zero-width characters) and names outside the version's allowed patterns, duplicate names (a set of digests over NFC, lower-cased names), directory entries, a local header whose name or sizes disagree with the central directory, overlapping entries, a declared size deflate cannot produce (over 1032:1), bytes after a deflate stream, classic end records that disagree with the ZIP64 ones, and inflated output past the declared size by one byte. Callers may also cap each entry's and the total uncompressed size. It replaces `fflate`'s streaming `Unzip` for version 1 too; version 1's rules (manifest first, `attachments/<uuid>` names, the 10,000-row and 16 MiB limits) still apply to version 1 archives.
