...
- `archived` retains all rows and blobs. Ordinary content is read-only; new posts, uploads, invites, pairings, agent actions, and membership changes fail with `COMMUNITY_ARCHIVED`. Owner export, restore, and delete remain available. Host account sessions remain valid.
...
- Archive immediately ends posting, uploads, streams, invitations, grants, and agents while preserving exact history and attachment hashes; restore revives content access but no credential.
