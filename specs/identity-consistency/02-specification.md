...
**Tracker:** DOR-676 (pulled in as W0) · DOR-677 (partially absorbed) · DOR-950 (partially absorbed) · DOR-949 (related, separate)
...
| `Identity Sizes`        | every kind at `xs`/`sm`/`md`/`lg` — **closes DOR-950's square-at-xs/sm question**, since the `compoundVariants` radius step (`identity-avatar.tsx:64-68`) is only reviewable side by side |
...
| **DOR-950**         | **Absorbed in part**                                      | **Absorbed:** click-to-profile on pills and avatars (→ §W3.2 — the drawer is the destination it was waiting for) and the square-at-`xs`/`sm` review state (→ §W4.3, `Identity Sizes`). **Stays on DOR-950:** touch long-press on the hover card, deliberately (§Open Questions) — a tap now opens the drawer, which is a better answer than a long-press card, so 950 re-evaluates whether the gesture is still wanted after Phase 3 ships.                                                                                                                    |
