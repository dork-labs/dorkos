...
- **`continuedFrom` is a deliberate forward-hook (written, not yet read).** `/clear` records the prior session id as a `continuedFrom` URL search param on the fresh session (client navigation state — the spec forbade a DB column). Nothing consumes it yet: it is the durable breadcrumb a future "continued from" affordance (sidebar link, session header) can read without a schema change. Kept intentionally per review; user-facing copy says only that the new session records where it came from, promising no visible affordance.
- **`trigger-turn.ts` export-widening (spec said "unchanged").** The spec's acceptance criteria pinned `trigger-turn.ts` as unchanged; in practice `guardTurnErrors` (already defined there) gained an `export` keyword so `trigger-command-intent.ts` could reuse the turn's error-terminal translation instead of duplicating it. No behavior change, no callers changed — a visibility widening only. Recorded here to correct the "unchanged" claim.
