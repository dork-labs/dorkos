...
**Status:** In Progress
**Tasks Completed:** 6 / 11 (1.1, 1.2, 1.4, 2.2, 3.1 and 3.2; 1.4 closed by DOR-2241 and 3.1 by DOR-2242 on 2026-09-23)
...
### Session 6 - 2026-09-23 (DOR-2242, branch `feat/community-switcher-create`)
...
- None beyond the open tasks in `03-tasks.md`.
...
- **Focus lands on the whole name.** A Community heading is marked `pending` while the Community's or channel's name is still being fetched; `focusPageHeading` waits for it, frame by frame, for at most 1.5s, and gives up if the person acts meanwhile. The Session page's heading is the session's title (from the cache the bar reads, no fetch), or "Session" before it has one.
- **Pre-existing content `h1`s** (rendered markdown, generated UI, READMEs) are not page headings and were left alone; `focusPageHeading` prefers a marked `PageHeading`, so they are only a fallback on a page with none.
