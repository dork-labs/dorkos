...
Late reads and SSE events carry the source community ref, context epoch, and an owner/session/connection authorization generation. An ordinary context switch leaves that generation valid, so late work may update only its source-qualified cache and never the visible target. Sign-out, local-owner change, membership removal, and connection revocation first invalidate or tombstone the affected generation, then close work and clear caches, drafts, anchors, and streams before routing away. Every late query, event, mutation receipt, optimistic commit, and retry result rechecks the captured generation before writing; invalidated work is discarded and cannot repopulate cleared state, including after A→B→A navigation. A send/upload/retry captures `{ ref, remoteCommunityId, roomId, threadId, clientMutationId, authorizationGeneration }` when submitted. Switching cannot retarget it. Its receipt stays with the source draft/history only while that authority remains valid; the new context shows no success, failure, attachment, or optimistic row from it.
