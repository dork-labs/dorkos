...
**The Library rule (model).** A room is a _hand-made 1:1 DM_ when `kind === 'dm'` **and** its roster is exactly one agent plus the viewer **and** it is not bridged (`bridge == null`). `build-library-sections.ts` **excludes** hand-made 1:1 DMs from the Direct messages section (reason `library:dm-suppressed-1to1`). They still appear in Today under the existing rules when they have a directed unread or recent activity (BC-16 unchanged) and in ⌘K. The agent row gains an **unread dot** when its hand-made 1:1 DM has a directed unread (`derive-unread-signal.ts` → `agentRow` via a `dmUnreadByAgentPath` map built once in `buildSidebarItems`). This needs no server migration: existing rooms simply stop being listed.
