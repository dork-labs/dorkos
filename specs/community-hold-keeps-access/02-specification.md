...
No migration. No new dependency. No wire schema change.
...
With `$5 = 'held'` an ordinary unrevoked grant passes the grant check. The lifecycle checks around it (`lifecycle !== 'active' && !(isReadOnlyLifecycle(lifecycle) && scope === 'read')`) keep refusing every non-read scope, so a kept grant reads and nothing else. Agent credentials need no predicate change: the agent path already allows `read` in a read-only lifecycle and checks `agents.active`, which the hold no longer clears.
...
- **Opening a stream while held** answers `423 COMMUNITY_HELD` before any event is written. DorkOS does not try, because the access it holds says `stream: false`.

### Server: invitations, admissions, pairings, agents while held

| Thing                                      | During the hold                                                                                                                                                                                                                                                                  | After release                     |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| Existing invitation                        | Not revoked. `POST /invites/preview` returns it with the community's `held` state so the page says "This community is on hold. You can join when the hold ends." `preflight`, `bind`, and `redeem` answer `423 COMMUNITY_HELD` and write nothing. Its expiry clock keeps running | Redeems normally if not expired   |
...
1. **Notice a release.** A timer in the remote connection runtime re-runs `RemoteCommunityPairingService.verify()` every 5 minutes for each connection whose stored `lastKnown.lifecycle` is `archived`. When access comes back `active` with `stream: true`, the existing 5-second reconcile in `remote-room-subscription-runtime.ts` resubscribes on its own (it already does when `effective.stream` flips). Connections in any other state are not polled. A `401` from that check marks the connection reconnect-required, as every `verify()` does.
2. **Do not replay refused posts.** `deliveryFailure()` in `community-adapter-outbox-delivery.ts` treats a `423` whose code is `COMMUNITY_HELD`, `COMMUNITY_ARCHIVED`, or `COMMUNITY_DELETION_PENDING` as permanent, with the read-only reason already mapped in `remote-community-refusal.ts`. Other `423`s keep retrying.
...
| `apps/community/src/data.ts`                                                                       | remove `readOnlyWord`; pass the stored lifecycle to the three predicates                |
...
| `apps/server/src/services/communities/remote/pairing-service.ts` (or the runtime that owns timers) | 5-minute re-verify for read-only connections                                            |
...
- **AC-2 — Held means read-only, release means everything back.** While held: G reads a channel's history and downloads a file (`200`); G posts → `423 COMMUNITY_HELD`; X reads (`200`) and posts → `423`; opening a stream with G → `423`; enrolling a new agent with G → `423`; `GET /me/connection-access` with G returns lifecycle `archived` and `{read:true, post:false, enrollAgent:false, stream:false}`, and the body parses with the unchanged `CommunityConnectionAccessSchema`. Release A. Then, with no new pairing or enrollment: G posts (`201`), X posts (`201`), G opens a stream (`200`), connection access returns `active` with every capability true, and I redeems (`201`). Fails if reads still require `history_only` in a hold, if capabilities ignore the lifecycle (schema parse fails), or if anything was revoked.
...
- **AC-8 — DorkOS notices release.** With a fake clock, a connection whose stored `lastKnown.lifecycle` is `archived` calls the access route again after 5 minutes and not before; when it returns `active` with `stream: true`, the subscription runtime resubscribes the joined rooms on its next reconcile. A connection whose lifecycle is `active` is never polled by this timer. Fails if release goes unnoticed without a person opening connection status.
- **AC-9 — DorkOS does not replay.** An outbox item refused with `423 COMMUNITY_HELD` is marked failed at once with the read-only reason; after the fake server switches to accepting posts, the item is not delivered. A `423` with another code still retries. Fails if held-era posts flood in on release.
...
- **Phase 1, task 1.2 — DorkOS app.** The re-verify timer and the permanent `423`. AC-8, AC-9. Independent of 1.1 in code; useful only once 1.1 ships.
