...
| 8   | DorkOS: noticing that a hold ended                                | While a connection's last known lifecycle is `archived`, the app re-checks access every 5 minutes                                                                      | Today it re-checks only when a person opens connection status, so after release an idle app would never resume streaming               |
