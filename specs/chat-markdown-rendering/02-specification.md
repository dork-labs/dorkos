...
Integrate Vercel's Streamdown library into the LifeOS Gateway chat UI so that assistant messages render rich markdown (headers, code blocks, lists, bold, italic, tables, links, blockquotes) with syntax-highlighted code blocks, instead of displaying raw markdown text.
...
- **Links**: Rendered links are clickable. Since this is an internal tool (LifeOS Gateway), the risk is minimal. If needed later, a `components` override can add `target="_blank" rel="noopener noreferrer"` to links.
