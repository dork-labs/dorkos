...
## v1.2 (PR4, DOR-439) — shipped 2026-07-23

Dogfood round 2, merged as PR #429: the three install spinners actually spin (root cause: `motion.span` `rotate: 360` stalls after one turn on re-render; replaced with CSS `animate-spin` gated on reduced motion, recheck spinner aligned); the Ollama not-installed state gained a plain-language explainer and a guided install mirroring the onboarding provision pattern — one-click via brew (macOS) / winget (Windows) when present (`execFile` args-only, no shell, no sudo, bounded + SIGKILL, in-flight dedup, cache-busted post-install re-probe), copyable official command on Linux (sudo stays in the user's terminal), download link always visible. Structural follow-ups (OllamaLocalPath 500-line split, server SSE scaffold DRY, dedup-guard note) consolidated on the opencode tidy ticket.
