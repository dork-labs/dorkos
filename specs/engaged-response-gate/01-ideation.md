...
## 0) Amendment, 2026-08-24 — what moved under this artifact

**Read this before §5, §6 and §12.** Ten days of room work landed between the trace this ideation was written against (`e370557f8`) and `02-specification.md`. Three of those changes touch its load-bearing arguments, and one of them removes an obstacle it called "the hard constraint nobody can design around". The specification is written against the tree as it stands now; the sections below are corrected here rather than rewritten in place, so the original reasoning stays readable.

| §             | What it said                                                                                                                                                                                                                          | What is true now                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| §3, §6        | `claimTargets` selects, charges, binds, advances the cursor and claims — **synchronously inside `RoomService.post`** — so "the gate cannot go inside `claimTargets` as it stands, and it must not make the poster's 202 wait" (§6.1). | **RP8 landed** (`room-collect.ts`, PR #1014). `claimTargets` no longer exists. `dispatch()` now runs only the per-message decisions synchronously (`selectCandidates`), then puts each target into a **collection** (`collectOne`). The claim tail — `chooseTrigger` → `busyWith` → `tryReserve` → `bindRoomSession` → `setReadCursor` → `holdClaim` — moved into `claimCollected`, which runs from `RoomCollector.sweep`'s **timer**, hundreds of milliseconds after the 202 already went out. **The `dispatch()` split §6.2 proposed is therefore unnecessary: the deferred pass it wanted already exists.** The gate slots into `runCollected`, ahead of the claim pass. |
| §12 D9, §12.1 | REACT is unimplementable because `RoomService.toggleReaction` refuses a non-person author; blocked on DOR-1202.                                                                                                                       | **Agents can react** (A-06 built, `react_to_room_entry`, 20/agent/room/hour; E16b reversed by ADR `260814-195522`). The blocker named here is gone. REACT stays deferred anyway — see `02-specification.md` §12 — but for a different and weaker reason: it is a third verdict with its own emoji vocabulary, its own budget question and its own scoring, and none of that is needed to make silence free.                                                                                                                                                                                                                                                                 |
| §3, §11       | Ambient chatter eats the per-room hourly ceiling.                                                                                                                                                                                     | **The ceilings got roughly 10× bigger** (DOR-1434: cascades ~10 deep, 1000 turns/room/hour, rooms settable to unlimited via `rooms.turn_limits_enabled`, resolved through `limits/room-limits.ts`). This makes the cost argument **stronger, not weaker**: the budget was the accidental brake on ambient spend, and it has largely been taken off. The `limits/` ladder also introduces per-room overrides, which §11's "one global switch" argument now has to be made against rather than merely asserted — `02-specification.md` §7 makes it.                                                                                                                           |

**Two locked decisions from the orchestrator supersede §12.** They are not re-argued in the specification:

1. **v1 ships tier 1 + tier 2 only.** Tier 1 is free routing rules; tier 2 is a cheap YES/NO classifier — two verdicts, no rationale, "judge like a thoughtful colleague, not an eager bot", prefer NO when unsure. REACT is a labelled future extension (D9 stays deferred).
2. **A NO never dispatches a turn, so it never counts against any turn limit.** Stated as an invariant with a test obligation (`02-specification.md` §4, §10).

**And one thing this ideation got right that the specification leans on entirely**: §5's finding that there is no server-side one-shot model call, and that building one means a new `AgentRuntime` seam. That is why the specification ships in two releases — **v1.0 is tier 1 alone, with no model call anywhere** — rather than waiting on plumbing to deliver a win that is available today.
...
- `WelcomeBackOfferSource.ask` (`services/rooms/welcome-back/greeter.ts`, moved there by DOR-1201) _reads_ like a one-shot port and is not one: its production implementation is `RoomService.askAside` → `RoomTriggerDispatcher.askAside` (`room-trigger.ts`), which binds a real `(room, agent)` session, holds a claim, and charges the budget. Its own TSDoc says so.
...
- **A reaction cannot cascade**, and this is a genuinely nice property worth preserving deliberately: reactions are rows in `room_entry_reactions` (`rooms/reactions/reaction-store.ts`), not `room_entries`, so `dispatch()`'s `if (entry.kind !== 'post') return` (`room-trigger.ts`) never sees one and no reaction can trigger anybody. **This survives agents being allowed to react** (ADR `260814-195522`): what changed is who may leave one, not where it is stored, and the storage is the whole reason it cannot cascade.
