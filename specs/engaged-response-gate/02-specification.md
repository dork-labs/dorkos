...
- **T9 — busy is a park, not a NO.** An ambient message that arrives while the agent is working is not gated, is re-judged when the claim releases, and is excused exactly once.
- **T9b — a roster that moved on.** An overheard message for an agent that has LEFT the room writes no `not_addressed_to_me` refusal, so no conduct decision nobody made reaches the §14 tuning signal.

**S3 and S4 are defensive pre-filters, and no test exercises their branches. Say so rather than implying otherwise.** T9 and T9b above pin real behaviour and are worth keeping, but neither reaches the guard it sits next to, because both are shadowed by machinery upstream of `gateBatch`:

| Guard         | Why it is unreached                                                                                                                                                            | Its one reachable shape                                                                                                                                                                                                                                           |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **S3** (busy) | A collection opened behind a live claim is parked at the `collectOne` seam with no deadline, so it is never swept and never arrives at the gate.                               | A race: the window opened while the agent was free, and something claimed that agent — an aside turn, or a burst in another room sharing its checkout — between the arming and the sweep firing. The same race `claimCollected` re-asks for, for the same reason. |
| **S4** (gone) | A departed member is never selected, so no collection exists; and archiving through `RoomService.updateRoom` calls `abandonHolds`, which drops every pending collection first. | Something that changes the room without going through the service — the case `claimCollected`'s identical second gate names.                                                                                                                                      |

Both stay: the cost is two comparisons, `claimCollected` re-checks them anyway, and the failure they prevent would be indistinguishable in the ledger from a real mute. But **deleting either leaves every test green**, and an implementer reading this should not budget review time defending them as load-bearing. Writing a test for the S4 archived path would mean writing the row behind the service's back — a scenario nothing in the product produces.
