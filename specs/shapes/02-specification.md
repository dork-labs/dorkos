...
**Cross-field validity** (a `.superRefine` on `ShapeManifestSchema`, so errors are precise):
...
Add one variant to `UiCommandSchema` (`packages/shared/src/schemas.ts:2892`), keeping the 20→21 count and the discriminated-union-on-`action` shape:
...
`applyShape(name, opts)` is **idempotent** and returns `{ ok, applied, warnings[], offeredAgents[] }`. Steps, each independently degradable:
...
- Register `'shape'` in the `MarketplaceInstaller` dispatch `switch` (exhaustive — TS refuses to compile until handled) and construct `ShapeInstallFlow` in the `index.ts` marketplace block (gated on `extensionManager && adapterManager`).
...
- `POST /api/shapes/:name/apply` → `{ ok, warnings[], offeredAgents[] }` (drives §5).
...
- **Unit — schema (`packages/marketplace`, `packages/shared`):**
  ...
  - Each `.superRefine` rule fails on a crafted invalid manifest (dangling `agentRef`; two `default` agents; `extension-secret` for a non-activated extension; an `agents[]` entry with neither `template` nor `matchName`). _Purpose: the cross-field invariants actually reject._
...
- **Phase 1 — Schema & types (unblocks P1/P2 authoring).** `shape` enum + `ShapeManifestSchema` + `.superRefine`; widen `DependencyDeclaration`; `apply_layout` UI command; `ui.shapes` config + migration; the Linear Ops fixture + schema round-trip test. _After this phase the manifest format is frozen and P1/P2 can be authored against it._
