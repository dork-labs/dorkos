...
- Task #4.9: Phase acceptance — worker: p4-builder
  ...
  - **Not walked by hand: the session half of the second sentence** (queue two messages mid-turn, then kill the server for the stalled notice). Both need a live model turn, which spends against the operator's own subscription, and the machine was at load 74 with four other agents on it. The wiring is unit-tested above and the rungs themselves were built and tested in P2. Flagged rather than claimed.
...
24. **The session half of the acceptance walkthrough was not walked.** Queueing two messages mid-turn and killing the server for the stalled notice both need a live model turn. Unit-tested, not hand-verified — see task 4.9.
