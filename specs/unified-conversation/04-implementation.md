...
4. **No verb glimpse in a room.** **Closed by DOR-1351** (`specs/presence-verb-glimpse`, ADR `260819-022127`). The room's presence signal now carries the same `SessionActivity` the session pane does, one agent's lane reads "Kai is reading standup.md", and the peek carries a reading per agent. What is deliberately still not done lives in that spec's own §12 — no verb for two agents in the lane, none while `working_late`, and the target never leaves this cockpit.
...
6. **Bridged DMs still drop the waiting notice.** Unchanged as of P5: `deliver.ts`'s `DELIVERABLE_NOTICES` carried only `turn_failed` and `halted` (it was at `:78` when P5 read it; the set is now at `:~116`). **Closed after the programme by DOR-1359 — see Post-programme follow-ups below.**
...
- ~~**Presence tier 3 (the verb glimpse) and the remaining approvals tiers** are the two open "What is not done" items with the clearest next shape.~~ **Presence tier 3 shipped as DOR-1351** (`specs/presence-verb-glimpse`); the remaining approvals tiers are still the clearest next shape.
...
- **`agent_busy` needed a damper of its own at the bridge; `awaiting_approval` did not.** The first draft of this change claimed neither did, and review proved that false on the only surface it ships to. `RoomNoticeLog.reportWaiting` damps a waiting line per `(room, agent)` for the life of the turn with no exception, so one wait really is one room entry. `reportSilence` damps the busy line the same way **except for a message that directly asked the agent** — and `directlyAsked` counts every human depth-0 message in a `dm` as asking, which is exactly what a bridged private chat is (`room-service.ts` seeds `deliverNotices` true for it). Measured: three messages typed in one breath at an agent busy elsewhere wrote three `agent_busy` notices, all `visibility: 'shown'`, and the spaced case is pinned deliberately by `room-silence.test.ts`. The room is right to answer every one of them; a phone buzzing three times is not, so `ChatBridgeDelivery` now damps `agent_busy` per `(room, agent)` itself and returns a new `damped` outcome. It re-arms on the same signal `RoomNoticeLog.recovered` uses — that agent's next outcome in that room, read off the log as an agent-authored post or a `turn_failed` about it — and arms only on a line that actually landed, so a blocked or undelivered busy line never silences the next one. `damped` is deliberately not a catch-up `STOP_OUTCOME`: a repeat never becomes news again, so the cursor must settle past it. Every clause here is pinned by a test with a seeded defect.
