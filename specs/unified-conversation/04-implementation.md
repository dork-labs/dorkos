...
Six things this phase decided rather than finished.
...
22. **`capabilities.toolCards` is still read by nothing.** P1's Known Issue 2 gave P4's renderer map the job of making it live. The renderer map landed; the flag did not find an honest consumer. The only branch anybody could write is "a surface that says it cannot draw tool cards gets the words without them", and no surface says that — a session declares `true`, a room's renderer has no tool parts to drop. Inventing the branch would be a check that cannot discriminate. **P5, or whoever gives a room tool cards, is where it becomes real**; until then it is a declaration two hosts make and nothing asks.
23. **The virtualizer is stood in for in every client test.** `test-setup.ts` mocks `@tanstack/react-virtual` globally, so every suite draws every row. It is the only way the room and chat suites could keep asserting what they assert (jsdom lays nothing out, so a real virtualizer answers with an empty window), and it is the same shape as the `motion/react` stand-in above it — but it does mean **no unit test can see a virtualization bug**. The two the browser suites caught this phase (the sticky action rail, the reply row's sibling) are exactly the kind that stay invisible here.
24. **The session lane now says the same thing as the status line under it.** `stalled` is wired (P2's Known Issue 13, closed in the direction P4 argued) and the browser shows its sentence six pixels above `ChatStatusSection`'s "Live updates lost" chip. **The fix is one line either way** — drop the chip, or dark the rung where `turnStatus` is true — and it is a decision about a component the spec moves unchanged, so it is flagged with its screenshot rather than settled here (task 4.9).
25. **The lane's `queued` rung cannot light on a session.** It sits below every `turn-*` rung, and a session only has a queue while a turn is open. The queue is still shown, in the composer's own panel. Making it reachable means reordering a priority table P2 argued three orderings of — a change with its own review.
26. **The mock browser leg has a cold-start race in `global-setup`.** Three failures in four runs, at load 74 and at load 7 alike: the Vite proxy answers 500 with `AggregateError [ETIMEDOUT]` against an express leg that has already logged `server running`. `webServer.url` proves the port answers, not that it can serve. It will flake in CI the same way; the fix is to probe the API leg directly or retry the first PATCH.
