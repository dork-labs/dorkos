...
6. **Bridged DMs still drop the waiting notice.** `services/relay/chat-bridge/deliver.ts:78` delivers only `turn_failed` and `halted`. A person on Telegram learns nothing while an agent waits. Untouched here. **(Delivered as of DOR-1359: `awaiting_approval` and `agent_busy` now reach a bridged room, in the room's own unchanged words. An actionable bridged Ask — Approve/Deny in the chat — remains open as DOR-1356.)**
