...
| 11  | The GitHub token                                               | The existing rewrite, extracted from `execGitClone` so every caller shares one gate; the lookup uses it too                                                            | The fetch refuses to run when the lookup fails, so the lookup must reach any private GitHub repository the fetch can. `git-subdir` gains the token for GitHub hosts, under the same host rule. |
