...
**Which route to `~/.claude` you took decides it, and only one route makes the pin explicit.** Review found the first implementation reading "absence names the default" as "the ambient variable is unset **and** the root is the default", which is a different and wrong test: launched from a shell exporting `~/.claude2`, selecting `~/.claude` then produced an explicit pin, Claude Code looked up the **suffixed** entry for `~/.claude`, and sign-in failed — breaking exactly the acceptance criterion 3 the pin exists to satisfy. Corrected: the pin is absent for `~/.claude` on every route **except** one, which is that the ambient variable already named `~/.claude` itself. That operator authenticated under that regime, so their suffixed entry genuinely exists and naming the path is right for them. The distinction is between which Keychain _branch_ Claude Code takes and whether the name for the _wanted_ account exists; conflating them is what produced the bug.
