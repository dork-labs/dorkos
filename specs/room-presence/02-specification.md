...
- ~~Should the room SSE snapshot carry current claims so cold connects render presence instantly?~~ **(RESOLVED)** No — the republish loop covers it within ≤10 s. **Answer:** keep the snapshot durable-only. **Rationale:** the snapshot/replay contract partitions on durability; putting ephemeral state in the snapshot makes every future reader reason about a mixed contract to shave ≤10 s off a line that is absent most of the time. Revisit only if dogfooding shows the gap is felt.
