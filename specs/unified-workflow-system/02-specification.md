...
- **PM-driven (Kai, away):** labels/assigns an issue `agent/ready` in Linear. The `flow-drain` Pulse schedule (server running) claims the top-ranked eligible issue each tick, carries it to the review gate in a fresh per-issue session, posts an evidence-bearing PR, assigns the issue back, and stops. A genuine question arrives as a ticket comment + `agent/needs-input` + optional Telegram nudge; answering it resumes the work. On approval (CI green) the agent auto-merges, closes, and tears down the worktree.
