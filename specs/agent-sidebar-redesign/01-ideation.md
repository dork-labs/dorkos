...
| 10  | Easy agent addition                     | `+` button in "AGENTS" section header + inline "Add agent" prompt for sparse lists                  | `+` button opens menu: Create agent, Import project, Browse Marketplace. Always visible regardless of agent count. Inline prompt provides softer onboarding for new users. Reuses existing CMD+K Quick Actions                                                         |
