...
- **`0016_host_legal_hold.sql` (DOR-2299).** Versions are contiguous, so this takes the next free number; single-item delete also wants `0016`, and whichever lands second renumbers. `communities.legal_hold_at`, `legal_hold_by_host_actor`, `legal_hold_reference` and their check; `host_api_keys` scope check gains `communities:legal_hold`. Additive: old code ignores the columns, and backing out means releasing every legal hold first (old code would purge a community the host must preserve).
