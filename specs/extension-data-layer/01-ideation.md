...
that runs pending migrations in one SQLite transaction (previous state intact on
failure — the ADR-0304 guarantee via SQLite's own atomicity; a file-level
backup/restore was considered and rejected in review as WAL-unsafe — see spec
§5). The bridge follows the **existing
...
| 3   | Query surface                         | Parameterized SQL: `query`/`run`/`transaction`, `?`+bound params, DDL forbidden at runtime; read surface enforced by `stmt.readonly` (keyword scan is a pre-filter only) | Zero escape hatches (CRM criterion); injection-proof via prepared statements; a keyword check alone cannot stop `WITH…DELETE` |
...
| 5   | Migration rollback                    | Apply pending migrations in one SQLite transaction; rollback via SQLite atomicity + refuse-enable on failure (no file backup — WAL-unsafe, spec §5)                      | Matches the marketplace install transaction's (ADR-0304) "previous state intact" guarantee                                    |
