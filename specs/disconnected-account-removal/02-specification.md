...
Authentication claims record private `cleanupSnapshotJson` with acknowledged generations, without external references. Migration marks all historical disconnected rows `unknown`; it never infers credential cleanup from a local applied authority command. Historical pending flows receive NULL snapshot and must terminate safely; a new explicit empty snapshot is encoded as `{}` and remains distinguishable. Completed historical flows stay intact. B owns final snapshot admission/revalidation tests for both race orders.
...
Real SQLite tests prove owner/state/cleanup gates, repeated removal, hidden inventory/detail, historical grants/usage retained, fresh replacement without inherited grants, passive no-resurrection, and migration defaults/index behavior. Cleanup-guard removal must fail its regression. B adds deferred-cleanup/auth race tests, disconnected/reconnect/remove UI states, pending/error recovery, and transport authority checks. Required verification runs on the combined source. No physical-history deletion or global identity framework is introduced.
