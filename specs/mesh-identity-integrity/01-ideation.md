...
- ADR `260726-170126`: author identity keys on the directory, never the ULID. The H12 fix stamps a _generation marker_ on the author row to distinguish successive occupants of one directory — it does not re-key identity on the ULID. This amends that ADR's consequences; the "moved agent splits in two" cost stays accepted and unchanged.
