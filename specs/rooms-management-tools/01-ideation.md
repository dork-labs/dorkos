...
- **`archive_room`, deliberately.** Archiving is not a roster edit; it is a lifecycle verb. It stops the room being triggered at all (`room-service.ts:1981` calls `abandonHolds`), it is the product's stand-in for delete (there is no delete), and on a bridged room it is a different function entirely (`archiveBridgedRoom`, `room-service.ts:1381`, operator-only). It also collides head-on with an open defect: `updateRoom` has no operator gate for `archived`, and closing that gate is blocked on DOR-608 because `createRoom`'s DM un-archive path runs through it (`room-service.ts:18-25`). Shipping an agent-callable archive on top of an ungated field, whose own gate is blocked, is how you ship two bugs at once. It gets its own item after DOR-608.
  **Amended 2026-09-03 — DOR-608 shipped, so the blocker named here is gone.** `updateRoom` is operator-only now; the write half moved to a private `applyRoomPatch` that asks nothing about the caller, so `createRoom`'s DM un-archive path no longer runs through a gated method; and the agent-facing `updateRoomFromTool` has no `archived` on its signature at all. `archive_room` is a scope decision on its own merits from here, not a deferral. Every other line in this document describing `updateRoom`'s gate as conditional or absent (§4.4's call-site inventory, §4.6's field table, S1 below) is historical for the same reason — the live rule is §D12 of `02-specification.md`, which is already true.
...
| --- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| S1  | Scope                     | Five verbs; `archive_room` deferred                                                                                           | Archiving is a lifecycle verb, not a roster edit, and its own gate was blocked on DOR-608 — closed 2026-09-03, see §1 |
...
~~Four are judgement calls the orchestrator may want to overturn at SPECIFY, flagged so they are easy to find rather than hidden in a table: **D7/D8** (no global default, so the global tab row has no switch — S7 says the group appears there, and this is my reading of what it should _do_ there); **D13** (`leave_room` restricted to channels, which is narrower than "self-removal" as literally worded in the brief); **D6's scope** (refusing the one nested key now, versus blocking on DOR-1506 closing the whole seam — I recommend the former plus an explicit note that the seam is still open); and whether the **global hourly turn cap's default** should move now that `create_room` makes rooms a thing an agent can mint (§6).~~ **(RESOLVED — orchestrator, 2026-08-27.)**

All four were ruled on before SPECIFY. The originals are struck through above as an audit trail; the answers are carried into `02-specification.md`.

| Flagged call                                         | Answer                                                                                                                                                                                                                                                                                                                    | Rationale                                                                                                                                   |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **D7/D8** — no global default; switchless global row | **Approved as proposed.**                                                                                                                                                                                                                                                                                                 | A global "all agents" switch is a wider grant nobody asked for, and a second grant path is a second thing to get wrong. Spec D7/D8.         |
| **D13** — `leave_room` channels-only                 | **Approved as proposed**, with the `kind !== 'channel'` spelling and the system-room refusal.                                                                                                                                                                                                                             | Leaving a DM is unrecoverable and the guard exists only in client menu logic today. Spec D9.                                                |
| **D6** — scope of the DOR-1506 closure               | **Narrow, in PR1.** Refuse the grant-bearing field on the agent-reachable manifest-write path, following `agent-updater.ts:157-160`, while the operator's `PATCH /api/mesh/agents/:id` keeps working. The full caller-identity policy for the whole manifest stays DOR-1506's own remit — reference it, do not absorb it. | The narrow closure is what this feature actually needs; absorbing the general policy would grow the blast radius of a boundary PR. Spec D6. |
| **Turn-cap default**                                 | **Deferred out of this programme**, recorded as a named non-goal; the orchestrator files it. This work still ships a test that _measures_ the multiplication without changing the number.                                                                                                                                 | Changing a shipped default on a prediction is the wrong order; let the measurement buy the change. Spec §Non-Goals, D17.                    |

**Three items absorbed into scope at the same time** (orchestrator): the `fallback_seat_author_id` cleanup when the seat-holder is removed or leaves, as `remove_member`/`leave_room` correctness (PR2); the stale `docs/guides/agents.mdx:92` approval-prompt claim and the `destructive-actions-prose.test.ts` `TOOL_ACCESS_CLAIM` carve-out (PR2 docs); and the `requireOwnerWitnessesAgents` TSDoc correction where PR1/PR2 touch it. **Not absorbed**, filed separately by the orchestrator: `findDmByMemberSet` atomicity, the turn-cap default, and DOR-608/`archive_room`.

One ruling changed the design rather than confirming it: the fallback seat is **cleared** on removal, not defended by a refusal. That is the better call — refusing would wedge an agent that holds the seat and could then never leave, contradicting the standing guarantee that taking an agent out is never refused.
