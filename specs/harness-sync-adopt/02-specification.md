...
member of two enums and one mapping table, it is named in nine doc comments, and the one test that
...
`move.link` is present exactly when the manifest **enables `claude-code`**, whatever root the skill
came out of (Deviation 17 — an earlier draft of this section keyed it off `root === '.claude/skills'`,
and that was wrong in both directions). The link is Claude Code's projection OF A CANONICAL SKILL, not
a link back into the folder the skill was taken out of: `planSkill` plans exactly this action for
every skill in `.agents/skills` whenever claude-code is enabled. So a skill adopted out of
`.opencode/skills` in a repository that also runs Claude Code needs it, and a `.claude/skills` skill
adopted where claude-code is NOT enabled must not get one. Every other enabled harness already lists
`.agents/skills` in its own documented project read paths
...
On success **the row repaints in place** from the status the route returned — **written into the
cache with `queryClient.setQueryData`, never an invalidate**, for the reason `use-harness-sync.ts:11`
gives about its own POST: the response already carries the recomputed status, so re-reading would
discard the authoritative answer and race the write. The chips flip, the advice and the command go
with the row's adoptability, and the button stops offering a move it has already made. On a refusal
the row draws the refusal sentence where the advice line was, in a polite live region, because that
sentence is the only thing on the row that changes.

> **Decision: no "What changed" summary above the list, unlike the Sync button (Deviation 18).**
> §8 freezes no adopt-summary sentence, and the sync's own summary is a receipt about files written
> and files deleted — the wrong words for a move. The chips are the summary, at the row the person
> pressed, and a banner would have needed copy invented outside §8 to say less than the row already
> says.
...
| **S7d**  | R7, DorkOS's own fields                               | `"{name}" uses {fields} in its settings, which are DorkOS's own and mean nothing to your other agents. [only when `schedule` is among {fields}: Moving it also changes what DorkOS does with it: a skill with a schedule starts running on a timer once it is in .agents/skills.] Take {fields} out and adopt it, or run dorkos harness adopt {name} --claude-only to keep it where it is.` |
...
| **S14b** | Confirm dialog, Claude Code not enabled               | `Move {name} so every agent can read it?` / `It moves from {source} to .agents/skills/{name}, where every agent reads it.` / `Move it`                                                                                                                                                                                                                                                      |
| **S15**  | CLI success, Claude Code enabled                      | `Moved {name} to .agents/skills/{name}. Claude Code still finds it through a link at {link.target}.` — `{link.target}` is `.claude/skills/{name}`, the path the link is AT, never the folder the skill came out of                                                                                                                                                                          |
| **S15b** | CLI success, Claude Code not enabled                  | `Moved {name} to .agents/skills/{name}, where every agent reads it.`                                                                                                                                                                                                                                                                                                                        |
...
`SkillFrontmatterSchema` declares? — and both answers were false for a real field. `hooks` is not in
the schema at all, so it fell to S7c's "DorkOS doesn't recognise", about a key DorkOS's own inventory
reads (HK-12) and the same run's manifest line called Claude Code's. `schedule` and `kind` **are** in
the schema and are DorkOS's own, so S7's "which only Claude Code understands" was false about them —
and about `schedule` it was false in the direction that matters, since DorkOS is the thing that would
start running it. So `allowlist.ts` classifies explicitly, in three sets:

| Set           | Contents                                                         | Sentence   | S18 reason                                                         |
| ------------- | ---------------------------------------------------------------- | ---------- | ------------------------------------------------------------------ |
| **BASE**      | the six agentskills.io fields                                    | no refusal | —                                                                  |
| **CLAUDE**    | `SkillFrontmatterSchema.shape` − BASE − DORKOS, **plus `hooks`** | **S7**     | `…use {fields}, which only Claude Code understands.`               |
| **DORKOS**    | `schedule`, `kind`                                               | **S7d**    | `…use {fields}, which are DorkOS's own.`                           |
| anything else | a typo, a third-party key                                        | **S7c**    | `Kept in Claude Code on purpose: …which DorkOS doesn't recognise.` |

Layer 2 is still read off the schema's shape, so a dialect field the schema gains tomorrow
classifies itself; the two corrections around it are spelled out because the shape cannot express
them. **A sentence that names a tool is printed only when EVERY offending key belongs to that tool** —
S7 when all are CLAUDE, S7d when all are DORKOS, and otherwise S7c, which claims nothing about who
understands what and still names every key the person has to take out. A body token is always
**S7b**, whatever the frontmatter holds, because that fact is about the text.
...
11. **A non-Claude root** (staged by hand until DOR-1902 lands): the move happens, **no link is
    left when claude-code is NOT enabled**, `checkPlan` is still clean, and the CLI prints **S15b**
    rather than S15 — the same
...
31. The button renders only on an `adoptable` row; the confirm names both paths and prints **S14** when the manifest enables Claude Code and **S14b** when it does not, whatever root the candidate is in (Deviation 18); **cancelling calls
...
| 16  | Which projects get a link left behind                                             | Every project whose manifest ENABLES `claude-code`, whatever folder the skill came out of — the link is Claude Code's projection of a canonical skill, not a link back into the source folder (corrected by Deviation 17; the first answer was "only a `.claude/skills` source") | Every other root's owner already lists `.agents/skills` in its documented read paths. A link elsewhere would be a path DorkOS wrote that no plan names — an orphan by construction                                                                                                                                                                                                                                                                    |
...
| 16  | This spec's own §8 picked S7 vs S7c by asking whether the offending key is one `SkillFrontmatterSchema` declares ("layer 2 plus `schedule` and `kind`"); §1.2 gave `AdoptMove` a `link`; §8's S2 left `{root}` singular for a run that searches six roots; and S18 had three forms, none of them for a key DorkOS cannot place                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Measured while implementing slice 1 (DOR-1943). `hooks` is **not** in the shape, so that rule sent Claude Code's most consequential field to S7c while the same run's manifest line said "only Claude Code understands"; `schedule` and `kind` **are** in the shape and are DorkOS's, not Claude Code's. `AdoptMove.link` could not be set until `planAdoptedSkillLink` exists, which is slice 2. And `joinNames([])` prints `in .`                                                                                                                                                                                                            | Four corrections, all in slice 1. (a) The classification is explicit — BASE / CLAUDE (shape − BASE − DORKOS, plus `hooks`) / DORKOS (`schedule`, `kind`) / everything else — and a new sentence **S7d** covers DORKOS, added to §8 with the table above; a mixed list gets S7c naming every key. (b) `link` is omitted from `AdoptMove` until slice 2 adds it beside the export that can fill it, since a field nothing sets is a field nothing checks. (c) S2's `{root}` is the harness-owned roots that actually hold a skill, joined `a, b and c`, falling back to all six — applied in BOTH the reader and the planner, so a hand-built empty list cannot print `in .`. (d) S18 gains a fourth form, `Kept in Claude Code on purpose: its settings use {fields}, which DorkOS doesn't recognise.`, because a manifest line read a year later must not claim Claude Code understands a key DorkOS has never heard of |
| 17  | §1.6, §8 (S15/S15b) and §Testing case 11 keyed the link adopt leaves off the SOURCE ROOT — `move.link` present exactly when `root === '.claude/skills'`, and "four of the five roots get no link"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Wrong in both directions, measured while implementing slice 2 (DOR-1944). The link is `planSkill`'s claude-code projection OF A CANONICAL SKILL, so it is owed whenever the manifest enables `claude-code`. With claude-code ON and a `.opencode/skills` source, adopt left the planned `.claude/skills/<name>` link unmade and the very next `--check` reported drift while S15b claimed "where every agent reads it"; with claude-code OFF and a `.claude/skills` source, adopt wrote a link no plan action names — an orphan at a path no sweep owns — while S15 claimed "Claude Code still finds it" about a tool the project does not run | The condition is the READER, not the source: `AdoptSkillInput` carries `enabledHarnesses` (the reader fills it from the manifest), `move.link` is planned iff `claude-code` is among them, and S15 names `link.target` rather than the folder the skill came out of. Both shapes are pinned in `adopt/__tests__/apply.test.ts` and `cli/__tests__/harness-adopt.test.ts`, each asserting the filesystem AND the sentence AND a clean `--check`                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 18  | §7 gave the confirm one variant, and §8's note on which one to print reasoned about S15/S15b alone — so the page had no rule of its own. **Built:** S14/S14b are picked by the same fact the link is, read where the CLIENT can read it: `status.enabled` includes `claude-code`. Deviation 17 corrected the ENGINE's condition and stopped there; the dialog is chosen before any plan exists, so it cannot ask `move.link` and has to ask the manifest's enabled list instead — the same question, one surface earlier. §7 is also amended: the row repaints in place after a move and there is no "What changed" summary above the list. §8 freezes no adopt-summary sentence, and `HarnessSyncSummary`'s copy is a sync's ("Agent files updated.", "Removed N files DorkOS put here") — a receipt about writes and deletions, which is not what a move is. The chips ARE the summary, at the row the person pressed, and inventing a sentence for a banner would have broken the frozen-strings rule to say less. |
...
8. **A `--claude-only` affordance on the row.** It is the way out of an R7 refusal, and today the
   sentence the row draws tells the person to run the CLI — so the app can state the problem and not
   the fix. The route already accepts `claudeOnly`; what is missing is a second choice in the
   confirm, and the reason it is not here is that a dialog with two verbs is a different design
   question from the one this slice answers. The client Transport method therefore takes no
   `claudeOnly` parameter at all: a parameter no surface can set is a decision documented in the
   wrong place.
9. **`manifest.claudeOnlySkills[].reason` is required and never surfaced.** Every entry must carry
...
10. **The adopt read walks the source tree a second time.** `projectAgentWorkspace` runs `project()`,
    ...
    boot, on a pass that already yields to the event loop between workspaces.
