...
| **S7d**  | R7, DorkOS's own fields                               | `"{name}" uses {fields} in its settings, which are DorkOS's own and mean nothing to your other agents. Moving it also changes what DorkOS does with it: a skill with a schedule starts running on a timer once it is in .agents/skills. Take {fields} out and adopt it, or run dorkos harness adopt {name} --claude-only to keep it where it is.` |
