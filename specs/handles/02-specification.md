...
> **As shipped (DOR-677):** the capture is a beat in the first-run conversation, but whether it is asked is gated by a profile timestamp, `profile.identityPromptDismissedAt`, not by an `OnboardingStateSchema` step. One field records that the question was put and closed (saved or skipped) across all three surfaces that ask it — the conversation beat, the getting-started row, and a one-time sidebar card for installs that onboarded earlier — and each also checks what is still missing, so a person with a name and a handle is never asked. It still lives in `~/.dork/config.json` and is still not gated on auth.
