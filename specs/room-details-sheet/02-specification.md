...
> [!WARNING]
> **The code at this commit does not do this yet, and the difference is a bug being fixed in the next commit.**
>
> `DM_RUNGS` in `entities/room/lib/response-mode.ts` offers three, and `rungOf` collapses a stored `engaged` onto `@only` in a direct message. That was written from a premise this spec inherited from `main` — the TSDoc on the retired `responseModeOptionsFor` says the engaged window cannot open in a direct message "because nobody `@`s anyone in a two-person conversation" — and the premise is false:
>
...
>
> The table above is the specification. The three-rung shape is the defect.
...
```ts
rungsFor(roomKind: RoomKind): readonly ResponseRungOption[]        // { rung, label }
rungOf(mode: ResponseMode, roomKind: RoomKind): ResponseRung        // total — never null
modeForRung(rung: ResponseRung, roomKind: RoomKind): ResponseMode
explainRung(
  rung: ResponseRung,
  roomKind: RoomKind,
...
`rungOf` is **total**. Every stored value maps to a rung in both room kinds. A membership can hold a value the current room kind would not newly offer — set by a script, by an older build, or through the operator surface — and a control that renders blank for a value that is really there is a setting nobody can fix. The current implementation has this instinct (`responseModeOptionsFor` widens rather than narrows for a stored value) and it must survive the rewrite.
...
| **@only**      | `Answers only when you @mention it.`                                                                                       | channel: none · DM: `In a conversation this small, that means it mostly stays quiet.` |
...
- **`@only` says something extra in a DM.** _Shipped, and it goes with the defect in §2.1._ The extra note exists because `engaged` currently lands on this rung in a direct message; once a direct message offers its own `Engaged` rung, this rung stops being reachable by two routes and the note stops being true of a two-person conversation that is really a group of four. It is listed here as what ships, not as what should.
...
`explainRung` is **total over rungs the room does not offer**, which is a property worth keeping whatever the rung set is: a membership can hold a value set by a script or an older build, and a control that renders blank for a value that is really there is a setting nobody can fix. _Its current DM branch — answering for `mention` when asked about `engaged` — is part of the §2.1 defect and goes with it._
...
- Unit: the rung model round-trips all five stored values in both room kinds; **both kinds yield four rungs** (§2.1 — the suite at this commit asserts three for a DM and changes with the fix); no two rungs in one room kind share an explanation; optimistic rollback leaves the **true** value on screen; Undo restores the captured mode; the unresolvable agent draws a letter.
...
| --- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1 | A direct message offers **three** rungs, because it has three behaviours | **Four, in both kinds.** The premise was inherited from `main` and is false — `engagementFor` runs for every room kind, mentions are offered in a direct message, and a group DM is still `kind: 'dm'`. **The code at this commit still ships three; that is the defect, fixed next.** |
| 2.2 | `explainRung → { headline, note? }`                                      | `{ sentence, note: string \| null }`                                                                                                                                                                                                                                                   |
| 2.3 | One `Everything` sentence; DM reads "Answers you every time"             | Two sentences, naming the room they are in. `Engaged` has three answers, not one. `@only` says something extra in a DM (and that goes with the 2.1 defect).                                                                                                                            |
...
1. The direct-message rung set (§2.1). Fixed in the next commit.
2. The `@only` DM note and `explainRung`'s DM branch, which move with it.
