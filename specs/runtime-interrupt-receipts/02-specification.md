...
- **The pump (DOR-1302).** A `query.close()` on the persistent path is settled by the
  ...
  nobody hands it one. **Closed 2026-09-03 without the receipt vocabulary** — somebody
  hands it one now: `persistent-dispatch.ts` reads the existing per-query `stoppedQueries`
  record on the crash seam and stamps `PumpCrash.stopRequested`, which the windower turns
  into an `'interrupted'` terminal and the crash budget skips. A Stop in the STARTING phase
  needed a second seam: it kills the process before `system/init`, so the dispatch is refused
  rather than streamed and the settled window reaches nobody — `explainRefusedDispatch`
  answers that refusal with the same interrupted settle. See the table entry below.
...
| **DOR-1302** — the pump settles `crashed` on the unacked shape           | **Was: subsumed. Shipped ahead of this spec instead — see the annotation below.** The plan was that the windower be handed the WHOLE receipt (not merely a "was closed" flag), recorded against the query on the record `stoppedQueries` already keeps. `session-turn-windows.ts` would consult it where an escalated `close()` reaching `onCrash` synthesises the `error_during_execution` result, and yield a stopped terminal — `terminal_reason: 'interrupted'` — instead. The session then settles `interrupted` on the pump exactly as on the resume path. Conformance case **I3** is the check.<br><br>**Shipped 2026-09-03 (DOR-1302), without waiting for the receipt vocabulary.** The mechanism is the one described, one step short of a whole receipt: `persistent-dispatch.ts` — the only place holding both the session's stop record and the query that died — answers `stopWasAimedAt` on the crash seam and stamps `PumpCrash.stopRequested`. The windower then branches at `onCrash`'s CALL SITE, choosing a new sibling `stoppedResult()` (`terminal_reason: 'interrupted'`, DorkOS's own reason for a close the CLI never got to name) over `crashResult()`, rather than conditioning inside `crashResult` as this row's sketch put it — one synthesiser per ending reads better than one that has to ask which ending it is. The mapper's existing DOR-1320 gate then drops the error frame and the projector settles `interrupted`. The SECOND half of the bug, found in review and not named in this table, shipped with it: a stop-caused close no longer spends a life of `SessionCrashRecovery`'s bound, which is what made the third message after two Stops refuse with "This chat's agent keeps stopping". The STARTING phase needed a second seam — `explainRefusedDispatch`, for the launch a close kills before `system/init` — which the receipt design would also have had to reach; it is named here so a later implementation does not rediscover it. A full receipt would still say WHICH stop this was; nothing here forecloses that. |
...
- **AC-7** — a Stop on the persistent path settles `turn_end{terminalReason:'interrupted'}`
  ...
  **Satisfied ahead of this spec, 2026-09-03**: the acked half shipped with DOR-1320 and the
  synthesised half with DOR-1302, both pinned through the real pump in
  `sessions/__tests__/persistent-dispatch.test.ts`. What the receipt would still add here is
  WHICH ending it was, not whether the turn settles honestly.
