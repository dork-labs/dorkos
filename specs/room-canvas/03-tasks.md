# Task breakdown — Canvas and Browser in rooms

**Spec:** [`specs/room-canvas/02-specification.md`](02-specification.md)
**Slug:** `room-canvas`
**Umbrella:** DOR-1995 · Linear project "Canvas and Browser in Rooms"
**Mode:** full · **Generated:** 2026-09-11

Seven tasks, one per phase of the spec's Implementation Phases section. Each task is one PR, each is
sized `xl`, and each is therefore promoted to its own tracker sub-issue under DOR-1995. Every
description is self-contained: it carries the phase's own create/modify list and acceptance criteria
plus every design, data-model, experience, testing, security and documentation passage that phase
implements, quoted in full, so an implementer can build from the task with the specification closed.
...
| Task  | Phase | Title                                        | Size | Priority | Depends on | Parallel with |
| ----- | ----- | -------------------------------------------- | ---- | -------- | ---------- | ------------- |
| `1.1` | P0a   | One generated catalog + notify-and-reconcile | xl   | high     | —          | 1.2, 1.3      |
| `1.2` | P0b   | `dorkos-ui` fences render in room bodies     | xl   | medium   | —          | 1.1, 1.3      |
| `1.3` | P1    | The Browser tab on `/session`                | xl   | high     | —          | 1.1, 1.2      |
| `2.1` | P2a   | Server: the table, the service, the routing  | xl   | high     | 1.1        | 1.2, 1.3      |
| `2.2` | P2b   | Client: the room canvas                      | xl   | high     | 1.3, 2.1   | —             |
| `2.3` | P2c   | Docs                                         | xl   | medium   | 2.2        | 3.1           |
| `3.1` | P3    | Pins, tab presence, the `#team` board        | xl   | medium   | 2.2        | 2.3           |

## Critical path

`1.1 (P0a) → 2.1 (P2a) → 2.2 (P2b) → 2.3 (P2c)` — four PRs deep, and `2.1` is by far the largest of
them: it owns the whole `canvas_documents` table (deliberately one PR, so the migration number is
minted once), the `RoomCanvasService` and its per-turn ledger, both routing callers and the stamp
that keeps them from doubling, the room capability, the context section and the config field. Start
`1.1` first; `1.2` and `1.3` can run beside it from minute one, and `1.3` must land before `2.2`
because the room client reuses the two-views split `1.3` builds. `2.3` and `3.1` are the only pair
that can run concurrently at the tail.

---

### Task 1.1: One generated catalog, `apply_layout` described, notify-and-reconcile

**Phase:** P0a — One generated catalog, `apply_layout` described, notify-and-reconcile
**Scope:** one PR. Independent of every other phase; unblocks task 2.1 (P2a).
...
#### Why this phase exists — the drift it fixes (Background / Problem Statement)
...
The fourth of those drifts — the `dorkos-ui` fence in room bodies — is task 1.2's; the first three
are this task's. Goal 7 of the spec states the deadline:

7. The four drifts above are fixed before rooms inherit them.

#### Detailed Design — the sections this phase implements
...
This task implements the fourth bullet of §10 on the session surface: the client records a held
update instead of dropping it, renders the banner, and the `ui-tools.ts` result says "held". The
server-side edit lock those refusals later ride in a room is task 2.1's; the rule it enforces is the
one bullet of §3.5 that names this behaviour:
...
#### Testing Strategy — the drift test
...
#### Testing Strategy — the client test this phase owns
...
- `apps/server/src/services/runtimes/shared/__tests__/ui-tool-contract.test.ts` exists and passes,
  asserting exactly 14 content types and 22 actions in both the rendered `control_ui` description and
  the rendered `<ui_tools>` block; it is red against the pre-change code (6 / 10 / 14).
- A generator test proves a 15th schema variant with no prose entry fails the build.
- An RTL test for the new banner: an agent update arriving while a person edits produces the banner
  with Reload and Keep mine, and the store records the held update rather than dropping it.
- A server test asserts the `ui-tools.ts` result for a held update says the update was held and does
  not claim success.
- `pnpm vitest run apps/server/src/services/runtimes/shared` and the client canvas store tests pass;
  `pnpm --filter @dorkos/server typecheck`, `pnpm --filter @dorkos/client typecheck` and both lint
  runs are clean.
- Browser proof: the notify-and-reconcile banner shown in a real browser (dev app), screenshotted in
  the PR — jsdom cannot prove its focus and layering behaviour.
- One changelog fragment in `changelog/unreleased/` with a `covers:` block and only the seven allowed
  headings, prettier-formatted before commit.
- Adversarial review per `REVIEW.md` before the PR.

---

### Task 1.2: Render `dorkos-ui` fences in room message bodies

**Phase:** P0b — `dorkos-ui` fences render in room bodies
**Scope:** one PR. Independent of every other phase; blocks nothing.
...
#### Detailed Design §12.4 — `dorkos-ui` fences in room bodies (P0b)
...
#### Documentation — the page that writes this behaviour up (the edit itself lands with task 2.3)
...
- An RTL test covering all four acceptance clauses: a `dorkos-ui` fence in a room message renders as
  a widget; an `agent`-channel control renders disabled with the existing tooltip; `ui` and `url`
  actions fire; a malformed fence falls back to a code block and the rest of the message still
  renders.
- The test is red against the pre-change `render-room-body.tsx`, which renders a code block today.
- `pnpm vitest run apps/client/src/layers/widgets/room-view` passes;
  `pnpm --filter @dorkos/client typecheck` and lint are clean.
- Browser proof: a widget fence posted into a real room, screenshotted in the dev app, showing the
  live `ui`/`url` controls and the disabled `agent` control with its tooltip.
- One changelog fragment in `changelog/unreleased/` with a `covers:` block and only the seven allowed
  headings, prettier-formatted before commit.
- Adversarial review per `REVIEW.md` before the PR.
...
### Task 1.3: The Browser tab on `/session` — two views over one document store

**Phase:** P1 — The Browser tab on `/session` (D4)
**Scope:** one PR. Independent of P0a and P0b; blocks task 2.2 (P2b).
...
#### Detailed Design §9.1 — The Browser tab: two views over one store (D4)
...
#### Open Question Q3, resolved — why `mcp_app` stays in Canvas
...
#### Data model changes — item 6, `Transport`
...
This phase adds the **workbench-serve capability flag** to that surface — a new boolean on
`Transport`, `true` on `HttpTransport` and `false` on `DirectTransport`, declared beside
`supportsTerminal`, and present in `createMockTransport`. The six room canvas methods themselves land
with task 2.2.
...
#### Detailed Design §11 — the Obsidian clause this phase implements
...
#### Security Considerations — item 4
...
#### Documentation — `docs/guides/workbench.mdx`
...
- RTL tests for the client bullets above: the two-view split across all 14 content types (`mcp_app`
  in Canvas), per-view active document across a tab switch, and the Browser tab hidden under a
  transport reporting no workbench-serve support.
- A dispatcher test for the reveal table: `browser_navigate` reveals Browser; `open_canvas` with
  `url`/`browser` content reveals Browser; `open_file` of a markdown file reveals Canvas.
- `tests/pulse/right-panel-tab-strip.spec.ts` re-measured at both window widths with its premise
  sentences rewritten — not a count bump — and `apps/e2e/pages/RightPanelPage.ts`,
  `apps/e2e/pages/canvas-dev-server.ts`, `tests/workbench/dev-server-preview.spec.ts`,
  `tests/responsive/touch-reach.spec.ts` and `tests/production/shipped-shell.spec.ts` updated and
  passing.
- Grep `apps/e2e` for the old tab-strip copy before pushing: browser specs assert literal strings and
  `browser-test` is a pass-through on PRs, so a missed string goes red only in the merge queue.
- `pnpm vitest run apps/client/src/layers/shared` passes; `pnpm --filter @dorkos/client typecheck`,
  `pnpm --filter @dorkos/shared typecheck` and lint are clean; the capability flag is present in
  `HttpTransport`, `DirectTransport` and `createMockTransport`.
- Browser proof: seven tabs on `/session` in a real browser, a screenshot of the Browser tab framing a
  dev-server preview, and a screenshot of the Canvas tab holding a markdown document at the same time.
- `docs/guides/workbench.mdx` updated in the same PR, and one changelog fragment with a `covers:`
  block, prettier-formatted before commit.
- Adversarial review per `REVIEW.md` before the PR.

---

### Task 2.1: Server — the table, the service, the routing

**Phase:** P2a — Server: the table, the service, the routing (depends on P0a)
**Scope:** one PR, deliberately: it owns the whole `canvas_documents` table rather than a stack that
mints two migration numbers. Depends on task 1.1 (P0a), because the room teaching is composed from
P0a's generated catalog. Blocks task 2.2 (P2b).
...
#### Detailed Design — every section this phase implements, in full
...
Item 6's workbench-serve capability flag lands with task 1.3 and the six room canvas methods with
task 2.2; this phase's share of the `Transport` surface is only what its routes need.
...
#### Testing Strategy — runtime conformance
...
#### Documentation — what this phase owns
...
- Every unit test listed above exists and passes: the four `packages/shared` schema tests, the two
  `packages/db` migration tests (fresh database and one seeded at `0095`, every index present in the
  generated SQL, cascade on delete and no cascade on archive), the whole
  `services/rooms/canvas/__tests__/` set, the routing-seam set, and the six route tests with their
  refusal codes, statuses and OpenAPI entries.
- The normalizer round-trip test exists: a stamped `ui_command` pushed onto `session.eventQueue`
  reaches `collectReply` with `applied` intact, through the real
  `session-event-normalizer.ts`. Without this edit every claude-code operation applies twice.
- The end-to-end test exists for `json` content (no source key, so no dedupe to mask a double write):
  one row, one `canvas` frame, one ledger entry, one named operation in the coalesced entry.
- The ledger test exists: an operation whose stamped event is delivered after the collector settles is
  still named in the entry and still has exactly one row.
- The per-turn ceiling is asserted twice, once per path — on the tool result the model reads, and on
  the applied count of a scripted four-operation test-mode turn.
- The new `runtimeConformance` case passes on claude-code and on the deterministic test-mode runtime.
- `tool-exposure.test.ts`'s two count guards are updated in this PR, and
  `pnpm vitest run apps/server/src/services/runtimes` is run before the PR is opened — a new verb has
  reddened those guards before and the failure reads like an unrelated break if you have not been told.
- A `config-manager.test.ts` migration test that boots a real `ConfigManager` and reads the file back
  off disk; `merged-migration-hashes.ts` pinned in the same PR; both default sites edited (the
  per-field default and the object literal), never one.
- The migration is generated with `drizzle-kit generate`, never hand-written, and the generated SQL is
  read to confirm every index survived; if another branch landed an `0096` first, this one is
  renumbered rather than theirs being touched.
- Both OpenAPI steps run in the same commit: `pnpm docs:export-api` and
  `pnpm --filter=@dorkos/site generate:api-docs`.
- `pnpm --filter @dorkos/server typecheck`, `pnpm --filter @dorkos/shared typecheck`,
  `pnpm --filter @dorkos/db typecheck` and lint are clean.
- Browser proof: with the dev app running, drive a test-mode room turn that opens a document, and show
  `GET /api/rooms/:id/canvas` returning it plus the single coalesced line in the room's log — the room
  tabs land in task 2.2, so the proof here is the API response and the room log, screenshotted.
- One changelog fragment with a `covers:` block, prettier-formatted before commit, plus the
  `contributing/architecture.md` and Settings Reference edits in the same PR.
- Adversarial review per `REVIEW.md` before the PR.

---

### Task 2.2: Client — the room canvas

**Phase:** P2b — Client: the room canvas (depends on P1 and P2a)
**Scope:** one PR. Depends on task 1.3 (the two-views split it reuses) and task 2.1 (the server
table, routes and stream frames it renders). Blocks tasks 2.3 and 3.1.
...
#### The server contract this client consumes
...
- RTL tests for every client bullet above: hydration from `snapshot.canvas`; `opened`/`updated`/
  `closed` frames applied; a lower-`rev` frame dropped; a resync replacing the whole set; a document
  another member opened not moving this viewer's active tab while one this member opened does; an
  editing viewer never moved; both views keeping their own active document.
- `apps/e2e/tests/rooms/room-canvas.spec.ts` exists and passes all seven steps on the deterministic
  test-mode runtime with no model spend, including step 5's two-context assertion and step 6's reload.
- `apps/e2e/pages/RoomsPage.ts` carries the canvas and browser tab selectors, the document tab strip
  and the unread dot, so the spec asserts through a page object rather than raw selectors.
- The edit-lock heartbeat posts every 15 s only while an editor is focused with unsaved changes, and
  clears on save, close and blur-with-no-changes — asserted, not assumed.
- File-backed documents (anything with `sourcePath`) open in the CodeMirror source editor, never
  Blintz — pinned by a test, because a ProseMirror round-trip rewrites the file and misattributes it.
- Grep `apps/e2e` for any tab-strip copy this PR changes before pushing; browser specs assert literal
  strings and `browser-test` is a pass-through on PRs.
- `pnpm vitest run apps/client/src/layers` passes; `pnpm --filter @dorkos/client typecheck` and lint
  are clean.
- Browser proof, and it is the feature: two browser contexts on one room, live, showing the same
  document appear in both without a reload, a reload restoring the whole table, and a close in one
  context removing it in the other — screenshots or a short clip in the PR, plus the phone-width sheet
  rendering both tabs.
- One changelog fragment with a `covers:` block, prettier-formatted before commit.
- Adversarial review per `REVIEW.md` before the PR.

---

### Task 2.3: Document the room canvas and the Browser tab

**Phase:** P2c — Docs (depends on P2b)
**Scope:** one PR, docs-only (`skip-changelog` does not apply — this phase ships its own fragment;
`review:light` is the usual label for a docs-only PR). Depends on task 2.2, so every page describes
shipped behaviour rather than planned behaviour.
...
#### Documentation — every page, in full
...
All user-facing prose follows the `writing-for-humans` skill: plain enough for a smart 9th grader who
does not code. Describe what happens for the person, not how the system works inside. The demo-claim
gate applies without exception: the Obsidian plugin is a staged, under-tested surface, so no page may
claim the Browser tab works there, and no page may claim a surface or pillar works that has not been
verified.

#### Definition of done

- All four docs pages edited, plus the Settings Reference table, `contributing/architecture.md` and
  the `chat:rooms-test` skill's canvas leg.
- `docs/concepts/rooms.mdx` gains "The room's canvas" beside "Files a room owns", with the five
  subsections named in the Documentation section, and its two new sentences under "What a room can
  never do to an agent"; the REST table under "Reference (for developers)" lists all six routes.
- The new rooms section is reachable from the docs sidebar (verified in a browser, not inferred from
  the file tree).
- `docs/guides/generative-ui.mdx` says 14 content types, matching the generated catalog, and names
  which two the Browser tab shows.
- No page claims the Obsidian shell has a Browser tab; the Obsidian canvas is described by what it
  renders.
- `pnpm build --filter=@dorkos/site` (the `site-build` check) passes locally before the PR; a page
  that type-checks can still fail the site build.
- Browser proof: each edited page rendered in the local site build, screenshotted — including the
  rooms page's new section reached from the sidebar.
- One changelog fragment with a `covers:` block, prettier-formatted before commit.
- Adversarial review per `REVIEW.md` before the PR.

---

### Task 3.1: Pins, tab presence, and the `#team` board

**Phase:** P3 — Pins, tab presence, the `#team` board (depends on P2b)
**Scope:** one PR. Depends on task 2.2. Can run beside task 2.3.
...
#### Detailed Design §9.4 — Tabs, authors, pins
...
#### Detailed Design §3.3 — Capacity, and why a pin is never evicted
...
#### Detailed Design §9.3 — What opens, and what never steals focus
...
#### Why presence rides `signal` and never the `canvas` frame

The `canvas` frame is durable state: whole-document, no `seq`, replayed as a whole set on resume
(§2). Tab presence is the opposite — it is ephemeral, it is nobody's history, and replaying it after a
reconnect would show a face on a document the person left ten minutes ago. It therefore rides the
existing `signal` frame, which carries no `seq` by design, and it is never replayed. An agent's face
on a document while a `read_canvas` claim is held is exempt from the etiquette rule against
narration, as a mechanical presence signal.

The references this phase leans on:
...
- A pin survives a reload and is never evicted — asserted against a room already at the twelve-document
  LRU ceiling, so the test proves the carve-out rather than the happy path.
- Pinned-first ordering asserted in the tab strip, and the pin action available to every member from
  the tab's own menu.
- Presence is ephemeral: a test asserts a tab-presence `signal` frame is never replayed after a
  reconnect, and that it carries no `seq`.
- `ROOM.md` is pinned by default in a room with a repo, and a room without one is unaffected.
- The `#team` board introduces **no new schema**: a test or a review note pins that it is documentation
  plus a seeded example widget over the existing primitives.
- `pnpm vitest run apps/client/src/layers` and the rooms server tests pass;
  `pnpm --filter @dorkos/client typecheck`, `pnpm --filter @dorkos/server typecheck` and lint clean.
- Browser proof: a pinned document surviving a reload at the LRU ceiling, two viewers showing each
  other's tab presence, and the `#team` board on Home — screenshots in the PR.
- One changelog fragment with a `covers:` block, prettier-formatted before commit.
- Adversarial review per `REVIEW.md` before the PR.
