...
- Single writer: a claimed session's continue calls advisor.move and starts no core session; its wait calls advisor.wait and arms no core timer; the auto timer never fires for it; claims/move/wait failure → 503 with nothing changed; markContinued sets continued. S5 needs: advised flag; cross-runtime continue only when offered; a person's move of a healthy session; carryOver:false kept on waiting.
...
0. Claimed sessions (task 5.1 step 4b) get NO resume timer and no auto-resume: flow resumes them. The boot scan skips rows whose session the advisor claims.
...
2. Keep every gate: default-off; the person's one-time `extensions.approvedToRun` approval (extension-load-policy.ts) before any server or client code runs; the install permission disclosure lists the carried extension; a local record never takes over a core or approved id; a duplicate id across plugins resolves by sorted plugin name with one warning.
