...
2. BIND APPROVAL TO SOURCE (security: approval is by id today, `approvedToRun.includes(id)`, and agents can install plugins through the marketplace tools): an approval records the extension's source (direct path, or plugin name + carried path, plus a content hash); an approved id that resolves to a different source needs a fresh approval before any code runs; migrate existing id-only approvals to bind to the source they resolve to today. Precedence: core, then direct `<dorkHome>/extensions/<id>`, then an already-approved source, then a new plugin-carried one. Keep every other gate: default-off; the one-time approval before any server or client code runs; the install permission disclosure lists the carried extension; a local record never takes over a core or approved id; a duplicate id across plugins resolves by sorted plugin name with one warning.
