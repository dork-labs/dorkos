...
- **A**: 1.1, 1.2, 2.5, 3.3, 6.1. No dependencies; different files.
...
7. `GET /api/runtimes/:runtime/accounts/usage` → `{ accounts }` for any runtime slug (400 unknown runtime) in routes/runtimes.ts + OpenAPI registration.
...
1. `packages/extension-api/src/server-extension-api.ts` `DataProviderContext` gains, with TSDoc:
   ...
   - `readonly accounts: { list(): Promise<AccountSummary[]>; usage(runtime?: string): Promise<AccountUsage[]>; onUsage(listener: (usage: AccountUsage) => void): () => void; markContinued(sourceSessionId: string, to: { sessionId: string; runtime: string; accountId: string }): Promise<void>; registerAdvisor(advisor: AccountAdvisor): () => void }`, `AccountSummary = { runtime; id; label; color; implicit }` over EVERY runtime: registered rows plus each runtime's implicit `default` (implicit true, color by position). `markContinued` sets the source session's plan to `continued` (task 5.1's store) when flow moved a claimed session itself.
   - The advisor types exactly as spec §6 X3: `AccountAdvisor { rank(candidates, ctx); onLimited?(info); modelFallback?(info) → { model } | null; carryOver?(info, targetAccountId); claims?(info: SessionInfo) → boolean; move?(info, target: { runtime; accountId }) → { sessionId }; wait?(info, resumeAt, autoResume) }`, `SessionInfo = { sessionId; cwd; runtime; accountId; trackerItem? }`, ranking rows gain optional `runtime` (another runtime = cross-runtime fallback), `LimitedPlan = { mode: 'auto'; target; delaySeconds } | { mode: 'wait'; resumeAt? } | { mode: 'ask' }`, `LimitedSessionInfo = { sessionId; cwd; accountId; window; resetsAt; scope: 'account' | 'model'; model; trackerItem? }`, `AccountCandidate`, `AdvisorContext { purpose: 'launch' | 'continue'; caller: 'person' | 'agent' | 'relay' | 'advisor'; cwd; runtime; sessionId?; excludeAccountId? }`, `AdvisorRanking { accounts: { id; eligible; reason; badge?: 'recommended' | 'reserved' }[]; recommendedId }`, `LimitedSessionInfo`, `LimitedPlan`, `CarryOverSeed { seedContext; prompt? }`. AccountUsage from `@dorkos/shared/account-usage`.
...
4. `services/extensions/extension-server-api-factory.ts` builds `dorkHome` and `accounts` (list via `readClaudeAccountSettings`, usage/onUsage via the D2 store, registerAdvisor keyed by extension id). `extension-server-lifecycle.ts` removes that extension's listeners and advisor on shutdown and reload.
...
1. New `apps/server/src/services/session/fleet/session-fleet-overlay.ts#applySessionFleetOverlay(page, { store, readAccounts, projectorFor, applyTrackerItems })`: `accountId`: claude-code where `session.account` matches a registered account (`path.resolve` equality), else `default` when the registry is empty; codex and opencode sessions `default`; `status: { lifecycle, limit }` from `projectorFor(session.id)?.status` when present, absent otherwise; `trackerItem` via task 2.4's `applyTrackerItems(page)`; returns `accountUsage = store.peek(distinct accountIds)` or undefined when none.
...
4. Auto fire: at `fireAt`, re-rank (caller 'advisor', purpose 'continue'); target still eligible → `continueOnAccount(…, 'advisor')`; else plan → `ask` and raise `account.limited` again with detail "could not move it automatically" (new dedupe suffix). At most one automatic carry-over per limit episode. Automatic carry-overs count against `AGENT_LAUNCH_MAX_LIVE` (the in-memory live set from task 3.2 in launch-session.ts; if 3.2 has not landed, add the set there); a fire that finds it full drops to `ask` and repeats the notification. Automatic carry-overs pass `unattendedApprovals: true` (and `unattended: true` to ensureSession) like a timer-fired schedule, so an approval card never waits on nobody; a person's continue does not.
   4b. SINGLE WRITER (spec X "One writer for a flow run"): ask `callAdvisor('claims', info)` ONCE when the limit is set (and when a person moves a healthy session), and again once for every unclaimed row still in ask or waiting when an advisor registers, and PERSIST it as `claimed_by` (advisor extension id) on the session_limits row (column in task 2.3's table; add it here if 2.3 merged). Every later continue, wait, cancel or auto fire reads the stored claim; a claimed row with no advisor present gets no core timer and a person's action answers 503. Claimed: `/continue` calls `advisor.move(info, target)` and records the returned id as plan `continued` (core starts nothing); `/wait` records `waiting` and calls `advisor.wait(info, resumeAt, autoResume)` with NO core timer; core's auto timer never fires for it (flow acts, then calls `accounts.markContinued`). `claims`/`move`/`wait` throwing or timing out → 503 "Flow could not be reached, so this was not changed."; only a row never claimed → the core path below.
   4c. Cross-runtime power: map the source mode to its trust stop via the SOURCE runtime's profile (unmapped → 'ask'), cap at 'act' (never autonomy/bypass across runtimes), pick the TARGET profile's mode at that stop (none → the target's most restrictive); same launch_origin allowlist and 'account-handoff' origin; one test per runtime pair. A person's move of a healthy session writes a synthetic session_limits row (scope 'account', window 'manual', plan continued) so the source shows moved and a repeat after restart returns the same id.
   4d. S5 needs: `continue-options` returns `advised: boolean` (whether an advisor ranking was used); `/continue` accepts `runtime` (default the session's) only when the advisor's ranking offered an account of that runtime, starting the target runtime with its own model/effort defaults and the power rule of step 4c, with a runtime-neutral default summary; states treat advisor-offered other-runtime accounts as "other accounts" (wait-only/all-accounts-out rules in spec D9); a person's `/continue` is allowed WITHOUT a limit for eligible launch origins (Activity reason "moved by a person"; in-flight marker keyed by the session alone); the `waiting` plan keeps `carryOver: false` from the `ask` it replaced and carries `unconfirmed?: true` in its type.
...
- Single writer: the claim is stored at limit time; with the advisor absent (boot, extension reload) a claimed row gets no core timer or action (503 to a person); a claimed session's continue calls advisor.move and starts no core session; its wait calls advisor.wait and arms no core timer; the auto timer never fires for it; claims/move/wait failure → 503 with nothing changed; markContinued sets continued. S5 needs: advised flag; cross-runtime continue only when offered; a person's move of a healthy session; carryOver:false kept on waiting.
...
0. Claimed rows (session_limits.claimed_by set, task 5.1 step 4b) get NO resume timer and no auto-resume, whether or not the advisor is currently registered: flow resumes them. The boot scan skips them by the stored column, never by asking live.
1. `apps/server/src/services/session/fleet/resume-service.ts`: for each session whose plan is `waiting` and not claimed, a timer at `resumeAt` clamped to >= the limit's resetsAt and >= now + 60 s (null resumeAt → no timer; state stays waiting-reset). Cleared by continue, cancel, a new turn_start, and shutdown. On boot, scan `session_limits` (task 2.3's store, `listWaiting()`) and re-arm every waiting row (past → check now); `auto` rows become `ask`.
...
2. Write-through: when a session's status carries context figures (contextTokens and contextMaxTokens, typically the terminal status of a turn), upsert them with observedAt now.
   2b. The field is the existing `SessionStatus.contextUsage`; `SessionContextUsageSchema` gains optional `observedAt` (set from the row, or now for a live reading).
...
### Task 6.1: Discover extensions that ship inside an installed marketplace plugin

- Size medium, priority high, tracker DOR-2383
- Depends on: nothing
- Parallel with: 1.1, 1.2, 2.5, 3.3

Tracker: DOR-2383 (the Flow extension carries the advisor) and DOR-2387. Spec X4; ADR 260926-153107 (drafted in the UI spec PR #2149). CRITICAL PATH for the Flow tab and the advisor.

Confirmed gap: `apps/server/src/services/extensions/extension-discovery.ts` scans only `<dorkHome>/extensions/` (global) and `<cwd>/.dork/extensions/` (local). A marketplace plugin carrying `.dork/extensions/<id>/` is compiled at install (`marketplace/flows/install-plugin.ts`, `lib/staged-extensions`) and `enable()` is called, but discovery never finds the record, so it never runs.

1. Discovery also scans `<dorkHome>/plugins/*/.dork/extensions/*` (origin global) and `<cwd>/.dork/plugins/*/.dork/extensions/*` (origin local). Confirm the plugin install roots against the marketplace installer (use the path it actually installs to; if it differs from the ADR, follow the installer and note it in the ADR).
2. BIND APPROVAL TO SOURCE (security: approval is by id today, `approvedToRun.includes(id)`, and agents can install plugins through the marketplace tools): an approval records the extension's source IDENTITY (direct path, or plugin name + carried path), NOT a content hash (edits and updates of an approved source must keep loading silently, DOR-504/506); an approved id that resolves to a different source needs a fresh approval before any code runs. Store it as a new operator-only `extensions.approvedSources` map (id → source) beside the unchanged `approvedToRun` list, with a semver-keyed config migration (next free key; adding-config-fields skill, merged-migration-hashes pin) that records each existing approval's current source. Precedence: core, then direct `<dorkHome>/extensions/<id>`, then an already-approved source, then a new plugin-carried one. Keep every other gate: default-off; the one-time approval before any server or client code runs; the install permission disclosure lists the carried extension; a local record never takes over a core or approved id; a duplicate id across plugins resolves by sorted plugin name with one warning.
3. Uninstalling the plugin disables its extensions and removes their `approvedToRun` entries; reinstalling asks again.
4. Record the source plugin on the extension record (for the Extensions UI and logs).

Tests: a second plugin carrying an already-approved id does NOT run until approved again; a direct extension beats a plugin-carried one of the same id; an existing id-only approval keeps working for its current source; a plugin-carried extension is discovered (global and local); it does not run before approval and runs after; a duplicate id resolves deterministically with a warning; a local plugin extension cannot shadow a core or approved id; uninstall disables it and clears approval; the install disclosure names it. User-facing: a changelog fragment ("Extensions that come inside a plugin now load, once you approve them").
