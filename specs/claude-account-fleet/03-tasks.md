...
0. Eligibility for carry-over: only sessions whose server-derived `Session.origin` is absent (a person) or `agent`. For `channel`, `room` with an outside author, connector-event and `task` origins: do NOT call onLimited; plan is `{ mode: 'ask', carryOver: false }`; `POST /continue` answers 409 "This conversation came in from <surface>, so it can only wait for the reset." (their text runs under that surface's grant; replaying it into a plain session escapes it). Add `carryOver?: false` to the `ask` member of LimitPlanSchema (task 1.2's schema; add it here if 1.2 has merged).
...
3. Routes in `routes/sessions.ts` (thin; OpenAPI-registered): `GET /api/sessions/:id/continue-options` → `{ plan, ranking }` (rankAccounts with caller 'person', purpose 'continue', excludeAccountId = the session's account; a person may pick any registered account, eligible or not); `POST /api/sessions/:id/continue` `{ account }` → 202 `{ sessionId }` (400 unknown/unregistered account or non-claude-code session; 409 while streaming; 409 when the session has no current `limit`; idempotent per limit episode via an in-flight marker keyed by (sessionId, limit.since) set SYNCHRONOUSLY before any await and shared with the auto timer: a second caller awaits the first promise and gets the same new session id); `POST /api/sessions/:id/wait` → plan `waiting`, cancels any timer; `POST /api/sessions/:id/continue/cancel` → `auto` becomes `ask`. Both answer 409 when the session has no current `limit`.
...
- Origins: one test per origin (person, agent carry over; channel, outside-author room, connector-event, task get ask/carryOver:false, onLimited not called, 409 on continue). No limit → 409 on continue, wait and cancel. A person call racing the auto timer, and two concurrent person calls, start exactly one session. Auto fire with the cap full → ask + notification. Auto carry-over runs unattended.
