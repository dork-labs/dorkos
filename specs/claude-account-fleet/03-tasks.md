...
RUNTIME-NEUTRAL (spec §6 R, contract revision 6 with fixtures 2.0.0): the ledger schema carries `runtime` (claude-code | codex | opencode), window entries gain optional `windowMinutes`, and the ledger gains optional `plan`, `credits { hasCredits, unlimited, balance }` and `spend { periodStart, costUsd, limitUsd?, observedAt, source }`; sources include the contract's Codex and error sources. Adopt the contract's exact names and version field; if the branch was built on the earlier (revision 5) shape, convert it before the PR. `AccountUsageSchema` gains `runtime`, `plan`, `credits`, `spend`; `toAccountUsage` state: a spend-only account is `limited` when `spend.limitUsd` is reached, an account with neither windows nor spend is `ok`. Add `IMPLICIT_ACCOUNT_ID = 'default'`.
...
Tracker: DOR-2380 (every runtime records usage, spec §6 R). Codex has one implicit account, `default`.
...
2. Map to ledger observations: key by window_minutes (300 → `five_hour`, 10080 → `seven_day`, other → `window:<minutes>`), `windowMinutes` kept, `usedPct = used_percent` (clamped), `resetsAt` ISO, `status: 'rejected'` for the window `rate_limit_reached_type` names (else null, and `usedPct` non-null), source per the contract's Codex source name. `plan` and `credits` as ledger meta. Record via the D2 store as (codex, 'default').
3. Limit (spec D4 for every runtime): a `rate_limit_reached_type`, or a `turn.failed` whose error is a rate limit, sets the session `limit` (window from the reached type, resetsAt from that window) through the same status path task 2.3 added, so `account.limited` and D9 apply. Codex has no carry-over target, so D9 reaches wait-only.

Tests: a real-shaped rollout record maps to five_hour and seven_day with plan and credits; an unknown window length becomes window:<minutes>; a reached limit sets rejected and the session limit; a record without rate_limits changes nothing; the existing context-usage behavior is unchanged.
...
- `account?: z.string().min(1)` — accepted only when the resolved runtime has `supportsAccounts` (task 2.8), or is the literal `default`; else 400. Registry id; unknown → error "No Claude account with id <x> is registered." and nothing starts (invariant 6); with a resolved runtime other than 'claude-code' → error;
...
The fixture folder `plugins/flow/conformance/fleet/` is merged on marketplace `main` (PR #57; contract revision 6 moves it to CONTRACT_VERSION 1.1.0: vendor the newest merged commit, and if revision 6 has not landed yet, wait for it rather than vendor 1.0.0). Vendor it from the current `origin/main` commit.
