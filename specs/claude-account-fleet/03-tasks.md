...
Tracker: DOR-2380 (with the DOR-2379 color helpers). Spec §5.1, §5.2, §11 Shared row. The shared contract is the marketplace spec `specs/flow-cli-core/02-specification.md` §1 (revision 4; branch `spec/flow-cli-core` until merged, a copy at `/private/tmp/claude-501/-Users-doriancollier-Keep-dork-os-dorkos/6843b882-e9ab-4de2-94de-492c4ebdda5e/scratchpad/fleet/CONTRACTS.md`). Read it first; it wins over anything restated here, and a difference is fixed in specs/claude-account-fleet/02-specification.md in the same PR.
...
1. Rows keep unknown fields: make `ClaudeCodeAccountSchema` a `z.looseObject` (applyConfigPatch re-parses the whole config, and a plain object strips per-row fields the contract says writers preserve). Client: `toWritableAccounts` in `apps/client/src/layers/features/settings/ui/runtimes/sections/ClaudeAccountsSection.tsx` rebuilds rows as {id,path,label}, which would wipe every stored color on any add/remove; make it carry each row's stored fields through, sending `color: null` for a row whose `colorIsDefault` is true. No visual change. Test the round trip (add an account, the others keep their colors).
2. Read rules (contract 1.1a) in `readClaudeAccountSettings`: skip a row whose path is missing or not absolute, keep the first of two rows sharing an id, read a bad color as null, each with one logged warning; the write path keeps its duplicate-id refusal.
...
6. Relay path: schedules can also run through `services/tasks/relay-dispatch.ts`, whose `TaskDispatchPayload` carries only model and effort. Add `account?: string` to `TaskDispatchPayloadSchema` (`packages/shared/src/relay-envelope-schemas.ts`), set it in relay-dispatch.ts, and have `packages/relay/src/adapters/claude-code/task-handler.ts` pass it as `accountHint` to sendMessage. No guard on either path: a schedule's account is the operator's approved choice.
