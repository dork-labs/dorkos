...
- **Rows keep what they do not know.** `ClaudeCodeAccountSchema` becomes a `z.looseObject`, because `applyConfigPatch` re-parses the whole config and a plain object strips any per-row field it does not declare (contract §1.1a: writers preserve unknown fields). The client's `toWritableAccounts` (`ClaudeAccountsSection.tsx`) today rebuilds rows as `{ id, path, label }`, so adding or removing any account would wipe every stored `color`: it must carry each row's stored fields through, sending `color: null` for a row on its default (`colorIsDefault`). A round-trip test pins it.
