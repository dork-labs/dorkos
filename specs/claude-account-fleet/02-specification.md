...
**Which sessions can be carried over.** Only a conversation a person or an agent on this machine started: `Session.origin` absent (person) or `agent`, as the server derives it. A conversation that came in through a room with an outside author, a chat binding (`channel`), a connector event or a schedule (`task`) runs under that surface's own grant, and replaying its text into a plain session would run a stranger's words outside that grant. For those, `onLimited` is not asked, the plan is `ask` with `carryOver: false`, and `POST …/continue` answers 409 ("This conversation came in from <surface>, so it can only wait for the reset."). `continue-options` still returns the ranking for display.
