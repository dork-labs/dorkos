...
> **What came out of this.** A six-document review exchange over this research split it into four items: **A** (the room model), **A′-policy** (thread write intent), **A′-mechanism** (resource coordination), **B** (the community server). A and A′-policy are specified in `specs/rooms/` and decided in ADRs 260726-170125, 260726-170126 and 260726-170127. A′-mechanism was sized by the DOR-500 contention measurement (`research/20260725_q3-contention-preregistration.md`) — tree-sharing is the collision, so the lock is keyed on the working tree, not on any conversation shape. **B is not decided.** Where this document and those ADRs disagree, the ADRs win: three claims asserted confidently here were falsified by reading source during the exchange, and the corrections are marked inline.
