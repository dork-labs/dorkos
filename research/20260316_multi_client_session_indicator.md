...
Exception: if the session is _locked_ (another client is actively writing), an inline "locked" state on the input area is appropriate — this is a blocking condition the user must be aware of. This is separate from the presence count.
