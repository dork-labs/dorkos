...
| 5   | **Agents shows recent + pinned, then "All N agents →" (Team)**                                                             | Slack doesn't list every person; an agent is also a project, so a cold project shouldn't cost a row forever. Also fixes the dead "N inactive" row (DOR-1105).                                                                                                         | Small–medium                          |
...
| 7   | **Make the shipped controls honest**                                                                                       | Group "Show ▸" filter never applied; "Mute" on a smart group is a no-op; 4 rollup rows look pressable and do nothing; "Chat with X" / "Message" open a _session_.                                                                                                     | Small each                            |
...
4. **Four roll-up rows look pressable and do nothing** (`section-count` "N inactive" is the visible one; also `automated`, `working`, `now-overflow`) — DOR-1105. A first-week user hits the first one.
...
| 19  | Four dead roll-up rows                                                                                                                                                   | `SidebarChrome.tsx:239-244` (DOR-1105)                                                                                              |
